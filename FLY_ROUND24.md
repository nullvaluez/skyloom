...
- **THE SEVENTH BUDGET SITE, offered by E and NOT taken — routed to A.**
  `sat-road-engine.js:526` bounds its finalize with a module const
  `FINALIZE_PER_FRAME = 1` that is **not multiplied by `budgetK()`**, while
  ...
  **concentrates the stall exactly where the phase marker points** — pass 2b's
  ...
  one-line idiom as the veg site (`59b4e97`), the budget-site count gate moves
  **6 → 7** with the expression, and the product path is byte-identical because
  `budgetK()` is 1 in production.
- **Residency needs an eviction policy with a cap** (A, from E's Owens caution,
  §4.2): retained tiles are issued and `residentMB` was still climbing at
  113.7 MB after 600 s, so the ceiling breach is reachable in a long live
  session before it is reachable in a fixture.
...
**The user is now testing `01ff6d8`** (`8240539` plus a scripts-only merge 13) — pushed early, at their request, before
...
| `TERRA_PACE` {`timerFix`, `mergeHysteresis`, `keepResident`, `skirtFast`, `walkWhileSaturated`, `bboxCache`} | 22/17/178 → 0/0/0; timer 10/12 → 4/12; the saturated walk strictly conservative; skirt output element-identical; **live-arm fixture evidence now in** — merges 1 → 0, refetchParent 1 → 0, Owens **161 → 185 ≤ 261**, Powell **161 → 183 ≤ 375**, draws rising because `keepResident` keeps more tiles drawn. `verify-terra-live` (6)'s 1 → 17 repeat fetches are **ATTRIBUTED to the harness's ~51°/frame wall-clock yaw at 1 fps, not to the feature** — the LRU had nothing to evict (peak 30–36 % of a 140 MB cap) and merges were 0; re-measured in pass 2 at 0.85°/frame over a full 360°. **OPEN at the close: pass 2b's ON arm read Owens 279 against the frozen 261** after a 600 s sweep (185 after 45 s in pass 1, same flags), with resident tiles 62 → 103 and `residentMB` 113.7 still climbing — **retention, not necessarily culling**, and the two requirements to A are a never-issue rule for off-frustum/superseded tiles and a capped LRU residency, proven in the node gate with zero-refetch-on-yaw preserved. **The 261 is not moved** | **ON** — `timerFix`, `mergeHysteresis`, `keepResident`, `skirtFast`, `walkWhileSaturated`, `bboxCache` all true; `skirtWorker` and `bendSphere` false, merged `5b13e35` |
