...
- **8 genuine imagery refetches on a pure yaw with merges 0** (A, from the
  `lod-fade` OFF arm: 8 of 567 distinct URLs, e.g. `/img/11/772/550` ×2, with the
  7 z15 DEM ceiling-shares correctly excluded) — transient-miss retries are the
  first guess, and A attributes from source (§4.2).
