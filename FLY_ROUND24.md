...
**THE FINAL TIP IS `c0f034c`** (merge 59: E's close sweep and the corrected
probe, scripts and ledger only) — **the tree under certification**. **This
record's own commit sits on top of it, and `main` is fast-forwarded to that record
commit**, so the tree `main` carries is `c0f034c` plus these documents. Earlier in
the close, **`main` WAS ADVANCED TO `0adb010` AT THE USER'S REQUEST — *"just push to main if
...
**THE ROUND'S HARNESS ARITHMETIC, E's, for the close: ELEVEN harness defects
across w5 and w6** — **five the same lesson** (an actionability wait, a 30 s
capture budget, a ramp clock, two draw-pairs, a JFK dwell), **two verdict-class
holes** (a SOFT path exiting 0, a missing gate count), **three attribution fixes
that kept a red off the wrong owner** (JFK's fixture gap, `lod-fade`'s scene
mismatch, the eviction leg's static-terrain premise) — **and NOT ONE PRODUCT
DEFECT AMONG THEM: every red that survived attribution this round was an
instrument, a venue limit, or a census that had not settled.** The converged row's
**tier and epoch columns stand UNFILLED, not inferred**, with the phantom probe's
two misses beside them; **`flicker`'s suburb re-run was not run this round**; and
the JFK `aeroway` feature is an R25 fixture item.
...
saved from a wrong single cause.** After the teardown the main worktree was
fast-forwarded to **`9962d15`** (merges 46–48: C's truth-arbiter APP file plus E's
harness repairs), and **the w6 server start is HELD for D's ruling, so that a GO
flip rides the tree w6 certifies.**
...
| `verify-depth-roundtrip.js` | **rc=1, 220 s, 1 passed / 1 failed — NOT RUNNABLE, not RED** (toy, tier high, renderer `reversedDepth=true`). Gate (0) refused because **`window.__flyDepthProbe(x, y)` was ABSENT** — *"this gate cannot reconstruct viewZ without the renderer's own conversion; re-implementing it in the harness would test the harness's copy of the bug"* — required signature `{ viewZ, coc, raw, reversed }` over drawing-buffer pixels, top-left origin, owner C under `DEPTH_FIX`; it printed `dof=null`, and gate (0b) "the DoF pass is present" then PASSED on an inference from style and tier while `dof` was null — **the fourth vacuous pass of the day**; E is making it read a handle. C has since built the hook (`e59445d`, §3), so **the PASS 2 marker stands**. Note for pass 2: an `error` there means **neither float target renders** — recorded with the probe's own string, and read as NOT RUNNABLE HERE rather than as a defect | **pass 2b: the hook ARRIVED and the gate still measured nothing.** `window.__flyDepthProbe` present and `__flyDof` true — **C's hook worked on the first run it existed for** — but 0 raycast hits of 15 probes and no finite CoC. Per-leg detail: `scripts/r24-close-sweep.md` §5.9. **RE-TAKE: rc=1, 443 s, 3 passed / 0 failed / 2 NOT CALIBRATED — the hook and the settle are proven, the round-trip is UNMEASURABLE FROM THE HARNESS SIDE.** On `9bcaace`, started 00:34:38, K=40, load 4.37 / 4.55 / 4.63. **(0)** `__flyDepthProbe` present **PASS**; **(0b)** the DoF pass is present — `__flyDof` true, style toy, tier high (*"the toy `DepthOfField` pass is the consumer this gate is about"*), renderer `reversedDepth=true` **PASS**; **the settle WORKED — SETTLED in 121 s, maxZ 15, load 4.49 — its first run as a CONTENT gate with K=40** (pass 2b's 0 hits was a world that had not streamed); **(1) THREE PIXELS WITH A KNOWN TRUE DISTANCE — NOT CALIBRATED**, 0 raycast hits of 15 probes, picked 0, every miss reading *"handle absent — THREE false, gl true, cam false, scene true"*; **(3)/(4) CoC — NOT CALIBRATED**, no finite `coc` (near undefined, far undefined), the DoF separation not asserted; **(5)** no page errors **PASS**; the row also printed **`dof=null` beside `__flyDof true`**. **ATTRIBUTION**: the gate tries to build a `Raycaster` **from OUTSIDE the bundle**, and the page exposes the renderer and the scene but **neither the camera nor the `THREE` namespace** — a bundled app has no `window.THREE` — **so no true distance can EVER be established from the harness side**. That is a **hook gap on C's probe, not a `DEPTH_FIX` result**, and **the miss table is what made it attributable in one read** where pass 2b's bare "0 raycast hits" could not say why. **ROUTED**: C extends the probe with an in-app truth raycast — `window.__flyDepthTruth(x, y)` (or a `truth` field on the probe result) through **the composer's ACTIVE camera** against **the world's depth-WRITING geometry only** (terrain tiles, toy chunks, sat buildings; no billboards, sprites or transparent non-depth-writers, **so the truth matches what the depth buffer holds**), returning `{ hit, distance, viewZ, object, source }` in the probe's own viewZ units and `{ hit: false, reason }` when nothing is hit, dev-only and on demand — and reconciles `dof=null` against `__flyDof true` with `cocSource` naming the pass it read. E consumes it: **(1)** picks three probes with `truth.hit` and asserts **\|probe.viewZ − truth.viewZ\| within 1 %** against C's **2.50–2.51 m** RED signature, NOTCAL with the truth's reason below three, **never a hand-built `Raycaster`**. **`depth-rt` joins the post-batch list after `fade`.** **POST-BATCH: rc=1, 434 s, 4 passed / 3 failed / 1 NOT CALIBRATED — the hook works, and the HEADLINE HOLDS.** On `14c1220`, which carries C's `__flyDepthTruth` and E's falsifier precondition; started 02:02:43, K=40, load 4.09 / 4.38 / 4.49. **THE HOOK WORKS**: (0) `__flyDepthProbe` present; (0b) the DoF pass present (`__flyDof` true, style toy, tier high); **(1) THREE PIXELS WITH A KNOWN TRUE DISTANCE — 12 truth hits** at 1,392 / 1,171 / 1,464 / 1,887 / 1,758 / 2,412 / 2,369 / 2,429 / 2,864 / 2,811 / 2,836 / 3,145 m on `Mesh`, *"in-app `Raycaster` through the composer's own camera against depth-writing meshes, un-bent by the live `uBendK`"* — per probe **`bendK` 0.000005, drops 10.05 – 30.12 m converging in 3–4 iterations, residual 0.000 – 0.007 m, reprojection 0.00 px on EVERY one: C's self-falsifying truth passed its own falsifiers on its first run**; (5) no page errors. **THE HEADLINE: the probe reconstructs in the KILOMETRE regime** — **1207.14 m against a 1171.4 m truth** (raw 0.00207; 2.5 / 0.00207 = 1207.7, i.e. z = near / raw, reversed with an infinite far and near ≈ 2.5) — **nowhere near C's 2.50–2.51 m double-un-reversal RED signature: `DEPTH_FIX`'s core claim, that reversed depth is no longer double-converted and DoF sees real depth, is CONFIRMED ON THE VENUE.** **THE THREE FAILS AND THE NOTCAL are instrument or open attribution, none of them the flag's**: **(2) near ~50 m — 3.05 %** (true 1171.4 m vs reconstructed 1207.14 m) and **(2) mid ~700 m — 3.26 %** (true 1171.4 m vs 1209.63 m), **both picks resolving to the SAME pixel (480, 464) with the same truth — two legs that are one measurement wearing two labels** (E's pick; fixed for the re-run as three DISTINCT pixels with distinct, ordered truths), with **the 36 m residual at 1.2 km left with C to attribute** (near-plane value, texel alignment between the two hooks, a skirt or LOD neighbour, float16 precision at raw 0.002) **before the ≤ 1 % bound is judged**; **(2) far ~4 km — 98.90 %** (true 3144.7 m vs reconstructed **34.45 m** at pixel (480, 281), raw 0.0726) — **the depth buffer holds a depth-WRITING object 34 m in front of the camera at that pixel that the truth raycast did not hit** (under reversed-Z a LARGER raw is nearer; the player aircraft under the chase cam is the obvious occupant, C confirming) — **two different surfaces, not a round-trip error**, so the re-run's pick treats a factor-of-ten disagreement as **a pick miss with its reason, never a `DEPTH_FIX` FAIL**, and C rules whether the truth's candidate set must include the player; **(3)/(4) CoC — NOT CALIBRATED**: the values are finite (near 0.141, far 0.165) but *"no `cocSource` is published — the gate would be asserting a number without knowing which pass produced it"*, and on a tree carrying C's `cocSource` publish it read null (with `dof=null` beside `__flyDof` true again) — C attributing. **The row re-runs at the tail.** **C CLOSED ALL THREE AT THE SOURCE** (`a44f4b7`, merge 37 `f22fcac`, `verify-c-flagoff` 50 → **54**, two RED-calibrated). **THE FAR PICK WAS C'S OWN DEFECT, not the candidate set**: a rigid vertical lift is the right correction for distant terrain and **fatal in the near field** — at `bendK` 5e-6 a 3,144 m hit means a lift of 3144² · 5e-6 = **49 m**, and a ray whose origin is lifted 49 m **passes ~49 m above anything a few tens of metres away**, so the first iteration hit distant terrain, the lift went to tens of metres, and every subsequent cast **flew straight over the player aircraft sitting ~30 m ahead under the chase cam**: *"the aircraft is a depth-writing visible Mesh and was in the list the whole time — my own rule was right and my correction defeated it."* The fix casts **BOTH** an unlifted ray (near field, where the drop is 0.05 m at 100 m and cannot matter) **and** the bend-solved ray, projects each corrected point back to the pixel, **rejects above 1.5 px, and takes the NEAREST survivor — "because nearest is what a depth buffer keeps"** — distant terrain found by the unlifted ray being pushed off the ray by its own drop and rejecting itself, **so there is no special case anywhere**; new fields `via` ('unlifted' \| 'bend-solved'), `tried`, `rejected`. **THE 36 m GAP, ruled out in order**: **NOT the near plane** (2.5 / 0.00207 = 1207.7 against a reported 1207.14 — **the probe reconstructs exactly near/raw for a reversed buffer, so the conversion is faithful and the disagreement is about which SURFACE**); **NOT texel addressing** (both hooks take `gl.getDrawingBufferSize()` and use exactly inverse conversions of the same px + 0.5 centre, **and `reprojectionPx` came back 0.00 on every probe — direct proof the truth point lands on the pixel asked for**); **NOT float16** (at raw 0.00207 a half-float carries ~0.05 % relative error, **60× too small for 3 %**). **What remains is the surface, and the log proves it was MOVING**: two probes of the SAME pixel (480, 464) read **1207.14 and 1209.63, 2.5 m apart** — *"a static surface cannot do that, so a truth taken in a different turn is a truth of a different world"* — so **`truth` is now a FIELD on the probe result, computed in the same synchronous turn as the depth read**, with no frame able to land between them (the standalone `__flyDepthTruth` stays). **THE CONTRACT WORDING MOVES, THE NUMBER DOES NOT**: ≤ 1 % is the wrong bound for a SURFACE comparison at 1.2 km, and it now reads **\|probe.viewZ − truth.viewZ\| ≤ 0.01·\|truth.viewZ\| + truth.slopeMPerPx**, because one pixel at 1.2 km subtends metres of ground and on a grazing face — a tile skirt, a cliff, a seam between two LODs — **the depth across one texel changes by tens of metres where no reconstruction error is being measured**; the truth casts the four neighbouring pixels and publishes `slopeMPerPx`, the largest one-texel view-Z step, **so the term is MEASURED at the probe's own pixel rather than guessed as a constant**: *"tight where the surface is flat, honest where it is not."* **`cocSource` WAS published — the gate read a different address**: `coc` and `cocSource` are assigned in the same branch off the same `cocTex`, so **a finite `coc` PROVES a published `cocSource`**, and the log printing `coc 0.1411764770746231` beside *"coc source: null"* is exactly that contradiction — the gate reads `__flyStats?.effects?.cocSource ?? __flyDof?.cocSource` (`verify-depth-roundtrip.js:381–383`) while the name lives on **the probe result the same gate already destructures at `:323`**: *"nothing wrote the address it read."* The probe now also mirrors the name onto `__flyStats.effects.cocSource` so both paths work, **and this class of "published, but somewhere else" is closed for the field**. **BONUS — THE CoC TEXTURE IS 8-BIT**: 0.1411764770746231 is float32(36/255) and 0.16470588743686676 is float32(42/255), **exact 1/255 steps**, so the CoC term is quantised to 0.0039 — comfortable against (3)'s 0.02 bound but worth stating, **because "0.141" reads like a continuous measurement and is not**; (4) is not blocked by it — it read 0.165 because that pixel held the aircraft at 34 m rather than terrain at 4 km, i.e. the far-pick defect, **and E's pick change plus the dual cast should clear the two together**. E consumes all of it for the tail re-run: `probe.truth` in the same turn, the slope term in (2), `cocSource` off the probe result, the quantisation stated. **TAIL RE-RUN, MEASURED: rc=1, 406 s, 7 passed / 2 failed** (toy / high, K=40, on `3231f7d`, started 03:19:16, load 3.93 / 4.17 / 4.37; `scripts/r24-out/tail/depth-rt.log`). **WHAT STOOD**: the hook published, the DoF pass present, **settled in 77 s** (maxZ 15), **13 distinct truths of 13 usable hits** with 2 misses where the un-bend did not converge in four passes ((672,297) residual 0.073 m, (288,281) 0.234 m, against the 0.05 m bound); **(2) NEAREST 35.9 m on `Jet_Cube024_1`, probe 35.94 m, err 0.10 % PASS**; **(2) FARTHEST 3210.4 m on `Mesh`, probe 3210.50 m, err 0.00 % PASS — the kilometre regime reconstructs, so the double-un-reversal headline stands a SECOND time**; **(4) far CoC 1 at 4 km PASS**; no page errors. **The CoC source is now NAMED in the log** — `DepthOfFieldEffect.renderTargetCoC`, half-resolution, via `__flyDof` — with the 8-bit quantum 0.0039 recorded. **THE TWO REDS ARE OPEN BY ATTRIBUTION — neither a defect nor a pass.** **(a)** (2) MEDIAN at px (480, 389): truth **1933.4 m** on `Mesh` against probe **1728.04 m**, \|Δ\| **205.36 m = 10.62 %** against a bound of **28.66 m** (1 % + a **9.32 m** one-texel slope), *"via bend-solved · 1 cast rejected of 2"*. **Nearest and farthest agree to 0.1 % ON THE SAME FRAME, so this is ONE PIXEL'S SURFACE disagreeing, not the decode**; C is reading the truth's rejected cast and whether a depth-WRITING actor (an `InstancedMesh`, a bent chunk) is missing from the candidate set — **instrument if so, and `DEPTH_FIX`'s if the buffer really reads 1728 m where every depth-writing surface is ≥ 1933 m.** **(b)** (3) *"CoC < 0.02 at the focus plane"* read near `coc` **0.161** — **on the PLAYER JET at 35.9 m**, because the nearest pick landed on `Jet_Cube024_1` under the chase cam: **the gate read an actor it does not control, which is R17 §7.1's lesson exactly**. E owns that premise and is rebuilding the clause as *"measured CoC ≈ the DoF material's own CoC formula at the truth distance, within the 8-bit quantum"*, **so it no longer assumes where focus sits**, with C supplying the focus-plane contract; the re-run rides a later server. **BOTH REDS ARE NOW ATTRIBUTED — INSTRUMENT, BOTH — with the re-run still pending.** **RED 1, the 205 m median, is C'S OWN TRUTH HOOK** (fixed at `81d803a`, merge 41 `066cc73`). The old truth **subtracted the GROUND drop from every hit and rejected anything that then missed the pixel by more than 1.5 px**, and that model failed twice at this pick: **on a GRAZING ray a vertical displacement is almost entirely PERPENDICULAR to the ray**, so at 1.7 km (ground drop 1728² × 5e-6 = **14.9 m**, one pixel ≈ 1.7 m) **a CORRECT candidate reprojects ~9 px away and is thrown out** — the row's own *"1 cast rejected of 2"*; and **the ground formula is WRONG BY CONSTRUCTION for an air-bent actor** (traffic, contrails and the player ride `world-bend-air-anchor`, whose `airDrop` carries R7's lift term), **so the one class of actor most likely to sit in front of terrain was the class GUARANTEED to be rejected**, and the truth fell through to the ground 205 m behind. **The candidate set is exonerated from source** — no raycast override, no layers manipulation anywhere in `components/fly` or `lib/fly`, every `InstancedMesh` pool already in the list. **The fix REMOVES the model rather than tuning it**: every bend variant displaces only in **Y**, so the rendered point shares the hit's XZ and **the truth is the point on the ORIGINAL ray at that XZ — reprojection 0 BY CONSTRUCTION, exact for ground, anchor and air bends**; `impliedDrop = hit.y − rayY(sameXZ)` becomes **a MEASUREMENT of how far the GPU moved the actor** and the validity test (0 ≤ impliedDrop ≤ groundDrop within tolerance) — **a ground hit reads them equal and an air-bent actor visibly less: the family tell, PRINTED not inferred**; both casts feed one pool of up to 8 hits each, nearest valid `t` wins, and the hook publishes every candidate (`hits`). **The two non-convergent misses (0.073 / 0.234 m) should vanish with it.** `verify-c-flagoff` **54 → 57**, RED-calibrated by forcing `valid=true` and by reversing the sort, **the gate file untouched**. C's line: *"your shape was right — it was an actor the ray DID see and my corrector discarded."* **RED 2, the CoC clause, is E'S INSTRUMENT and worse than "an actor it does not control": the toy DoF runs on WORLD CONSTANTS that do not follow the chase** — `TOY.dofFocusM` **700 m**, `dofRangeM` **2600 m**, `bokehScale` 2.6 (`Effects.jsx:238–246`), normalised to focus 0.001167 / range 0.004333 — **so the near pick at 35.9 m sat 664 m IN FRONT of the focus plane, and CoC 0.161 there is the DoF working as configured**. **The same defect voids (4): it PASSED at 3210 m while naming 4 km.** Both clauses picked their pixel **by RANK** (nearest / farthest truth) and never checked WHERE that pixel was, and nothing in this pose's 13 truths is near 700 m (closest 1171 m) or past 4 km (max 3487 m) — **so on this pose both are NOT CALIBRATED BY CONSTRUCTION: a pose problem, and never a reason to move 0.02 or 0.5. One cause, a false red AND a vacuous green.** **C's contract confirms the rebuild**: `CircleOfConfusionMaterial` computes `smoothstep(0, focusRange, abs(linearDepth − focusDistance))`, and evaluated at this run's own truths it reproduces the measurements within the 8-bit quantum (35.9 m → **0.1624** against a measured 0.1608; 3210 m → **0.9965** against a measured 1); **a "< 0.02" pick must land in 482–918 m of world distance**, and `window.__flyDof.cocMaterial` exposes the live `focusDistance` / `focusRange` uniforms. E rebuilds (3)/(4) as **"measured CoC ≈ the material's own formula at the truth distance, within the 8-bit quantum, at all three picks"** — **stronger than a focus-plane assertion, because it tests that the DoF reads REAL DEPTH without assuming where focus sits** — RED-calibrated on the flag-off tree where CoC must not track truth distance. The re-run rides the night-gates server after row (6) if E's rebuild is merged by then, else the `lod-fade` server. Marker stands for the re-run **RE-RUN ON THE REBUILT GATE: rc=1, 379 s, 7 passed / 4 failed** (`scripts/r24-out/w5/depth-rt.log`; served app `645d08c`, harness at `e27c4ed` — the player PARKED via `visible=false`, (3)/(4) rebuilt against the live `cocMaterial`), started 05:48:43, load 4.03 / 3.97 / 3.98. **The player is parked**; settled in **58 s** (maxZ 15); **15 truths of 15 usable, all terrain (`Mesh`), ZERO misses — C's arbiter removed the convergence misses exactly as predicted**. **(2)** nearest 1211.1 m: probe **1211.09 m, err 0.00 % PASS**; median 2282.3 m: probe 2356.32 m, **+74.01 m, 3.24 %** against a 35.69 m bound **FAIL**; farthest 2667.3 m: probe 2788.60 m, **+121.30 m, 4.55 %** against 43.96 m **FAIL** — **every pick "via unlifted", every truth line "drop 0.00 m"**. **(3)** nearest CoC 0.1451 against a formula 0.1434 at the truth's Euclidean 1319.8 m, \|Δ\| 0.0017 ≤ 0.0052 **PASS**; median 0.8118 vs 0.7725 **FAIL**; farthest 0.9725 vs 0.9375 **FAIL**. **(4)** the measured CoC **orders the picks the way true distance from focus does** — worst backwards step **0.0000**, spread 0.8275 ≥ half the predicted 0.7942 **PASS**. **(5)** no page errors. **THE READING — put by the orchestrator and now CONFIRMED BY C IN C'S OWN WORDS — is ONE CAUSE, and it is C's NEW arbiter (`81d803a`), not the flag and not E's clauses**: *"your diagnosis is exactly right and I got the geometry backwards … applied to an unlifted cast it is circular — a raycast hit lies on its own ray by construction, so `impliedDrop` ≡ 0, the interval test 0 ≤ 0 ≤ groundDrop passed HONESTLY, and the truth became the un-bent geometry with no correction at all … I removed the thing that was working."* A CPU raycast hit **lies on the ray by construction**, so `impliedDrop = hit.y − rayY(sameXZ)` is **identically zero for an unlifted hit** — validity passes trivially, nearest-`t` wins, and **the truth degenerated to the UN-BENT geometry with no bend correction at all**. The (2) errors are **exactly the ground drop projected along the grazing ray**: the drop is ≈ d² × 5e-6 ≈ **26 m at 2282 m** and **36 m at 2667 m**, and divided by the ray's tan θ that is **74 m** and **121 m** — **the GPU lowers the surface, so the pixel's ray meets the lowered surface FARTHER along.** *"The rendered point shares the CPU hit's XZ"* is **true for a rigid displaced POINT — the air-bent actor case C fixed — and FALSE for a continuous SURFACE**; the previous tail run's bend-solved iteration agreed to 0.00 % at 3210 m, and **the new arbiter regressed it**. **WHAT SURVIVES IS THE `DEPTH_FIX` HEADLINE, PROVEN A DIFFERENT WAY: the DoF's CoC agrees with the DEPTH BUFFER** — the material's own formula evaluated at **the PROBE's** Euclidean distance gives 0.8112 against a measured 0.8118, and 0.9713 against 0.9725, **inside the 8-bit quantum at all three picks** — **(3) fails only because it references the TRUTH's distance while the truth is wrong.** So **DoF ↔ buffer is PROVEN, and buffer ↔ geometry is OPEN ON THE INSTRUMENT.** C restores the bend-solved iteration **for surfaces** while keeping the variant-correct displacement plus reprojection **for rigid actors**; E moved (3)'s reference distance to the probe's. **C'S FIX IS IN** (`ea30f64`, merge 46 `09a7cdb`, smoke 17/17, pushed — **NOT yet on the served tree**, `lib/fly/depth-probe.js` being an app file, so the re-run rides a FRESH server after `lod-fade`, with `terra-live` last). **SURFACES GET THE ITERATION BACK** — lift the ray by the drop at the current hit, re-cast, converge at 0.05 m, **and at convergence `hit − (0, drop, 0)` lies on the ORIGINAL ray, so residual and reprojection become the validity TEST rather than a construction**; **RIGID AIR-BENT ACTORS GET THEIR OWN DISPLACEMENT** — traffic, contrails and the player carry `world-bend-air*` keys and drop by `airDrop(d, y)` with R7's altitude lift, **not d²k** — with **the family read from the material's own `customProgramCacheKey`** (`world-bend.js:716`/`777`/`847`), one iteration per family matching only its own hits, **which is what lets an air-bent actor in front of terrain be found at all**; and **validity = convergence + residual + reprojection, because nothing else excludes a degenerate zero**. The hook publishes `hits` (per-family candidates with converged / iters / residual / reprojection), `rayHits` (every raw hit any cast met, with its bend key) and `eye` (the camera position the answer belongs to). **THE CALIBRATION MOVED, for exactly lesson 89's reason**: `verify-c-flagoff` was **structurally incapable** of catching this — in SOURCE a degenerate zero is indistinguishable from a converged answer, so forcing `valid=true` or reversing a sort proves nothing — so it now lives in `scripts/r24-c-depth-roundtrip-proof.mjs` **as geometry solved BACKWARDS from the w5 median pick**: `bendK` 5e-6, ray depressed 19.4734°, eye 760.85 m — **RED** (the degenerate "ray point at the hit XZ") 2282.30 m, implied drop 0.00, **the interval test ACCEPTS**, off the buffer by **74.02 m**; **GREEN** (iterated lift, 4 passes, converged) drop 24.68 m, 2356.30 m, off the buffer by **0.02 m**, residual 0.0056 m — **the w5 log measured +74.01 m and the proof reproduces +74.00 m**. `verify-c-flagoff` **57 → 58** (RED by forcing `valid=true` and by deleting the air-drop branch). **ATOMICITY CONFIRMED**: `probe()` samples `composer.depthTexture` and calls `truth()` **in the same synchronous turn** — no rAF can fire between; one depth texture, one scene graph, one set of camera matrices, and `FlyScene` applies and removes the floating-origin offset inside one frame (`:1712`/`:1735`). **AND w6's `eye` PRINT RETIRED THE MOTION THEORY**: the camera moves only **0.8–1.3 m** between the pick sweep and each per-pick assertion, **so the w5 truth SHIFTS of hundreds of metres (median 2628 → 2282, farthest 3053 → 2667) were NOT the aircraft travelling — they were the degenerate arbiter picking DIFFERENT SURFACES between the two reads**, the same instrument defect §22 and lesson 89 describe. In E's words: *"those shifts were the arbiter picking different surfaces, not the aircraft travelling. Printing the number retired the theory in one line."* **"Expected motion" is the right label for the METRE the eye print actually shows, and for nothing more.** **AND C RETRACTED THE SECOND OF ITS OWN TWO ATOMICITY ARGUMENTS** (`cae48d3`, merge 54 `2f9bff3`, ledger-only, which also carries the w6 certification): the first stands, and the second — *"a single instrument cannot produce a drift between two of its own readings; a constant bias cancels"* — **is refuted by the eye print**, since a metre cannot move a ground intercept by hundreds of metres. **C names the category error, and it is the keepable part**: *"That arbiter was not a BIAS, it was a SELECTOR. Its validity test was trivially satisfied by every candidate, so nearest-valid-`t` was choosing among a pool on a knife edge, and a metre of camera travel could flip which surface won. A bias cancels between two readings of one instrument; a selector amplifies the smallest input change into a discontinuous output. I reasoned about the first and shipped the second — the same shape as R21 §5's 'a probe green on a quiet boot is not a probe green under load': an instrument whose answer is stable only while nothing moves. The `eye` field exists because I wanted 'expected motion' to be a measurement rather than an argument. It became one, and it decided against me."* **C stands node-only: no browser or dev server was ever started from C's worktree in the close.** C's ledger closes `DEPTH_FIX`'s own claim **on E's two-instrument agreement**. **AND E MEASURED THE SAME ERROR INDEPENDENTLY**: inverting the smoothstep on the median's MEASURED CoC 0.8118 gives **2578.5 m** Euclidean against the truth's 2496.5 — **82 m, or 74 m in view-Z** — and gate (2) measured **74.01 m**. **A depth-texture read and the GPU's own CoC texture, sharing no code, agree to a metre on the SIZE of the truth hook's error — which is what makes the attribution safe rather than plausible.** E's `(3)` now references **the PROBE's** Euclidean distance with both distances printed, and **that clause thereby LEAVES the round's RED set, written into the source**: both sides come from the same buffer, so **a green (3) is a CONSISTENCY CHECK, not a detector.** Marker stands **W6, ON THE FLIPPED TREE: rc=0, 384 s — 11 passed, 0 failed. `DEPTH_FIX` IS CERTIFIED ON THE VENUE.** (`scripts/r24-out/w6/depth-rt.log`, tree `d960fdb` — `LOD_CROSSFADE` ON, C's restored arbiter `81d803a` → `ea30f64`, E's rebuilt clauses — started 08:09:52, load 4.13 / 3.80 / 3.34.) Player parked; settled in **71 s** (maxZ 15); **15 truths of 15 usable, all terrain, every pick "via bend-solved · family ground · converged true"**. **(2)** nearest 1201.7 m truth against probe **1201.71 m, \|Δ\| 0.00 m, 0.00 %**; median 2399.3 against **2399.39, \|Δ\| 0.05 m, 0.00 %**; farthest 2836.6 against **2836.79, \|Δ\| 0.22 m against a 48.58 m bound, 0.01 %** — **the median that read +74 m on w5 under the degenerate arbiter reads 0.05 m under the restored iteration**. **(3)** the measured CoC ≡ the material's own formula **at the PROBE's Euclidean distance** within the 8-bit quantum at all three picks (0.1373 / 0.8353 / 0.9804) — **DoF ↔ depth buffer, independent of the truth hook**. **(4)** the measured CoC orders the picks the way true distance from focus does — worst backwards step **0.0000**, spread **0.8431 ≥ half the predicted 0.8422**, \|signed\| 610 → 1927 → 2389 m. **(5)** no page errors. **E's `eye` print shows the camera moved 0.8 / 1.3 / 1.1 m between the pick sweep and each read, labelled EXPECTED motion** (the read is atomic; the pixel looks at a different ground point) — **C's atomicity confirmation is now a PRINTED READING, not an argument**. **So `DEPTH_FIX` is certified BOTH WAYS on the venue — buffer ↔ geometry through C's truth, and DoF ↔ buffer through E's CoC clause — the R24 red signature (every fragment collapsing to −cameraNear) is ABSENT, and every red this row ever showed was the INSTRUMENTS', none the flag's.** | — | needs `window.__flyDepthProbe`; absent ⇒ the row reads NOT RUNNABLE, never RED |
| `verify-terra-live.js` | **rc=1, 715 s, 8 passed / 1 failed — both arms, armed through `__flyTerraPaceOverride`.** Content probe **0 URL and 0 position mismatches in BOTH arms** (62/64 off, 59/65 on) — **a cache/URL mix-up is RULED OUT as a separate cause of "tiles swapping" in this code path**; **(4) merges 1 → 0** and **(5) replaced-on-screen 0 → 0**; **(7) the Owens ceiling 161 → 185 ≤ 261** and (8) Powell 161 → 183 ≤ 375, the rise being `keepResident` keeping more tiles DRAWN. **(6) FAIL, 1 → 17 repeat URLs — ATTRIBUTED TO THE INSTRUMENT, not `TERRA_PACE`**: the harness stepped `PIN_YAW` ~51° per rendered frame at 1 fps, so upstream started a refine, downloaded the children and DISCARDED them before the next evaluation — a node probe on the vendored classes reads duplicates 1 → 0 at 0.85°/frame and 28 → 28 at 51°/frame — and the byte LRU is ruled out by arithmetic (peak 41.3 MB ≈ 30 % of a 140 MB cap, merges 0). `PIN_YAW` became frame-based with `arcOk` guarding gate 6. **Two limits at exactly A's strength**: the fixture serves deterministic bytes per (z,x,y) and Esri does not, so a merge that replaces four children with a coarser parent reads here as a resolution change and on the user's machine as **a DIFFERENT CAPTURE** the probe cannot see; and the probe only tests tiles whose material already has a map, so its denominator excludes the mid-load tiles that are the likeliest moment for a mismatch. Per-leg detail: `scripts/r24-close-sweep.md` §5.10| **rc=1, 1973 s, K=40, both arms pinned explicitly — the residency trio HOLDS its yaw contracts on the flipped tree and BREAKS the frozen desert ceiling.** Started 22:04:16, load 4.20 / 4.63 / 4.71, `FLY_TERRA_SWEEP_MS=600000` with FRAME-based sweeps. **Arm A (trio OFF)**: content probe **62 resident tiles, 58 with an imagery URL, 0 URL / 0 position mismatches**; yaw arc **383° over 450 frames (0.8 fps)**; Powell draws **211** / tris 359,004; Owens draws **152** / tris 170,932; `residentMB` 0 (the OFF arm does not report residency). **Arm B (`timerFix` + `mergeHysteresis` + `keepResident` + `skirtFast`)**: **103 resident tiles, 99 with a URL, 0 URL / 0 position mismatches**; yaw arc **305° over 359 frames (0.6 fps)**; Powell **366** / 616,368 / `residentMB` **112.6**; Owens **279** / 409,846 / `residentMB` **113.7**. Gates: **(1)** every resident tile displays the imagery of its OWN z/x/y (arm A, 58 checked) **PASS**; **(2)** its quadtree address matches its world position (arm A, 62 checked) **PASS**; **(3)** the same with the trio ON (arm B, 99 URL / 103 position) **PASS** — the content half is answered in BOTH arms on the flipped tree; **(4) the engine stops merging tiles the camera merely turned away from — merges 35 → 0 PASS**; **(5) no tile is replaced while it is on screen — 27 → 0 PASS** — *that pair is the round's headline for Symptom B, measured on the venue*; **(6) SKIP / NOT CALIBRATED** — the duplicate-URL comparison needs ≥ 360° per arm within 10 % of each other and got 383°/450 frames off against 305°/359 frames on, **and the gate's own message named the right knob: raise `FLY_TERRA_SWEEP_MS` (arc = frames × 0.85°, so a slow venue needs proportionally longer), NOT `FLY_TERRA_YAW_MIN_ARC`** — hence 900 s on the re-take; **(7) CEILING Owens ≤ 261 in every arm that ran — FAIL, off 152 / on 279**; **(8)** satellite ≤ 375 at the suburb pose in every arm that ran — off 211 / on 366 **PASS**; **(9)** no page errors in either arm **PASS**. **THE BREACH: +127 draws with 41 more resident tiles and 2.4× tris at the same pose — retained tiles are ISSUED.** Pass 1's 45 s wall-clock sweep read Owens **off 161 / on 185**; this 600 s frame-based sweep reads **off 152 / on 279** at the SAME flag state, so **E's caution — accumulation with sweep DURATION — is the working hypothesis**, not (or not only) a cull-margin leak; **E's resident/visible census separates them in one read** (resident up with the drawn fraction flat = retention; drawn fraction up = cull). **Routed to A with two requirements**: a retained tile that is off-frustum or superseded is **never issued**, and residency gets an **eviction policy with a cap** (LRU by last-visible frame, tiles or MB — `residentMB` 113.7 after 600 s **and nothing says it stops**; a real GPU at 60+ fps accumulates an order of magnitude faster than this fixture, **and the user is flying this build now**), proven RED/GREEN in the node terra-residency gate with **zero-refetch-on-yaw preserved**. **The frozen 261 is NOT moved.** **A'S FIX HAS SINCE LANDED** (`dead5e5`/`c573d08`, merged to integration and pushed in `83462eb`): the breach is attributed to `keepResident` ALONE by a switch-by-switch measurement, and PATCH 26 `parkOffscreen` cuts off-frustum issued tiles **142 → 0** with residency itself unchanged (§3 A). **This row is DEFERRED, not closed** — E re-runs it at 900 s on that fix, and whether Owens lands under 261 is measured there, not asserted here. **The expected reading is on the record IN ADVANCE**: resident high, **`parked` high**, `visible` ≈ flag-off, drawn fraction well under 100 % — **read before the draw number** — and if Owens still breaches 261 with the drawn set frustum-bounded, that is A's named residual (z17 detail where upstream collapsed to z13) and the lever is `LODThreshold`, **never the ceiling**. The census itself was repaired first (§5.2) **POST-BATCH (`14c1220`, K=40, `FLY_TERRA_SWEEP_MS=900000` on both arms): KILLED BY THE ROW TIMEOUT — rc=124 at 2,400 s during arm B's Owens settle.** Two 900 s sweeps plus four settles cannot fit `run()`'s ceiling, the same arithmetic that sent `lod-fade` standalone. **What survived**: the arms were COMPARABLE for the first time (647 vs 650 frames, 550° vs 553°). Arm A (trio OFF) — 56 resident / 52 with a URL, **0 / 0 mismatches**; merges 74, `replacedOnScreen` **65**, `refetchParent` 74; Powell 210 draws / 531,267 tris; **Owens 175 draws**, resident 205 (128 with a model, **parked 0 — correct BY CONSTRUCTION**, `parkOffscreen` acting only with `keepResident` on). Arm B (trio ON, `parkOffscreen`, cap 260) — 62 resident, **0 / 0 mismatches**; merges **47**, `replacedOnScreen` **0**, `refetchParent` 47; **the poses were never reached**. **Two facts**: `replacedOnScreen` 65 → 0 at a full revolution, and merges 47 where pass 2b's ON arm read 35 → 0 — **the R21 symptom reduced from 74 to 47, not removed**. **A ATTRIBUTED IT TO A'S OWN BRAKE AND FIXED IT** (`f3cf43b`, merge 39 `dd6502c`, `verify-terra-residency` 35 → **40**): gate 26 asserted the cap exceeds the MEASURED working set, and A had measured that on a **240-frame synthetic yaw saturating at 190** then shipped **260**, while a 650-frame revolution at z17 holds more (the OFF arm alone 205 resident / 152 with a model at Owens) — **so 260 BOUND, the LRU elected off-frustum subtrees, their parents collapsed and the return re-fetched them**: *"74 → 47 reduced by my own brake rather than by the policy."* **`parkOffscreen` is REFUTED from source** (it writes `model.visible` and `_LODEvaluate`, `LOD`, `_update`, `_loadSubTiles`, `_removeSubTiles` and `tile-residency.js` never read it) — **and the near-miss A checked anyway is worth keeping**: `getElevationAt` → `getLocalInfoFromGeo` raycasts the tile meshes for every satellite drape sample, and had three's raycaster skipped invisible meshes **every sample would have returned null and retried forever, exactly the shape of the arm-B timeout** — but r185's raycaster tests LAYERS only (`three.core.js:56188`). **THE FIX is not a bigger constant** — *"a constant measured on one path is wrong on the next one, which is how we got here"* — the cap **FOLLOWS THE DRAWN SET**, `cap = max(260, k × drawn)`, with **`k` sized on the WORST measured retained/drawn ratio** (yaw 240f 3.96 · 650f 4.87 · bob 240f **9.20** · 650f 5.75 · serpentine 240f 5.34 · 650f 4.94 ⇒ **k = 14**); after it **the BYTE budget is the trigger that guards memory**, which is right because bytes are the resource and a count is a proxy, and pass 2b's 113.7 MB against a 140 MB budget says lifting the count trigger spends no memory the byte rule was going to refuse. **`replacedOnScreen` 65 → 0 held BY DESIGN** — election is out-of-frustum ONLY — now pinned by gate 33; the new rows are RED-calibrated by REMOVING the adaptive term (34 and 35 fail at 56 merges / 56 refetches on an effective cap of 150 against a 190-tile set), **gate 36 PUBLISHES the eviction count** — *"a merge count is never again ambiguous between 'the LOD policy did it' and 'the memory brake did it'; that ambiguity is what cost this re-take"* — **and gate 26 is REPAIRED rather than patched around**. **THE STANDALONE IS THE CERTIFICATION THIS ROUND WAS WAITING FOR: rc=0, 2,412 s, 9 passed / 0 failed** (tree `645d08c`, started 04:59:40, load 4.44 / 4.26 / 4.03; `scripts/r24-out/w5/terra-live.log`). **BOTH ARMS SWEPT FULL REVOLUTIONS for the first time on the venue** — OFF **671° / 789 frames** (0.9 fps), ON **607° / 714 frames** (0.8 fps), **symmetry 90.5 %, inside gate 6's ±10 %** (E: *"one slow arm from a coin"*). Content **0 URL / 0 position mismatches on both arms** (53 / 80 resident). **THE ON ARM'S COUNTERS ARE ALL ZERO against an EXPLICIT PRE-SWEEP CENSUS**: merges Δ0, `replacedOnScreen` Δ0, `refetchParent` Δ0, evictions Δ0, imagery requests **570 → 352**, effective cap **1722** (14 × 123 drawn) against **461 resident**, `overBudgetPasses` Δ0 — **the cap never bound and nothing was shed, so the post-batch 47 merges WERE the old cap and the adaptive cap removed them.** **(4) merges 74 → 0**; **(5) ON-SCREEN REPLACEMENTS 59 → 0 — the user's "tiles swapping for other ones", measured to ZERO over a full turn**; **(6) repeat URLs 301 → 24** (img 149/282 worst 12× → **8/344 worst 2×**; dem 151/250 worst 12× → **15/304 worst 6×**). **Growth** (resident/visible/parked): OFF 81/59/0 → 129/96/0 (93°) → 129/96/0 (222°) → **241/179/0**; ON 113/30/53 → 189/56/84 (95°, 112 f) → 293/74/144 (218°, 256 f) → **461/123/223**; `residentMB` 128.7, peak 129. **Ceilings: Owens OFF 211 / ON 104 ≤ 261** — the frozen desert control, **no re-baseline** — Powell OFF 231 / ON 201 ≤ 375; no page errors. **E'S TWO READINGS**: **(1) the ON arm's resident set had NOT reached steady state** — 113 → 461 climbing monotonically past a full revolution while visible went 30 → 123 and parked 53 → 223, nothing bounding it; **no gate asserts saturation, so not a failure** — an R25 soak item. **BUT E WITHDRAWS THE OTHER HALF OF THIS READING**: the claim that **the OFF arm was unsaturated too (81 → 129 → 129 → 241)** does not survive w6, where from a SETTLED start the OFF arm reads **269 → 265 → 249 → 241 — stable to DECLINING** — because **the w5 figure sampled `start` before the pose had converged**. In E's words: *"the instrument I added to answer that question was, on its first run, answering it wrong for exactly the reason everything else this round was wrong."* **The 'not the trio's' inference goes with it**; what stands is the ON arm's own climb. **(2) A's decoding rule** (merges with 0 evictions = the LOD policy; merges tracking evictions = the cap) **is a real inference only on the ON arm**: the OFF arm's cap / eviction / `residentMB` zeros are **a TAUTOLOGY, the residency pass not being installed when `keepResident` is off** — which is why E took an explicit pre-sweep census rather than trusting a reset that is a no-op there. **ONE READING HELD**: at the Owens pose **the ON census is COARSER** — tris **104,582 vs 276,704 (38 %)**, draws **104 vs 211**, *"resident 465 (349 with a model, parked 74) visible 275 = 59 % drawn"* against *"resident 237 (176 with a model, parked 0) visible 176 = 74 % drawn"* — while Powell reads 519,309 vs 576,692 (**90 %**), **so it is Owens-specific**. **A'S READING: it is (i), and the defect is in A'S OWN HARNESS, not the product** — `verify-terra-live.js:249`'s `settleDraws()` is `await page.waitForTimeout(30000)`, **a fixed wall-clock sleep with NO convergence condition** (not maxZ stability, not downloads-in-flight, not a frame count), and at 0.8 fps that is **~24 rendered frames**, far too few to warp continents and descend five to eight levels to z17 — **A had already made the YAW SWEEP frame-based for exactly this venue and left the settle on the clock: same venue, same mistake, one function away.** **THE DISCRIMINATOR IS THE POSE ORDER**: `POSES = { powell, owens }` in that order with **the sweep running AT Powell**, so Powell needs no re-streaming and reads 90 % while Owens needs a warp and a descent from scratch and reads 38 % — **under (ii) the policy would apply at BOTH poses and Powell would be coarse too; it is not.** Three corroborations: the ON arm arrives carrying 461 resident / 128.7 MB by `keepResident`'s own contract, so **its re-descent is slower (465 nodes to walk against 237), not its resting state coarser**; `visible 275` against `draws 104` is **the walk being behind in one number** — `visible` reads the LOD walk's `_inFrustum` stamps while three's cull uses live matrices, so after a warp the stale stamps still say "in frustum at the OLD position"; and `parked 74` at Owens against 220 at Powell says most tiles had not been revisited. **A's sentence, verbatim**: *"The ON arm's coarser Owens census is the harness settling on wall clock rather than frames — a fixed 30-second sleep is about 24 rendered frames at 0.8 fps, far too few for a tree to warp continents and descend to z17 — and the discriminator is that Powell, which needed no re-streaming, reads 90 % of the OFF arm's triangles while Owens, which needed a full re-stream, reads 38 %; the ceiling gate is green either way, but the ON arm's Owens draw count is not yet a settled number."* **OFF's 211 sits on the same sleep and is in family with prior runs (175, 152, 275), so the OFF column is usable**; **the lower ON number is a CEILING PASS, not a merit, and must never be quoted as headroom.** **(ii) is NOT excluded until one more measurement** — if it survives, what the user would see is **terrain that stays coarse for a beat after a big turn or a warp and then sharpens, the milder cousin of the reported symptom**, answered by a knob (`mergeHysteresisK`, or the refine side of `LODThreshold`), **never a re-baseline**. **THE FIX**: A is committing a **convergence settle** — maxZ stable AND downloads-in-flight 0 for N consecutive RENDERED frames, a frame floor plus a wall-clock cap, frames and seconds printed, a capped settle printed as **NOT SETTLED**, with the old 30 s clock kept behind an env knob as the RED — merged scripts-only, and **E runs `terra-live` once more LAST, after `lod-fade`**: **if ON's Owens triangles rise to meet OFF's, (i) closes and the settled ON number is the certified one; if ON plateaus materially below OFF with the tree quiescent, (ii) goes to R25 as a product finding.** **Everything else in this row is certified without hedging; this one number is NOT YET SETTLED** **THE CONVERGED RE-RUN, ON A'S FRAME-COUNTED SETTLE: rc=1, 4,906 s — 8 passed, 1 failed** (`scripts/r24-out/w6/terra-live.log`, tree `0025614` stamped `0adb010`, `LOD_CROSSFADE` ON, A's settle from merge 44; a first launch at 10:33:08 hit the 240 s cap on the content-probe pose and E stopped it (rc 143), relaunched 10:43:36 with `FLY_TERRA_SETTLE_CAP_MS` 600000 and a 7,200 s row budget; load 4.23 / 4.17 / 4.18). **THE SETTLES, PER POSE PER ARM, exactly as A built them to print**: OFF content-probe **SETTLED 381 s / 327 frames** (maxZ 17, 202 modelled); **OFF Powell NOT SETTLED** — the 600 s cap after 431 frames, still moving, **downloads in flight (8)**; **OFF Owens NOT SETTLED** — the cap after 1,159 frames, **in flight (6)**; ON content-probe **SETTLED 430 s / 392 frames** (238 modelled); **ON Powell SETTLED 34 s / 24 frames** (349 modelled); **ON Owens NOT SETTLED** — the cap after 1,159 frames, **in flight (8)**. **THE SWEEP, from a settled start this time**: OFF merges **Δ100**, `replacedOnScreen` **Δ87**, `refetchParent` **Δ100**, evictions 0, imagery 496, growth 269 → 265 → 249 → 241 — **the OFF arm merges its way DOWN**; ON merges **Δ0**, `replacedOnScreen` **Δ0**, `refetchParent` **Δ0**, evictions **Δ0**, imagery 156, **the effective cap 1722 never bound**, growth 317/122/116 → 357/123/145 → 445/121/213 → 465/123/226 (**levelling**), `residentMB` 129.8 / peak 130.1; arcs OFF **569° / 669 frames**, ON **533° / 627 frames**. **(4) merges 100 → 0**, **(5) on-screen replacements 87 → 0**, **(6) URLs fetched twice 289 → 18** all PASS; the content probe reads **0 mismatches on both arms**; no page errors; **(8) Powell OFF 269 / ON 194 ≤ 375 PASS**. **THE POSES**: Powell OFF (not settled) 269 draws / 614,271 tris / resident 265 / 199 modelled / visible 199; **Powell ON (SETTLED) 194 draws / 508,840 tris / resident 465 (349 modelled, 232 parked) / visible 117 = 25 %**; Owens OFF (not settled) 186 draws / 197,254 tris / resident 153 / 120 modelled / visible 120; **OWENS ON (NOT SETTLED) 279 draws / 441,206 tris / `residentMB` 144.3 / resident 509 (383 modelled, 303 parked) / visible 80 = 16 % → **(7) FAIL: Owens ON 279 > 261**. **WRITE IT AS IT IS**: this is **the SAME 279 as pass 2b's breach, the one the adaptive cap was built against**; **the w5 standalone's 104 was A's case (i), an UNSETTLED census**, and the fuller tree reads **441,206 tris against 104,582**; **the ON census at Owens is itself NOT SETTLED** (downloads in flight a constant 8), **so it is a breach measured on a tree that never went quiet**; **the frozen 261 does not move**; and **the attribution is OPEN between three owners** — A (residency composition: 303 parked against 383 modelled, and whether the trio holds a DEEPER drawn set at Owens, with `LODThreshold` pre-staged as the lever), D (whether `parentBlend` can add ANY draw at a HELD pose — **D pre-committed to answer for this red**), and E (the governor tier at each pose per arm, a draw DECOMPOSITION BY CLASS at Owens ON against the OFF arm's 186, and whether the 8 stranded tiles are ONE CONSTANT SET). **AND A READ THE PERPETUAL "downloads in flight" FROM SOURCE**: `downloading` counts TILES in flight — one `loader.update` per tile fetches both sources and is finally-decremented, **so it cannot leak, only hold a perpetual count** — and at a held pose refine and merge cannot flap (constant ratio, complementary tests), so **the count is a VERSION-DIRTY TILE THAT CAN NEVER CLEAR ITS OWN DIRTY FLAG**: `_updateModel` advances `_loadedEpoch` **only when `loader.update` reports content CHANGED**, a tile whose content is already correct returns false, `_needVersionUpdate` stays true, and `_update` re-enters `_updateModel` on every walk **forever** (`index.js:506` / `:1301` / `:200`). **The trigger is any `_epoch++` after those tiles loaded** — `rootTile.reload(false)` via `TileMap._updateSource` on a source recompute, and **a governor TIER STEP rebuilds the imagery source with `maxLevel satMaxZoomFor(tier)`** — **flag-independent, upstream, and an R25 item of its own: *a tier step can leave a permanent phantom download count on any machine.*** **AND THE PROBE BUILT TO REPRODUCE THIS MECHANISM CAME BACK NOT CALIBRATED — honestly, and only after E corrected its own instrument** (`scripts/r24-e-phantom-dl.js`; `phantom-dl.log`, `phantom-dl2.log`). **Run 1's verdict is WITHDRAWN**: it tested whether the count ever TOUCHED zero — printing *"reached zero = true"* and **`dl: 4` on the next line** — and declared the after-leg drained in one frame, **before the rebuild it existed to observe had issued anything**, publishing a verdict FALSIFYING A's mechanism that its own output did not support; **E caught it by reading its own PASS line sceptically**. **Run 2 requires a SUSTAINED drain and a treatment check**: the control drained honestly (**dl 0 held for 30 consecutive RENDERED frames after 327 s**; min 0 / max 9 / last 0), and then the tier step — with `maxLevel` really moving **17 → 16 → 17** via `satMaxZoomByTier` {high 17, medium 16} — **issued NO downloads at all, peak 0 over 122 s**. **No treatment, so two hypotheses survive**: the tier change did not actually rebuild the source in the running app, **or** it did and **every invalidated tile was already correct** — which is exactly A's precondition, and would mean **a tier step strands DIRTINESS without issuing DOWNLOADS**. **And that second, more interesting outcome cuts against A's own use of the mechanism**, because perpetual re-entry into `_updateModel` WITHOUT downloads does not explain a stuck `downloading` of 8 — **so the certification row's perpetual count would be something else again**. **One measurement separates them**: a version-dirty scan immediately after the step, which `r24-e-owens-decomp.js` already knows how to do — ten minutes, R25 (§5b). **So A's stale-epoch mechanism stands as INFERRED FROM SOURCE, NOT REPRODUCED, with the probe's result beside it.** **A's prediction that the ON arm would cap too HELD at Owens and FAILED at Powell** — composition at the bump, not policy — **and A refuses "the streamer never rests without the trio"**. **The `dl === 0` criterion refusing to certify a tree that never goes quiet is the criterion WORKING**, and a *"quiescent apart from N stranded tiles"* verdict is an R25 amendment **A declined to make mid-certification**. **THE ATTRIBUTION, ALL THREE OWNERS IN AGREEMENT.** **(1) A, from the vendored source: a PARKED TILE CANNOT BE DRAWN** (`model.visible=false` skips the subtree in `projectObject`), so drawn terrain ≤ the visible stamp — **Owens ON 279 = terrain ≤ 80 + non-terrain ≥ 199**, against **Owens OFF 186 = terrain ≤ 120 + non-terrain ≥ 66**: **the TERRAIN term FELL while the total rose 93**, and the fixture census names the remainder (**149 toy chunks at the fixture's Owens**). **Why the content grew**: the chunk engines drape on the terrain and gate their commits on `tileZ ≥ demZ`, and on the OFF arm the terrain merges constantly (100 merges / 87 on-screen replacements in this very run) **so drape samples come back from tiles replaced underneath them and chunks RETRY rather than finalize**, while on the ON arm nothing merges, the field is stable and deep, the drape succeeds and the chunks commit — in A's words, ***"the trio is not adding terrain draws; it is UNSTARVING a content pipeline that was previously failing to finalize"*** — which is also why **the frozen 261, set on a tree where that pipeline was partly starved, meets a fuller world**. The stranded 8 **cannot add draws** (`loader.update` returns false, `syncGroups` never runs), **but A found a real interaction of its own**: a version-dirty tile **returns in `_update` BEFORE `_inFrustum` is recomputed and before `r24Park` runs**, so a stranded tile is **frozen in its last visibility** — ≤ 8 draws, a one-line fix, R25. **"(b) is FALSE as posed"**: the ON arm holds MORE resident (509 against 153) and draws FEWER terrain tiles (80 against 120) — §18's retention/issue separation. **And the settle is one-sided the other way**: a FAIL on an unsettled census is real and **the settled number is ≥ 279** — *A is not hiding behind NOT SETTLED*. **THE NUMBER A WOULD SHIP: NONE.** `LODThreshold` could clear the 18 draws (a ~23 % cut in drawn terrain) but **would cut the component that IMPROVED to pay for the one that grew**, trades terrain sharpness at the tier the user flies for a number on a venue whose Owens carries 149 toy chunks (**live Owens is the empty-desert control precisely because R20's lock places near zero there, and the 261 was frozen against the LIVE one**), and R19 certified 0.86 against 209 — ***"a fixture content layer grew" is not the evidence that moves it***. **(2) D, from source and from the log header: THE CROSSFADE IS NOT EVEN THE VARIABLE** — arm A is `TERRA_PACE` OFF, arm B `TERRA_PACE` ON, and **`LOD_CROSSFADE` is ON in BOTH (no pin)**, so the 186 → 279 delta is measured with D's flag CONSTANT; the tells are `residentMB` 0 / parked 0 against 144.3 / 303, **A's levers**. And it cannot add a draw at a held pose in any of three senses: PATCH 6 inserts **before** the upstream return expression and the parent mesh is removed exactly when upstream removes it (**E's co-display census 0 confirms**); `attachLodFade` creates **no material and no mesh**, and `lod-crossfade.js` has no reference to `.visible` / `frustumCulled` / `.add(` / `.remove` / `removeFromParent` / `renderOrder`; PATCH 7 delays `add(model)` and **never ran** (merges Δ0); the stranded tiles re-enter `_updateModel`, **which calls neither hook** — `tile-loaded` re-runs `attachLodFade` idempotently and arms nothing; and **a live blend is three uniform values on a material already being drawn — draws are invariant to `active` at any count**. D's sentence: ***"the crossfade cannot add a draw at a held pose, so the 279 goes to A's residency lever."*** D's residual, **stated and excluded**: an exception inside `onRefine` could reject `_loadSubTiles`' promise and leave `_subTiles` set — throw-safe by inspection and excluded by **zero page errors on both arms**. **(3) E, LIVE RE-READ** (`scripts/r24-out/w6/owens-decomp.log`; a FRESH warp to Owens on the flipped tree, trio ON, no sweep): **two censuses 60 s apart IDENTICAL** — **dl 0 (SETTLED), draws 140, tris 193,522**, terrain resident 225 / withModel 169 / parked 77 / stamp 141 / drawableTiles 92 / meshesInView 92, byClass satBuilding 0 / skyline 0 / roads 0 / clouds 0 / cirrus 0 / player 4, stranded set **0 of 0 stable**, tier **high** at boot and at both censuses (**no governor step**), epoch not exposed. **THE READING: at rest, a fresh Owens on the ON arm SETTLES at 140 ≤ 261 with NO stranding — the first SETTLED Owens ON number of the round, inside the frozen ceiling with room.** The row's 279 / dl 8 is **the POST-SWEEP state** — 465 tiles carried from Powell by `keepResident` / `parkOffscreen`, 303 parked, visible 80, non-terrain ≈ 195 against ≈ 44 at the fresh pose — **whose causal chain (a source rebuild during the 900 s sweep stranding 8; carried residency letting the content pipeline finalize) is INFERRED FROM SOURCE AND NOT MEASURED**: the re-read did not replicate the sweep and saw no tier step. **RULINGS**: **the frozen 261 does not move**; **gate 7 reads NOT CALIBRATED when an arm did not settle** (D's proposal, the (18) shape; E's scripts-only change, RED = the w6 numbers); **no `LODThreshold` change**; and **the row's 279 stands as an OPEN POST-SWEEP FINDING on the fixture's Owens — not a certified breach and not a pass.** **AND A CORRECTED ITSELF ONE LAST TIME, AGAINST ITS OWN FLATTERING VERSION** (`d0aba81`, merge 56 `38d4948`): E's fresh census printed **stamp 141 AND meshesInView 92 — both terms, for the first time — so A's visibility STAMP OVERSTATES what three actually draws by about a third** (it reads the last walk's `_inFrustum` flags while three culls on live matrices). **A's post-sweep bound "terrain ≤ 80" was therefore LOOSE**: the real terrain term is plausibly nearer **55**, **the non-terrain share of the 279 is LARGER than A argued**, and **the no-change ruling on `LODThreshold` stands on a WIDER margin than the one A gave** — with A's ledger recording the 279's causal chain as **INFERRED FROM SOURCE, NOT MEASURED**. **A is done for the round** — node-only, nothing in flight, its three R25 items standing as recorded. **E'S THREE READINGS, and what E would not fake**: the governor TIER is **not recorded by the row and is unrecoverable for that run**, so E read it LIVE at Owens with the trio ON — **high at boot and at both censuses 60 s apart, no governor step, no source rebuild** — and **two columns E declined to invent**: `map._epoch` (the epoch lives on the ROOT tile; the per-tile dirty scan read `n._root._epoch` correctly) and `__flyGov.state()` (a FUNCTION, read as a field). **THE DECISIVE READING, in E's words**: *"Warped straight to Owens from boot with the trio ON, the pose reaches dl 0 with zero version-dirty tiles by A's own getter's rule and settles at 140 draws, resident 225. The gate's Owens, reached after a 900 s sweep and a Powell pose, capped at dl 8, 279 draws, resident 509. Same pose, same flags, same venue, opposite outcome. The streamer resting is not a property of the POSE; it is a property of the HISTORY — exactly the shape of a trigger requiring an epoch bump after tiles loaded."* **AND THE DECOMPOSITION'S CORRECTION**: even fully settled, A's `_inFrustum` stamp reads **141** while only **92** tile meshes are inside the live frustum — **so "visible 80" at Owens ON is a STAMP, not a drawn count, and differencing it against draws is TWO DIFFERENT INSTRUMENTS, not a decomposition**; the class census rules out new geometry (satBuilding, skyline, roads, clouds, cirrus **all zero** at Owens; player 4), and **the remainder scales with the RETAINED resident set — `keepResident` doing what it says**. **A'S §23a PUTS THE FIGURE AGAINST THE RIGHT DENOMINATOR** (`e2aae15`, merge 58 `4c2e4d5`): 141 against 92 is **49 extra — 35 % of the STAMP is not drawn, a 53 % OVERCOUNT relative to the DRAWN count**, and **53 % is the number to carry wherever the claim is phrased that way**; A's reading used **drawn ≤ stamp, a genuine BOUND**, so **non-terrain ≥ draws − stamp still holds and E's measurement makes it TIGHTER** (terrain nearer 55; the no-change ruling on `LODThreshold` stands on more room) — **a bound built from two instruments is sound where an EQUALITY is not**. And **A retracts "186 an underestimate": it was as inadmissible as 279.** **GATE 7'S ADMISSIBILITY CHANGE** (`r24/e c73b3b1`, merge 57 `40ef1e0`, scripts-only; with `52cfa2a` making flicker print the one number that decides D's prediction): **an arm that did not converge contributes NOTHING to the ceiling clause**, and with neither converged the gate reads **NOT CALIBRATED with both numbers and their status** — with E's framing correction that **it was never only the ON arm: BOTH Owens censuses capped (OFF dl 6, ON dl 8), so 186 was exactly as inadmissible as 279**. **261 and 375 stand FROZEN, and the change can only WITHHOLD a verdict, never grant one.** | PENDING — the residency trio's live draw evidence, and whether gate (6)'s repeat fetches appear on real bytes | the LIVE capture difference behind a merge; a fixture draw count bounds nothing live |
...
| **the three NIGHT gates** — `verify-sat-night` / `-dusk` / `-flicker`, run because C's hill-follows-moon ruling moves satellite NIGHT ground pixels | R19/R16 pixel bands | **FIRST ATTEMPT VOID ON THE HARNESS, plus ONE REAL READING.** `verify-sat-night` **rc=1 after 172 s with ZERO gates run** — *"locator.screenshot: Timeout 23971 ms exceeded"* after *"waiting for element to be stable"*: the R16 harness's screenshot waits on Playwright's ACTIONABILITY check, **which a canvas re-rendering continuously at fixture frame rates never satisfies** — a venue-vs-legacy-harness mismatch, **not a lighting result**; E is adapting the screenshot path (`page.screenshot` with a clip, or the harness's own `readPixels` probe) and re-running all three. `verify-dusk` **rc=1 after 177 s** and got **ONE reading out before the same timeout**: *"pinned noon is the certified DAY sky (day bucket, no blend, env/bg exactly 0.85 / 1.0) — FAIL — state=day s=0 env=0.7382 bg=0.8801 el=67.954°"* — **both `SatEnvironment` intensities scaled by ~0.868 at a PINNED NOON where `moonK` is 0**, so it is **NOT** the hill-follows-moon change: **something that shipped ON this round dims the satellite environment BY DAY**, and since this frozen R19 gate never ran on an R24 flipped tree until tonight it has read that way **since the flips at `ec53fd3`**. **C CLOSED IT AT THE SOURCE, and it is NOT an R24 flag** (`739d3ee`, merge 40 a fast-forward): it is **a per-FRAME ramp starved by a ~0.5 fps venue**. `SatEnvironment.jsx:553` ramps both intensities with `k = 1 − Math.exp(−(delta > 0.25 ? 0.25 : delta) / SKY_LIVE.hdriFade.rampSec)` where `delta` is r3f's FRAME delta — **a per-frame exponential approach capped at 0.25 s of advance PER FRAME, not a wall-clock ramp** — and `git log -L` puts the clamp and `rampSec` 1.5 in R16's `4a1fe1f` and the `day: { env: 0.85, bg: 1.0 }` anchor in R13's `d72adb1`: **neither moved this round.** **The arithmetic says it twice**: env is 13.15 % short and bg 11.99 % short, implying **3.04 s and 3.18 s** of ramp time, i.e. **12–13 frames at 0.25 s each inside a 26 s wait — ~0.48 fps, exactly the SwiftShader regime**; and the two ratios are **NOT equal (0.8685 vs 0.8801) precisely because they ramp from different seeds toward different targets — one starved ramp measured on two channels, not two symptoms**. On a machine with frames, 26 s buys 26 s of ramp (`exp(−26/1.5)` = 3.3e-8) and the explicit snap two lines down lands on **EXACTLY** 0.85 / 1.0, which is what the frozen cell's "exactly" requires: *"the gate is a settle contract and the venue cannot satisfy it at half a frame per second, whatever is flagged."* **Every candidate cleared FROM SOURCE**: `LAMBERT_ENV` sets `material.reflectivity` on Lambert materials only (four named sites), per-material and never a scene scalar; `CLOUD_LIT` is a material; `ONE_SUN`'s `hill.dayK` is 1.0, the identity; **POST_ORDER and tone mapping cannot show in these numbers BY CONSTRUCTION** — the gate reads `__flyStats.envIntensity` / `bgIntensity`, written straight from `env` / `bgOut` at `SatEnvironment:598–599`, **pre-tonemap scalars before any pass runs**; `ENV_UNIFORM` is shipped OFF; `FRAME_STATS` adds no draw and no material state. **The decisive structural fact**: the ONLY writers of `scene.environmentIntensity` in the whole tree are `FlyScene.jsx:2468` (the TOY `<Environment>`) and `SatEnvironment.jsx:347`/`:594` — **nothing R24 added writes either scalar**. **WHAT CHANGES IS THE HARNESS** (E, scripts-only): a settle contract **waits on the VALUE, not the clock** — `await page.waitForFunction(() => window.__flyStats?.envIntensity === 0.85 && window.__flyStats?.bgIntensity === 1)` with a generous timeout, **strictly stronger than the 26 s sleep and immune to venue speed**, applied to **all fifteen legs** that sleep on the same assumption; **the assertions do not move — a venue adaptation of a settle, not a re-baseline**. C's offered `envTarget`/`bgTarget` publish was DECLINED by the orchestrator: the constants are known to the gate. **`verify-sat-night`'s FIRST w5 RUN: rc=1, 424 s, and the NIGHT LEG PASSED EIGHT GATES** (app `645d08c`, merge-42 harness, started 05:40:26, load 4.61 / 4.38 / 4.14): the sun pinned night (mix night 1 / day 0 / frac 0, hdri night), **13 of 16 road chunks ready** with 3 draping, road stats reported, the toy pipeline never built in satellite, the informational night road-pixel probe Δ **29.14/255** against a noise floor 23.47 (corrected **5.67**), and **draws 83 ≤ 375**. **Three reds, ALL attributed by E from source to the HARNESS meeting the venue, none of them C's night**: **(a)** *"night roads draw measurably"* read `layerΔ=0` (107 → 107) — **zero BY CONSTRUCTION**: the A/B waited 2,500 ms and read `__flyStats.drawCalls`, **which republishes every 60 FRAMES** (the file's own comment) — one second at 60 fps, **60–120 s at this venue's 0.5–0.9 fps** — so **both halves read the same stale publish whatever the roads did**; the pair now nulls the counter and waits for a FRESH publish. **(b)** *"eye AGL is the low-AGL worst case"* read agl **560** against a 600–1000 band — `pinScene` pins the aircraft to the ABSOLUTE world Y captured right after `warpToGeo` and `eyeAgl = pos.y − groundElev`, and **`groundElev` is only right once the DEM streams**: 792 − 560 = **232 m of ground where real Manhattan has ~10**, i.e. the FIXTURE's DEM — **a pose precondition the venue cannot hold**, now **NOT CALIBRATED with `pos.y` and `groundElev` printed beside it, and never a reason to widen the band**. **(c)** the NOON leg pinned the sun (mix day 1) and then died on `page.screenshot: Timeout 30000ms` — **the actionability wait was gone (merge 42), but the page-level capture itself has a 30 s budget a starved compositor exceeds**; a rendered frame is now **witnessed via rAF before the capture**, with a venue timeout. `verify-sat-night` also gains **the NOTCAL verdict and a gate count in its summary line** — the file that once printed "0 gates". **`sat-night2`, ON THE `3de0c50` HARNESS: rc=1 by the verdict convention, 2,063 s — 26 passed, 2 failed, 1 NOT CALIBRATED** (`scripts/r24-out/w5/sat-night2.log`, served app `645d08c`, started 06:02:29, load 4.17 / 4.12 / 4.02). **THE FIX MEASURED**: *"night roads draw measurably"* now reads **layerΔ=30** (141 → 111) where the old pair read **0 BY CONSTRUCTION** — the counter nulled and a fresh 60-frame publish awaited — with the retired instrument line printing layerDraws night 30 / day 15 against scene totals 141/111 and 192/177. **AND THE ONE THAT SPEAKS TO C'S NIGHT IS GREEN**: *"SUN DRIVES UNIFORMS ONLY — same meshes, same material, only the uniform moved"* **PASS** — night 8/8 meshes ready, noon 12/12, materials **1/1 same**, `uRoadNight` **1 → 0**. Noon roads near-invisible **PASS** (Δ 11.15 / noise 12.65 / corrected **0.00**); every fade-band leg **PASS** (solid ≤ 1985 m; mid-dissolve **0.537** at 2685 m; **0** by 3085 m; eviction on invisible geometry, gone 2.55 against solid 26.22 with noise 1.04; ring off by 3400 m); beacons placed 2 with `nightK` 1; layers unmount off-satellite; **zero page and console errors**. **NOT CALIBRATED**: the AGL precondition, agl **575** with `pos.y` 792 m and `groundElev` **217 m** printed beside it — the fixture DEM, exactly as attributed. The night road-pixel probe is an informational **WARN** (Δ 26.89 / noise 28.02, corrected **−1.13**, inside the noise). **THE TWO REDS ARE BOTH AT JFK, and the attribution is IN PROGRESS (E)**: *"JFK streams road chunks (ready ≥ 2)"* read **ready=1**, and *"runway edge lights baked into the road chunks (cls-7 verts present)"* read **cls7=0** with classes 1/2/4/5/6 present. **The orchestrator read them as ONE cause; E CHECKED, and the single-cause reading was WRONG — the attribution is E's, and it took arithmetic rather than another run.** **(1) `ready=1` IS the fifth clock**: the leg warps to JFK and **sleeps a flat 26 s** before reading `satRoads.ready` — 1,560 frames of streaming at 60 fps, **about thirteen here** — fixed like the others, with the dwell kept as a FLOOR and the leg then waiting on `ready ≥ 2` under a 240 s cap, reading **NOT CALIBRATED if the cap hits, because a ring that never streamed cannot testify about roads**. **(2) `cls7=0` is NOT downstream of it.** The fixture DOES synthesize `aeroway`, but it places **ONE airport per scene at a fixed offset from the scene centre** (`scripts/r24-fixture/features.mjs`: `alon = lon + r·0.35`, `alat = lat + r·0.28`), which for the manhattan scene lands at **40.8196, −73.8880 — 21.9 km from the JFK pose** — while `SAT_ROADS.ring` is `{ z: 13, r: 12000 }` in MERCATOR units (≈ metres / cos lat, per the block's own comment), reaching about **9.1 km of ground at latitude 40.64**: **the only runway in the scene is outside the ring by a factor of 2.4, so a perfectly streamed ring at JFK still contains no aeroway**. It becomes a **fixture-column NOT CALIBRATED with the distance and the ring reach named**, **GUARDED on `FLY_TILE_FIXTURE` so a LIVE run still FAILS on a missing runway — that regression is what the leg is for** — and the fixture gains an `aeroway` feature at the real JFK **next round**, because a fixture change mid-certification would move what every other Manhattan-scene row sees (§5b). **Merge 47 `85b2016`** (`r24/e 55f91e9`, scripts-only, smoke 17/17, pushed) carries it, **not yet on the served tree**; **`sat-night` is NOT re-run on this server** — its JFK and AGL legs report their new verdicts on the w6 server, in the order **depth-rt → sat-night → terra-live last**.The airfield pixel probe passed informationally (net sparks 4906). **`verify-dusk`, ON THE SAME HARNESS: rc=1 by the verdict convention, 1,086 s — 14 passed, 1 failed** (`scripts/r24-out/w5/dusk.log`, started 06:37:58, load 4.03 / 4.13 / 4.06). **THE HEADLINE CLOSES THE OPEN ITEM: pinned noon reads the CERTIFIED DAY SKY — day bucket, no blend, env 0.85 and bg 1.0 EXACTLY** (state day, s 0, el 67.954°, the kloofendal day HDRI, HalfFloat 1016) — **on E's value-wait, where the clock-only run had read 0.7382 / 0.8801**: **the starved R16 per-frame ramp C attributed from source is CONFIRMED ON THE VENUE as the whole cause, with the assertion unmoved.** Golden-hour strength and star weight **exactly 0** at noon, and the pinned noon frame is **indistinguishable from the live scene's own noise with the round switched off** (mean \|Δ\| **0.0510/255** against a control's 0.1198/255). **Blend ladder**: endpoints pure buckets, 8 steps of 1/N, monotone; the elevation re-keys exact at both ends (stars 0 above −4°, 1 below −12°, glow 0 outside the band); procedural weather deterministic per cell + bucket and shipped as `'procedural'` (cover 82). **el +4 (6.152°)**: the golden lobe warms the sun-side horizon **Δwarm 1.84** (R−B) and brightens it **Δluma 7.47** against a noise of 0.001. **P9 (el 2.064°, frac 0.047 < `nightFrac` 0.06 — the LEGACY rule would have called this night)**: **DUSK**, the night bucket refused, star weight exactly 0, glow **0.783** alive. **el −10 (−8.343°)**: night bucket, no blend, glow exactly 0, stars **0.564** — the sat-night contract. **Overcast dusk**: the lid is up (`uOvercast` 1.000) and **keeps a top-vs-horizon gradient Δluma 29.24, not one flat tan dome**. Zero page and console errors. **THE ONE RED**: *"the cirrus deck is mounted and costs EXACTLY +1 draw"* read armed **172** against parked **172**, Δ 0 — **the same draw-pair shape as sat-night's road A/B before E's fix** (a toggle, then a read of the 60-frame-republished `drawCalls`) — **attributed in progress to E as the SIXTH clock in lesson 90's series**, with **the R19 assertion (exactly +1 draw, 214 against 213 live) untouched**; the leg rides w6 or a standalone knob. **So the dusk column reads 14 of 15 on the venue, the pinned-noon contract met EXACTLY, and the one remainder an instrument pair.** **`verify-flicker`, ON THE SAME HARNESS: rc=1, 1,861 s — 4 passed, 2 failed** (`scripts/r24-out/w5/flicker.log`, started 07:00:54, load 3.89 / 4.14 / 4.03). **GREEN**: **(1)** the park census is complete — **44 tracked / 44 parked** (hero, traffic, tracers, clouds, cirrus, boats, plumes; 14 siblings, 13 instanced, 10 letters); **(4)** the R21-sanctioned informational count of pixels swinging > 120 luma between consecutive frames reads **urban 2** (max swing 121.7, one grid cell) and **suburb 0** (max 119) of **547,200** pixels against a bound of 32 — **the R21 FLICKER SIGNATURE, the 380 → 146 → 73 → 49 series, is ABSENT on the flipped tree**; **(4a)** the Powell control holds at **exactly 0**; **(5)** under `reversedDepthBuffer` the sat-tint polygon offset is authored FOR it (factor −2, units 2); **(6)** zero page and console errors, with the tint layer at the slope pose reading 54 polys / 124 verts / 6 chunks visible. **THE TWO REDS, attributed in progress (E), with the bound `FLICK_P99` = 12 NEVER MOVED**: **(2)** urban flicker **p99 29.491 > 12** over 12 settled frames (window B p50 **0.124** / p95 17.787 / p999 40.107 / max 52.5, `movingFrac` **0.1444**); **(3)** suburb **p99 15.01 > 12** (window B p50 **0** / p95 4.503 / p999 28.4, `movingFrac` **0.0872**). **THE READING (the orchestrator's, E confirming): window B's MEDIAN pixel is dead still — p50 0.124 and 0 — and the tail is the 14 % / 8.7 % of pixels that CHANGED inside the window.** A 12-frame window is **0.2 s on the user's machine and ~24 s on this venue**, long enough for tiles and chunks to stream in during it, and **the A→B decay says the scene is SETTLING, not settled** (urban p99 40.3 → 29.5 with `movingFrac` 0.95 → 0.14; suburb 46.5 → 15.0, 0.995 → 0.087). The quiescence probe **passed BEFORE window A** (0.008 / 0.0491) **but did not hold THROUGH window B**, and E's stated demotion rule — informational when `movingFrac` > 0.05 — **did not fire on window B's own fraction**, so E is reading whether the implementation keys on the PROBE rather than on the ASSERTED window; the fix shape is **quiescence held THROUGH the window** (re-probe and re-capture, a frame-witnessed cap, NOT CALIBRATED on the cap). **So: the p99 measured STREAM-IN, not flicker; the flicker signature itself is clean; the bound is untouched; and this is NOT a verdict on B's `CHUNK_FADE` / `HEAL_IN_PLACE` or A's `parkOffscreen` until the window is quiescent.** **E'S ATTRIBUTION CONFIRMS IT AND CORRECTS E'S OWN ACCOUNT**: *"Your first hypothesis is correct, and my earlier description of this gate to you was wrong. The demotion keyed on the PROBE, not on window B's own fraction."* The pre-fix sequence was **window A → a 20 s gap → a SEPARATE 3-frame probe looped until its `movingFrac` ≤ 0.05 → then window B (12 frames) asserted**, with `quiet` set by the probe and **never re-checked** — so the leg printed *"QUIET (0.008)"* from the probe **while the frames it asserted on measured 0.1444**. **AND THE REASON IS SAMPLE SIZE, NOT TIMING**: `movingFrac` counts pixels that moved AT ALL across the window, so a 12-frame window catches slower and more intermittent change than a 3-frame one — **a 3-frame sample cannot bound a 12-frame statistic no matter when it is taken** — and 0.008 admitted a window measuring **0.1444, eighteen times the number that let it in** (suburb's 0.0491 admitted 0.0872). **So both reds are unattributable to the product and the distribution reading stands.** **THE FIX (`66f61ba`) is one step stricter than the shape proposed**: the separate probe is **REMOVED** — each attempt grabs a FULL window and **the window that gets asserted is the very window found quiet**, so control and assertion are **the same frames measured the same way and cannot disagree by construction**, with retries printing the rejected window's own `movingFrac` and p99. **A SECOND DEFECT in the same block**: the never-quiet path printed a bare SOFT console line and **exited 0** — two never-quiet legs would have been **indistinguishable in a sweep table from two passes**, which is §6a again, *inside the escape hatch built for §6a* — it is now NOT CALIBRATED, reaches the exit code, and is counted in the summary. **`FLICK_P99` 12, `FLICK_QUIET` 0.05, `FLICK_QUIET_TRIES` 4, `FLICK_GAP_MS`, `FRAMES` and (4)'s R21 demotion DO NOT MOVE.** **Because the fix changes WHICH frames get asserted, the w5 numbers cannot carry forward: both legs re-run on w6 (31 min) and stand until then as ATTRIBUTED-NOT-MEASURED, not as reds.** **Merged with it**: `bbd9118` gives **the cirrus pair the same `freshDraws` fix — the sixth clock — with no standalone knob**, E's reason being that threading a skip path through four settle legs and three pixel A/B blocks of a frozen 900-line gate **risks a misplaced brace that misbehaves only when the knob is unset**, so 18 minutes once on w6 is cheaper; and `be31852` makes `depth-rt` print `eye` per pick, label the sweep-versus-assertion camera displacement as **EXPECTED MOTION** now that C confirmed atomicity, and dump C's `hits` / `rayHits` with family / converged / impliedDrop-vs-groundDrop **whenever a pick disagrees by more than 1 %**. **Merge 48 `9962d15`, smoke 17/17, pushed** — the served tree is still `3de0c50`'s harness, merges 46–48 waiting for the teardown behind the depth-probe APP file — and **the w6 order is depth-rt → sat-night → dusk → flicker → `terra-live` LAST on the converged settle, every row stamped `9962d15`.****W6 `sat-night`, ON THE FLIPPED TREE (`d960fdb`, `LOD_CROSSFADE` ON, with the JFK ready-wait and the AGL NOTCAL from merges 45/47), started 08:16:35: 27 passed, 1 failed, 1 NOT CALIBRATED.** **What the fixes measured**: the AGL precondition **PASSES this run** (agl **683**, inside 600–1000 — the fixture DEM read differently at this pose's read time; **the precondition met, not a change in the fixture**); *"night roads draw measurably"* **layerΔ 32** (153 → 121); **SUN DRIVES UNIFORMS ONLY** 9/9 night and 12/12 noon on ONE material with `uRoadNight` 1 → 0; **JFK STREAMS ROAD CHUNKS `ready=4` PASS — the fifth clock fixed and MEASURED**, where w5 read `ready=1`; **the runway cls-7 leg reads NOT CALIBRATED exactly as designed** (the fixture's only runway 21.9 km from the pose against a ~9.1 km ring reach, guarded on `FLY_TILE_FIXTURE`); beacons 2 with `nightK` 1; JFK total draws **158 ≤ 375**; and everything else that passed on w5 passed again. **THE ONE RED IS NEW ON THE FLIPPED TREE**: *"eviction happens on INVISIBLE geometry (A/B < 15 % of the solid Δ)"* — **gone 8.69 / solid 23.47 / noise 4.12**, against w5's 2.55 / 26.22 / 1.04 with the crossfade OFF. **Attributed in progress, with what is already established**: **(a)** D from source, exhaustively — `attachLodFade` has **two call sites** (`FlyScene.jsx:1019`, `prewarm.js:362`), **both inside `applyHillshade` on the TileMap's materials**, and `applyBendAnchorSat` (the sat-building drape) has **zero references to the `uLodFade*` uniforms**, so **the crossfade cannot change a building pixel**; **(b)** `fadeSec` 0.25 is a FADE CLOCK advancing `min(dt, 0.05)` per RENDERED frame — **a blend is a floor of five rendered frames at any rate, 2.5–10 s at this venue** — so **a 2–4 s capture pair routinely STRADDLES a blend**, its mid-blend texels interpolating between parent and child (**the orchestrator's own "one to two frames" was inverted; the correction is recorded**); **(c)** the tell is in the numbers — **noise rose ×3.96, MORE than gone's ×3.41**, gone/noise **2.45 → 2.11**, solid ×0.90: a same-state pair holds no eviction and no building change, **so a common ADDITIVE term lifted both pairs in lockstep and the eviction did not become visible** — and the fixture's per-(z,x,y) hue stamps make parent↔child texels **maximally different**, so the blend is more visible in this measurement than it would be on real imagery; **(d)** D's verdict pending E's frames is **an INSTRUMENT case** — the control's static-terrain premise meeting a feature that makes terrain **a time-varying surface for seconds** — with **the 15 % bound NOT to move** and the fix witnessing `terra.fades.active === 0` before each capture, **restoring the premise rather than widening the tolerance**; **(e)** what only E's frames settle is **WHERE the 8.69 lives** — spread across the tile field (the blend) or **on building footprints** (not the blend, and then the flip perturbed something indirectly, a different owner, and **D's one-line revert applies as recorded**). **D's honest gap**: no measured refine rate for a climb at 3400 m — **printing `fades.active` and `faded` at each capture converts the inference into a reading, and it is the same line that fixes the gate.** **ATTRIBUTED: IT IS E'S INSTRUMENT, NOT D'S FLIP.** **(1) THE FRAMES** (E's `scripts/r24-e-evict-diag.js`, PNGs and `evict-diag.log` under `scripts/r24-out/w6`) put **BOTH pairs' deltas across the WHOLE TERRAIN BAND, not on footprint cells** — the A/B band mean **13.0/255** with a 16×8 grid running 0.44 to 41.9, and the SAME-STATE noise pair **22.2/255** running 2.12 to 70.7. With the control rising FASTER than the signal (×3.96 against ×3.41, gone/noise 2.45 → 2.11) and D's source check closing the footprint branch, **a common ADDITIVE term lifted both pairs: the terrain surface changed between captures, and the eviction never became visible.** **(2) A §6 DEFECT INSIDE THE INSTRUMENT BUILT TO SETTLE A §6 ARGUMENT**, in E's words: the diagnostic's `fades` numbers **read null at all four captures** because it read `window.__flyTerra.fades()`, **which nothing writes** — D publishes the live object at **`window.__flyStats.terra.fades`, where `verify-lod-fade` has read it all along** — *"not a timing question, not a style-path question — mine."* **(3) THE WITNESS ALONE WAS NOT ENOUGH**: draw counts across the four captures at ONE pinned pose read **80 / 80 / 100 / 127** — **tile arrivals live inside the very window being measured** — so the witness before a capture requires **BOTH `terra.fades.active === 0` AND `downloading === 0`** (read the way A's converged settle reads it), **counted in RENDERED frames with a cap**, and **a capped wait is reported OUT LOUD as a capture on moving terrain whose A/B carries the terrain's own delta**. It is wired in `sat-night` before EACH capture of `abProbe` (a blend can start between the two), in `dusk` before the noon A/B series, and in `flicker` before each window opens **and deliberately NOT between a window's frames — a stillness wait inside the window would destroy the temporal statistic the gate computes**. **RED = `FLY_TERRAIN_WITNESS=0`, which reproduces the w6 numbers.** **NO BOUND MOVED**: the 15 % eviction ratio, `FLICK_P99` 12, `FLICK_QUIET` 0.05, dusk's 250 ms spacing, every luma floor (`r24/e 391956d`, merge 55 `0025614`, smoke 17/17, pushed; **the served app unchanged**). **(4) DECISION: the eviction leg is NOT re-run** — a 34-minute row for one leg — **it stands ATTRIBUTED with the RED reproducible**, and **D's confirmation stands in the record: the crossfade cannot change a building pixel, and the one-line revert was never triggered.** **W6 `dusk` CLOSES THE COLUMN: rc=0, 1,586 s — 15 passed, 0 failed** (tree `0025614` — `LOD_CROSSFADE` ON, the terrain witness before the noon A/B series, the cirrus pair on `freshDraws` — started 09:21:12, load 4.20 / 4.15 / 3.62). **THE CIRRUS DECK NOW COSTS EXACTLY +1 DRAW — armed 171 against parked 170, Δ 1 — the SIXTH CLOCK fixed and MEASURED**, R19's +1 assertion met **with nothing moved**, where w5 had read 172 against 172 BY CONSTRUCTION. **Pinned noon reads the certified day sky exactly again** (env 0.85 / bg 1.0 on the value-wait); the blend ladder, both elevation re-keys, the procedural weather, the golden band, **P9** (el 2.064°, **dusk not night**, stars exactly 0, glow 0.783), **el −10 night** (glow 0, stars 0.564), the **overcast lid** (`uOvercast` 1.000, gradient Δluma 29.24) and the error leg are all **green on the FLIPPED tree**. **So the dusk column reads 15 of 15 on the venue, EVERY earlier red an instrument's and now measured green, and the crossfade clean through every dusk leg.** **W6 `flicker` IS A PARTIAL ROW, and honestly so**: `rc=124` — **the runner's ROW TIMEOUT at 2,400 s**, which E's own rc-124 line says outright — on tree `0025614` (stamped `0adb010`, the same app) with the witness harness and the **quiet-window-IS-the-asserted-window** fix, started 09:50:48, load 4.53 / 4.27 / 4.10. **THE URBAN LEG IS MEASURED AND GREEN ON THE FLIPPED TREE**: two windows were **rejected on their own `movingFrac`** — 0.1703 with p99 29.288, then 0.0976 with p99 19.182, the gate saying *"the scene is still streaming; waiting and taking another FULL window rather than asserting on this one"* — and **the third was found quiet at 0.0417 and asserted: p50 0, p95 1.624, p99 6.138 ≤ 12, p999 20.87, max 55.8, swing pixels 0**, with the park census 44/44. **So `LOD_CROSSFADE` on Manhattan reads INSIDE R21's `FLICK_P99` bound with a factor-of-two margin, on the window the fix GUARANTEES is the quiet one.** **THE SUBURB LEG IS NOT MEASURED**: two windows rejected (0.1232 with p99 28.554; 0.0785 with p99 11.642) and **the third attempt was in progress when the row's wall clock killed the browser** — the honest quiescence search (full ~24 s windows at this venue, 20 s gaps, four tries) **plus** the terrain witness exceeds the default 2,400 s row budget here. **That leg is neither a red nor a pass.** **And the four rejected windows are worth reading as a result in themselves: they are the instrument DECLINING to assert on a streaming scene — every one of them is a window the PRE-FIX gate would have asserted on.** **Decision**: `terra-live` runs now, last, on A's converged settle; **`flicker` re-runs once more with `CERT_ROW_TIMEOUT=4800` as the final row if the session budget holds, and otherwise this row stands as URBAN MEASURED / SUBURB NOT MEASURED.****From here, `dusk` → `flicker` → `terra-live` last run on the WITNESS harness, and any leg whose witness CAPS prints "capture on moving terrain" and is recorded as MEASURED-ON-MOVING-TERRAIN — neither a pass nor a fail.** **ALL THREE NIGHT ROWS NOW EXIST — sat-night 26/2/1, dusk 14/1, flicker 4/2 — and every red among them is an INSTRUMENT UNDER THE VENUE, none a defect of the flag it certifies; three re-runs ride w6 where they are cheap.** | **PENDING** — the night look on a real display, and whether the day dimming is visible |
...
- **Give the fixture an `aeroway` feature at the REAL JFK** (next round, not
  now): the generator places one airport per scene at a fixed offset from the
  scene centre, 21.9 km from the JFK pose against a road ring that reaches
  ~9.1 km — so the runway-lights leg cannot be satisfied on this fixture at all.
  **A fixture change mid-certification would move what every other Manhattan-scene
  row sees**, which is why it waits (§4.3).
- **R25: A's park-before-return fix** — a version-dirty tile returns in `_update`
  BEFORE `_inFrustum` is recomputed and before `r24Park` runs, so a stranded tile
  is frozen in its last visibility (≤ 8 draws, one line).
- **R25: a "post-sweep warp" pose in `verify-terra-live` with a PER-CLASS draw
  split** — the state the user actually flies into, since a fresh Owens settles at
  140 while the post-sweep Owens reads 279. **And the note that goes with any
  Owens number**: the fixture's Owens carries **149 toy chunks**, and the live
  desert the 261 was frozen against carries near zero by R20's lock — **so the
  Owens ceiling on the ON arm is readable only on the user's machine.**
- **R25: ONE measurement separates the phantom-download hypotheses** — a
  version-dirty scan immediately after a tier step, which
  `r24-e-owens-decomp.js` already knows how to do, ten minutes. The probe built
  for it came back NOT CALIBRATED (no downloads issued at all after a real
  `maxLevel` 17 → 16 → 17 move), leaving two live: **the source was not rebuilt in
  the running app**, or **it was and every invalidated tile was already correct** —
  the second being A's own precondition, and **cutting against A's use of the
  mechanism, since re-entry without downloads does not explain a stuck count of
  8** (§4.2).
- **R25: a tier step can leave a PERMANENT phantom download count on any
  machine** (A, from source, flag-independent and upstream): a tile whose content
  is already correct returns false from `loader.update`, so `_updateModel` never
  advances `_loadedEpoch`, `_needVersionUpdate` stays true and `_update`
  re-enters it on every walk forever — triggered by any `_epoch++` after those
  tiles loaded, including **the governor rebuilding the imagery source on a tier
  step** (§4.2). It is why a settle criterion of `downloading === 0` can never be
  satisfied on such a tree — **and the criterion refusing to certify a tree that
  never goes quiet is the criterion working**; a "quiescent apart from N stranded
  tiles" amendment was declined mid-certification.
- **R25: a soak that reads whether the resident set SATURATES or LEAKS past one
  revolution.** The standalone's growth column ends with the ON arm at 461
  resident and still climbing (113 → 461 across 607°), the OFF arm at 241 and
  also unsaturated — **so this is not the trio's** — and no gate asserts
  saturation, which is exactly the question the column was added to answer.
- ~~8 genuine refetches on a pure yaw~~ **ATTRIBUTED** (A, §22 as corrected by
  §22a): DISCARDED refines — 74 starts against 72 commits — of which **ONE
  re-bought its four children as 4 imagery + 4 DEM**, the other duplicating
  nothing because it never re-refined before the sweep ended. **The R25 item is
  the DISCARD, not the retry**: a switch-gated vendored patch committing
  already-downloaded children and letting the merge policy decide later, **not
  built this round for want of a measurement budget** (§4.2). **With it, two E
  items for R25, both queued scripts-only and neither re-running in w6**:
  `verify-lod-fade` will dump the classified duplicate LIST — kind / z / url /
  count / bucket — **and print the classification RULE beside it, so a reader can
  re-classify by hand and DISAGREE** (*"a reader with the list alone has to trust
  the classifier, which is exactly what let a wrong count stand through two rounds
  of derivation"*); and it will print D's `noParentMapFirst`
  (`{z, x, y, hasModel, matCount, matName}`) **so P3's attribution half becomes a
  READING**. Two facts ride with them: **E's raw `byUrl` did not survive the
  teardown**, and E's earlier reply to A — *"whether any of the 8 were DEM is
  unrecoverable"* — **was true of what survived and WRONG as a conclusion, because
  the printed aggregate (15 duplicates in the window) constrained it all along:
  the number that refuted §22 was on screen in E's own log**; and **one blind spot
  survives A's correction — a discarded child at z ≥ 16 re-requests the z15
  ancestor and lands in SHARED by construction, so "no DEM refetch appeared" is
  never evidence on its own** — a property of the classification, not of the
  streamer.
...
- ~~Residency needs an eviction policy with a cap~~ **CLOSED TWICE** — first
  with `parkOffscreen` and a constant cap (A `dead5e5`, pushed in `83462eb`),
  then with **the adaptive cap** (`f3cf43b`) after the constant one was measured
  BINDING on this venue; the standalone then read **9/0 with every ON-arm counter
  at Δ0** (§4.2): `parkOffscreen` stops issuing off-frustum tiles and
...
6. **A per-frame budget is a per-frame-RATE contract**; at 1 fps it is a
   ...
   (E CERT.)
6f. **A shared file edited from a STALE BRANCH is a silent revert.** E's branch
    predated A's `SETTLE_CONVERGE`, so editing `verify-terra-live.js` on that base
    **would have reverted A's converged settle inside E's own commit — no
    conflict, no warning, git merging disjoint hunks with no marker.** E caught it
    by **grepping the file for A's newest symbol before touching it**, and merged
    the orchestration tip first. **The round paid for this class twice, from both
    sides**: A's duplicate `budgetK` import at merge 15 was the mirror image — an
    addition where this would have been a deletion. *"The only cheap defence is to
    grep for the other owner's newest symbol before you edit."* (E CERT and
    A PACE, jointly.)

6a. **A SETTLED NUMBER AND A LIVED NUMBER ARE DIFFERENT NUMBERS.** A's sentence
    for the round, over anything else in its ledger: *"Every ceiling this round
    was certified on a FRESH WARP, and the 279 came from a pose reached the way a
    player actually reaches one — fly, turn, go somewhere else. It was 2× the
    settled number. The gate was not wrong; it was answering a question narrower
    than the claim it was being read as supporting."* A fresh Owens settles at
    **140**; the same pose reached by flying there reads **279** against a ceiling
    frozen on fresh warps. **Certify the pose the player arrives in, or say which
    pose the number belongs to** — and note where this leaves the round: **the
    user's machine reads the LIVED numbers first.** E reached the same place from
    the other side, and the sentence belongs beside A's: *"the streamer resting is
    not a property of the POSE; it is a property of the HISTORY."* (A PACE, with
    E CERT.)

6b. **OUR GATES READ STATE; THESE DEFECTS LIVE IN TRANSITIONS.** A's sentence for
    the round, which E reached from the other side: *"what a tile IS gets
    censused every round; what a tile was in the middle of DOING was invisible to
    every instrument we had until someone read the transition by hand."*
    `_loadSubTiles` discarding four completed downloads, `de()` clamping to an
    ancestor past a source's ceiling, `setIndex` handed a container three wraps
    only when it is a plain Array — **three of this round's findings live in the
    MIDDLE of a method, and no census of what exists could have seen any of
    them.** **A fifth landed at the close**: a tile whose content is already
    correct returns false from `loader.update`, so its dirty flag never clears and
    the walk re-enters `_updateModel` forever — **a perpetual "downloads in
    flight" that no draining-queue account explains**. (A PACE, with E CERT.)

6d. **"No number I would ship."** A knob existed that would have cleared the
    failing gate — `LODThreshold` could cut ~23 % of drawn terrain and take the
    18 draws with it — and A refused it, because it **cuts the component that
    IMPROVED to pay for the one that grew**, trades terrain sharpness at the tier
    the user flies for a number on a venue whose Owens is not the live desert the
    ceiling was frozen against, and rests on evidence that does not support it:
    *"a fixture content layer grew" is not the evidence that moves it*. **A knob
    that clears a gate numerically is not a fix; naming the number you would NOT
    ship is part of the measurement.** (A PACE.)

6e. **Check whether your flag is even the variable before arguing the
    mechanism.** D was asked to defend the crossfade against a draw delta measured
    between `TERRA_PACE` OFF and ON — with `LOD_CROSSFADE` **ON in both arms and
    no pin** — so the flag under suspicion was constant across the comparison. The
    mechanism argument was still worth making and D made it three ways, but **the
    log header settled it first**. (D ATMOS.)

6c. **A CONSTANT is a signature.** The stranded count did not decay, drift or
    spike: it sat at 8 and 6 across a 600 s cap and 1,159 rendered frames, and
    **no account of a draining queue produces a constant** — which is what sent
    A into the source instead of into another wait. **When a number refuses to
    move, stop timing it and read what writes it.** (A PACE.)
...
89. **A validity test that a DEGENERATE value passes honestly needs a
    calibration with a known non-zero.** The new arbiter's `impliedDrop` is
    identically 0 for a hit that lies on the ray — which every CPU raycast hit
    does — so the check passed trivially and the truth silently fell back to
    un-bent geometry, off by exactly the ground drop projected along the grazing
    ray. `verify-c-flagoff`'s RED for it (forcing `valid=true`, reversing the
    sort) **could not catch a zero that was legitimately zero** — **in SOURCE a
    degenerate zero and a converged answer are indistinguishable**, so the
    calibration had to move to a geometry proof solved BACKWARDS from the failing
    pick, where the RED reproduces the measured error to 0.01 m. And the
    conceptual half, in the author's own words: *"applied to an unlifted cast it
    is circular … I removed the thing that was working"* — *"the rendered point
    shares the CPU hit's XZ"* is true for a rigid displaced POINT and false for a
    continuous SURFACE, and **one corrector cannot serve both**. **And the
    category error the author named when the eye print refuted his own defence of
    it**: *"that arbiter was not a BIAS, it was a SELECTOR — its validity test was
    trivially satisfied by every candidate, so nearest-valid-`t` was choosing
    among a pool on a knife edge, and a metre of camera travel could flip which
    surface won. A bias cancels between two readings of one instrument; a selector
    amplifies the smallest input change into a discontinuous output. I reasoned
    about the first and shipped the second."* Its shape is R21 §5's *"a probe
    green on a quiet boot is not a probe green under load"* — **an instrument
    whose answer is stable only while nothing moves** — and the field added to
    make "expected motion" a measurement rather than an argument is what decided
    it. (C LIGHT; pairs with 86.)

90. **Four clocks where the venue needed frames.** `SatEnvironment`'s per-frame
    ramp read as a day-dimming regression; `terra-live`'s `settleDraws()` slept
    30 s and censused an unstreamed tree; `verify-sat-night`'s draw A/B waited
    2,500 ms against a counter that republishes every 60 FRAMES, so both halves
    read one stale publish; and `page.screenshot`'s own 30 s budget killed a leg
    on a starved compositor. **Every one was a duration standing in for a
    condition** — and on a venue at half a frame per second a duration is a
    measurement of the venue. Wait on the value, the frame, or the publish.
    (E CERT; the round's fourth instance, pairing with 85 and 88.)

91. **Two reds at one pose are not evidence of one cause — and the cheap
    confirmation is arithmetic, not another run.** A single-cause reading tied
    `cls7=0` to a ring that had not streamed; the fixture's own generator says
    otherwise, placing its one airport 21.9 km from the pose against a ring that
    reaches ~9.1 km, so **no amount of streaming could have put a runway in that
    ring**. In E's words: *"if I had accepted the single-cause reading, fixed the
    clock and re-run, `cls7=0` would very likely have gone green anyway once the
    ring streamed — a runway-free neighbourhood rendering correctly — and the leg
    would have passed having proved nothing."* **A green that arrives because the
    scene has nothing to fail on is the most expensive kind**, and thirty seconds
    of geometry caught it where a 34-minute re-run would have hidden it.
    (E CERT; pairs with 90.)

92. **I read the block's own comment as if it were the code — and I wrote
    both.** E's account of `verify-flicker` to the orchestrator was that it
    asserts on window B and demotes when `movingFrac` > 0.05, "structurally
    immune"; the implementation set `quiet` from a SEPARATE 3-frame probe and
    never re-checked it, so the leg printed QUIET (0.008) while the frames it
    asserted on measured 0.1444. **The comment described the guarantee; the code
    delivered a weaker one.** And the deeper half is sampling, not timing: a
    3-frame sample cannot bound a 12-frame statistic no matter when it is taken,
    because the longer window catches slower and more intermittent change — so
    **the control must be the very window that gets asserted**, which is what the
    fix makes true by construction. (E CERT; pairs with R20 §5's "it matched
    prose as code".)

95. **A §6 defect inside the instrument built to settle a §6 argument.** The
    diagnostic written to decide whether a red belonged to a feature or to a
    harness read `window.__flyTerra.fades()` — **which nothing writes** — and
    printed null at all four captures, while the owner publishes the live object
    at `window.__flyStats.terra.fades`, **where the round's own `verify-lod-fade`
    had been reading it all along**. In E's words: *"not a timing question, not a
    style-path question — mine."* **The failure mode this record names most often
    is the one that reappears inside the tool built to catch it**, and the fix is
    the same every time: read the handle the owner publishes, and print what you
    read. (E CERT; pairs with 92 and with §6's own lesson.)

96. **A stillness witness must witness everything that moves — and it must never
    run where the statistic lives.** Waiting for `terra.fades.active === 0` was
    not enough: draw counts across four captures at ONE pinned pose read
    80 / 80 / 100 / 127, because **tile arrivals live inside the very window being
    measured**, so the witness takes `downloading === 0` as well, counts rendered
    frames, caps, and **says out loud when it capped** — a capture on moving
    terrain whose A/B carries the terrain's own delta, which is neither a pass nor
    a fail. And the boundary: it runs BEFORE a window opens and **deliberately not
    between a window's frames**, because a stillness wait inside the window would
    destroy the temporal statistic the gate exists to compute. (E CERT.)

93. **A discard is a refetch you have not seen yet — and I derived a count from
    a mechanism I liked without checking it against a number I had not chosen.**
    Work thrown away AFTER it completed appears in no wasted-request counter
    until the second purchase: children downloaded and discarded because their
    tile left the frustum mid-download, surfacing later as duplicate URLs.
    **Count STARTS and COMMITS separately** — 74 against 72 — and **ask which
    direction the venue biases the number** (a real network makes the discard
    window LONGER, so the fixture is the favourable case and the FAIL is honest).
    But the sharper half is A's own, verbatim: *"I derived a count from a
    mechanism I liked and never checked it against a number I had not chosen, and
    the 15-duplicate line was on screen the whole time."* Two discards would have
    meant SIXTEEN duplicate URLs, because every re-bought child fetches imagery
    AND DEM; the window reported fifteen in total. **A derivation that never
    meets an independent total is a hypothesis wearing a number.** (A PACE.)

94. **Two honest counters that disagree by a small integer are not a discrepancy
    to reconcile — they are a measurement to read.** One wrapper counted refines
    STARTED at the top of `_loadSubTiles`; another hook counted refines
    COMMITTED, returning early on `aborted` without incrementing. 74 against 72,
    and **the 2 is the discard rate** — the integer neither counter was built to
    count, and it exists only because that early return exists. **Two owners'
    instruments composed into a third measurement neither designed.** (A PACE
    and D ATMOS jointly; pairs with 93.)
...
**CLOSED AND NOW CONFIRMED ON THE VENUE**: on E's value-wait the pinned noon reads
**env 0.85 and bg 1.0 EXACTLY**, where the clock-only run read 0.7382 / 0.8801 —
so C's source attribution was the whole cause and **the frozen assertion never
moved**. It was **the venue starving an R16 PER-FRAME ramp** — `SatEnvironment.jsx:553` advances both
...
| `DEPTH_FIX` | node proof (`depth-roundtrip-proof`: RED flat **0.176–0.177** CoC, viewZ **−2.50 m**; GREEN error 0.000000; the mirror proven by EXTRACTING three's own formula, 8,004 comparisons bit-identical) + `verify-depth-offset` **7/7**; the **hook is proven present and published** (pass 2b: `__flyDepthProbe` present, `__flyDof` true) and **THE DOUBLE UN-REVERSAL IS GONE ON THE VENUE**: the probe reconstructs **1207.14 m against a 1171.4 m truth**, in the kilometre regime and nowhere near the 2.50–2.51 m RED signature, over **12 truth hits that passed C's own falsifiers** (residual ≤ 0.007 m, reprojection 0.00 px; the iterative un-bend converged in 2–4 passes and **has since been REPLACED** by C's `81d803a` arbiter, which takes the truth on the original ray at the hit's XZ — reprojection 0 by construction, no iteration at all). **All three open items are CLOSED AT THE SOURCE** (C `a44f4b7`): the far pick was the un-bend's own near-field blindness (dual cast, nearest survivor), the 36 m gap was a surface moving between turns (`truth` is now a field on the probe result, same turn), and `cocSource` was published at an address the gate did not read. **The round-trip contract is restated with a MEASURED slope term** — `0.01 · abs(truth.viewZ) + truth.slopeMPerPx` — because one pixel at 1.2 km subtends metres of ground. **THE ROUND-TRIP IS NOW MEASURED AT BOTH ENDS**: nearest 35.94 m against a 35.9 m truth (**0.10 %**) and farthest 3210.50 m against 3210.4 m (**0.00 %**) on one frame, with 13 usable truths and the CoC source named — **CERTIFIED ON THE VENUE — 11/11 on the FLIPPED tree** (w6): 15 usable truths all converging via the restored bend-solved arbiter, the round-trip reading **0.00 / 0.00 / 0.01 %** where the degenerate arbiter had read +74 m, the CoC clause agreeing with the material's own formula at all three picks **independently of the truth hook**, the ordering clean and the spread above half the prediction, and the camera's 0.8–1.3 m of motion between sweep and read **printed and labelled EXPECTED** — so **buffer ↔ geometry AND DoF ↔ buffer are both proven, the collapse-to-`−cameraNear` signature is absent, and every red this row ever showed was the INSTRUMENTS', none of them `DEPTH_FIX`'s** — and the rebuilt gate proved the headline a SECOND way: **the DoF's CoC agrees with the DEPTH BUFFER within the 8-bit quantum at all three picks** (the material's formula at the PROBE's Euclidean distance: 0.8112 against 0.8118, 0.9713 against 0.9725), so **DoF ↔ buffer is PROVEN and only buffer ↔ geometry is open, on C's new arbiter**, whose degenerate `impliedDrop` (identically 0 for an unlifted hit) let the truth fall back to UN-BENT geometry and produced errors that are exactly the ground drop projected along a grazing ray (74 m at 2282 m, 121 m at 2667 m) — **C has CONFIRMED that reading in C's own words** (*"I got the geometry backwards … applied to an unlifted cast it is circular … I removed the thing that was working"*) **and FIXED it** (`ea30f64`, merge 46): the iteration is restored for SURFACES, rigid air-bent actors get `airDrop` with their family read off the material's own cache key, validity is convergence + residual + reprojection, and **the calibration moved out of the flag-off gate into a geometry proof that reproduces the w5 error to 0.01 m** (RED 74.02 m, GREEN 0.02 m, the log 74.01 m); c-flagoff 57 → 58 (§4.2). Earlier: the 205 m median was C's own truth hook rejecting the correct candidate — a ground-drop correction is perpendicular to a grazing ray and wrong by construction for an air-bent actor, so the class most likely to sit in front of terrain was the class guaranteed to be discarded (fixed at `81d803a`, the truth now taken on the original ray at the hit's XZ with reprojection 0 by construction; c-flagoff 54 → 57); and the CoC clause read the toy DoF's WORLD constants (focus 700 m, range 2600 m) at a 35.9 m pick, one cause producing both a false red and a vacuous green at 3210 m, with E rebuilding both clauses against the material's own formula (§4.2). **The re-run is pending.** Historically — the near and mid picks resolved to ONE pixel, the far pick hit a different SURFACE (a depth-writing object 34 m out, the player the obvious candidate), and CoC has no `cocSource` to attribute — re-run at the tail (§4.2). Historically: the re-take proves the hook and the settle (121 s, maxZ 15, the gate's first content run at K=40, `reversedDepth` true), but **(1) cannot be calibrated from the harness side AT ALL** — the page exposes the renderer and the scene and neither the camera nor `THREE`, so an out-of-bundle `Raycaster` can never establish a true distance. **the truth is now PUBLISHED with its own self-check** (C `514eddd`): `__flyDepthTruth` through the composer's own camera and scene over depth-WRITING geometry only, **un-bent by the live `uBendK`** because the CPU geometry is not the surface the depth buffer recorded, reporting `residualM` and `reprojectionPx` so a wrong space reads as a large number rather than a plausible distance; E asserts the probe against it within 1 % of the 2.50–2.51 m RED signature, reading the falsifiers first (§3 C, §4.2) | **ON**, merged `a60bf17` |
...
| `TERRA_PACE` {`timerFix`, `mergeHysteresis`, `keepResident`, `skirtFast`, `walkWhileSaturated`, `bboxCache`} | 22/17/178 → 0/0/0; timer 10/12 → 4/12; the saturated walk strictly conservative; skirt output element-identical; **live-arm fixture evidence now in** — merges 1 → 0, refetchParent 1 → 0, Owens **161 → 185 ≤ 261**, Powell **161 → 183 ≤ 375**, draws rising because `keepResident` keeps more tiles drawn. `verify-terra-live` (6)'s 1 → 17 repeat fetches are **ATTRIBUTED to the harness's ~51°/frame wall-clock yaw at 1 fps, not to the feature** — the LRU had nothing to evict (peak 30–36 % of a 140 MB cap) and merges were 0; re-measured in pass 2 at 0.85°/frame over a full 360°. **THE YAW CONTRACTS ARE CERTIFIED TWICE, AND THE OWENS CEILING IS OPEN AGAIN.** On the CONVERGED settle (w6, 8/1, both arms from a settled start): **merges 100 → 0**, **on-screen replacements 87 → 0**, repeat URLs **289 → 18**, content correct in both arms, every ON counter Δ0 with the adaptive cap never binding — **and Powell ON SETTLED in 34 s / 24 frames at 194 draws ≤ 375** — while **(7) reads Owens ON 279 > 261 — now ATTRIBUTED by all three owners and standing as an OPEN POST-SWEEP FINDING rather than a certified breach**: **the TERRAIN term FELL (≤ 80 against ≤ 120) while non-terrain rose**, because the trio **unstarves a content pipeline** that the OFF arm's constant merging had been making retry rather than finalize; **the crossfade is not even the variable** (ON in both arms, no pin); and **a FRESH Owens on the ON arm SETTLES at 140 ≤ 261 with zero stranding** — the first settled Owens ON number of the round. **The 261 does not move, no `LODThreshold` change ships, and gate 7 now reads NOT CALIBRATED when an arm did not settle** (§4.2). Earlier, on the constant-cap tree: **CERTIFIED AT A FULL REVOLUTION (9/0, both arms 671° and 607°, symmetry 90.5 %)**: **merges 74 → 0**, **on-screen replacements 59 → 0** — the user's "tiles swapping" measured to zero over a full turn — repeat URLs **301 → 24**, content correct per tile in both arms, **Owens draws 104 ≤ 261 with no re-baseline**, and every ON-arm counter Δ0 against an explicit pre-sweep census with the cap at 1722 against 461 resident, **so the earlier 47 merges were the OLD cap and A's adaptive cap removed them** (§4.2). Two readings ride with it: the resident set **had not saturated** when the sweep ended (113 → 461, and the OFF arm 81 → 241, so not trio-specific — an R25 soak item), and at Owens the ON census is **coarser** (38 % of the tris), **a ceiling pass and not a merit**, with A reading whether that is venue timing or a coarser leaf set. **The zero-merge headline's loss on the CAPPED tree is ATTRIBUTED AND FIXED** — merges 47 with 47 refetches were **A's own brake**, `maxResidentTiles` 260 having been sized on a 240-frame yaw that saturates at 190 while this venue's revolution holds more, so the LRU elected off-frustum subtrees and the return re-fetched them; **the cap now follows the drawn set** (`max(260, 14 × drawn)`, `k` sized on the worst measured retained/drawn ratio of 9.20), `parkOffscreen` is refuted from source as a cause, and **`replacedOnScreen` 65 → 0 held BY DESIGN because election is out-of-frustum only** (§4.2); and **the Owens draw number on the parked tree is STILL UNMEASURED**, the row having been killed by `run()`'s 2,400 s ceiling and re-running standalone (§4.2). Earlier, on the un-capped flipped tree: merges 35 → 0, on-screen replacements 27 → 0. **FIX LANDED, re-measure PENDING: the same row's ON arm read Owens 279 against the frozen 261** after a 600 s sweep (185 after 45 s in pass 1, same flags), with resident tiles 62 → 103 and `residentMB` 113.7 still climbing — **retention, not necessarily culling**, and **both requirements are met** — PATCH 26 `parkOffscreen` (off-frustum issued **142 → 0**, resident unchanged, the drawn set BOUNDED by the frustum instead of growing with sweep duration) and `maxResidentTiles` **260**, an LRU by last-visible frame with distance breaking ties, `verify-terra-residency` **22 → 32** RED-calibrated by neutering the park. **`keepResident` alone was the cause** (switch-by-switch, §3 A). **The 261 is not moved, and whether Owens lands under it is E's `terra-live` re-run to measure** | **ON** — `timerFix`, `mergeHysteresis`, `keepResident`, `skirtFast`, `walkWhileSaturated`, `bboxCache` all true; `skirtWorker` and `bendSphere` false, merged `5b13e35` |
