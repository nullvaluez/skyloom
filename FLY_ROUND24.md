...
`0.01·|truth.viewZ| + truth.slopeMPerPx`, the slope **measured by casting the
...
bounded concurrency, a 31-frame blend window — while TWO of D's six preconditions
fail on the ON arm** (the arc capped at 264°, the frame ratio 0.75 at the edge of
the ±25 % guard) **and one fade was denied for `noParentMap`, a defect-shaped
