...
89. **A validity test that a DEGENERATE value passes honestly needs a
    ...
    sort) **could not catch a zero that was legitimately zero**. And the
    conceptual half: *"the rendered point shares the CPU hit's XZ"* is true for a
    rigid displaced POINT and false for a continuous SURFACE — one corrector
    cannot serve both. (C LIGHT; pairs with 86.)
