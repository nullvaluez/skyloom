...
- **W3 (certification)** — and the round's worst hour. All five merged, 17/17
  ...
  **A `6aa2030`**, **C `961135c`**, **D `3f379ff`**, **B `8b68ae5`** (the
  ...
  with E's `FRAME_STATS` flip `6c26fe9` completing the set. **A dry-run merge of all five
...
| `verify-fixture.js` | running at K=200 with a 900 s settle cap per pose; **gates 1–3 PASS so far** — satellite boots the fixture in **124.1 s** (`pct 100` at **54.6 s**), img 74 / dem 74 / mvt 67 / tilejson 20 <!-- CERT:verify-fixture PASS1 PENDING --> | <!-- CERT:verify-fixture PASS2 PENDING --> | — | n/a — it certifies the venue |
...
- **`verify-terra-live` on a real GPU.** The FIXTURE column is now in (§4.2:
  merges 1 → 0, refetchParent 1 → 0, Owens 161 → 185 ≤ 261, Powell 161 → 183 ≤
  375) and the URL↔position half is answered — 0 mismatches in both arms — so
  what remains is the live draw evidence and the resolution of gate (6)'s
  1 → 17 repeat-fetch count.
...
34. **The constants an instrument DECLARES must be the ones it measured, and a
...
| `FRAME_STATS` | the round's only frame-pace instrument; flag-off byte-identical, and **flag-gated with no runtime pin**, so `verify-frame-pace` runs for the first time in pass 2 (flip `6c26fe9`) | <!-- FLIP:FRAME_STATS PENDING --> |
