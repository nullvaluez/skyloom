...
reason per row, the three exposed night gates last.
...
**The user is now testing `9db481d`** on the integration branch (`8240539` plus merges 13–22: the scripts-only ones, the import dedupe, **A's Owens fix**, C's dead-import removal, E's census repair, B's `__noFade` declarations + heal throughput floor, **C's night hill ruling** and E's waiting sun legs) — **`main` is at `9bf5f8a`** — **on `main` as well as on the integration branch**, `main` having been fast-forwarded to it at the user's request (§4.1) — pushed early, at their request, before
