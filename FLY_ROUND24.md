...
**The user is now testing `7540f20`** (`8240539`, plus merges 13–19: the scripts-only ones, the import dedupe, **A's Owens fix**, C's dead-import removal, E's census repair and **B's `__noFade` declarations + heal throughput floor**) — **on `main` as well as on the integration branch**, `main` having been fast-forwarded to it at the user's request (§4.1) — pushed early, at their request, before
