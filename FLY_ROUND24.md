...
**A boundary D put on the record before the pass-2 number exists:** A's yaw
...
**E's instrument-defect count for the session is TWELVE**, each with a defence in
...
41. **The constants an instrument DECLARES must be the ones it measured, and a
...
| `CHUNK_FADE` | pops 92 → 2, both attributable; `maxDying` 4 is the only draw term. **ON per the ship table; green leg pending the standalone RE-TAKE**, whose expected reading is on the record in advance — births SOFT, deaths spanning ≥ 3 samples, hard deaths ≤ `fadeBudgetMiss` (§4.2) — and which runs with B's frame-count ramp floor (`minFrames` 3) in | **ON**, merged `7face8f` |
| `HEAL_IN_PLACE` | heals 16 (0 in place) → 21 (all in place); evictions 40 → 24. **ON per the ship table; green leg PENDING ATTRIBUTION** with `CHUNK_FADE` (§4.2) | **ON**, merged `7face8f` |
