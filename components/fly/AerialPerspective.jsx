...
import { getEdgeFade, linearHazeOn } from '@/lib/fly/toy-world/world-bend';
 ...
 * depth reconstruction and introduces two shared atmosphere channels:
 *
...
void mainImage(const in vec4 inputColor, const in vec2 uv, const in float depth, out vec4 outputColor) {
  ...
  if (uLivingAir.x > .5) {
    ...
    float transmission=livingAirTransmission(length(metricRay),uCamPos.y-uGroundY,h);
    float authority=clamp(uMaxMix/.55,0.,1.);
    // Scale optical depth, not the final blend. A blend capped at authority
    // retains 35% ground even at infinite distance and cannot meet the sky.
    // Near-field first-order extinction is unchanged; the far limit is the
    // exact directional horizon shared with the cloud composite.
    vec3 transmittance=pow(vec3(transmission),vec3(.86,1.,1.16)*authority);
    // The curved world is finite. Finish extinction before geometry starts
    // fading, so its last pixels meet the same sky instead of the raw HDRI.
    // Only the distant band is affected; local terrain keeps optical depth.
    if(uR25Mode.x>.5)transmittance*=1.-smoothstep(uR25WorldFade.x,uR25WorldFade.y,dXZ);
...
export class AerialPerspectiveEffect extends Effect {
    ...
    super('AerialPerspectiveEffect', law ? lawFragmentShader : r25 ? enhancedFragmentShader : fragmentShader, {
      ...
      uniforms: new Map(law ? [
        ...
        ...(r25 ? [
          ...
          ['uR25WorldFade', new Uniform(new Vector2(1e9,2e9))],
  ...
  _updateR25(u) {
    ...
    const edge = getEdgeFade();
    u.get('uR25WorldFade').value.set(edge.startM * .6, edge.startM);
