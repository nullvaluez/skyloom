...
| `verify-shadow-bubble.js` RED leg | `R25_LIGHT=off` — the same file, no rig (`…-RED-pass1.txt`). Run on the pass-2 code: the two clauses whose TEXT changed afterwards are (2b) and (3b), and **neither verdict can move with the rig unmounted** — there is no live texel to publish and no rung to hold. Not re-run, to spare a shared container | **5 passed, 8 failed** — and every failure is the defect: the cascade **1500 m at 80 m AGL, 1.465 m/texel, the same as at cruise**; no rung; the AO radius **24.00 at cruise and 24.00 at the deck**; `hemi.groundColor` never written; the night key 0.09 at every phase; nothing on the bus. The five passes are the rig-absent checks, the cruise cascade (1500 on both trees, by design) and the desert ceiling (**draws 56 ≤ 261**) |
| `verify-shadow-bubble.js` pass 3 | armed, with the SECOND-TRAVERSE instrument | **15 passed, 0 failed** — §2.2 |
| `verify-shadow-bubble.js` pass 4 | armed, re-run ALONE after E's `pkill` took the server down mid-pass-3 | **15 passed, 0 failed** again, on an independent program baseline (first traverse 121 → 122, second excursion **122 → 122 → 122**) and the same moon A/B (**0.045 → 0.180**). The saved artifact is this run; pass 3's file is kept as `…-ARMED-contended.txt` because a stale concurrent run was writing to it |
