...
(async()=>{
 ...
 try{
  ...
  const sample=async(label)=>{
    ...
    const s=await page.evaluate(()=>({review:window.__graphicsReview,gov:window.__flyGov.state(),night:window.__fly.satBuildings?.nightEnabled,position:{...window.__fly.flight.pos}}));
  ...
  r.status=r.errors.length||!present||!r.effectsBeforeDetail||!r.recovered?'FAIL':'PASS';
