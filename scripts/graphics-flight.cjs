...
(async()=>{
 const report={...require('./graphics-source.cjs')(),status:'BLOCKED',native:[1920,1080],scenario:args.scenario||'urban',errors:[],networkFailures:[],samples:[],transitions:[],seconds:duration};let browser;
 ...
 try{
  ...
  await page.addInitScript(()=>{localStorage.setItem('fly-map-style-2','satellite');localStorage.setItem('fly-quality-tier','high');localStorage.setItem('fly-sound-on','0');localStorage.setItem('fly-controls-seen','1');window.__flyWeatherOverride='baseline';window.__flySunOverride=Date.UTC(2026,6,18,4);});
  await page.goto((args.url||'http://localhost:3000')+'/?graphics=cinematic&graphicsReview=1',{waitUntil:'domcontentloaded',timeout:90000});
  ...
  await page.evaluate(()=>{
    ...
    const bench=window.__graphicsFlight={frames:[],gpu:[],longTasks:[],start:performance.now(),last:0,running:true,disjoint:0,loads:0,unloads:0};
    ...
    function tick(now){
      ...
      if(bench.last)bench.frames.push(now-bench.last);bench.last=now;
      ...
      f.pos.y=Math.max(f.groundElev+100,500+120*Math.sin(sec/20));
      if(ext){
  ...
  while(Date.now()<end){
    ...
    if(args.scenario==='mixed' && report.samples.length%18===0){
      const places=[[40.2083,-83.0701,600],[-37.683,144.582,500],[48.8579,2.301,600],[36.601,-118.06,1700],[40.7028,-74.017,500]];
      await page.evaluate(p=>window.__fly.warpToGeo(p[0],p[1],{altM:p[2],name:null}),places[(report.samples.length/18-1)%places.length]);
    }
  }
  const values=await page.evaluate(()=>{const b=window.__graphicsFlight;b.running=false;b.observer?.disconnect();return{frames:b.frames,gpu:b.gpu,disjoint:b.disjoint,longTasks:b.longTasks}});
  ...
  report.status=report.errors.length||absent?'FAIL':unsuitable||!budgetsMeasured||missingTiles||absentTraffic?'BLOCKED':report.timing.p95<=20&&report.timing.p99<=33.3&&budgetsPass?'PASS':'FAIL';
