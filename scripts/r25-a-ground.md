...
| **E CERT** | **`R24_WORKER_TAIL_RE` is FIXED here** (§6.5) and your r25/e gate 2b goes 1 of 2 → **2 of 2** on this branch. Two consequences for you: (a) this tree's own copy of `verify-skirt-worker.mjs` still carries the OLD three-argument regex in its gate 2 and therefore still reads 8/1 — **your copy supersedes it at merge**; (b) `scripts/vendor-three-tile-integration.json` now carries one new reviewed entry (`r24SpliceWorkerTail (modified)`) and a new `index.js` digest, which is what returns `verify-vendor-three-tile` to 34/0. |
| **E CERT** | **`scripts/_boot.js:181` swallows `FLY_BOOT_SCALE`** — the options object sits in `waitForFunction`'s SECOND parameter, which Playwright reads as `arg`, so the boot-screen wait silently uses the 30 s page default. It killed two of my runs outright. §7.4. One-line fix: move the object to the third parameter. Every browser gate in the fleet is exposed to it under load. |
| **E CERT** | `lib/fly/r25-pins.js` now imports through `@/…` so `_alias-loader.mjs` gates can load any R25 module chain (§7.6). If you would rather fix it in `verify-lod-fade.mjs` / `verify-atmo-law.mjs` by registering `_node-resolve.mjs` instead, that works too and the constants change can be reverted — but ONE of the two has to happen or B, C, D and F will each rediscover it. |
| **Fable (W0)** | The three out-of-row touches are listed in §3 with reasons: `lib/fly/r25-pins.js` (3 import specifiers), `lib/fly/terrain-engine.js` (1 line, charter-assigned), `lib/fly/vendor/three-tile/index.js` + its receipt (charter item 6, handed to me by the orchestrator). |
