...
// A bounded venue wait reports missing readiness rather than pretending to pass.
const limitWait = ms => Math.min(ms, Math.max(30000, Number(process.env.FLY_PROBE_TIMEOUT_MS) || ms));
...
(async () => {
  ...
  // Windows' bundled headless shell can silently use software rendering.
  // Select the same hardware-backed Chrome channel as the flight harnesses;
  // other venues retain their bundled browser or explicitly select a channel.
  const channel = process.env.FLY_BROWSER_CHANNEL || (process.platform === 'win32' ? 'chrome' : undefined);
  const browser = await chromium.launch({ channel, args: ['--enable-gpu', '--enable-webgl', '--ignore-gpu-blocklist'] });
  ...
  try {
    ...
    await page.addInitScript(() => {
      // Component regression: isolate the saved sky branch. Full Painterly
      // acceptance is measured separately by verify-painterly-browser.cjs.
      window.__flyPainterlyOverride = 0; // requires ?graphicsReview=1
      try {
    ...
    const boot = await bootFly(page, {
      style: 'satellite', timeoutMs: limitWait(1800000),
    ...
    const measurePose = async (tag, P, { lumaLeg = true, drawLeg = false } = {}) => {
      const { readiness, ms } = await warpToPose(page, P, { sun: 'noon', timeoutMs: limitWait(900000), pollMs: 5000, pin: true, pitchRad: P.pitchRad ?? -0.1 });
    ...
    if (LEGS.includes('cloud')) {
      ...
      const { readiness } = await warpToPose(page, P, { sun: 'noon', timeoutMs: limitWait(900000), pollMs: 5000, pin: true, pitchRad: -0.06 });
    ...
    if (LEGS.includes('look')) {
      ...
      for (const [tag, hdg] of [['toward', az + Math.PI], ['away', az]]) {
        ...
        const { readiness } = await warpToPose(page, P, { sun: tMs, timeoutMs: limitWait(600000), pollMs: 5000, pin: true, pitchRad: -0.05 });
        if (!readiness?.ready) {
