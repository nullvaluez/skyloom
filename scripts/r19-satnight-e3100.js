/**
 * R19 — WHAT still churns the (E) ground crop at 3100 m after the v3 park?
 *
 * Reproduces verify-sat-night's (E) climb (2000 → 2700 → 3100, same pose, same
 * dwells), applies the harness's v3 park + foreground hide, then measures the
 * NO-TOGGLE band delta (the A/A "noise" the gate divides by) repeatedly under
 * successively wider parks. Whichever park collapses the noise names the actor.
 *
 * Configs, applied cumulatively:
 *   0  harness v3 park only (clouds + siblings + cirrus + traffic root +
 *      tracer ribbon + beacons) + setForegroundVisible(false)
 *   1  + POI letters (troika Text children of the scene-root letter groups —
 *      the frame loop rewrites group.visible every frame, so the park has to
 *      go on the child MATERIAL, the CloudField lesson)
 *   2  + the satellite skyline mass (window.__satSkyline.object)
 *   3  + SatCityGlow dome/core
 */
const { chromium } = require('playwright');
const { bootFly } = require('./_boot');

const NOON_MS = Date.UTC(2026, 6, 17, 17, 0, 0);

const pinScene = ([lat, lon, altM, heading, pitch]) => {
  window.__fly.warpToGeo(lat, lon, { altM, name: null });
  const f = window.__fly.flight;
  f.heading = heading;
  f.pitch = pitch;
  f.bank = 0;
  const p = { x: f.pos.x, y: f.pos.y, z: f.pos.z };
  if (window.__pin) clearInterval(window.__pin);
  window.__pin = setInterval(() => {
    f.pos.x = p.x;
    f.pos.y = p.y;
    f.pos.z = p.z;
    f.heading = heading;
    f.pitch = pitch;
    f.bank = 0;
    f.speed = 0;
  }, 8);
};

(async () => {
  const browser = await chromium.launch({
    channel: 'chrome',
    headless: true,
    args: ['--enable-gpu', '--ignore-gpu-blocklist'],
  });
  const page = await browser.newPage({ viewport: { width: 1600, height: 900 } });
  page.on('pageerror', (e) => console.log('PAGEERROR', e.message));
  const canvas = () => page.locator('.fixed.inset-0 canvas').first();
  const shot64 = async () => (await canvas().screenshot()).toString('base64');
  const bandDelta = (a, b) =>
    page.evaluate(
      async ([sa, sb]) => {
        const load = (s) =>
          new Promise((res, rej) => {
            const i = new Image();
            i.onload = () => res(i);
            i.onerror = rej;
            i.src = 'data:image/png;base64,' + s;
          });
        const [ia, ib] = await Promise.all([load(sa), load(sb)]);
        const w = Math.min(ia.width, ib.width);
        const h = Math.min(ia.height, ib.height);
        const y0 = Math.floor(h * 0.55);
        const y1 = Math.floor(h * 0.98);
        const bh = Math.max(1, y1 - y0);
        const grab = (img) => {
          const cv = document.createElement('canvas');
          cv.width = w;
          cv.height = bh;
          const ctx = cv.getContext('2d', { willReadFrequently: true });
          ctx.drawImage(img, 0, -y0);
          return ctx.getImageData(0, 0, w, bh).data;
        };
        const da = grab(ia);
        const db = grab(ib);
        let sum = 0;
        let sparks = 0;
        let px = 0;
        // Per-16th-row histogram: WHERE in the crop the change lives.
        const rows = new Array(16).fill(0);
        for (let i = 0; i < da.length; i += 4) {
          const d =
            Math.abs(da[i] - db[i]) +
            Math.abs(da[i + 1] - db[i + 1]) +
            Math.abs(da[i + 2] - db[i + 2]);
          sum += d;
          if (d > 75) {
            sparks += 1;
            const y = Math.floor(i / 4 / w);
            rows[Math.min(15, Math.floor((y / bh) * 16))] += 1;
          }
          px += 1;
        }
        return { mean: sum / Math.max(1, px * 3), sparks, rows };
      },
      [a, b]
    );

  await bootFly(page, { style: 'satellite' });
  await page.evaluate(() => window.__flyStore.getState().setQualityTier('high'));
  await page.evaluate((t) => {
    window.__flySunOverride = t;
  }, NOON_MS);
  await page.mouse.move(800, 450);

  // the harness's (E) history, verbatim
  await page.evaluate(pinScene, [40.7075, -74.0113, 2000, 2.6, -0.3]);
  await page.waitForTimeout(24000);
  await page.evaluate(pinScene, [40.7075, -74.0113, 2700, 2.6, -0.3]);
  await page.waitForTimeout(9000);
  await page.evaluate(pinScene, [40.7075, -74.0113, 3100, 2.6, -0.3]);
  await page.waitForTimeout(9000);

  const park = (cfg) =>
    page.evaluate((c) => {
      const out = {};
      const hide = (o) => {
        if (!o) return false;
        o.visible = false;
        if (o.material) o.material.visible = false;
        return true;
      };
      if (c === 0) {
        const cl = window.__flyClouds;
        hide(cl);
        hide(window.__flyCirrus);
        let n = 0;
        cl?.parent?.children?.forEach((o) => {
          if (o === cl || !o.isInstancedMesh) return;
          hide(o);
          n += 1;
        });
        out.siblings = n;
        out.traffic = hide(window.__flyTraffic);
        out.tracers = hide(window.__flyTracers);
        out.beacons = hide(window.__satBeacons);
        if (window.__flyPlayer) window.__flyPlayer.visible = false;
        let scene = window.__flyPlayer ?? null;
        while (scene && scene.parent) scene = scene.parent;
        scene?.traverse((o) => {
          if (o.isInstancedMesh && (o._isModel !== undefined || o._painted !== undefined))
            o.visible = false;
        });
      }
      if (c === 1) {
        let scene = window.__satRoads?.object ?? null;
        while (scene && scene.parent) scene = scene.parent;
        let n = 0;
        scene?.children?.forEach((g) => {
          if (!g.isGroup || !('popT' in (g.userData ?? {}))) return;
          g.children.forEach((ch) => {
            if (ch.material) {
              ch.material.visible = false;
              n += 1;
            }
          });
        });
        out.letters = n;
      }
      if (c === 2) out.skyline = hide(window.__satSkyline?.object);
      if (c === 3) {
        out.glowDome = hide(window.__satCityGlow?.dome);
        out.glowCore = hide(window.__satCityGlow?.core);
      }
      return out;
    }, cfg);

  const stats = () =>
    page.evaluate(() => ({
      sb: window.__flyStats?.satBuildings ?? null,
      sky: window.__flyStats?.satSkyline ?? null,
      fade: window.__flyStats?.satBldgFade ?? null,
      draws: window.__flyStats?.drawCalls ?? -1,
    }));

  for (let cfg = 0; cfg <= 3; cfg++) {
    console.log(`\n=== CONFIG ${cfg}: ${JSON.stringify(await park(cfg))}`);
    console.log('   stats:', JSON.stringify(await stats()));
    for (let i = 0; i < 5; i++) {
      const a = await shot64();
      const b = await shot64();
      const d = await bandDelta(a, b);
      console.log(
        `   A/A mean=${d.mean.toFixed(3)} sparks=${String(d.sparks).padStart(6)} rows=[${d.rows.join(',')}]`
      );
      await page.waitForTimeout(1200);
    }
  }

  await browser.close();
})();
