...
// Optional venue deadline. Missing readiness still reports NOT CALIBRATED;
// no visual/resource assertion changes and the historical default is retained.
...
(async () => {
  // Use the flight harnesses' hardware-backed browser on Windows. Assertions
  // and fixture inputs are unchanged; other venues may select their channel.
  ...
  try {
    ...
    await page.addInitScript(() => {
      // Saved ground component regression; the complete painterly profile has
      // its own production acceptance gate. Run with ?graphicsReview=1.
      window.__flyPainterlyOverride = 0;
      (window.__r22Unpinned ??= {}).__flyVisualsOverride = 'enhanced';
      window.__flyR25Sky = 0; // D-only column: C forced Classic even in Enhanced
    });
    const b = await bootFly(page, { style: 'satellite', timeoutMs: limitWait(1500000) });
    ...
    for (const P of POSES) {
      ...
      const { ms, readiness } = await warpToPose(page, P, { sun: 'noon', timeoutMs: limitWait(1200000 * Math.min(SCALE, 2)), pollMs: 5000, pin: true });
      r.readyMs = ms;
      if (!readiness?.ready) {
