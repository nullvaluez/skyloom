...
> re-run, and the user's machine has confirmed nothing yet. **MERGE NOTE
> (2026-08-12): R22.1 and R23 below were PARALLEL rounds, both closed against
> R22's 5d6c09d and merged together at push time — the combined R22.1×R23
> tree passed `npm run build` but has had NO cross-round harness sweep; the
> first session on this tree should run the R22.1 trio (flash-guard /
> frame-pace / step-clean) + R23's night gates before trusting both greens.**
>
> Also new — PARALLEL round, closed hours earlier the same day: **Round 23
> "Night Alive" is BUILT (2026-08-11):
