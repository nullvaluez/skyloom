...
export function isCargo(aircraft) {
  ...
  // Check common cargo airline callsigns
  const cargoAirlines = ['FDX', 'UPS', 'GTI', 'ABX', 'ATN', 'ADB'];
  if (aircraft.flight) {
    const callsign = aircraft.flight.trim().toUpperCase();
  ...
  // Check aircraft types commonly used for cargo
  const cargoTypes = ['B744', 'B748', 'B763', 'B77L', 'B77F', 'MD11', 'A306', 'A30B'];
  if (aircraft.t && cargoTypes.includes(aircraft.t.toUpperCase())) {
    return true;
  }
...
export function getAircraftIconType(aircraft) {
  ...
  switch (classification) {
    ...
    case 'special':
      return 'jet';
    default:
      return 'airliner'; // Default to airliner for unknown flying objects
  }
}
