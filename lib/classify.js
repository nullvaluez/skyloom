...
// R15 "Ground Truth": canonical exact type table. This module is a normal
// bundler module so it imports the source of truth directly; the worker keeps
// an inlined copy because it cannot import. NOTE: the warbird table is
// deliberately NOT consulted here — its archetype values (warbird-heavy,
// classic-transport…) are traffic archetypes, not members of this module's
// 9-shape icon domain.
import { EXACT_TYPE_CLASS } from './aircraft-type-tables';
...
export function isHelicopter(aircraft) {
  ...
  // EXACT designator only (R15). The old substring list matched 'R22' inside
  // 'SR22' and made every Cirrus a helicopter; ICAO codes are <= 4 chars so a
  // 2-3 char pattern can always swallow a longer, unrelated type.
...
export function getAircraftIconType(aircraft) {
  ...
  if (typeCode) {
    // R15 "Ground Truth": exact modern code wins outright. The helicopter and
    // military substring lists that used to lead this chain are GONE — they
    // were built from 2-3 char patterns ('R22', 'C17', 'C2', 'A6', 'E3'…) and
    // ICAO designators are <= 4 chars, so each pattern could swallow a longer
    // unrelated type (SR22→heli, C172/C177→military, C206/C208/C210/C25x→
    // military via 'C2', DA40→military via 'A4', DA62→military via 'A6',
    // BE20→military via 'E2', B212→military via 'B2'). Every code they carried
    // now lives in EXACT_TYPE_CLASS. The 4-char lists below survive as the
    // curated fallback tail — no doc8643 code contains another 4-char code, so
    // they are exact by construction.
    if (EXACT_TYPE_CLASS[typeCode] !== undefined) {
      return EXACT_TYPE_CLASS[typeCode];
    }
    ...
    const propTypes = ['C172', 'C152', 'C150', 'C177', 'C182', 'C185', 'C206', 'C207', 'C208', 'C210',
      ...
      'M7', 'AA5', 'TB20', 'TB21', 'DR40', 'PITA', 'VENT'];
    // R15: 'C25' removed — it swallowed the CitationJets C25A/C25B/C25C/C25M
    // (jets) two lists before the bizjet check. 'PC24' removed — the PC-24 is a
    // jet. 'M8'/'M9' removed — not doc8643 designators.
