...
const CLASSIFICATION_RARITY = {
  ...
  // R17 "Your Wings": the five FLY archetypes that had no base at all.
  //
  // Every key above this block is 2D-map vocabulary ('commercial', 'private',
  // 'special', 'government'…) from a classifier that was DELETED in round 9.
  // Fly feeds `_classification` = the worker's archetype string, so an
  // airliner, a bizjet, a Cessna, a glider and a drone all scored base 0 —
  // three quarters of everything in the sky was worth nothing, and a widebody
  // read the same as an unknown blip. The legacy keys stay (dormant, and the
  // provenance ledger verify-classify audits); these five are additive.
  //
  // Calibrated so nothing floods SPICY (which pings at legendary, 85):
  //   airliner 10 + A380's 40 = 50 rare — the everyday heavy is a nice find,
  //   not an alarm; jet 15 + GLF6's 35 = 50 rare likewise. The ONE new ping
  //   class is a civil aircraft squawking an emergency (airliner 10 + 7700's
  //   80 = 90 legendary), which is precisely when a spotter should be told.
  //   glider/drone sit high because they are genuinely uncommon on ADS-B and
  //   carry no type bonuses of their own — both still land short of a ping
  //   (drone 50 + the B6 category bonus 25 = 75 epic).
  //
  // Persisted history is NOT migrated: a spot's rarity is what it was worth
...
const TYPE_RARITY_BONUS = {
  ...
  C17: 30,
  KC135: 25,
  K35R: 25, // R16 — the real ICAO designator for the KC-135R ('KC135' never matched)
  KC46: 35,
  E3: 45,   // AWACS
  ...
  // Special aircraft
  AN124: 55,  // Antonov
  A124: 55,   // R16 — the real ICAO designator ('AN124' is 5 chars, unmatchable)
  AN225: 100, // Mriya (if it existed)
  A225: 100,  // R16 — ditto for the Mriya
  C130: 20,
  C30J: 20,   // R16 — C-130J Super Hercules (its own designator, not a C130 substring)
  ...
  G650: 35,
  GLF6: 35, // R16 — the ICAO designator ADS-B actually reports for the G650
  GLEX: 30,
  GL7T: 35,
...
//
// R16 convention: new bonuses land in BOTH tables; the legacy table is the
// provenance ledger verify-classify i2 audits. (Practically: EXACT_TYPE_BONUS
// is what the runtime reads, TYPE_RARITY_BONUS is what the gate diffs against
// — an entry in only one of them either fails i2 as "invented" or shows up in
// i6 as an unaudited move. Same key, same value, both tables.) The R16 batch
// is the FIVE unreachable designators the 5-char legacy keys never matched:
// K35R (KC-135R), C30J (C-130J), GLF6 (G650), A124 / A225 (Antonovs) — all
// 4 chars, so they only ever self-match among <=4-char designators and no
// other code's score moves.
const EXACT_TYPE_BONUS = {
  ...
  K35R: 25, // R16: the KC-135R's real designator — 'KC135' could never match
  KC46: 35,
  E3: 45,
  ...
  // Special aircraft
  C130: 20,
  C30J: 20,  // R16: the J-model flies under its own designator, not 'C130'
  A124: 55,  // R16: An-124 Ruslan — 'AN124' (5 chars) was dead in substring space
  A225: 100, // R16: An-225 Mriya — likewise
  ...
  // Business jets
  G650: 35,
  GLF6: 35, // R16: what ADS-B actually reports for a G650 (G650 is the model)
  GLEX: 30,
  GL7T: 35,
