 ...
 * press(key) the one-shot actions (F/C/P) that have no on-screen equivalent.
...
export class InputController {
  constructor() {
    ...
    // Round 17 (photo mode): wheel accumulator + a flag that turns a plain
    // button-0 drag into the free-look orbit (see setPhotoLook).
    this.photoLook = false;
    this._zoom = 0; // accumulated wheel deltaY, eaten by consumeZoom()
    this._dragLast = null; // {id, x, y} — relative-drag orbit tracker
    this._el = null;
    this._handlers = [];
  ...
  setSpeedPreset(preset) {
    ...
    const index = ['slow', 'cruise', 'boost'].indexOf(preset);
    if(index >= 0)this.press(String(index+1));
  }
  ...
  /**
   * Round 17 — photo mode. While on, ANY primary-button drag on the canvas
   * (left mouse OR a finger) feeds the same free-look delta channel RMB
   * already uses, so the photo rig orbits without a modifier and mobile gets
   * an orbit with zero new plumbing. RMB keeps working exactly as today.
   */
  setPhotoLook(on) {
    if (this.photoLook === on) return;
    this.photoLook = on;
    this._dragLast = null;
    this._zoom = 0;
    if (!on) {
  ...
  /** One-shot wheel scrub (accumulated deltaY px). Consuming clears it. */
  consumeZoom() {
    const z = this._zoom;
    this._zoom = 0;
    return z;
  ...
  /**
   * Round 17 (A5) — inject a synthetic EDGE press from a source that is not a
   * keyboard (the on-screen touch cluster).
   *
   * It writes into the SAME `_pressed` set the keydown listener writes, which
   * is the entire point: FlyScene's `consumePress('f' | 'c' | 'p')` state
   * machines — intercept toggle, cinema toggle, photo toggle, each with its own
   * guards (`!paused`, `autopilot.mode !== 'off'`, the photo/chase swap and its
   * `snap()`) — run UNMODIFIED and cannot drift from the keyboard behaviour,
   * because there is only one implementation of each.
   *
   * Edge, not hold: `keys` is deliberately NOT touched. `_pressed` is drained
   * by exactly one consumePress() call, so a double-tap is two presses and a
   * press nobody consumes is dropped by the next neutralize() — the same
   * lifecycle a real keydown gets.
   */
  press(key) {
    this._pressed.add(String(key).toLowerCase());
  }
  ...
  neutralize() {
    ...
    // Round 17: photo mode neutralizes the STICK, not the interface. It runs
    // this EVERY frame (the plane keeps flying while you compose, so the stick
    // must stay dead), and the orbit drag + the P key ARE the whole photo
    // interface — clearing them here would eat every camera delta and make P
    // un-pressable. Every other neutralize caller is unchanged.
    if (this.photoLook) return;
    this._pressed.clear();
    this.freeLook.active = false;
    this.freeLook.dx = 0;
    this.freeLook.dy = 0;
  ...
  setBrake(on) { this.touchBrake = !!on; }
  ...
  attach(el) {
    ...
    // Round 17: wheel = photo-mode zoom scrub. Accumulated unconditionally
    // (nothing else calls consumeZoom, so this is inert outside photo mode)
    // and PASSIVE — the fly canvas is a fixed full-viewport element, there is
    // nothing under it to scroll and nothing to preventDefault.
    on(el, 'wheel', (e) => {
      this._zoom += e.deltaY;
    }, { passive: true });

    on(el, 'pointermove', (e) => {
      // Round 17 photo drag: a RELATIVE client-position delta, not
      // movementX/Y — touch pointers report movement* as 0 on several
      // engines, and this same path has to serve finger and mouse alike.
      // Sits ABOVE the touch gate on purpose: in photo mode the stick is
      // neutralized every frame, so a canvas drag can only mean "orbit".
      if (this._dragLast && e.pointerId === this._dragLast.id) {
        const rect = el.getBoundingClientRect();
        this.freeLook.dx += (e.clientX - this._dragLast.x) / rect.width;
        this.freeLook.dy += (e.clientY - this._dragLast.y) / rect.height;
    ...
    on(el, 'pointerdown', (e) => {
      ...
      } else if (this.photoLook && e.button === 0 && e.isPrimary) {
        // Photo mode: plain drag orbits (see setPhotoLook).
        this.freeLook.active = true;
        this.freeLook.dx = 0;
        this.freeLook.dy = 0;
        this._dragLast = { id: e.pointerId, x: e.clientX, y: e.clientY };
        try {
          el.setPointerCapture(e.pointerId);
        } catch {
          /* capture unsupported — drag still works on-canvas */
        }
      }
    });
    ...
    on(el, 'pointercancel', (e) => {
      this.freeLook.active = false;
      if (!this._dragLast || e?.pointerId === this._dragLast.id) this._dragLast = null;
    });
  ...
  read() {
    ...
    return {
      ...
      throttleDelta: (k.has('+') || k.has('=') ? 1 : 0) - (k.has('-') ? 1 : 0),
      brake: k.has(' ') || this.touchBrake ? 1 : 0,
