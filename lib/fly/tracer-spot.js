...
export function spotPalette(L, P) {
  ...
  const ocGlow = K.overcastGlow * oc * v; // R18: overcast ADDS emission — WHITE light
  P.emitPeak = B.emitDay + (B.emitNight - B.emitDay) * n + ocGlow;
  ...
  // Both ends are luma 1, so the white share of the overcast glow re-weights
  // the hue without moving the brightness (R16: never a tinted day line).
  const ocWhite = P.emitPeak > 0 ? ocGlow / P.emitPeak : 0;
  for (let b = 0; b < BAND_N.length; b++) {
    mix3(P.bodyEmit[b], mix3(_tmp, BAND_N[b], WHITE, B.whiteDay * v), WHITE, ocWhite);
...
/**
 * Prominence follows REACH (TRUE metres). `body` weights vapour and emission:
 * full within fullM, a faint floor (bodyFar) by farM. `glint` carries the
 * head dot, head boost and white core: traffic you can act on only, gone by
 * glintFarM. `emit` is the NIGHT law on every additive mark — over a black
 * sky even a faint line reads, so past reach emission also dims to
 * nightFloor by nightFarM, on top of the day's gentle ramp. The
 * inspected/locked target keeps its glint and emission at any range.
 */
export function spotProminence(Dt, n, pinned, out) {
  const PR = S.prominence;
  out.body = 1 - (1 - PR.bodyFar) * smoothstep(PR.fullM, PR.farM, Dt);
  if (pinned) {
    out.glint = 1;
    out.emit = 1;
    return out;
  }
  out.glint = 1 - smoothstep(PR.glintFullM, PR.glintFarM, Dt);
  out.emit = 1 - n * (1 - PR.nightFloor) * smoothstep(PR.fullM, PR.nightFarM, Dt);
  return out;
}
...
export function newSpotRec(pool, hex) {
  return {
    ...
    slot: false,
    near: false,
    slotFrame: -2, // the frame the rec last held a rank (keep* hysteresis reads frame − 1 only)
    missingSince: 0,
  };
}
