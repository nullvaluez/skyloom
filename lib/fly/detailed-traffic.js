import { Color, DoubleSide, DynamicDrawUsage, Group, InstancedMesh, MeshStandardMaterial, Object3D } from 'three';
...
export function createLiveAircraftMaterial(){
  const m=new MeshStandardMaterial({vertexColors:true,roughness:.43,metalness:.06,envMapIntensity:.62,side:DoubleSide});
...
roughnessFactor=vSurfaceRole<.5?.43:vSurfaceRole<1.5?.18:vSurfaceRole<2.5?.32:.93;`).replace('#include <metalnessmap_fragment>',`#include <metalnessmap_fragment>
metalnessFactor=vSurfaceRole>1.5&&vSurfaceRole<2.5?.72:.04;`);
  };
  m.customProgramCacheKey=()=>`${key}-living-airframe-v1`;return m;
}
