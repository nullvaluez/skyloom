 ...
 */
export function createSatelliteArchitectureMaterial({ distant = false, lighting = true } = {}) {
  ...
  (distant ? applyBendAnchorSatSkyline : applyBendAnchorSat)(material);
...
`);
  ...
  material.customProgramCacheKey = () => `${bendKey}|cinematic-architecture-v3-${distant ? 'far-coverage' : 'near'}${immersive ? '|immersive-surface-v1' : ''}`;
  return material;
}
