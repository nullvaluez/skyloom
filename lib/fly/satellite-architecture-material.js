...
${earth ? `
// A clear wall/roof value hierarchy reads from cruise without added geometry.
diffuseColor.rgb *= mix(1.16,1.04,isWall);
...
float floorBand=1.-smoothstep(.035,.12,abs(fract(cell.y)-.025));
float roofSeam=1.-smoothstep(.012,.045,abs(fract(vFacadeMetres.x/2.4)-.5));
float surfaceDetail=(1.-minify)*uArchitectureDetail;
diffuseColor.rgb*=1.-surfaceDetail*(floorBand*isWall*.12+roofSeam*(1.-isWall)*.14);
...
`);
  ...
  material.customProgramCacheKey = () => `${bendKey}|cinematic-architecture-v3-${distant ? 'far-coverage' : 'near'}${immersive ? '|immersive-surface-v1' : ''}${groundLighting ? '|ground-window-v2' : ''}${earth ? '|earth-architecture-v1' : ''}`;
