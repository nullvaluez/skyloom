...
import { immersiveOn } from './immersive';
...
import { BUILDING_EMISSION_GLSL } from './night-lighting-policy';
import { applyDaylightSurface } from './daylight-depth';
import { stylizedEarthOn, STYLIZED_EARTH } from './stylized-earth';
...
export function createSatelliteArchitectureMaterial({ distant = false, lighting = true, fadeUniform } = {}) {
  const immersive = immersiveOn('materials');
  const earth = stylizedEarthOn();
  const groundLighting = nearGroundOn('lighting');
...
varying vec2 vArchitectureRoof;
...
float cmArchitectureAmount = uCinematicMaterials*uArchitectureDetail;
float cmArchitectureDay = uCinematicMaterials*(1.0-uArchitectureNight);
vec2 cmArchitectureUV=mix(vArchitectureRoof*uCinematicMetres,vFacadeMetres,isWall)/${STYLIZED_EARTH.materials.repeatM.toFixed(1)}+vec2(fract(seed*.127),fract(seed*.317));
float cmArchitectureLayer=isWall<.5?3.0:family<2.5?2.0:1.0;
vec4 cmArchitectureTex=vec4(.5,.5,.5,.9),cmArchitectureRelief=vec4(.5,.5,.5,1.0);
if(cmArchitectureAmount>.001){cmArchitectureTex=cmColor(cmArchitectureUV,cmArchitectureLayer);cmArchitectureRelief=cmDetail(cmArchitectureUV,cmArchitectureLayer);}
${earth ? `
// Broad material identity survives the detail ladder and the near/far handoff.
// Roofs sit below sunlit facades in value, including when seams have minified.
float roofValue = mix(1.16,${STYLIZED_EARTH.materials.roofValue.toFixed(2)},cmArchitectureDay);
diffuseColor.rgb *= mix(roofValue,1.04,isWall);
diffuseColor.rgb *= vec3(1.025,1.008,0.975);
` : ''}
...
cmArchitectureAmount *= (1.0-isGlass*.88)*(1.0-minify);
diffuseColor.rgb *= mix(vec3(1.0),clamp(cmArchitectureTex.rgb*2.05,vec3(.58),vec3(1.35)),cmArchitectureAmount*.48);
// Tall, low-frequency weather streaks remain resolved after fine masonry retires.
float weatherStreak=archHash(vec2(floor(vFacadeMetres.x/1.7),seed));
diffuseColor.rgb *= 1.0-cmArchitectureAmount*.045*weatherStreak*isWall;
if(uPainterly>.5){
  // Existing regional vertex palettes still own each building's identity.
  // Reduce photographic grit while retaining windows, courses and roof seams.
  float wash=sin(vFacadeMetres.y*.21+seed)*sin(vFacadeMetres.x*.13+seed*.37);
  float resolved=1.0-smoothstep(.25,1.0,max(width.x,width.y));
  diffuseColor.rgb*=1.0+wash*.035*resolved*(1.0-uArchitectureNight);
}
...
// A recessed opening has a dark upper reveal and a sun-catching lower sill.
// These are metre-scale bands, filtered with the same footprint as the pane.
vec2 paneLocal=fract(cell)-.5;
float revealTop=(1.0-smoothstep(.025+width.y,.065+width.y,abs(paneLocal.y-opening.y*.5)))*pane.x;
float sill=(1.0-smoothstep(.018+width.y,.050+width.y,abs(paneLocal.y+opening.y*.5+.035)))*pane.x;
float revealSide=(1.0-smoothstep(.018+width.x,.045+width.x,abs(abs(paneLocal.x)-opening.x*.5)))*pane.y;
float revealDetail=cmArchitectureDay*isWall*(1.0-minify);
diffuseColor.rgb*=1.0-revealDetail*(revealTop*.34+revealSide*.16);
diffuseColor.rgb+=vec3(.055,.052,.046)*sill*revealDetail*(1.0-isGlass*.65);
...
${immersive ? `
// Metre-space panels and roof seams disappear before becoming subpixel noise.
float floorBand=1.-smoothstep(.035+width.y,.10+width.y,abs(fract(cell.y)-.025));
vec2 roofCell=vArchitectureRoof*uCinematicMetres/vec2(2.4,12.0);
vec2 roofWidth=max(fwidth(roofCell),vec2(.001));
vec2 roofJoint=1.-smoothstep(vec2(.009)+roofWidth,vec2(.025)+roofWidth,abs(fract(roofCell)-.5));
float roofSeam=max(roofJoint.x,roofJoint.y)*(1.-smoothstep(.18,.7,max(roofWidth.x,roofWidth.y)));
float surfaceDetail=(1.-minify)*uArchitectureDetail;
diffuseColor.rgb*=1.-surfaceDetail*(floorBand*isWall*mix(.12,.30,cmArchitectureDay)+roofSeam*(1.-isWall)*.14);
float groundOcclusion=mix(.72,1.,smoothstep(0.,5.,vFacadeMetres.y));
diffuseColor.rgb*=mix(1.,groundOcclusion,isWall);
` : ''}
`).replace('#include <roughnessmap_fragment>', `#include <roughnessmap_fragment>
roughnessFactor = mix(baseRough, mix(0.38,0.19,isGlass), paneMask);
roughnessFactor = mix(roughnessFactor,cmArchitectureTex.a,cmArchitectureAmount*(1.0-paneMask)*.45);
...
if(cmArchitectureAmount>.001) normal=cmNormal(normal,-vViewPosition,cmArchitectureUV,cmArchitectureRelief.xy,cmArchitectureAmount*.45);
if (uArchitectureDetail > 0.5 && isWall > 0.5) {
  float relief = (-mix(.035,${STYLIZED_EARTH.materials.facadeReliefM.toFixed(2)},uCinematicMaterials) * paneMask - 0.006 * isMasonry * mortar) * (1.0-minify);
...
${groundLighting ? BUILDING_EMISSION_GLSL : 'float darkBuilding = step(0.09, fract(seed * 0.173));'}
...
${groundLighting ? '' : 'vec3 lamp = mix(vec3(0.60,0.72,0.86),vec3(1.0,0.51,0.20),clamp(warmth+(fract(seed*0.071)-0.5)*0.18,0.0,1.0));'}
float glow = paneMask * occupied * darkBuilding * ${groundLighting ? 'buildingGlow' : '(0.14 + fract(seed*0.113)*0.24)'};
glow += entrance * ${distant ? '0.0' : '0.19'} * darkBuilding;
totalEmissiveRadiance += lamp * glow * uArchitectureNight * uArchitectureLighting;
`);
  };
  material.customProgramCacheKey = () => `${bendKey}|cinematic-architecture-v6-${distant ? 'far-coverage' : 'near'}${immersive ? '|immersive-surface-v1' : ''}${groundLighting ? '|ground-window-v2' : ''}${earth ? '|earth-architecture-v2' : ''}`;
  // Upper facades and every roof are excluded from pavement spill; their own
  // visible window emission still varies with the shared building seed.
  if (!distant) applyNightGroundReceiver(material, 'buildings', {
    weight: 'isWall * (1.0-smoothstep(1.0,5.0,vFacadeMetres.y))', heightM: 5, coolFill: 0.015,
  });
  applyDaylightSurface(material, 'building');
  applyPainterlySurface(material, 'architecture');
  return material;
}
