...
function eiffelTower(detail = false) {
  ...
  for (let i = Math.ceil(SEGS / 6); i < SEGS - 2; i += detail ? 1 : 2) {
    const y0 = (TOP * i) / SEGS;
  ...
  for (let c = 0; c < 4; c++) {
    ...
    for (let i = 0; i < ARCH_SEGS; i++) {
      ...
      const pt = (t) => [
        (ax + (bx - ax) * t) * w,
        detail ? 26 + 13 * Math.sin(Math.PI * t) : ARCH_Y - 13 * Math.sin(Math.PI * t),
        (az + (bz - az) * t) * w,
      ];
      const p0 = pt(t0);
      const p1 = pt(t1);
