 ...
 *  { x, z, topY, r } — x/z/r all in ABSOLUTE world units (the engine derives
 *  the radius from the same draped position buffer the x/z come from, so the
 *  horizontal test compares world against world with no unit conversion);
 *  topY is post-drape absolute Y — the same frame as flight.pos.y. */
...
export class CrashSystem {
   ...
   *                  autopilot: boolean — an intercept/formation is FLYING it.
   *                  flight, satellite, satBuildings, mercK (accepted,
   *                    unused since the W1 world-unit reconciliation — the
   *                    column test is world-vs-world; respawnPose still
  ...
  update(dt, ctx) {
    ...
    // THE AUTOPILOT IS EXEMPT — the same call the plan makes for the boost
    // meter, for the same reason: an assist you explicitly asked for must not
    // kill you. An intercept follows a REAL aircraft, and real aircraft land;
    // chasing one down its approach had the assist flying you into the
    // ground at a sink the terrain rule would happily call a crash. (Measured:
    // a live intercept in the NYC pattern descended through 136 m AGL at
    // -27 m/s, three metres a second under the threshold. That is not a
    // margin, it is a coin toss.) It is also what makes verify-chase-cam,
    // verify-fly-formation and verify-inspect-actions — all of which fly armed
    // intercepts at whatever live traffic is nearest — green by construction
    // instead of by luck. Break out (hard stick / F) and you own it again the
    // same frame.
    ...
    if (ctx.satellite && flight.speed >= CRASH.building.minSpeedMps) {
      ...
      if (cols && cols.length) {
        for (let i = 0; i < cols.length; i++) {
          ...
          // W1 integration (Fable): the engine derives x/z AND r from the same
          // draped position buffer, so both sides of this comparison are world
          // units already — dividing the delta by mercK (the original draft)
          // made every cylinder ~k× too fat at temperate latitudes. Compare
          // world-vs-world; topY is post-drape absolute Y, same frame as pos.y.
