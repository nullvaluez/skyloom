 ...
 *
 *      L(view) = R(e) · (1 + mu²)  +  M(e) · lobe(g, mu)  +  A(e)
 *
 ...
 *   tableR/M/A  Float32Array(24) — the GLSL uniform arrays, radiance-scaled
 *
...
export function createSkyState() {
  return {
    ...
    tableA: new Float32Array(SKY_ROWS * 3), // isotropic (multiple-scatter) radiance
    mieG: 0.76,
    dip: 0,
    veil: 0,
    fog: 0,
...
function rowRadiance(s, k, mu, out) {
  ...
  const r = 1 + mu * mu;
  for (let c = 0; c < 3; c++) out[c] = s.tableR[k * 3 + c] * r + s.tableM[k * 3 + c] * lobe + s.tableA[k * 3 + c];
  return out;
}
...
export function computeSkyModel(s, inp, cfg) {
  ...
  if (stale) {
    ...
    // view extinction only (rawS). It is ISOTROPIC — a third row array, not
    // folded into the Rayleigh (1 + mu^2) phase, which would double its
    // contrast around the sun and paint dark wedges 90 deg from it.
    ambientOf(s, sinEl, s.ambientSS);
    const ms = cfg.msK ?? 0;
    if (ms > 0) {
      for (let k = 0; k < SKY_ROWS; k++)
        for (let c = 0; c < 3; c++) s.tableA[k * 3 + c] = ms * s.ambientSS[c] * s.rawS[k * 3 + c];
    }
    s.keyAlt = alt;
    s.keySinEl = sinEl;
    s.keyCfg = cfg;
...
export function skyRadianceJS(s, ray, out = [0, 0, 0]) {
  ...
  for (let c = 0; c < 3; c++) {
    ...
    const A = s.tableA[i * 3 + c] + (s.tableA[(i + 1) * 3 + c] - s.tableA[i * 3 + c]) * t;
    const L = R * r + M * lobe + A;
 ...
 *   uR25SkyA[8]               isotropic multiple-scatter rows (tableA)
...
uniform vec3 uR25SkyA[${SKY_ROWS}];
...
vec3 r25SkyHorizon( float mu ) {
  vec3 L = uR25SkyR[0] * ( 1.0 + mu * mu ) + uR25SkyM[0] * atmoMieLobe( uR25SkyP.x, mu ) + uR25SkyA[0];
...
vec3 r25Sky( vec3 ray, vec3 sd ) {
  ...
  vec3 A = mix( uR25SkyA[i], uR25SkyA[i + 1], t );
  float mu = dot( ray, sd );
  vec3 L = R * ( 1.0 + mu * mu ) + M * atmoMieLobe( uR25SkyP.x, mu ) + A;
