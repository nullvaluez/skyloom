...
export function applyImmersiveFoliage(material) {
  if (material.userData.immersiveFoliage) return;
  material.userData.immersiveFoliage = true;
  atlas??=leafAtlas();users++;
  material.alphaMap=atlas;material.alphaTest=.42;material.side=DoubleSide;
  const dispose=()=>{material.removeEventListener('dispose',dispose);if(--users===0){atlas.dispose();atlas=null;}};
