...
import { createCinematicTreeMaterial, createCinematicHomeMaterial } from './cinematic-ground';
...
// R25 A: the warm entries for SatGroundDetailLayer's two materials, guarded by
// the same predicate their mount is (see the (b2) block below).
...
function buildWarmScene() {
  ...
  if (satelliteVisualsOn()) {
    ...
    const tree = add(tri(['normal','color','aCanopyPhase']), createCinematicTreeMaterial(applyBendAnchor), {instanced:true,instanceColor:true,cast:true});
    ...
    add(tri(['normal','color','uv','aHomeVariant']), createCinematicHomeMaterial(applyBendAnchor), {instanced:true,instanceColor:true,cast:true});
  ...
  // (b2) R25 A — SatGroundDetailLayer's two instancers. They take the SAME
  // 'world-bend-anchor-r8' variant verbatim (no sway, no new key), but three's
  // OWN program key is the material's parameter set, and neither of these is
  // any of (a)/(b)/(c): the scrub is Lambert + vertexColors + instanceColor +
  // alphaMap/alphaTest/DoubleSide (the shared leaf atlas), and the hedge is
  // Lambert + vertexColors + instanceColor with no map at all. Without these
  // two entries the first descent through the bubble compiles two programs
  // mid-flight — which is the class of hitch this round exists to remove, so
  // "the variant is already warm" would have been the wrong reading of the
  // warm-set rule. Guarded by the same predicate the mount is, so with
  // GROUND_DETAIL_R25 off the warm set is exactly the R24 one.
  if (groundDetailOn('scrub')) {
    const m = new MeshLambertMaterial({ vertexColors: true });
    applyImmersiveFoliage(m); // alphaMap + alphaTest + DoubleSide, as shipped
    applyBendAnchor(m);
    add(tri(['normal', 'color', 'uv']), m, { instanced: true, instanceColor: true });
  }
  if (groundDetailOn('hedges')) {
    const m = new MeshLambertMaterial({ vertexColors: true });
    applyBendAnchor(m);
    add(tri(['normal', 'color']), m, { instanced: true, instanceColor: true });
  }
