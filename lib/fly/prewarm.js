...
export async function runPrewarm({ gl, camera, scene, style, tier }) {
  ...
  try {
    ...
    // R24 B (ENV_UNIFORM, recon WB-4) — WARM THE OTHER SHADOW STATE TOO.
    // three folds `shadowMapEnabled && shadows.length > 0` into every lit
    // material's program key, and the satellite shadow rig is gated on tier
    // 'high' — so a single governor step flips `shadows.length` 1↔0 and every
    // lit material in the scene compiles its other permutation, mid-flight,
    // exactly when frames are already slow. `compile()` derives that count by
    // traversing the TARGET scene, so flipping the live directional's
    // castShadow for the duration of a second compile mints the other key.
    // The flag is restored in a finally: this must never leave the scene in a
    // different shadow state than it found it.
    if (ENV_UNIFORM.enabled && ENV_UNIFORM.warmBothShadowStates && scene) {
      const casters = [];
      scene.traverse((o) => {
        if (o.isLight && o.shadow) casters.push(o);
      });
      if (casters.length) {
        const saved = casters.map((o) => o.castShadow);
        try {
          for (const o of casters) o.castShadow = !o.castShadow;
          await compileUnder(gl, dummyRT(), () =>
            gl.compileAsync(warmScene, camera, scene)
          );
          _state.shadowStates = 2;
        } catch {
          // the alternate state failed to warm — the boot state is still good
        } finally {
          casters.forEach((o, i) => {
            o.castShadow = saved[i];
          });
        }
      }
    }
...
// ---------------------------------------------------------------------------
// R24 B (ENV_UNIFORM, recon A5) — THE LATE-HDRI RE-QUEUE.
//
...
export function requeueForEnvironment({ gl, camera, scene }) {
  ...
  if (!scene || !scene.environment || _requeue) return false;
  const meshes = [];
  _keep.scene?.traverse((o) => {
    if (o.isMesh) meshes.push(o);
  });
  if (!meshes.length) return false;
  _requeue = { gl, camera, scene, meshes, i: 0 };
  _state.requeuePending = meshes.length;
...
export function pumpRequeue(lastFrameMs) {
  ...
  while (n < ENV_UNIFORM.idleBudget && q.i < q.meshes.length) {
    ...
    try {
      ...
      compileUnder(q.gl, dummyRT(), () => q.gl.compileAsync(holder, q.camera, q.scene));
      _state.requeued += 1;
    } catch {
  ...
  holder.clear();
  _state.requeuePending = q.meshes.length - q.i;
  if (q.i >= q.meshes.length) _requeue = null;
  return n;
}
