...
import { createSatelliteArchitectureMaterial } from './satellite-architecture-material';
import { createSatelliteWaterMaterial } from './satellite-water';
import { createCinematicTreeMaterial, createCinematicHomeMaterial, createCinematicHomeDepthMaterial } from './cinematic-ground';
import { createCinematicModelMaterial } from './cinematic-models';
...
import { stylizedEarthOn } from './stylized-earth';
import { applyR25Terrain } from './r25-ground';
import { buildGroundScrubGeometry, createGroundScrubMaterial } from './near-ground-detail';
import { applyNightGroundReceiver, createNightGroundSplatMaterial } from './night-ground';
...
import { LitCloudMaterial } from '@/lib/fly/cloud-material';
...
import {
  ...
  applyHillshade,
  applyNavLights,
  applyOverlayCloudGate,
  applyRoadPulse,
  applyRunwayGlow,
  offsetUnits,
} from '@/lib/fly/toy-world/world-bend';
// Traffic trails: the SAME factories the components use, so a warmed program
// can never differ from the drawn one.
import { makeSpotTracerMaterial, patchAirWake } from '@/lib/fly/tracer-spot';
import { makeWakeMaterial } from '@/lib/fly/aircraft-wake';
import { attachLodFade } from '@/lib/fly/lod-crossfade';
import { getFacadeAtlas } from '@/lib/fly/toy-world/sat-building-engine';
import { nightCityOn, satelliteRoadLighting } from '@/lib/fly/night-city';
import { settleOn, sinceRevealMs } from '@/lib/fly/settle';
import { buildPassList } from '@/components/fly/Effects';
// R25 C SKY: the lazy ALTERNATE-profile warm of the sky programs (below).
import { AerialPerspectiveEffect } from '@/components/fly/AerialPerspective';
import { warmLiveCloudR25 } from '@/lib/fly/immersive-cloud-pass';
import { aerialR25Wanted } from '@/lib/fly/r25-sky';
import { visualsPinned } from '@/lib/fly/visuals-profile';
...
// ---------------------------------------------------------------------------
const SIZES = {
  aBuildingStyle: 3, aWaterShore: 3, aModelSurface: 2, aModelLight: 3, aModelLocal: 4, aCanopyPhase: 1, aHomeVariant: 1,
  normal: 3,
  uv: 2,
  color: 3,
  aFoam: 1,
...
function buildWarmScene() {
  ...
  if (satelliteVisualsOn()) {
    for (const distant of [false, true]) {
      add(tri(['normal','color','uv','aBendAnchor','aBuildingStyle']), createSatelliteArchitectureMaterial({distant, lighting: satelliteVisualsOn('lighting')}), {cast: !distant, receive: !distant});
    }
    add(tri(['normal','uv','aWaterShore']), createSatelliteWaterMaterial());
    add(tri(['normal','color','aAnchor','aModelSurface','aModelLight','aModelLocal']), createCinematicModelMaterial(applyBendAnchorMonument, {merged:true}));
    const tree = add(tri(['normal','color','uv','aCanopyPhase']), createCinematicTreeMaterial(applyBendAnchor), {instanced:true,instanceColor:true,cast:true});
    tree.customDepthMaterial = createCinematicTreeMaterial(applyBendAnchor, {depth:true});
    _keep.materials.push(tree.customDepthMaterial);
    const homes = add(tri(['normal','color','uv','aHomeVariant']), createCinematicHomeMaterial(applyBendAnchor), {instanced:true,instanceColor:true,cast:true,receive:true});
    homes.customDepthMaterial = createCinematicHomeDepthMaterial(applyBendAnchor);
    _keep.materials.push(homes.customDepthMaterial);
    const scrub = add(buildGroundScrubGeometry(), createGroundScrubMaterial(applyBendAnchor), {instanced:true,instanceColor:true,cast:true,receive:true});
    scrub.customDepthMaterial = createGroundScrubMaterial(applyBendAnchor, true);
    _keep.materials.push(scrub.customDepthMaterial);
    scene.userData.cinematicWarmMeshes = scene.children.slice();
    add(tri(['aPool','aLight']), createNightGroundSplatMaterial());
    const clutter = new MeshLambertMaterial({ vertexColors:true, emissive:new Color(0xffffff), emissiveMap:pixel() });
    applyBendAnchor(clutter);
    applyNightGroundReceiver(clutter, 'clutter', { heightM:8, coolFill:0.012 });
    applyDaylightSurface(clutter, 'clutter');
    add(tri(['normal','color','uv']), clutter, {instanced:true,instanceColor:true});
  }
  ...
  // SatTintLayer (MeshBasic, vertexColors, multiply blend, position+color only).
  {
    const m = new MeshBasicMaterial({
      ...
      // R24 C (recon T11): this warm TWIN carried a raw -2 while production
      // (SatTintLayer) has applied the R21 P8 reversed-depth sign flip since
      // that round — the twin and the material it warms disagreed about the
      // depth convention. Caught by `scripts/verify-depth-offset.mjs`, which is
      // exactly the drift a one-implementation rule exists to prevent. (No
      // program moves: polygonOffset is GL state, not a shader define — so this
      // is a state-fidelity fix, not a cache-key one.) `null` here means "no
      // renderer in scope": the value FlyCanvas latched at context creation.
      polygonOffsetUnits: offsetUnits(null, -2),
    });
    applyBendFade(m);
    add(tri(['color']), m);
  }
  ...
  // { transparent:false, side:FrontSide } and a `.map` assigned post-clone.
  {
    ...
    m.map = pixel();
    applyBendFade(m);
    // Round 24 (D): the warm tile material takes the SAME LOD-crossfade slot
    // the live one does (attachLodFade returns null with LOD_CROSSFADE off), so
    // the '-r19-d24' program is compiled at boot rather than on the first
    // refine — the whole point of the warm set.
    ...
    applyR25Terrain(m); // R25 W0: D GROUND's warm twin of the live terrain chain
    add(tri(['normal', 'uv']), m);
  }
  ...
  // (f2) R24 (Fable, post-merge audit — C review finding F8): under ONE_SUN the
  //      SATELLITE archetypes are MeshLambert (LandmarkMonuments.jsx), so the
  //      Toon twin above no longer covers them — warm the Lambert program too.
  if (ONE_SUN.enabled && ONE_SUN.monumentsLambert) {
    const m = new MeshLambertMaterial({ color: LANDMARKS_3D.satStyle.color, vertexColors: true });
  ...
  // R24 (Fable, post-merge audit — C review finding F8): the marquee mesh is
  // MeshLambert in satellite under ONE_SUN (MonumentModels.jsx) — its own program.
  if (ONE_SUN.enabled && ONE_SUN.monumentsLambert) {
  ...
  // R24 (Fable, post-merge audit — C review finding F9): SHADOW_CALM arms the
  // SatShadowCatcher (FlyScene.jsx); its ShadowMaterial never had a warm twin,
  // so its first compile would land mid-flight over the first city.
  if (SHADOW_CALM.enabled) {
    const m = new ShadowMaterial({
      ...
      polygonOffsetUnits: offsetUnits(null, -1),
    });
    applyBend(m);
    add(new CircleGeometry(1, 3), m);
  }
  ...
  // will); disarmed it warms '-r19' with the R19 attribute list exactly.
  {
    ...
    const r23 = satelliteRoadLighting();
  ...
  // ---- 'world-bend-air' × 3 ------------------------------------------
  {
    ...
    // Satellite SPOTTER TRAILS ('world-bend-air-tracer-v1'). The colour is
    // itemSize 4 (vertexAlphas is part of the key). Its draw happens in the
    // sky-overlay pass, whose holder-scene variant SkyOverlayPass compiles on
    // first sight during boot; this lit entry covers the frames before the
    // deferral takes effect.
    const spotGeo = tri(['aTracer']);
    spotGeo.setAttribute('color', new BufferAttribute(new Float32Array(12), 4));
    add(spotGeo, makeSpotTracerMaterial());

    // Engine plumes + player contrail ('world-bend-air|ovgate-v1|optical-
    // wake-v1'): previously never warmed at all.
    add(tri(['aWake']), makeWakeMaterial(patchAirWake));
  }
  ...
  // ---- 'world-bend-air-anchor' (traffic billboards) ------------------
  {
    ...
    // Same predicate as TrafficLayer: 'world-bend-air-anchor|ovgate-v1'.
    if (SKY_OVERLAYS.cloudGate.enabled && SKY_OVERLAYS.cloudGate.billboards)
      applyOverlayCloudGate(m, { floor: SKY_OVERLAYS.cloudGate.markFloor });
    add(new PlaneGeometry(1, 1), m, { instanced: true, instanceColor: true });
  }
...
function buildExtraScene() {
  ...
  for (const Klass of [LitCloudMaterial ?? MeshLambertMaterial, MeshBasicMaterial]) {
    const m = new Klass();
    m.map = pixel();
    m.transparent = true;
    m.depthWrite = false;
...
async function warmShadowDepth(gl, camera, warmScene) {
  ...
  const wasAutoUpdate = sm.autoUpdate, wasNeedsUpdate = sm.needsUpdate;
  ...
  try {
    ...
    sm.autoUpdate = wasAutoUpdate;
    sm.needsUpdate = wasNeedsUpdate;
...
/** Match both effects tiers without mutating a live light during an async warm.
 * FlyScene has one directional sun; it may already be shadow-free at low DPR.
 * Clone only lights, preserving their count/type/layers plus the real fog/env.
 */
function cinematicLightingTarget(source, shadows) {
  const target = new Scene();
  target.environment = source?.environment ?? null;
  target.fog = source?.fog ?? null;
  const lights = [];
  source?.traverseVisible((object) => { if (object.isLight) lights.push(object); });
  const hasCaster = lights.some((light) => light.castShadow);
  const sun = lights.find((light) => light.isDirectionalLight);
  for (const light of lights) {
    const copy = light.clone(false);
    copy.castShadow = shadows && (hasCaster ? light.castShadow : light === sun);
    target.add(copy);
  }
  return target;
}

async function warmCinematicVariants(gl, camera, source, owner, depth = true) {
  const meshes = owner.userData.cinematicWarmMeshes;
  if (!meshes?.length) return;
  const holder = new Scene();
  for (const mesh of meshes) {
    // Share the factory-created material with its session-retained original.
    // Separate objects avoid reparenting or changing the queued warm order.
    const copy = mesh.clone(false);
    copy.customDepthMaterial = mesh.customDepthMaterial;
    holder.add(copy);
  }
  for (const shadows of [true, false]) {
    const target = cinematicLightingTarget(source, shadows);
    const previous = gl.shadowMap.enabled;
    let pending;
    try {
      gl.shadowMap.enabled = shadows;
      // compileAsync dispatches compile synchronously, then polls readiness.
      pending = compileUnder(gl, dummyRT(), () => gl.compileAsync(holder, camera, target));
    } finally {
      gl.shadowMap.enabled = previous;
    }
    await pending;
  }
  // Custom canopy depth uses the runtime factory and stays in _keep.materials.
  // Drive the shadow renderer now, rather than leaving its first compile in
  // the optional post-reveal queue. warmShadowDepth restores state synchronously.
  if (depth) await warmShadowDepth(gl, camera, holder);
}
...
function buildWarmPasses(camera, style, tier) {
  ...
  const effects = [];
  for (const s of specs) {
    // A standalone cloud pass splits aerial perspective from the color grade.
    // Preserve that boundary so warm and runtime compile the same effect groups.
    if (s.boundary) { effects.push(null); continue; }
    try {
  ...
  // R24 C (POST_ORDER): the SAME per-pass policy the live composer applies, so
  // the warmed merged program carries the same DITHERING define. Without this
  // the warm would compile a program the real chain never binds — the exact
  // failure mode the el()/raw() twin rule exists to prevent.
  return finishPassChain(passes);
}

/**
 * ROUND 25 (C SKY) — the LAZY ALTERNATE-PROFILE warm of the sky programs.
 *
 * The live Visuals toggle swaps the AerialPerspectiveEffect TEXT (Classic <->
 * Enhanced) and the cloud pass's march/composite pair. `buildWarmPasses` above
 * already warms the CURRENT profile's aerial (it calls buildPassList, which
 * asks the same r25On predicate production does). This is one more unit on the
 * same queue (drained behind the boot overlay, else sliced onto idle frames),
 * so the first toggle finds its programs resident and the program count is
 * flat from the first cycle on.
 *
 * NEVER UNDER THE FLEET PIN: with `__flyVisualsOverride` set (every legacy
 * harness pins 'classic'), the alternate is never compiled, so no frozen
 * program census moves (plan ruling 6). The aerial group is tier-independent
 * (the cloud pass is a boundary on both sides of it), so one pass covers every
 * tier; the cloud pair belongs to the live pass instance and is compiled there.
 */
async function warmR25SkyAlternate(gl, camera, alpha, size) {
  const enhancedNow = aerialR25Wanted();
  const enhancedExists = !!(R25_SKY.aerialSun?.enabled || R25_SKY.exposure?.enabled);
  if (enhancedNow || enhancedExists) {
    const alt = enhancedNow ? new AerialPerspectiveEffect() : new AerialPerspectiveEffect({ r25: true });
    const pass = new EffectPass(camera, alt);
    pass.initialize(gl, alpha, HalfFloatType);
    pass.setSize(size.width || 1, size.height || 1);
    pass.renderToScreen = false;
    pass.setDepthTexture(null);
    await compileUnder(gl, dummyRT(), () => gl.compileAsync(pass.scene, pass.camera));
    _keep.passes.push(pass); // retained: the program stays refcounted
    _state.passes++;
  }
  // The cloud pair: compiled on the live pass (its uniforms are shared with the
  // Classic pair). Cheap no-op when the pass is not mounted yet.
  try {
    compileUnder(gl, dummyRT(), () => warmLiveCloudR25(gl));
  } catch {
    // a missed warm is a stutter at the first toggle, never a broken frame
  }
}
...
function queueExtraEnvironmentWarm({ gl, camera, scene }) {
  if (_envRequeued || !settleOn() || !_keep.scene) return false;
  _envRequeued = true;
  if (_keep.scene.userData.cinematicWarmMeshes) {
    enqueue('env-rekey-cinematic-shadow-tiers', () => warmCinematicVariants(gl, camera, scene, _keep.scene, false));
  }
  for (const owner of [_keep.extraScene]) {
    if (!owner) continue;
...
export async function runPrewarm({ gl, camera, scene, style, tier }) {
  ...
  try {
    ...
    // Required cinematic variants precede the optional/sliced queue. A DPR
    // effects step removes sun shadows while retaining these same materials.
    await warmCinematicVariants(gl, camera, scene, warmScene);
    ...
    if (PREWARM.warmEffectsAltTier) {
      ...
      // R25 C SKY: the other Visuals profile's sky programs — one more queue
      // unit (drained behind the overlay, else on the idle slice), never under
      // the harness pin (see warmR25SkyAlternate).
      // Classic never allocates/compiles the Enhanced alternate speculatively.
      if (style === 'satellite' && R25_SKY.enabled && aerialR25Wanted() && visualsPinned() == null) {
        enqueue('r25-sky-alt', () => warmR25SkyAlternate(gl, camera, alpha, size));
      }
    }
...
export function requeueForEnvironment({ gl, camera, scene }) {
  ...
  queueExtraEnvironmentWarm({ gl, camera, scene });
