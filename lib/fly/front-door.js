...
import { FRONT_DOOR, FLIGHT_PLAN, AUDIO, HANGAR } from './fly-constants';
...
/**
 * R25 A: the continuous bed (engine + wind) is silent while not flying (the
 * title's frozen flight, the hangar); one-shots and UI sounds still play.
 *
 * INCLUDING the 'prop' tremolo (fix pass, adversarial review): its LFO is
 * CONNECTED to engGain.gain (audio-engine _applyProfile), and Web Audio SUMS a
 * connected signal onto the param's intrinsic value — ramping the intrinsic
 * to 0 alone left a ±engineMaxGain·thrumDepth (0.121) 24–30 Hz thrum on the
 * title for 'prop' and 'warbird-prop'. The LFO depth goes to 0 too, whatever
 * the context state (a graph built by the first title click starts at full
 * depth). `immediate` sets the depth at once — for a graph (re)built while
 * quiet (a hangar pick, the first click), which has not sounded yet.
 * Measured in verify-r25-front-door [9] on the real FlyAudio.
 */
export function quietBed(audio, immediate = false) {
  const ctx = audio?.ctx;
  if (!ctx) return;
  const t = ctx.currentTime;
  const lfo = audio._lfoGain?.gain;
  if (lfo) {
    if (immediate) lfo.value = 0;
    else lfo.setTargetAtTime(0, t, 0.05);
  }
...
/**
 * Leaving the quiet (entering flight): restore the 'prop' tremolo depth a
 * freshly built graph has (audio-engine _applyProfile's value — [9c] compares
 * the two, so a drift there reads FAIL). update() owns everything else.
 */
export function wakeBed(audio) {
  const ctx = audio?.ctx;
  const lfo = audio?._lfoGain?.gain;
  if (!ctx || !lfo) return;
  lfo.setTargetAtTime(AUDIO.engineMaxGain * HANGAR.audioDefaults.thrumDepth, ctx.currentTime, 0.25);
}

/**
 * The soft-lock blip plays on an acquisition while actually flying only — the
 * title's frozen flight still soft-locks (verify-r25-title t8 measured 3
 * acquisitions in one dwell) and the hangar did too. Flag off: `hex && !prev`,
 * today's rule.
 */
export function lockBlipWanted(hex, prev, s) {
  return !!hex && !prev && gameplayLive(s);
}
