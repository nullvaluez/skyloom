import { groundAudioCue, nearGroundOn } from './near-ground';
...
export class ImmersiveAudio {
  ...
  update(runtime,store,active){
    ...
    const ground=store.mapStyle==='satellite'&&nearGroundOn('audio')?groundAudioCue(runtime.groundImmersion):0;
    ...
    for(const node of this.nodes){
      ...
      node.gain.gain.setTargetAtTime(gain*(1+ground*(wind?.07:.04)),t,.35);
      node.filter.frequency.setTargetAtTime(wind?3800-inside*1800-ground*160:1600+speed*2800,t,.5);
      node.source.playbackRate.setTargetAtTime(wind?.8+speed*.25:.65+speed*.55,t,.5);
    }
    runtime.immersiveAudio={status:this.status,loops:this.nodes.length,inside,ground,paused:!audible};
  }
