...
import { CANVAS, PERF_GOVERNOR, LADDER_FIX } from '@/lib/fly/fly-constants';
...
function buildLadder(dpr0, tier0) {
  ...
  for (let d = dpr0; d > CANVAS.dprMin + 1e-6; d -= step) {
    rungs.push({ dpr: +d.toFixed(4), tier: tier0 });
  }
  rungs.push({ dpr: CANVAS.dprMin, tier: tier0 });
  // R24 A (LADDER_FIX) — SUB-NATIVE render-scale rungs, before the first tier
  // rung. On a devicePixelRatio-1 display the loop above runs ZERO times
  // (dprMax 1.5 clamps to 1, which is already dprMin), so the governor's very
  // first step is a structural TIER step: the post chain rebuilds and the
  // high-tier layers unmount. That is the "buildings appearing and
  // disappearing" the user reports, triggered by a transient. Two cheap rungs
  // (0.875, 0.75 at the shipped values) stand in front of it — the browser
  // upscales, so the trade is a slightly softer image instead of a content
  // change. CANVAS.dprMin is deliberately NOT moved: it is the canvas's own
  // boot arithmetic, and these rungs exist for the governor alone.
  if (LF.enabled) {
    const sub = LF.subStep;
    for (let d = CANVAS.dprMin - sub; d > LF.dprMin - 1e-6; d -= sub) {
      rungs.push({ dpr: +d.toFixed(4), tier: tier0 });
    }
  }
  const t0 = Math.max(0, TIERS.indexOf(tier0));
  for (let i = t0 - 1; i >= 0; i--) rungs.push({ dpr: CANVAS.dprMin, tier: TIERS[i] });
  return rungs;
}
  ...
  g.tick = (dt, clock, opts = {}) => {
    ...
    if (g.refresh === 0) {
      ...
      if (g.samples.length >= cfg.refreshFrames) {
        ...
        g.targetFps = LF.enabled && LF.nativeRefresh ? best : Math.min(60, best);
        g.samples.length = 0;
      }
