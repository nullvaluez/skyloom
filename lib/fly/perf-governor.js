...
import { CANVAS, PERF_GOVERNOR, LADDER_FIX, STEP_SAFE } from '@/lib/fly/fly-constants';
// R24 A (STEP_SAFE): the module cell the -99 rig drains. See lib/fly/step-safe.js.
import { parkDpr } from '@/lib/fly/step-safe';
...
function buildLadder(dpr0, tier0) {
  ...
  if (LADDER_FIX.enabled) {
    const sub = LADDER_FIX.subStep;
    for (let d = CANVAS.dprMin - sub; d > LADDER_FIX.dprMin - 1e-6; d -= sub) {
      rungs.push({ dpr: +d.toFixed(4), tier: tier0 });
    }
  }
...
export function createGovernor({ dpr0, tier0, applyDpr, applyTier, cfg = PERF_GOVERNOR }) {
  ...
  g.tick = (dt, clock, opts = {}) => {
    ...
    if (LADDER_FIX.enabled && LADDER_FIX.longFrameFrac > 0 && g.targetFps > 0 && dt > 0) {
      g.lfTotal++;
      if (dt > LADDER_FIX.longFrameK / g.targetFps) g.lfLong++;
      if (g.lfTotal >= LADDER_FIX.longFrameWindow) {
    ...
    if (g.refresh === 0) {
      ...
      if (g.samples.length >= cfg.refreshFrames) {
        ...
        g.targetFps = LADDER_FIX.enabled && LADDER_FIX.nativeRefresh ? best : Math.min(60, best);
        g.samples.length = 0;
      }
    ...
    // is fine.
    const r24Stutter =
      LADDER_FIX.enabled && LADDER_FIX.longFrameFrac > 0 && g.longFrac > LADDER_FIX.longFrameFrac;
...
export function PerfGovernor({ setDpr }) {
  ...
  const gov = useMemo(() => {
    const g = createGovernor({
      ...
      applyDpr: (d) => (STEP_SAFE.enabled ? parkDpr(d) : setDpr(d)),
      applyTier: (t) => {
