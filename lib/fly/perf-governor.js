...
import { CANVAS, PERF_GOVERNOR, LADDER_FIX, SETTLE_CALM } from '@/lib/fly/fly-constants';
// R24 A (STEP_SAFE): the module cell the -99 rig drains. See lib/fly/step-safe.js.
import { parkDpr, resolveStepSafe } from '@/lib/fly/step-safe';
 ...
 */
/**
 * R24 A — the live LADDER_FIX config with a harness/diagnosis override on top
 * (`window.__flyLadderFixOverride`, set before Fly mode mounts). Same idiom as
 * TERRA_PACE and STEP_SAFE: the flags can be flipped for an A/B on the user's
 * machine without editing constants, and production reads nothing extra.
 */
export function resolveLadderFix() {
  const pin = typeof window !== 'undefined' ? window.__flyLadderFixOverride : null;
  return pin ? { ...LADDER_FIX, ...pin } : LADDER_FIX;
}

function buildLadder(dpr0, tier0) {
  const LF = resolveLadderFix();
  const rungs = [];
  ...
  const fix = LF.enabled ? LF : (settleOn() && SETTLE_CALM.ladderFix ? SETTLE_CALM.ladderFix : null);
...
export function createGovernor({ dpr0, tier0, applyDpr, applyTier, cfg = PERF_GOVERNOR }) {
  // Resolved ONCE per governor: the pin is set before mount and never moves
  // mid-session, and re-reading a global on every frame would be a cost for
  // nothing.
  const LF = resolveLadderFix();
  ...
  const g = {
    ...
    samples: [],
    sampled: 0,
    // R24 A (LADDER_FIX): rolling long-frame accounting. Additive only — the
    // R21 state shape the stability gates read is untouched.
    lfTotal: 0,
    lfLong: 0,
    longFrac: 0,
  };
  ...
  const targetForRefresh = (refresh) => {
    const cinematic = useFlyStore.getState().mapStyle === 'satellite' && satelliteVisualsOn();
    return !cinematic && LF.enabled && LF.nativeRefresh ? refresh : Math.min(60, refresh);
  };
  ...
  g.setWarpGrace = (clock) => {
    ...
    g.aboveSince = null;
    g.emaDt = 0;
    // This hook also runs for style changes. The display estimate survives a
    // transition, while its policy target follows the newly selected world.
    if (g.refresh > 0) {
      const target = targetForRefresh(g.refresh);
      if (target !== g.targetFps) {
        g.targetFps = target;
        g.lfTotal = 0;
        g.lfLong = 0;
        g.longFrac = 0;
  ...
  g.tick = (dt, clock, opts = {}) => {
    ...
    // ---- R24 A (LADDER_FIX.longFrameFrac) — the stutter term -----------
    // Counted BEFORE the outlier drop above, which exists so one stall is not
    // a hardware verdict but also makes the controller blind to exactly the
    // thing users call "not smooth": a healthy MEAN with frequent 30-80 ms
    // frames (recon FL-13). A rolling FRACTION of frames longer than
    // longFrameK / targetFps is a stutter signal the EMA cannot carry, and
    // being a fraction over a window, one hitch can never trigger it.
    if (LF.enabled && LF.longFrameFrac > 0 && g.targetFps > 0 && dt > 0) {
      g.lfTotal++;
      if (dt > LF.longFrameK / g.targetFps) g.lfLong++;
      if (g.lfTotal >= LF.longFrameWindow) {
        g.longFrac = g.lfLong / g.lfTotal;
        g.lfTotal = 0;
        g.lfLong = 0;
      }
    }
    ...
    if (g.refresh === 0) {
      ...
      if (g.samples.length >= cfg.refreshFrames) {
        ...
        g.refresh = best;
        // R24 A (LADDER_FIX.nativeRefresh) — govern against the display's own
        // vblank. R21 capped the target at 60, so a 144 Hz monitor running at
        // ~100 fps never steps while presentation alternates 7/14 ms (FL-04).
        // The cinematic profile targets 60 fps even on high-refresh displays.
        // Preserve main's native-refresh policy for the other visual profiles.
        g.targetFps = targetForRefresh(best);
        g.samples.length = 0;
      }
    ...
    // ---- down ----------------------------------------------------------
    // R24 A (LADDER_FIX): a stuttering session steps down even when the mean
    // is fine.
    const r24Stutter =
      LF.enabled && LF.longFrameFrac > 0 && g.longFrac > LF.longFrameFrac;
    if (g.emaFps < cfg.downFrac * g.targetFps || r24Stutter) {
      g.aboveSince = null;
    ...
    if (g.emaFps > cfg.upFrac * g.targetFps) {
      ...
      if (g.latched && clock >= g.latchReleaseAt && useFlyStore.getState().mapStyle === 'satellite' && satelliteVisualsOn()) { g.ceilingIdx = 0; g.latched = false; g.ascendedAt.clear(); }
      const next = g.idx - 1;
  ...
  g.state = () => ({
    ...
    sampled: g.sampled,
    // R24 A (LADDER_FIX) — additive fields; nothing existing moved.
    longFrac: +(g.longFrac || 0).toFixed(3),
    subNativeRungs: g.ladder.filter((r) => r.dpr < CANVAS.dprMin - 1e-6).length,
  });
...
export function PerfGovernor({ setDpr }) {
  ...
  const gov = useMemo(() => {
    const g = createGovernor({
      ...
      // R24 A (STEP_SAFE) — park the value; the -99 rig applies the renderer
      // resize, the composer resize and the React setState in ONE tick inside
      // the frame that draws it (recon A3). Flag off: the R21 call, verbatim.
      applyDpr: (d) => (resolveStepSafe().enabled ? parkDpr(d) : setDpr(d)),
      applyTier: (t) => {
  ...
  useFrame((_, delta) => {
    // Demand-rendered hangar background frames are not a hardware FPS sample.
    // R25: any menu (title or hangar) — frames there are not a hardware sample.
    if(menuOpen(useFlyStore.getState()))return;
