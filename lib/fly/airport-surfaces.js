...
import { operationsFootprint } from './operations-scenery.js';
...
export function buildAirportSurfaces(vt,span,k,frame){
  const pos=[],uv=[],style=[];
  ...
  const tri=(a,b,c,depth=0)=>{
    ...
    if(frame&&operationsFootprint([a,b,c],frame,span/scale))return;
      ...
      for(const ring of rings){let travelled=0;
        for(let j=1;j<ring.length;j++){
          ...
          for(let q=0;q<steps;q++){
            ...
            if(frame&&operationsFootprint(corners,frame,layer.extent))continue;
