import { REGIONAL_PALETTES } from './living-regional-palettes';
...
export const BUILDING_PROFILES = [
  ...
  { name: 'attached', walls: ['#a37763', '#c4b5a0', '#b49b80', '#d0c7b7'], roofs: ['#45484a', '#725447', '#656665', '#66514a'], pitch: 2.7, floor: 3.0, window: [0.36, 0.62], roughness: 0.88, occupancy: 0.52, warmth: 0.83 },
  { name: 'airport', walls: ['#abb3b6', '#b7bab8', '#8c989c', '#c3c7c5'], roofs: ['#777f82', '#939a9b', '#687477', '#a5a9a8'], pitch: 6, floor: 4.8, window: [0.84, 0.65], roughness: 0.52, occupancy: 0.72, warmth: 0.42 },
];
...
export function inferBuildingStyle({ id = 0, height = 10, areaM2 = 100, aspect = 1, density = 0, landuse = '', tags = '', region = 'temperate' } = {}) {
  ...
  if (/terminal|airport/.test(tag)) family = 7;
  else if (/terrace|row.?house/.test(tag)) family = 6;
  else if (/industrial|warehouse|hangar|manufactur/.test(tag)) family = 5;
  ...
  else if (landuse === 'airport') family = areaM2 > 2500 ? 7 : 5;
  ...
  const seed = hash & 255;
  const regional = REGIONAL_PALETTES[region];
  const local = regional && [0,1,2,6].includes(family) ? regional : {};
  return { family, variant, seed, region, roofRise:1, ...BUILDING_PROFILES[family], ...local };
}
