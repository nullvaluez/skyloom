/** Opt-in visual slice. Current cinematic and Neon remain their own baselines. */
...
let search, enabled = false;
export function immersiveOn(feature) {
  if (typeof window === 'undefined') return false;
  if (search !== window.location.search) {
    search = window.location.search;
    enabled = new URLSearchParams(search).get('graphics') === 'immersive';
  }
  const arm = window.__flyImmersiveArm;
  const on = typeof arm === 'boolean' ? arm : enabled;
  if (!on || !feature) return on;
  return window.__flyImmersiveFeatures?.[feature] ?? IMMERSIVE.features[feature] === true;
}
