...
export function immersiveProfile(tier = 'high') {
  return STYLIZED_EARTH.profiles[tier] || STYLIZED_EARTH.profiles.medium;
}
...
export function immersiveLighting(sun = {}, weather = {}) {
  ...
  const lighting=cinematicFlightOn('lighting')?STYLIZED_EARTH.lighting:{sun:2.7,fill:.25,environment:.24};
  return {
    day, night, golden, overcast,
    sun: (0.09+(earth ? lighting.sun : 2.5)*day)*(1-0.83*overcast),
    fill: (0.10+(earth ? lighting.fill : 0.34)*day)*(1+0.42*overcast),
    environment: (0.14+(earth ? lighting.environment : 0.38)*day)*(1+0.15*overcast),
