...
import detailAssets from './monument-detail-assets.json' with { type: 'json' };
...
export const MONUMENT_MANIFEST = [
  ...
  {
    poi: 'Burj Khalifa',
    file: '/models/monument-burj-khalifa-far-v1.glb',
    targetHeightM: 828, yawFixRad: 0, exclusionM: 90,
    accents: [{ yMin: 0.88, yMax: 1, color: '#d6eaff' }],
  },
  {
    poi: 'CN Tower',
    file: '/models/monument-cn-tower-far-v1.glb',
    targetHeightM: 553, yawFixRad: 0, exclusionM: 32,
    accents: [{ yMin: 0.62, yMax: 0.65, color: '#ffe3bd' }],
  },
];

// Generated inventory supplies exact byte/triangle budgets without duplicating URLs.
for (const entry of MONUMENT_MANIFEST) {
  const assets = detailAssets.filter(a => a.poi === entry.poi && a.level !== 'far');
  if (assets.length) entry.detail = Object.fromEntries(assets.map(a => [a.level, a]));
  if (['Empire State Building','Willis Tower'].includes(entry.poi)) {
    entry.detail.medium = entry.detail.high; // Both authored towers also fit the medium budget
    entry.detail.high.yawFixRad = 0;
    // Tall tubes cannot interpolate a roof-only vertex band: the fragment mask owns it.
    if (entry.poi === 'Willis Tower') entry.detail.high.accents = [];
  }
}
 ...
 *
 *   • HISTORICAL R20 SOURCING: One World Trade / CN Tower — the only free models found live on a GitHub
 ...
 *   • HISTORICAL R20 SOURCING: Burj Khalifa — no free model was found at any acceptable quality (the Poly
