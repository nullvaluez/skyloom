...
export class Autopilot {
  ...
  update(dt, flight, target, userCmd) {
    ...
    if (this.mode === 'intercept') {
      ...
      // needs ~7km, not 1km.
      // Round 17: accel + the boost ceiling are the AIRCRAFT's (flight.cfg),
      // not the global FLIGHT block — a glider braking on a fighter's 40 m/s²
      // would overshoot every intercept. (Accepted quirk, FLY_ROUND17_PLAN §3:
      // slow aircraft simply cannot close on a fast target.)
      ...
      const vAllowed = Math.sqrt(arrive * arrive + 2 * F.accel * 0.85 * brakeD);
      cmd.speedOverride = Math.min(F.speeds.boost, vAllowed);
    ...
    const Ff = flight.cfg ?? FLIGHT; // round 17: per-aircraft envelope
    const excess = 0.8 * Math.sqrt(2 * Ff.accel * errDist);
    const desiredSpeed = clamp(tSpeed + excess, 40, Ff.speeds.boost);
