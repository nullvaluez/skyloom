...
export class FlightOperations {
  ...
  warp(flight){if(this.phase==='hangar')return;this.assisted=true;this.phase='airborne';this.approachRequested=false;this.parkingBrake=false;this.gear=this.profile?.fixedGear?1:0;this.flaps=0;this.lowSpeed=false;this.vy=Math.sin(flight.pitch)*flight.speed;this.accumulator=0;this.contactCount=0;this.lastTouchdown=null;this.emit('warp');}
  ...
  advance(dt,flight,cmd,held=false){
    ...
    // Free Flight deliberately has no operations profile. Its airborne start
    // must not inherit the hangar/departure gear value and hold it forever.
    if(!this.profile){this.gear=0;this.flaps=0;return false;}
    ...
    // Gear follows the flight phase, not the low-speed physics owner. A low
    // pass or a go-around must not extend the wheels just by nearing an airport.
    // Keep them through initial lift-off, then take 2.5 seconds to retract.
    const gearDown=this.profile.fixedGear||this.grounded||this.phase==='approach'||
      (this.gear===1&&agl<this.profile.clearance+15);
    const gearStep=Math.min(Math.max(dt,0),.1)/2.5;
    this.gear=gearDown?Math.min(1,this.gear+gearStep):Math.max(0,this.gear-gearStep);
    if(!this.lowSpeed){this.flaps=0;this.vy=Math.sin(flight.pitch)*flight.speed;return false;}
    this.flaps=1;
