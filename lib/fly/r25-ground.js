...
//   s  ONE SUN     albedo *= clamp((N.L+a)/(up.L+a), lo, hi) x the Classic
//                  flat-ground hill value; direct light on the flat normal
...
/** Free every Enhanced resource; the uniforms return to their identity. */
function teardown(engine) {
  _st.live = false;
  _st.pool?.dispose();
  _st.pool = null;
  _st.atlas?.dispose();
...
function attachEngine(engine) {
  _st.offTile?.();
  _st.offTile = null;
  if (_st.live) teardown(_st.engine);
  _st.engine = engine;
...
function onToggle() {
  ...
  if (!r25GroundOn() && _st.live) teardown(engine);
...
export function r25GroundFrame(runtime, ctx) {
  ...
  r25GroundStats.pool = pool
    ? { allocated: pool.allocated, bytes: pool.bytes(), resident: pool.entries.size, ...pool.stats }
    : null;
  r25GroundStats.atlas = at ? { x0: at.x0, y0: at.y0, loaded: at.loaded, bytes: at.bytes(), ...at.stats } : null;
  r25GroundStats.textureBytes = (pool ? pool.bytes() : 0) + (at ? at.bytes() : 0);
