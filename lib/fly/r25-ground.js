...
import { ResidentReliefBackfill } from './relief-backfill';
...
export function applyR25Terrain(material) {
  ...
  material.onBeforeCompile = function (shader, renderer) {
    ...
    // Three r185 caches programs per material, but keeps only ONE uniforms
    // dictionary on that material. A cached Enhanced program revisited after
    // Classic does not call onBeforeCompile again. Preserve the dictionary
    // identity and its superset of bindings so that program cannot lose its
    // relief/reference samplers. Classic's shader and cache key stay unchanged.
    if (h.uniforms) Object.assign(h.uniforms, shader.uniforms);
    else h.uniforms = shader.uniforms;
...
const _st = {
  ...
  offDispose: null,
  backfill: new ResidentReliefBackfill(),
  unsubVisuals: null,
...
function teardown(engine, keepCpu = false) {
  ...
  r25GroundStats.textureBytes = 0;
  r25GroundStats.pool = null;
  r25GroundStats.atlas = null;
}

function attachEngine(engine) {
  _st.offTile?.();
  _st.offDispose?.();
  _st.offTile = _st.offDispose = null;
  // Also discard a CPU atlas retained by an Enhanced -> Classic toggle.
  // It belongs to the old imagery source even when no GPU resources are live.
  if (_st.live || _st.atlas) teardown(_st.engine);
  else _st.backfill.reset();
  if (_st.warm) {
    _st.warm.scene.traverse((o) => o.geometry?.dispose());
    _st.warm.material.map?.dispose();
    _st.warm.material.dispose();
    _st.warm.rt.dispose();
    _st.warm = null;
    r25GroundStats.classicWarm = 'idle';
  }
  _st.frame = 0;
  _st.engine = engine;
  _st.offDispose = engine?.onDispose?.(() => attachEngine(null)) || null;
...
function warmClassic(ctx) {
  ...
  const warm = (_st.warm = { scene, material, rt });
  ...
  Promise.resolve(p).then(
    () => { if (_st.warm === warm) r25GroundStats.classicWarm = 'done'; },
    () => { if (_st.warm === warm) r25GroundStats.classicWarm = 'failed'; }
  );
}
...
export function r25GroundFrame(runtime, ctx) {
  ...
  r25Uniforms.uR25Sharp.value =
    sat && r25GroundOn('sharpen') && (TIER_RANK[ctx?.tier] ?? 0) >= (TIER_RANK[sh.minTier] ?? 2)
      ? Math.min(0.35, sh.k)
      : 0;
  ...
  // SAT_QUILT retired: the colour reference replaces the grey-out. This hook
  ...
  // Classic -> Enhanced works at a stationary pose too: fill the visible
  // resident tiles in bounded batches, highest zoom first. No tile
  // unload, epoch bump, imagery request, or geometry replacement is involved.
  if (px && engine?.loadR25Relief && _st.backfill.active.size < _st.backfill.limit && _st.frame % 6 === 0) {
    const candidates = [];
    engine.forEachLoadedTile((t) => {
      if (t.model.visible && !t.model.geometry.userData.r25Relief) candidates.push(t);
    });
    candidates.sort((a, b) => b.z - a.z);
    for (const t of candidates) {
      if (_st.backfill.active.size >= _st.backfill.limit) break;
      _st.backfill.request(t, (tile) => engine.loadR25Relief(tile), (tile, geometry, bytes) => {
        if (!_st.live || _st.engine !== engine || !engine.isResidentTile(tile, geometry)) return false;
        geometry.userData.r25Relief = bytes;
        bindTile(tile);
        return true;
      });
    }
  ...
  // Colour reference: recentre on the camera's z11 tile, stream, upload.
  if (_st.atlas) {
