...
export const MONUMENT_LIGHTING_PROFILES = {
  ...
  'Burj Khalifa': { role: 'glass', flood: '#dae8f1', intensity: 0.1, accent: 0.7 },
  'CN Tower': { role: 'stone', flood: '#ffdfbf', intensity: 0.19, accent: 0.9 },
...
export function attachCinematicModelAttributes(geometry, entry = {}) {
  ...
  const bakedSurface=geometry.getAttribute('_model_surface'),bakedLight=geometry.getAttribute('_model_light');
  const kinds = {'Empire State Building':1,'Willis Tower':2,'Burj Khalifa':3,'Big Ben':4,'Eiffel Tower':5,'CN Tower':6};
  const kind = entry.baseline ? 0 : (kinds[entry.poi] ?? 0);
  const metres = entry.targetHeightM ?? 1;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const band = kind === 4 ? null : modelAccentAt(entry.accents, y, radial);
    // Ground-mounted wash diminishes up the facade; curated accent bands stay independent.
    const ny = geometry.attributes.normal?.getY(i) ?? 0;
    const nx = geometry.attributes.normal?.getX(i) ?? 0, nz = geometry.attributes.normal?.getZ(i) ?? 0;
    const orientation = entry.baseline ? 1 : .2 + .8 * Math.max(0, nx*.55 - ny*.55 + nz*.62);
    const wash = p.intensity * (0.28 + 0.72 * (1 - y) ** 0.7) * orientation * (kind === 5 ? .6 : 1);
    local.set([pos.getX(i)*metres, y*metres, pos.getZ(i)*metres,kind],i*4);
    surface[i * 2] = bakedSurface?.getX(i) ?? role.roughness;
    surface[i * 2 + 1] = bakedSurface?.getY(i) ?? role.metalness;
    if (band) accent.set(band.color).multiplyScalar(p.accent);
    else accent.setRGB(0, 0, 0);
    light[i * 3] = (bakedLight?.getX(i) ?? flood.r * wash) + accent.r;
    light[i * 3 + 1] = (bakedLight?.getY(i) ?? flood.g * wash) + accent.g;
    light[i * 3 + 2] = (bakedLight?.getZ(i) ?? flood.b * wash) + accent.b;
  }
  ...
  geometry.setAttribute('aModelLocal', new BufferAttribute(local, 4));
  geometry.deleteAttribute('_model_surface');
  geometry.deleteAttribute('_model_light');
  geometry.userData.materialRole = p.role;
  geometry.userData.lightingMask = 'first-party-unit-height-radial-v1';
  return geometry;
}

// Fragment masks preserve crisp detail without tessellating every window or adding lights.
const LANDMARK_ARCHITECTURAL_LIGHT = `
if (uModelNight > 0.001) {
  vec3 lp = vModelLocal.xyz;
  float kind = vModelLocal.w;
  vec3 localNormal = normalize(cross(dFdx(lp),dFdy(lp)));
  if (kind > 0.5 && kind < 3.5) {
    float face = abs(localNormal.x) > abs(localNormal.z) ? lp.z : lp.x;
    vec2 grid = vec2(face / 3.3, lp.y / 4.1);
    vec2 cell = fract(grid);
    vec2 aa = max(fwidth(grid), vec2(0.001));
    vec2 pane = smoothstep(vec2(0.18)-aa,vec2(0.18)+aa,cell) *
                (1.0-smoothstep(vec2(0.78)-aa,vec2(0.78)+aa,cell));
    float hash = fract(sin(dot(floor(grid),vec2(12.9898,78.233))) * 43758.5453);
    float lit = step(0.63,hash) * pane.x*pane.y;
    float resolve = 1.0-smoothstep(0.25,0.8,max(aa.x,aa.y));
    float wall = 1.0-smoothstep(0.3,0.7,abs(localNormal.y));
    float occupiedTop = kind < 1.5 ? 372.0 : (kind < 2.5 ? 442.0 : 690.0);
    wall *= 1.0-step(occupiedTop-0.7,lp.y);
    totalEmissiveRadiance += vec3(0.42,0.29,0.15)*lit*resolve*wall*uModelNight;
  }
  if (kind > 1.5 && kind < 2.5) {
    float roof = 1.0-smoothstep(0.4,0.9,abs(lp.y-442.0));
    float mast = smoothstep(520.0,523.0,lp.y);
    totalEmissiveRadiance += (vec3(0.65,0.45,0.22)*roof + vec3(0.5,0.65,0.8)*mast)*uModelNight;
  }
  if (kind > 3.5 && kind < 4.5) {
    float face = abs(lp.x)>abs(lp.z) ? lp.z : lp.x;
    vec2 clock = vec2(face,lp.y-62.5);
    float dial = 1.0-smoothstep(3.35,3.65,length(clock));
    float hands = max((1.0-smoothstep(.10,.22,abs(clock.x)))*step(0.0,clock.y),
                      (1.0-smoothstep(.10,.22,abs(clock.y+.6*clock.x)))*step(0.0,clock.x));
    totalEmissiveRadiance += vec3(0.85,0.66,0.32)*dial*(1.0-hands)*uModelNight;
  }
}`;
...
attribute vec4 aModelLocal;
varying vec4 vModelLocal;
varying vec2 vModelSurface;
varying vec3 vModelLight;`)
      .replace('#include <begin_vertex>', '#include <begin_vertex>\nvModelSurface = aModelSurface;\nvModelLight = aModelLight; vModelLocal = aModelLocal;');
...
varying vec3 vModelLight;`)
    ...
    if (merged) shader.fragmentShader = shader.fragmentShader.replace('#include <emissivemap_fragment>', `#include <emissivemap_fragment>
${LANDMARK_ARCHITECTURAL_LIGHT}`);
  };
  m.customProgramCacheKey = () => `${key}-cinematic-model-v2${merged ? '-merged' : ''}`;
  return m;
}
...
export function cinematicAircraftParameters(src, isCanopy = false) {
  ...
  const exposed = !isCanopy && (src?.metalness??0)>.65 && !src?.map;
  const cinematic = cinematicFlightOn('aircraft') ? isCanopy
    ? {roughness:.12,metalness:.06,clearcoat:.85,clearcoatRoughness:.12,envMapIntensity:1.15}
    : exposed ? {roughness:.39,metalness:.72,clearcoat:.08,clearcoatRoughness:.38,envMapIntensity:.5}
    : {roughness:.36,metalness:.08,clearcoat:.38,clearcoatRoughness:.24,envMapIntensity:.85} : {};
  return {
    ...role, ...cinematic, normalMap: src?.normalMap ?? null, roughnessMap: src?.roughnessMap ?? null,
