...
export function proceduralWeather(lat, lon, tMs, cfg) {
  ...
  // R19: derive the temperature from the CELL latitude, not the raw one.
  // Everything else here is already cell-quantised; this term was not, so two
  // positions inside one cell produced different payloads — and because the
  // rain/snow tie-break reads tempC, flying a few hundred metres near the
  // freezing line could flip precipitation kind back and forth. Caught by
  // verify-dusk's determinism gate when the fallback was switched on.
  const tempC = 24 - Math.abs(cy * cell) * 0.45;
  return {
    found: true,
    source: 'procedural',
    cloudCoverPct: cover,
