...
export function attachGroundShadowLight(runtime,light){
  ...
  return()=>{if(runtime.sunLight===light){runtime.sunLight=null;delete runtime.shadowRadiusM;delete runtime.shadowFocus;delete runtime.shadowCoverage;}};
}
// These values belong to the mutable scene bus, never to React render state.
export function publishGroundShadowCoverage(runtime, census) { runtime.shadowCoverage = census; }
export function releaseGroundShadowCoverage(runtime, census) {
  if (runtime.shadowCoverage === census) runtime.shadowCoverage = null;
}
export function publishGroundShadowFocus(runtime, x, y, z, dx, dz) {
  const focus = (runtime.shadowFocus ??= {});
  focus.x = x; focus.y = y; focus.z = z; focus.dx = dx; focus.dz = dz;
}
