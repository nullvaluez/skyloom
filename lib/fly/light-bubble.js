...
/**
 * The shadow radius FlyScene's two readers consume: the SHADOW_CALM texel snap
 * in the satellite key-light branch, and the `nearReceive` sweep's reach.
 * `undefined` — the flag-off state — sends both back to their frozen literals
 * through `??` (`shadowRigRef.current.radiusM` and `SAT_SHADOWS.orthoRadiusM`
 * respectively; they are deliberately different, because the sweep's literal is
 * the CONSTANT and the snap's is the rig, which carries TOY.shadowRadiusM in
 * the toy style).
 */
