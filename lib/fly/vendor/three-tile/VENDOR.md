...
| 26 | A | `TERRA_PACE.parkOffscreen` | `index.js` · module scope (`r24Park`) + `Tile.update()` + `Tile._update()` (both branches) | T1 (the DRAW half of `keepResident`) — a retained tile is still ATTACHED, so three walks it and can draw it. Pass 2b measured ONE fixed Owens pose going 152 → 279 draws between a 45 s and a 600 s sweep with identical flags (62 → 103 resident tiles, tris ×2.4): residency charging rent in draw calls forever, against a frozen ceiling that was never set on it. The patch parks the MODEL (`model.visible = inFrustum`), never the tile — a tile's children hang off the tile, and a parent whose box contains an in-frustum child is itself in frustum, so a visible tile can never be orphaned behind a parked ancestor. Nothing is disposed and nothing re-downloaded; return is one boolean. The same visit stamps `_r24LastVisible`, which is what makes `lib/fly/tile-residency.js`'s LRU a recency order rather than a distance heuristic. | every site is a single `if (R24_SWITCHES.parkOffscreen)` that returns; NO upstream statement is edited (the `_update` pre-pass recomputes `_inFrustum` on its own added line so the upstream comma-expression stays byte-verbatim) |
