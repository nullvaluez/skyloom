// R24 A PATCH 20 (TERRA_PACE.skirtWorker) — the ONE non-three import this
// bundle is allowed. It is generated from a readable source next door by
// scripts/build-tile-worker.mjs; the bundle still imports NO app code.
import { R24_WORKER_SKIRT_TAIL } from "./workers/skirt-tail.built.js";
...
// SATELLITE INTEGRATION: independent cache/retry/LOD rollback controls.
const FLY_PATCH = {
  parallelFetch: !1,
  lodBailFix: !1,
  demErrorTable: null,
  skirtEdges: !1,
  // R24 (A "MOTION HOLD"). Every one of the four below is OFF at the value it
  // carries here, and OFF means the patched site evaluates to the byte-verbatim
  // upstream expression — 0 ms of dwell, the literal ×5, the literal threshold,
  // a re-thrown rejection, an un-stamped error material.
  mergeDwellMs: 0,
  frustumPenalty: 5,
  lodHysteresis: 1,
  unlockOnReject: !1,
...
const FLY_NOW = typeof performance < "u" && performance.now ? () => performance.now() : () => Date.now();
const FLY_HOLD_STATS = {
  dwellArmed: 0,
  dwellHeld: 0,
  dwellFired: 0,
  rejectLoad: 0,
  ...
  errorTiles: 0,
  rasterRetryActive: 0,
  rasterRetryStarted: 0,
  rasterRetryRecovered: 0,
  rasterRetryDiscarded: 0,
  lastError: null
};
function flyTileHoldStats() {
  return FLY_HOLD_STATS;
}
function flyRasterError(material) {
  return !!material?.userData?.flyError;
}
function flySetPatch(i) {
  return i && Object.assign(FLY_PATCH, i), FLY_PATCH;
}
function flyGetPatch() {
  return FLY_PATCH;
}
function flySetDemErrorTable(i) {
  FLY_PATCH.demErrorTable = i || null;
}
let _flyECap = 0;
let _flyEGen = 0;
let _flyEG = null;
...
let _flyEP = null;
let _flyEOcc = null;
function flyBoundaryEdgesFast(i) {
  const n = i.length;
  if (n === 0 || n % 3 !== 0) return null;
  let cap = 16;
  while (cap < n * 2) cap <<= 1; // load factor <= 0.5 ⇒ a free slot always exists
  if (cap > _flyECap) {
    _flyECap = cap;
    _flyEG = new Int32Array(cap);
    _flyEU = new Int32Array(cap);
    _flyEV = new Int32Array(cap);
    _flyEP = new Int32Array(cap);
    _flyEOcc = new Int32Array(cap);
    _flyEGen = 0;
  }
  // No per-call clear (an O(cap) fill was measurable) and no per-call scan of
  // the whole table: the generation stamp retires the previous contents and
  // `occ` remembers exactly which slots this call touched.
  const gen = ++_flyEGen;
  const HG = _flyEG, HU = _flyEU, HV = _flyEV, HP = _flyEP, OCC = _flyEOcc, mask = cap - 1;
  let occN = 0;
  for (let t = 0; t < n; t += 3) {
    for (let e = 0; e < 3; e++) {
      const a = i[t + e], b = i[t + (e === 2 ? 0 : e + 1)];
      if (!(a >= 0) || !(b >= 0)) return null;
      const u = a < b ? a : b, v = a < b ? b : a;
      let s = ((Math.imul(u, 73856093) ^ Math.imul(v, 19349663)) >>> 0) & mask;
      for (;;) {
        if (HG[s] !== gen) {
          HG[s] = gen;
          HU[s] = u;
        ...
        if (HU[s] === u && HV[s] === v) {
          const p = HP[s];
          if (p < 0) return null; // third occurrence — non-manifold
          const pe = p & 3, pt = (p - pe) / 4;
          // Must be the EXACT reverse, or upstream would emit both halves.
          if (i[pt + pe] !== b || i[pt + (pe === 2 ? 0 : pe + 1)] !== a) return null;
          HP[s] = -1;
          break;
        }
        s = (s + 1) & mask;
  ...
  // Only the boundary survives — a tile perimeter, a few hundred entries — so
  // the one remaining sort is O(perimeter log perimeter), not O(3·T log 3·T).
  const slots = [];
  for (let j = 0; j < occN; j++) if (HP[OCC[j]] >= 0) slots.push(OCC[j]);
  slots.sort((x, y) => (HU[x] !== HU[y] ? HU[x] - HU[y] : HV[x] - HV[y]));
  const out = new Array(slots.length);
  for (let j = 0; j < slots.length; j++) {
    const p = HP[slots[j]], e = p & 3, t = (p - e) / 4;
    out[j] = [i[t + e], i[t + (e === 2 ? 0 : e + 1)]];
  }
  return out;
}
const FLY_SKIRT_STATS = { fast: 0, bail: 0, upstream: 0, tris: 0 };
function flySkirtStats() {
  return FLY_SKIRT_STATS;
}
const FLY_EDGE_CAPTURE = { max: 0, rows: [] };
function flyEdgeCapture(max) {
  if (max !== void 0) {
    FLY_EDGE_CAPTURE.max = max | 0;
    FLY_EDGE_CAPTURE.rows.length = 0;
  }
  return FLY_EDGE_CAPTURE;
}
function flyBoundaryEdgesRef(i) {
  const was = FLY_PATCH.skirtEdges, mainFast = R24_SWITCHES.skirtFast;
  R24_SWITCHES.skirtFast = false;
  FLY_PATCH.skirtEdges = !1;
  try {
    return We(i);
  } finally {
    FLY_PATCH.skirtEdges = was;
    R24_SWITCHES.skirtFast = mainFast;
  }
}
...
export const R24_SWITCHES = {
  // R24 A PATCH 26 (TERRA_PACE.parkOffscreen) — see the patch site in _update.
  parkOffscreen: false,
  // R24 A PATCH 26 — monotonically increasing walk counter, stamped on every
  // tile the moment it is seen in frustum. lib/fly/tile-residency.js orders its
  // LRU by this, so "least recently VISIBLE" is a real reading and not a guess
  // from distance.
  r24Frame: 0,
  ...
  bboxCache: false,
  // Set from C's own block (TERRAIN_LIGHT.workerNormals), read defensively.
  workerNormals: false,
  // R24 A PATCH 25 (FRAME_STATS attribution) — an optional callback the APP
  // installs (lib/fly/terrain-engine.js sets it to frame-stats' markPhase).
  // Inverted control on purpose: the bundle still imports no app code, and
  // when nothing installs it these are two null checks on paths that already
  // do real work. This is what turns "it froze for 40 ms" into "it froze
  // during a skirt build" on the user's machine.
  onPhase: null,
  // R25 D PATCH 30 (R25_GROUND.relief) — R25 D GROUND relief switches, set by
  // lib/fly/terrain-engine.js applyR25GroundSwitches (no app import here):
  //   r25Relief    the FLAG (armed at engine creation): the LERC worker is
  //                created from the grid-capturing splice (PATCH 31)
  //   r25ReliefPx  the live REQUEST size (0 = Classic / off): LERC requests
  //                carry `r25Relief` only while this is > 1 (PATCH 32)
  //   r25ReliefFn  the app's main-thread relief builder (terrain-rgb, PATCH 33)
  // All three off = no splice, no request field, no main-thread work.
  r25Relief: false,
  r25ReliefPx: 0,
  r25ReliefFn: null,
...
/** Counters the patches publish so a gate can prove which path actually ran. */
export const R24_STATS = {
  skirtFastCalls: 0, // boundary-edge scans answered by the fast path
  skirtBails: 0, // …handed back to the verbatim upstream body
  skirtEdges: 0, // triangles*3 walked by the last fast scan
  skirtWorkers: 0, // DEM workers created from the spliced R24 source
};
...
// texture lifetime). With NO hook installed this is null and patches 6 and 7
// are a single short-circuited `&&` in front of statements that are byte-
// verbatim upstream — which is the off-state for patches 5, 6 and 7.
let ir = null;
function Xr(e) {
  ir = e;
}
...
class T extends oe {
  ...
  // Living Earth arrival prefetch. A held camera cannot refine ground behind
  // itself, but nearby mapped buildings need that DEM before their chunk can
  // finalize. Refine only the local evidence ring to the requested floor.
  // The owner clears this per-map field when arrival ends; normal LOD is exact.
  _arrivalGroundRequired() {
    const f = this._root.flyArrivalGround;
    if (!f || this.z >= f.zoom) return false;
    const world = 40075016.68557849, span = world / 2 ** this.z;
    let dx = Math.abs((this.x + 0.5) * span - world / 2 - f.x) % world;
    dx = Math.min(dx, world - dx);
    const dz = Math.abs((this.y + 0.5) * span - world / 2 - f.z);
    return Math.hypot(Math.max(0, dx - span / 2), Math.max(0, dz - span / 2)) <= f.radius;
  }
  /** 是否为叶子瓦片 */
  get isLeaf() {
   ...
   */
  get BBox() {
    // Stylized Earth: Hudson DEM tiles can be wholly below sea level. The
    // old [0, maxZ] box was empty, so applyMatrix4 skipped its world transform
    // and the tile walker hid valid terrain. Use pre-skirt surface extrema:
    // skirts descend hundreds of metres and must not enlarge surface culling.
    const minZ = Math.min(0, this._model?.geometry?.userData?.surfaceMinZ ?? this._maxZ);
    const maxZ = Math.max(0, this._maxZ);
    // R24 A PATCH 24 (TERRA_PACE.bboxCache) — recon T3, the allocation half.
    // Every tile visit allocates a Box3 and two Vector3s here, and the walk
    // touches every tile. PATCH 22 makes the walk run even while the loader is
    // saturated, which is correct but roughly doubles the visits — so the
    // per-visit allocation has to go. The box only moves when the tile's own
    // max height changes or the map's world matrix does (the floating-origin
    // rebase is the only thing that moves it; the map's rotation and scale are
    // set once at construction), so those five numbers are the whole key.
    if (R24_SWITCHES.bboxCache) {
      const m = this.matrixWorld.elements;
      if (
        this._r24Box &&
        this._r24BoxK0 === this._maxZ &&
        this._r24BoxMin === minZ &&
        this._r24BoxK1 === m[12] &&
        this._r24BoxK2 === m[13] &&
        this._r24BoxK3 === m[14] &&
        this._r24BoxK4 === m[0]
      ...
      const { x: a, y: b } = this.scale;
      this._r24Box = new ke(new I(-a, -b, minZ), new I(a, b, maxZ)).applyMatrix4(this.matrixWorld);
      this._r24BoxK0 = this._maxZ;
      this._r24BoxMin = minZ;
      this._r24BoxK1 = m[12];
      this._r24BoxK2 = m[13];
    ...
    return new ke(new I(-e, -t, minZ), new I(e, t, maxZ)).applyMatrix4(this.matrixWorld);
  }
  /** 是否需要更新 */
  ...
  update(e) {
    ...
    // R24 A PATCH 25 (FRAME_STATS attribution) — the quadtree walk runs INSIDE
    // renderer.render (three calls it from projectObject for isLOD objects),
    // so a stall here is otherwise indistinguishable from a draw.
    if (R24_SWITCHES.onPhase) R24_SWITCHES.onPhase('lod-walk');
    const { camera: t } = e;
    // R24 A PATCH 26 — one tick per tree walk, not per tile.
    if (R24_SWITCHES.parkOffscreen) R24_SWITCHES.r24Frame++;
  ...
  LOD(e, r24NoStart) {
    ...
    // ── R24 PATCH #6a (`mergeDwellMs`) — OFF (0) = the verbatim upstream line ──
    //
    // THE DEFECT. `_getDistRatio` multiplies a tile's ratio by 5 instead of 0.8
    // the instant its BBox leaves the frustum (a 6.25x step = 2.64 zoom levels),
    // and `_LODEvaluate` has NO hysteresis in either direction. An interior node
    // has no model, so `_update`'s "skip a non-visible leaf" guard never covers
    // it and LOD() runs on it in or out of frustum. Every yaw, bank and roll
    // reversal therefore MERGES the ground leaving the view — which downloads a
    // coarse parent and then `unloadSubTiles()` DISPOSES the entire deep subtree
    // — and re-refines it from scratch on the way back, one level per 50 ms LOD
    // tick plus a round trip each.
    //
    // MEASURED (R22.1's own instruments, scripts/r22p1-b-stutter.md 2.3): a 350
    // kt / 233 m serpentine over Powell built 2,123 DEM tile meshes in 22 s —
    // ~96/s against a resident set of 223 tiles (r22p1-close.md 1.2 gate (6)),
    // i.e. the whole terrain rebuilt about every two seconds, against a
    // transport-limited steady-state rate of ~5-10/s. R22.1 made each of those
    // builds 5-6x cheaper (patch #5) and the stutter went away; the CHURN did
    // not, and it is what the user sees as the ground going blurry and snapping
    // sharp on every turn.
    //
    // THE FIX IS HYSTERESIS IN TIME, NOT IN SPACE, and that is the whole point:
    // a node must WANT to merge continuously for `mergeDwellMs` before it may.
    // A yaw sweep no longer destroys a subtree; a genuine climb or departure
    // still merges, `mergeDwellMs` later. Because the dwell only delays, the
    // SETTLED tree at any frozen pose is bit-for-bit the tree upstream reaches —
    // same tiles, same draws, same triangles, same texture bytes — which is why
    // this can ship without moving a single frozen assertion in a fleet whose
    // every terrain harness freezes the aeroplane before it measures.
    //
    // The return value is `s` on every path here, exactly as upstream; the only
    // difference is WHEN `_removeSubTiles` is called.
    if (r24NoStart === !0) return s;
    // A failed raster replacement must not retry on every 20 Hz LOD walk.
    if (FLY_PATCH.rasterMark && this._rasterLodAt > FLY_NOW()) return s;
    // Residency budget evictions already have their own cooldown; do not defer them again.
    const _budgetEviction = !this.inFrustum && this._r24Collapse > FLY_NOW();
    const _dwell = _budgetEviction ? 0 : FLY_PATCH.mergeDwellMs;
    if (_dwell > 0) {
      if (s !== 2)
        this._mergeWantSince = 0;
      else {
        const _now = FLY_NOW();
        this._mergeWantSince > 0 || (this._mergeWantSince = _now, FLY_HOLD_STATS.dwellArmed++);
        if (_now - this._mergeWantSince < _dwell)
          return FLY_HOLD_STATS.dwellHeld++, s;
        this._mergeWantSince = 0, FLY_HOLD_STATS.dwellFired++;
      }
    }
  ...
  _update(e) {
    const { loader: t } = e;
    // R24 A PATCH 22 (TERRA_PACE.walkWhileSaturated) — recon T3, second half.
    // Upstream freezes the ENTIRE tree walk while `downloadingThreads + 4 >=
    // maxThreads`: with TILES.maxThreads 10 that is SIX loads in flight, i.e.
    // one refine batch plus one. Nothing is re-evaluated then — not frustum
    // flags, not merges, not the descent — so a queue that stays busy pins the
    // tree at whatever depth it had reached. E CERT measured exactly that on
    // the R24 fixture: dl 9/10 in flight, the tree stuck at maxZ 6, ground
    // samples answering from a z6 tile (193 m vs the true 276 m at Powell), and
    // every satellite building drape restarting forever because tileZ 6 is
    // below SAT_BUILDINGS.demZ.
    // The fix is per-tile, not global, and STRICTLY CONSERVATIVE: keep walking
    // and keep the frustum/LOD state current, but START no load while
    // saturated — neither a refine (PATCH 8) nor a version reload. It can only
    // evaluate more; it can never issue a download upstream would not have.
    if ((R24_SWITCHES.walkWhileSaturated || FLY_PATCH.lodBailFix)) {
      if (this.loadState === "loading") return;
      const r24Sat = t.downloadingThreads + 4 >= t.maxThreads;
      this._updateShadow();
      ...
      this._inFrustum = q.intersectsBox(this.BBox) || this._arrivalGroundRequired();
      r24Park(this);
      FLY_PATCH.rasterMark && !this._rasterRetryActive && this._model?.material?.some(flyRasterError) && this._retryErrorMaterial(t);
      if (!this._inFrustum && this._model && this.isLeaf) return;
      this.LOD(e, r24Sat);
      this.children.forEach((r) => r instanceof T && r._update(e));
      return;
    }
    if (!(t.downloadingThreads + 4 >= t.maxThreads || this.loadState === "loading")) {
      ...
      // R24 A PATCH 26 — parking needs `_inFrustum` for a tile that the
      // upstream line below then EARLY-RETURNS on (an off-frustum loaded leaf
      // — exactly the tile that must stop drawing). Computed on its own added
      // line so the upstream statement is left byte-verbatim; the assignment
      // is idempotent and PATCH 24 caches the box, so the repeat is a compare.
      if (R24_SWITCHES.parkOffscreen) {
        this._inFrustum = q.intersectsBox(this.BBox) || this._arrivalGroundRequired();
        r24Park(this);
      }
      this._inFrustum = q.intersectsBox(this.BBox) || this._arrivalGroundRequired();
      FLY_PATCH.rasterMark && !this._rasterRetryActive && this._model?.material?.some(flyRasterError) && this._retryErrorMaterial(t);
       !(!this._inFrustum && this._model && this.isLeaf) && (this.LOD(e), this.children.forEach((r) => r instanceof T && r._update(e)));
    }
  }
  /**
   * 瓦片阴影保持和跟瓦片阴影一致
   ...
   */
  _LODEvaluate(e, t, r) {
    if (this._arrivalGroundRequired()) return 1;
    ...
    const _h = Math.max(1, FLY_PATCH.lodHysteresis);
    return this.isLeaf && this.z < t && n <= r ? 1 : !this.isLeaf && this.z >= e && n > r * _h ? 2 : 0;
  }
  /**
  ...
  _getDistRatio(r24AsInFrustum) {
    ...
    return this.inFrustum ? t * 0.8 : t * (FLY_PATCH.frustumPenalty || 5);
  }
  /**
   * 创建子瓦片并设置位置和缩放
   */
  ...
  async _loadSubTiles(e) {
    // ── R24 PATCH #7 (`unlockOnReject`) — OFF = re-throw, i.e. upstream ────────
    // `LOD()` DISCARDS the promise these three methods return (`:215`), and
    // `TileLoader.update` has a try/FINALLY with no catch (`:1067`), so a single
    // rejection anywhere below leaves `this._subTiles` set forever — and the
    // guard `!this.subTiles` in LOD() means THIS NODE CAN NEVER REFINE AGAIN for
    // the life of the session. That is a permanently blurry patch with the
    // loader idle and zero requests issued: the exact shape of R22.1 follow-up
    // F11. The reachable throw surfaces are `LoaderFactory.getMaterialLoader` /
    // `getGeometryLoader` (both `throw` synchronously, outside the inner
    // `.catch`) and `_materialClip`'s OffscreenCanvas.
    //
    // The catch below restores the pre-call invariant and nothing else. On the
    // SUCCESS path a `try` block around unchanged statements is a no-op by
    // construction — the catch is unreachable when nothing throws — which is
    // what makes the flag-off/flag-on identity argument exact rather than
    // empirical.
    try {
      ...
      // Every sibling must release its async owner before staging is disposed.
      const settled = await Promise.allSettled(a), failed = settled.find((result) => result.status === "rejected");
      if (failed) throw failed.reason;
      o.forEach((c) => c._loadState = "loaded");
      if (FLY_PATCH.rasterMark && this.model?.material.some((m) => m.map?.image) &&
          o.some((c) => c.model?.material.some(flyRasterError))) {
        this._rasterLodFails = Math.min(6, (this._rasterLodFails || 0) + 1);
        this._rasterLodAt = FLY_NOW() + Math.min(30000, 1000 * 2 ** (this._rasterLodFails - 1));
        this.unloadSubTiles();
        return false; // parent image and geometry remain attached
      }
      this._rasterLodFails = 0, this._rasterLodAt = 0;
      ...
      if (!FLY_PATCH.unlockOnReject) throw err;
      FLY_HOLD_STATS.rejectLoad++, FLY_HOLD_STATS.lastError = String(err?.message ?? err);
      this._rasterLodFails = Math.min(6, (this._rasterLodFails || 0) + 1);
      this._rasterLodAt = FLY_NOW() + Math.min(30000, 1000 * 2 ** (this._rasterLodFails - 1));
      // `unloadSubTiles()` is the exact undo: it clears `_subTiles` (re-opening
      // LOD()'s guard) and unloads whichever of the four orphans were built, so
      // the node returns to being the ordinary leaf it was before the call. Its
      // `subTiles || children` fallback is safe here — `_loadSubTiles` only runs
      // when `!this.subTiles`, so `children` holds this tile's own model, which
      // is a TileMesh and not a Tile and is therefore never touched by it.
      return this.unloadSubTiles(), !1;
    }
  }
  /**
   * 移除子瓦片（合并操作）。
  ...
  async _removeSubTiles(e) {
    // R24 PATCH #7 (`unlockOnReject`) — OFF = re-throw, i.e. upstream verbatim.
    // This one is WORSE than the `_loadSubTiles` case: the first two statements
    // below tear down the interior-node invariant (`_subTiles = void 0`) and set
    // `_loadState = "loading"`, and `_update`'s guard skips a "loading" node AND
    // ITS ENTIRE SUBTREE. A rejection here therefore freezes a whole region of
    // the map permanently — it stops updating its frustum, stops refining and
    // stops merging. There is also no timeout on the raster fetch it awaits
    // (TILE_HOLD.raster adds one engine-side), so a hung connection reaches the
    // same state without any error at all.
    try {
      ...
      if (FLY_PATCH.rasterMark && o.material.some(flyRasterError)) {
        this.unloadModel();
        const kids = this.children.filter((c) => c instanceof T);
        this._subTiles = kids.length ? kids : void 0;
        this._rasterLodFails = Math.min(6, (this._rasterLodFails || 0) + 1);
        this._rasterLodAt = FLY_NOW() + Math.min(30000, 1000 * 2 ** (this._rasterLodFails - 1));
        return false; // never replace valid fine coverage with an error material
      }
      this._rasterLodFails = 0, this._rasterLodAt = 0;
      // Patch #7 completion repair: _update skips this loading node while the
      // root still refreshes q/N. Re-read visibility before the async verdict,
      // otherwise a return to view retains the stale off-screen x5 penalty and
      // destroys fine children that the current camera still needs.
      if (FLY_PATCH.unlockOnReject) this._inFrustum = q.intersectsBox(this.BBox);
      let l = this._LODEvaluate(r, n, s) !== 2;
      if (!l && ir) {
        const wait = ir.onMerge(this, o);
        if (wait) {
          this._loadState = "loading";
          await wait;
          this._loadState = "loaded";
          ir.onMergeEnd?.(this, o);
          if (FLY_PATCH.unlockOnReject) {
            this._inFrustum = q.intersectsBox(this.BBox);
            l = this._LODEvaluate(r, n, s) !== 2;
          }
        }
      }
      if (l && FLY_PATCH.unlockOnReject) {
        this.unloadModel();
        // Cancellation must restore the same child index as rejection. The
        // meshes stayed attached, but LOD cannot merge again without subTiles.
        const kids = this.children.filter((c) => c instanceof T);
        this._subTiles = kids.length ? kids : void 0;
        return l;
      }
      ...
      if (!FLY_PATCH.unlockOnReject) throw err;
      FLY_HOLD_STATS.rejectMerge++, FLY_HOLD_STATS.lastError = String(err?.message ?? err);
      this._rasterLodFails = Math.min(6, (this._rasterLodFails || 0) + 1);
      this._rasterLodAt = FLY_NOW() + Math.min(30000, 1000 * 2 ** (this._rasterLodFails - 1));
      // Undo, in the order the invariants were torn down.
      // (1) The half-built replacement mesh: dispose it ONLY if it never made it
      //     into the scene graph. If it has a parent the merge effectively
      //     landed and it is this node's real model, so it must be kept.
      const m = this._model;
      ir?.onMergeEnd?.(this, m);
      m && !m.parent && (this._model = void 0, m.dispose());
      // (2) `_subTiles`. Restoring it is not optional: the children are still
      //     attached, so without it the node can neither merge (LOD() guards on
      //     `this.subTiles`) nor refine (`isLeaf` is false with 5 children) —
      //     a different permanent brick from the one we are fixing.
      const kids = this.children.filter((c) => c instanceof T);
      this._subTiles = kids.length ? kids : void 0;
      // (3) The state that freezes the subtree. Never leave it at "loading".
      return this._loadState = this._model ? "loaded" : "empty", this._mergeWantSince = 0, !1;
    }
  }
  /**
   * 更新瓦片模型数据（用于脏瓦片重新加载）。
  ...
  async _updateModel(e) {
    // R24 PATCH #7 (`unlockOnReject`) — OFF = re-throw, i.e. upstream verbatim.
    // Same freeze as `_removeSubTiles`: `_loadState` is left at "loading" and
    // `_update` then skips this node and everything under it, forever.
    try {
      ...
      if (!FLY_PATCH.unlockOnReject) throw err;
      FLY_HOLD_STATS.rejectUpdate++, FLY_HOLD_STATS.lastError = String(err?.message ?? err);
      // `_loadedEpoch` is advanced DELIBERATELY. `_needVersionUpdate` is
      // `_loadedEpoch < _epoch && _loadState !== "empty"`, so clearing the
      // "loading" state without it would re-enter this method on every 50 ms LOD
      // tick against a source that has just proven it cannot answer — a 20 Hz
      // retry storm. Accepting the STALE imagery for this epoch is strictly
      // better than both a frozen subtree and a storm, and the next genuine
      // epoch bump re-asks.
      this._loadState = "loaded", this._loadedEpoch = this._root._epoch;
    }
  }

  async _retryErrorMaterial(loader) {
    const model = this.model;
    if (!FLY_PATCH.rasterMark || !model || model.parent !== this || this.loadState !== "loaded" ||
        !model.material.some((m) => m?.userData?.flyError) || this._rasterRetryActive) return false;
    const root = this._root, epoch = root._epoch;
    const resident = () => {
      let tile = this;
      while (tile && tile !== root) tile = tile.parent;
      return tile === root && !!root.parent && this.model === model && model.parent === this &&
        this.loadState === "loaded" && root._epoch === epoch && FLY_PATCH.rasterMark;
    };
    if (!resident()) return false;
    const now = FLY_NOW();
    if (this._rasterRetryModel !== model || this._rasterRetryEpoch !== epoch) {
      this._rasterRetryModel = model, this._rasterRetryEpoch = epoch;
      this._rasterRetryAttempts = 0, this._rasterRetryAt = now + 1000;
    }
    if (now < this._rasterRetryAt || FLY_HOLD_STATS.rasterRetryActive >= 2 ||
        loader.downloadingThreads + 4 >= loader.maxThreads) return false;
    this._rasterRetryActive = true;
    this._rasterRetryAttempts++;
    FLY_HOLD_STATS.rasterRetryActive++, FLY_HOLD_STATS.rasterRetryStarted++;
    try {
      const changed = await loader.updateResidentMaterial(this, model, resident);
      if (!resident()) { FLY_HOLD_STATS.rasterRetryDiscarded++; return false; }
      if (changed) {
        model.syncGroups();
        this._updateShadow();
        // Same event as ordinary material loads: TerrainEngine.onTileMaterial
        // installs bend/grading hooks synchronously before the next render.
        root.dispatchEvent({ type: "tile-loaded", tile: this });
        if (!model.material.some((m) => m?.userData?.flyError)) FLY_HOLD_STATS.rasterRetryRecovered++;
      }
      return changed;
    } catch (error) {
      FLY_HOLD_STATS.lastError = String(error?.message ?? error);
      return false;
    } finally {
      this._rasterRetryActive = false;
      this._rasterRetryAt = FLY_NOW() + Math.min(30000, 1000 * 2 ** Math.min(this._rasterRetryAttempts, 5));
      FLY_HOLD_STATS.rasterRetryActive--;
    }
  }
}
class ae extends Te {
...
// R24 A PATCH 26 (TERRA_PACE.parkOffscreen) — recon T1, the DRAW half of
// keepResident.
//
// THE DEFECT THIS CLOSES. keepResident stops the field behind the camera from
// collapsing, which is the round's headline — but a retained tile is still
// ATTACHED to the scene graph, so three walks it, culls it and, where the bent
// world or a generous bound lets it through, DRAWS it. Pass 2b measured the
// retained set growing with sweep duration on one fixed pose — Owens draws 185
// after a 45 s sweep and 279 after a 600 s sweep with the SAME flags, resident
// tiles 62 -> 103, tris x2.4 — i.e. residency that costs draw calls forever.
// The frozen Owens ceiling (261) was never set against that, and on a real GPU
// at 60+ fps the accumulation is an order of magnitude faster than the fixture.
//
// The fix separates RETENTION from ISSUE. A tile that is out of frustum keeps
// its model, its textures and its place in the tree — nothing is disposed and
// nothing will be re-downloaded — but its model is not visible, so three skips
// the whole subtree in projectObject: no draw, and no per-mesh cull either.
// Coming back into view is one boolean.
//
// It is the MODEL that is parked, never the tile: a tile's children hang off
// the tile, so hiding the tile itself would hide in-frustum descendants. And a
// parent whose box contains an in-frustum child is itself in frustum, so a
// visible tile can never be orphaned behind a parked ancestor.
//
// The same visit stamps `_r24LastVisible`, which is what makes the residency
// LRU an LRU rather than a distance heuristic.
function r24Park(tile) {
  if (!R24_SWITCHES.parkOffscreen) return;
  if (tile._inFrustum) tile._r24LastVisible = R24_SWITCHES.r24Frame;
  const m = tile._model;
  if (m && m.visible !== tile._inFrustum) m.visible = tile._inFrustum;
}

// R24 A PATCH 4 (TERRA_PACE.skirtFast) — recon T2 / FL-02 / A2.
// -----------------------------------------------------------------------
// Upstream's getBoundaryEdges (`We`, below) allocates THREE two-element
// arrays per triangle, sorts all 3T of them with a boxed JS comparator that
// does four min/max per comparison, then dedupes. R22.1 profiled that
// function at 37% + its comparator at 30% = 67% of every stalled millisecond
// while streaming (scripts/r22p1-b-stutter.md §2.2, archived).
//
// The same answer is O(E) with no per-tile allocation: count each UNDIRECTED
// edge in a module-scoped, generation-stamped open-addressed table (so it is
// never cleared, only re-stamped), keep the ones seen exactly once, emit each
// in its single occurrence's direction, and sort only the PERIMETER — a few
// hundred edges, not 3T.
//
// It is output-IDENTICAL for every input it claims, because upstream's output
// is precisely "the edges whose (min,max) key occurs once, ordered by
// (min,max)": its sort groups equal keys together, and its dedupe pass drops
// exactly the adjacent reverse pairs. For anything else — a length that is
// not a multiple of 3, a negative index, a degenerate a===b edge (whose
// min/max collapse makes it its own reverse), an edge seen three times, or
// two occurrences with the SAME winding (the one case upstream KEEPS both) —
// this returns null and the verbatim upstream body below runs instead.
let r24EdgeCap = 0;
let r24KeyMin = null;
let r24KeyMax = null;
let r24Meta = null; // count in bits 0-1, direction flag in bit 2
let r24Stamp = null;
let r24Occ = null;
let r24Gen = 0;
function r24EnsureEdgeTable(edges) {
  let cap = 1024;
  while (cap < edges * 2) cap <<= 1; // load factor <= 0.5
  if (cap <= r24EdgeCap) return;
  r24EdgeCap = cap;
  r24KeyMin = new Int32Array(cap);
  r24KeyMax = new Int32Array(cap);
  r24Meta = new Int32Array(cap);
  r24Stamp = new Int32Array(cap);
  r24Occ = new Int32Array(cap);
  r24Gen = 0; // fresh stamps for a fresh table
}
function r24BoundaryEdgesFast(idx) {
  const n = idx.length;
  if (n === 0 || n % 3 !== 0) return null;
  r24EnsureEdgeTable(n);
  const mask = r24EdgeCap - 1;
  const gen = ++r24Gen;
  let occN = 0;
  for (let t = 0; t < n; t += 3) {
    ...
    for (let e = 0; e < 3; e++) {
      ...
      const dir = a === lo ? 0 : 1;
      let s = (Math.imul(lo, 2654435761) ^ Math.imul(hi, 2246822519)) & mask;
      for (;;) {
        if (r24Stamp[s] !== gen) {
          r24Stamp[s] = gen;
          r24KeyMin[s] = lo;
        ...
        if (r24KeyMin[s] === lo && r24KeyMax[s] === hi) {
          const m = r24Meta[s];
          const count = m & 3;
          if (count >= 2) return null; // seen 3x: upstream's pairwise dedupe
  ...
  for (let k = 0; k < occN; k++) {
    const s = r24Occ[k];
    if ((r24Meta[s] & 3) === 1) keep.push(s);
  }
  keep.sort((x, y) =>
    r24KeyMin[x] !== r24KeyMin[y] ? r24KeyMin[x] - r24KeyMin[y] : r24KeyMax[x] - r24KeyMax[y]
  );
  const out = new Array(keep.length);
  for (let k = 0; k < keep.length; k++) {
    const s = keep[k];
    out[k] = (r24Meta[s] >> 2) === 0 ? [r24KeyMin[s], r24KeyMax[s]] : [r24KeyMax[s], r24KeyMin[s]];
  }
  R24_STATS.skirtEdges = n;
  return out;
}
// R24 A PATCH 20 (TERRA_PACE.skirtWorker) — recon T2 / FL-02 / A2 (the F2 half).
// -----------------------------------------------------------------------
// PATCH 4 made the main-thread boundary scan O(E). This moves the whole skirt
// build — scan, skirt vertices, attribute concatenation — INTO the DEM worker
// and returns the finished arrays as TRANSFERABLES, so the main thread does
// nothing but wrap them in BufferAttributes.
//
// The worker source is three-tile's own minified string, VERBATIM, with only
// its `self.onmessage = …` tail replaced by a readable module
// (lib/fly/vendor/three-tile/workers/skirt-tail.src.js, stringified by
// scripts/build-tile-worker.mjs). Hand-editing the minified blob is what
// VENDOR.md forbids; this is the sanctioned way to add worker-side work, and
// it is the seam another agent extends for worker-side DEM normals (recon T6).
//
// If the tail shape is ever not found, the splice returns null and the
// verbatim upstream worker is created instead — the switch degrades to off.
const R24_WORKER_TAIL_RE =
  /self\.onmessage=(\w+)=>\{const (\w+)=\1\.data,(\w+)=(\w+)\(\2\.demData,\2\.z,\2\.clipBounds(?:,\2\.errTable)?\);self\.postMessage\(\3\)\}/;
const r24WorkerSrcCache = new Map();
function r24SpliceWorkerTail(src) {
  if (r24WorkerSrcCache.has(src)) return r24WorkerSrcCache.get(src);
  const m = R24_WORKER_TAIL_RE.exec(src);
  // R24 C PATCH 8 (TERRAIN_LIGHT.workerNormals) — recon T6 / L8.
  // A Blob worker cannot see R24_SWITCHES, and the built tail must stay
  // flag-independent (gate 19 diffs it against the .src.js), so the switch is
  // substituted into the source at splice time exactly the way `__DECODE__` is.
  // False leaves the whole normals branch dead and A's skirt output
  // element-for-element identical, which is what keeps verify-skirt-worker's
  // identity leg meaningful with C's switch off.
  const r24Tail = R24_WORKER_SKIRT_TAIL.replace(
    /__R24_NORMALS__/g,
    R24_SWITCHES.workerNormals ? "true" : "false"
  );
  const out = m
    ? src.slice(0, m.index) + r24Tail.replace(/__DECODE__/g, m[4]) + src.slice(m.index + m[0].length)
    : null;
  r24WorkerSrcCache.set(src, out);
  return out;
}
function r24MakeWorker(src, opts) {
  const spliced = r24SpliceWorkerTail(src);
  if (!spliced || typeof self === "undefined" || !self.Blob) return null;
  let url;
  try {
    url = (self.URL || self.webkitURL).createObjectURL(
      new Blob([spliced], { type: "text/javascript;charset=utf-8" })
    );
    const w = new Worker(url, { name: opts?.name });
    w.addEventListener("error", () => {
      (self.URL || self.webkitURL).revokeObjectURL(url);
    });
    R24_STATS.skirtWorkers++;
    return w;
  } catch {
    return null;
  } finally {
    if (url) (self.URL || self.webkitURL).revokeObjectURL(url);
  }
}
// R25 D PATCH 31 (R25_GROUND.relief) — the relief splice, LAYERED on A's
// PATCH 20 splice (whose output is untouched when this is not called). A's
// splice leaves the two R25 markers in the readable tail inert (the skirt
// literal `true`, the mesh-wrap comment), which is exactly the R24 worker; this
// substitutes them: the worker's grid->Martini function (found by SHAPE,
// `function N(g,..){return new M(g.width).createTile(g.dem)`) is wrapped so the
// handler sees the full decoded, clipped grid, and — spliced for R25 ALONE —
// the skirt literal becomes `false` so the main thread keeps building the skirt
// exactly as upstream does. Cached per source; null (no tail shape, no mesher)
// falls back to the caller's verbatim path.
const R25_MESH_FN_RE = /function (\w+)\((\w+),\w+(?:,\w+)?\)\{return new \w+\(\2\.width\)\.createTile\(\2\.dem\)/;
const r25WorkerSrcCache = new Map();
function r25SpliceWorkerTail(src) {
  const r24Skirt = !!(R24_SWITCHES.skirtWorker || R24_SWITCHES.workerNormals);
  const key = (r24Skirt ? "s" : "-") + src;
  if (r25WorkerSrcCache.has(key)) return r25WorkerSrcCache.get(key);
  const base = r24SpliceWorkerTail(src);
  const mesh = R25_MESH_FN_RE.exec(src);
  const out = base && mesh && base.includes("/*__R25_MESH_WRAP__*/")
    ? base
      .replace("true /*__R24_WORKER_SKIRT__*/", r24Skirt ? "true" : "false")
      .replace("/*__R25_MESH_WRAP__*/", mesh[1] + " = r25CaptureMesh(" + mesh[1] + ");")
    : null;
  r25WorkerSrcCache.set(key, out);
  return out;
}
function r25MakeWorker(src, opts) {
  const spliced = r25SpliceWorkerTail(src);
  if (!spliced || typeof self === "undefined" || !self.Blob) return null;
  let url;
  try {
    url = (self.URL || self.webkitURL).createObjectURL(
...
// R25 D PATCH 32 (R25_GROUND.relief) — the relief bytes ride the geometry's
// userData (the app binds them into a pooled texture on tile-loaded).
function r25CarryRelief(geometry, relief) {
  if (relief && relief.length) geometry.userData.r25Relief = relief;
  return geometry;
}
function We(i) {
  // R24 A PATCH 25 (FRAME_STATS attribution) — the boundary scan is the
  // measured hot spot (67% of every stalled ms on the R22 tree); tagging it is
  // how a stall in the user's ring buffer gets a name.
  if (R24_SWITCHES.onPhase) R24_SWITCHES.onPhase('skirt');
  if (R24_SWITCHES.skirtFast) {
    const fast = r24BoundaryEdgesFast(i);
    if (fast) {
      R24_STATS.skirtFastCalls++;
      return fast;
    }
    R24_STATS.skirtBails++;
  }
  if (FLY_EDGE_CAPTURE.max > 0) {
    FLY_EDGE_CAPTURE.rows.push(i.slice());
    if (FLY_EDGE_CAPTURE.rows.length > FLY_EDGE_CAPTURE.max) FLY_EDGE_CAPTURE.rows.shift();
  }
  if (FLY_PATCH.skirtEdges) {
    const fast = flyBoundaryEdgesFast(i);
    FLY_SKIRT_STATS.tris += i.length / 3;
    if (fast) {
      FLY_SKIRT_STATS.fast++;
      return fast;
    }
    FLY_SKIRT_STATS.bail++;
  } else {
    FLY_SKIRT_STATS.upstream++;
    FLY_SKIRT_STATS.tris += i.length / 3;
  }
...
class A extends Ie {
   ...
   */
  setAttributes(e, t = 0) {
    // Capture the surface extent before adding skirts. Worker-built geometry
    // supplies the same scalar; raw/plugin geometry takes this one-time path.
    let surfaceMinZ = e.surfaceMinZ;
    if (!Number.isFinite(surfaceMinZ)) {
      surfaceMinZ = 0;
      const pos = e.attributes.position.value;
      for (let i = 2; i < pos.length; i += 3) surfaceMinZ = Math.min(surfaceMinZ, pos[i]);
    }
    this.userData.surfaceMinZ = surfaceMinZ;
    // R24 A PATCH 21 (TERRA_PACE.skirtWorker) — the worker already built the
    // skirt and transferred the finished arrays, so the only work left is
    // wrapping them. When the worker DECLINED (a non-manifold input), it does
    // not set the flag and the verbatim upstream build below runs as today.
    if (R24_SWITCHES.skirtWorker && e.r24Skirted === !0) {
      const { attributes: r24a, indices: r24i } = e;
      return this.setIndex(new z(r24i, 1)), this.setAttribute("position", new z(r24a.position.value, r24a.position.size)), this.setAttribute("uv", new z(r24a.texcoord.value, r24a.texcoord.size)), this.setAttribute("normal", new z(r24a.normal.value, r24a.normal.size)), this;
    }
...
class Ge {
  ...
  async update(e, t) {
    ...
    let r = !1;
    try {
      if (FLY_PATCH.parallelFetch || R24_SWITCHES.parallelLoad) {
        // R22 PATCH #1 (parallelFetch, OFF = the verbatim serial pair below).
        // Imagery and DEM come from two independent Esri endpoints and touch
        // disjoint mesh state (syncMaterials vs setGeometry); awaiting them in
        // series costs 2 round-trips per tile, which the z2→z16 warp descent
        // pays once per level. syncGroups() still runs after BOTH resolve.
        // A rejected DEM must not dispose staging while imagery still writes it.
        const settled = await Promise.allSettled([this.updateMaterial(e, t), this.updateGeometry(e, t)]);
        const failed = settled.find((result) => result.status === "rejected");
        if (failed) throw failed.reason;
        const [n, s] = settled.map((result) => result.value);
  ...
  async updateGeometry(e, t) {
    ...
    if (t.geometry.userData.source === r)
      return !1;
    // Transport/decoder failures are failed replacements, never a new flat plane.
    const s = await v.getGeometryLoader(r).load({ source: r, ...e }).then((o) => (o.userData.source = r, o));
   ...
   * @returns 材质是否发生变化
   */
  async updateMaterial(e, t, canCommit) {
    const r = t.material, n = this.imgSource.filter((l) => this._checkBounds(l, e)), s = [];
    const borrowed = canCommit ? new Set(r) : null;
    let o = r.length !== n.length;
    // ── R24 PATCH #8 (`rasterMark`) — OFF = the un-stamped clone, verbatim ────
    // On any imagery failure upstream hands the tile a 20%-opacity BLACK
    // MeshBasicMaterial and forgets it happened: no reason code, no retry, no
    // telemetry, and — because `_updateModel` is the only path that re-runs this
    // method and it needs a root epoch bump — the tile keeps that black square
    // until the LOD collapses it away. R21 built reason-coded TTL/backoff for
    // the VECTOR chunk path (STREAM_KEEPER.retry) and it was never ported to
    // raster; TILE_HOLD.raster is that port, and it lives engine-side in
    // lib/fly/raster-cache.js. This site is the half that has to be here.
    //
    // Stamping `userData.source` is what makes the failure censusable. Note
    // carefully that stamping it ALONE would have the OPPOSITE of the intended
    // effect — the reuse test immediately below would then MATCH the error
    // material and keep it forever — so the marker `flyError` is what actually
    // re-opens the re-ask, by excluding it from that test. `flyError` is never
    // set with the patch off, so the guard is byte-equivalent to upstream then.
    const a = async (l) => await v.getMaterialLoader(l).load({ source: l, ...e }).then((c) => (c.userData.source = l, c)).catch((c) => {
      this.log("Failed to load material:", c);
      const m = this._errorMaterial.clone();
      return FLY_PATCH.rasterMark && (m.userData.source = l, m.userData.flyError = 1, FLY_HOLD_STATS.errorTiles++), m;
    });
    let committed = false;
    const retained = [];
    try {
      for (const l of n) {
        const h = r.find((c) => c?.userData?.source === l && !c?.userData?.flyError);
        if (h)
          s.push(h);
        else {
          o = !0;
          const c = await a(l);
          const previous = FLY_PATCH.rasterMark && flyRasterError(c) &&
            (r.find((m) => m?.userData?.source === l && m.map?.image) ||
              (r.length === 1 && n.length === 1 && r[0].map?.image ? r[0] : null));
          if (previous) {
            c.dispose();
            s.push(previous), retained.push([previous, l]);
          } else s.push(c), this._materialClip(c, l, e);
        }
      }
      // Material-only retries may finish after LOD disposal or a source epoch
      // change. Ordinary loading calls omit the guard and retain their behavior.
      if (canCommit && !canCommit()) return false;
      // Mark only after the async commit guard: an obsolete result cannot
      // mutate a still-live material now owned by a newer source epoch.
      for (const [material, source] of retained) Object.assign(material.userData, { source, flyError: 1 });
      return t.syncMaterials(s), committed = true, o;
    } finally {
      if (canCommit && !committed) for (const material of s) {
        if (!borrowed.has(material)) Z.prototype.disposeMaterial.call({ material: [material] }, 0);
      }
    }
  }
  /** 将材质纹理裁剪到地图范围 */
  ...
  updateResidentMaterial(e, t, canCommit) {
    return this.updateMaterial(e, t, canCommit);
  }
}
class H {
  constructor() {
...
class Qe extends H {
  ...
  // Regional Esri coverage can end below the advertised max zoom. LERC's
  // finite -3.4028e38 no-data value must never become terrain. Retry the same
  // geographic footprint against measured parent data, keeping fine imagery.
  async load(params) {
    const { source, x, y, z } = params;
    if (z < source.minLevel) return super.load(params);
    this._demNoData ??= new Map();
    const now = FLY_NOW(), first = Math.min(z, source.maxLevel);
    // A 257x257 source can be clipped at most eight levels to a 2x2 grid.
    const last = Math.max(source.minLevel || 0, z - 8);
    let failure;
    for (let level = first; level >= last; level--) {
      const parent = He(x, y, z, level), coord = parent.coord;
      const url = source.getUrl(coord.x, coord.y, coord.z);
      const region = url + '|' + parent.bounds.join(',');
      if ((this._demNoData.get(url) || 0) > now || (this._demNoData.get(region) || 0) > now) continue;
      try {
        const geometry = await this.doLoad(url, { ...params, clipBounds: parent.bounds });
        geometry.userData.demSourceLevel = level;
        v.manager.parseEnd(geometry);
        return geometry;
      } catch (error) {
        if (!error.demNoData) throw error;
        failure = error;
        // A missing quadrant must not suppress valid data elsewhere in the
        // same source tile. Only an entirely empty DEM gets a whole-URL entry.
        const key = error.demNoDataFull ? url : region;
        this._demNoData.delete(key);
        this._demNoData.set(key, now + 300000);
        while (this._demNoData.size > 256) this._demNoData.delete(this._demNoData.keys().next().value);
      }
    }
    if (!failure) { failure = new Error('No measured DEM coverage for tile'); failure.demNoData = true; }
    throw failure;
  }
  geometryFromData(data, level) {
    if (data.demNoData) {
      const error = new Error('LERC tile contains no-data elevations');
      error.demNoData = true;
      error.demNoDataFull = data.demNoDataFull === true;
      throw error;
    }
    // R25 D PATCH 32 (R25_GROUND.relief) — carry the worker's relief map
    // (present only when the request asked for it) onto the geometry.
    if (data.r25Relief) return r25CarryRelief(new A().setAttributes(data, level), data.r25Relief);
    return new A().setAttributes(data, level);
  }
  /**
  ...
  async doLoad(e, t) {
    const { z: r, clipBounds: n } = t, o = {
      ...
      z: r,
      clipBounds: n,
      // R25 D PATCH 32 (R25_GROUND.relief) — ask for the relief map only while
      // the app requests it (Visuals Enhanced); off, the spread adds no key.
      ...(R24_SWITCHES.r25Relief && R24_SWITCHES.r25ReliefPx > 1 ? { r25Relief: R24_SWITCHES.r25ReliefPx } : {}),
      // R22 PATCH #3 (demErrorTable): undefined ⇒ the worker uses its own
      // embedded upstream curve, i.e. verbatim behavior. See flySetDemErrorTable.
      errTable: FLY_PATCH.demErrorTable || void 0
    }, a = (await this._workerPool.postMessage(o)).data;
    return this.geometryFromData(a, r);
  }
}
...
class tt extends H {
  ...
  async doLoad(e, t) {
    ...
    // R25 D PATCH 33 (R25_GROUND.relief) — terrain-rgb meshes the decoded grid
    // on the MAIN thread (setData below), so its relief map is built from that
    // same returned grid by the app's function; off, one short-circuited &&.
    R24_SWITCHES.r25ReliefPx > 1 && R24_SWITCHES.r25ReliefFn && r25CarryRelief(h, R24_SWITCHES.r25ReliefFn(l, Math.round(Math.sqrt(l.length)), Math.round(Math.sqrt(l.length)), s, R24_SWITCHES.r25ReliefPx));
...
class ut extends Ge {
  ...
  updateResidentMaterial(e, t, canCommit) {
    return super.updateResidentMaterial(this._getTileCoords(e), t, canCommit);
  }
}
...
export { flyGetPatch, flySetDemErrorTable as setDemErrorTable, flySetPatch as setFlyPatch, flyBoundaryEdgesFast, flyBoundaryEdgesRef, flySkirtStats, flyEdgeCapture, flyTileHoldStats };
