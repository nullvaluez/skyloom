...
class T extends oe {
  ...
  LOD(e, r24NoStart) {
    ...
    // R24 A PATCH 23 (TERRA_PACE.walkWhileSaturated) — PATCH 7's companion. The
    // decision is still computed and returned (so `_LODEvaluate` keeps the
    // tree's state honest); only the ACTION is withheld while the loader is
    // saturated. Every upstream call site passes one argument and falls
    // through to the unchanged line below.
  ...
  _update(e) {
    ...
    if (R24_SWITCHES.walkWhileSaturated) {
