...
// ─────────────────────────────────────────────────────────────────────────────
// R22 A TERRA — vendored patch switches. Every patch below sits behind one of
// these flags and EVERY flag defaults OFF, in which case the code path is
// byte-equivalent to upstream three-tile 0.12.1. lib/fly/terrain-engine.js is
// the only caller of setFlyPatch(); it drives them from TERRA_PIPE /
// TERRA_SHARP (both enabled:false by default). See VENDOR.md for the ledger.
//   parallelFetch  — patch #1, TileLoader.update  (imagery ‖ DEM)
//   lodBailFix     — patch #2, Tile._update       (skip subdivide, keep walking)
//   demErrorTable  — patch #3, TerrainLercLoader  (martini decimation targets)
//   skirtEdges     — patch #5, getBoundaryEdges   (R22.1 B STUTTER; see below)
//   mergeDwellMs   — patch #6a, Tile.LOD          (R24 A MOTION HOLD; see below)
//   frustumPenalty — patch #6b, Tile._getDistRatio
//   lodHysteresis  — patch #6c, Tile._LODEvaluate
//   unlockOnReject — patch #7, the three async Tile mutators
//   rasterMark     — patch #8, TileLoader.updateMaterial (error-material policy)
// ─────────────────────────────────────────────────────────────────────────────
const FLY_PATCH = {
  parallelFetch: !1,
  lodBailFix: !1,
...
/** Monotonic clock for the merge dwell. `performance` exists in every browser
 *  and in Node >= 16, but the fallback keeps the module importable anywhere. */
const FLY_NOW = typeof performance < "u" && performance.now ? () => performance.now() : () => Date.now();
/**
 * R24 receipts, exported beside `flySkirtStats` for the same reason: a gate must
 * be able to prove which arm it measured and that the patched path was
 * EXERCISED, not merely armed. `dwellHeld` counts LOD ticks where a merge was
 * deferred; `dwellFired` counts merges that survived the dwell and went ahead;
 * `dwellArmed` counts the transitions into "this node wants to merge".
 * `rejectLoad`/`rejectMerge`/`rejectUpdate` count recoveries that, before patch
 * #7, permanently bricked a node or froze a whole subtree.
 */
...
/** Merge patch switches. Returns the live object (also readable via getFlyPatch). */
function flySetPatch(i) {
  return i && Object.assign(FLY_PATCH, i), FLY_PATCH;
}
/** Read-only view of the live patch switches. */
function flyGetPatch() {
  return FLY_PATCH;
}
/**
 * R22 patch #3 — replace the martini mesh-error table for the LERC DEM path.
 * `table` is a plain map {zoom: maxVerticalErrorMeters}; a zoom absent from it
 * falls back to upstream's `Math.round(7000*(1-z/17)^3)` curve. Pass null to
 * restore upstream exactly.
 *
 * NOTE (measured, R22): the module-private `le = Ee(21)` at the bottom of this
 * comment block is NOT the table our DEM actually uses — the LERC loader
 * decodes AND decimates inside its embedded worker, which carries its own copy
 * of the curve (`P = W(21)`, consumed by `ie()` in the worker source string).
 * The table therefore travels with each postMessage instead of mutating a
 * closure. `le` is left byte-verbatim: it serves only the Mapbox terrain-rgb /
 * terrain-dem loaders and TileGeometry.setData(), none of which this app uses.
 */
...
class T extends oe {
  ...
  constructor(e = 0, t = 0, r = 0) {
    // R24 patch #6a: `_mergeWantSince` is declared HERE rather than lazily in
    // LOD() so every Tile shares one V8 hidden class on the hottest object in
    // the library. It is written by nothing and read by nothing while
    // FLY_PATCH.mergeDwellMs is 0, so the field is inert when the patch is off.
    super(), this.isTile = !0, this._root = this, this._sizeInWorld = -1, this._maxZ = 0, this._loadState = "empty", this._inFrustum = !1, this._epoch = 0, this._loadedEpoch = -1, this._mergeWantSince = 0, this.x = e, this.y = t, this.z = r, this.name = `Tile ${r}-${e}-${t}`, this.up.set(0, 0, 1);
  }
  /** 瓦片数据加载状态 */
  get loadState() {
  ...
  LOD(e) {
    ...
    const _dwell = FLY_PATCH.mergeDwellMs;
    if (_dwell > 0) {
      if (s !== 2)
  ...
  _update(e) {
    const { loader: t } = e;
    const _busy = t.downloadingThreads + 4 >= t.maxThreads;
    // R22 PATCH #2 (lodBailFix, OFF = the verbatim upstream expression below).
    // Upstream RETURNS from this node when the loader is busy, and because the
    // walk is depth-first from the root that truncates the ENTIRE remainder of
    // the tree for the frame — including the deep, near-camera tiles that sort
    // last in DFS order and are exactly the ones a descent is waiting on. The
    // patch keeps walking while busy but starts NO work: no _updateModel, no
    // LOD() (the only path that reaches _loadSubTiles/_removeSubTiles). Every
    // node re-reads downloadingThreads live, so the budget is spent by whoever
    // the walk reaches once threads free, instead of by a fixed DFS prefix.
    if (_busy && FLY_PATCH.lodBailFix && this.loadState !== "loading") {
      this._updateShadow(), this._inFrustum = q.intersectsBox(this.BBox), !(!this._inFrustum && this._model && this.isLeaf) && this.children.forEach((r) => r instanceof T && r._update(e));
      return;
    }
    if (!(_busy || this.loadState === "loading")) {
  ...
  // Patch #8a: flyError excluded reuse but never scheduled a resident re-ask.
  // Retry material only, retaining geometry/LOD/epoch and the old material until
  // replacement. At most two retries globally; 1s then 2/4/8/16/30s backoff.
  ...
  _LODEvaluate(e, t, r) {
    ...
    // R24 PATCH #6c (`lodHysteresis`) — BUILT, NOT ARMED (ships at 1).
    // The classic deadband: refine at `n <= r`, merge only past `n > r*H`. It is
    // the textbook cure for the missing hysteresis, and it is deliberately left
    // at 1 because — unlike patch #6a — it MOVES THE SETTLED TREE at every
    // frozen pose, i.e. it would move Owens draws, P-LEWIS triangles and the
    // texture-bytes gate. Arming it is a measured decision, not a default.
    // At H <= 1 the `_m` binding is `r` itself and the expression below is the
    // verbatim upstream one.
    const _h = FLY_PATCH.lodHysteresis, _m = _h > 1 ? r * _h : r;
    return this.isLeaf && this.z < t && n <= r ? 1 : !this.isLeaf && this.z >= e && n > _m ? 2 : 0;
  }
  /**
  ...
  _getDistRatio() {
    ...
    // R24 PATCH #6b (`frustumPenalty`) — SHIPS AT 5, i.e. `t * 5`, the verbatim
    // upstream literal. This is the multiplier that makes off-camera ground 2.64
    // levels coarser than on-camera ground (log2(5/0.8)); patch #6a holds the
    // CONSEQUENCE at bay without touching the settled state, and lowering this
    // number attacks the cause instead. It is a knob and not a default because
    // it DOES move the settled tree: fewer levels of collapse means more
    // resident tiles, and the binding budget is NOT the Owens 261 draw ceiling
    // (out-of-frustum tiles issue zero draws) but verify-aerial's texture-bytes
    // gate. Sweep 5 -> 3 -> 2 -> 1.6 against texture bytes and gpuFrameMs on an
    // egress-enabled machine before proposing a move (scripts/r24-a-churn.js
    // header carries the protocol). `|| 5` also restores upstream for a null,
    // 0 or absent value, so a malformed constants block cannot change the world.
...
/**
 * R22.1 PATCH #5 (`skirtEdges`) — the boundary-edge finder used to build every
 * DEM tile's skirt, and MEASURED as the single largest cause of the live
 * low-AGL stutter (scripts/r22p1-b-stutter.md: 67% of all stall-window CPU
 * samples, 1,107 ms out of a 30 s window, concentrated into ~4-tile bursts
 * because `_loadSubTiles` resolves four children in one microtask drain).
 *
 * Upstream (below, kept verbatim and still reachable) allocates 3·T
 * two-element ARRAYS, sorts them with a JS comparator that runs four
 * Math.min/max per comparison (~165k comparator calls for a 4k-triangle tile),
 * then de-duplicates adjacent reverse pairs. Both halves are pure garbage:
 * the answer is "which undirected edge occurs exactly once".
 *
 * IDENTICAL OUTPUT BY CONSTRUCTION, not by hope:
 *   - the surviving slots are explicitly sorted by the (min, max) vertex pair,
 *     which IS upstream's comparator; every boundary edge is unique under that
 *     key, so no tie-break exists to disagree about;
 *   - a boundary edge is emitted in the DIRECTION of its single occurrence,
 *     which is exactly the entry upstream's dedup loop pushes;
 *   - the fast path RETURNS NULL — deferring to the verbatim upstream body —
 *     for every input it does not claim: a non-multiple-of-3 index array, a
 *     negative (or NaN) index, a third occurrence of one undirected edge
 *     (non-manifold), or
 *     two occurrences that are not exact reverses (inconsistent winding, the
 *     one case where upstream emits BOTH halves as boundary edges).
 * verify-frame-pace gate (2) proves the equality on live tile index buffers.
 */
/* Open-addressed edge table, module-scoped and REUSED. A Map keyed by
 * min*2^21+max was tried first and measured only 2.3x (475 ms of a 30 s
 * window): those keys are heap doubles, so every probe boxes and hashes one.
 * Three parallel Int32Arrays with linear probing keep the whole hot loop on
 * SMIs and allocate nothing per tile. Not re-entrant — the call site is
 * synchronous, single-threaded and non-recursive. */
let _flyECap = 0;
let _flyEGen = 0; // generation stamp, so the table is never cleared
let _flyEG = null; // slot generation — a slot is EMPTY unless HG[s] === gen
let _flyEU = null; // min vertex of the slot
let _flyEV = null; // max vertex of the slot
let _flyEP = null; // first-occurrence code (triStart*4 + edge), -1 once paired
let _flyEOcc = null; // compact list of the slots actually written this call
...
/** Receipts, so a gate can prove the fast path was EXERCISED rather than
 *  merely armed (a patch that silently bails on every real tile would pass an
 *  "output is identical" gate perfectly). */
const FLY_SKIRT_STATS = { fast: 0, bail: 0, upstream: 0, tris: 0 };
function flySkirtStats() {
  return FLY_SKIRT_STATS;
}
/**
 * Harness-only capture of the REAL index buffers this function is handed.
 * `max` 0 (the default, and the only value production ever has) costs one
 * integer compare per tile. It exists because the resident tile geometries are
 * the wrong corpus for an identity proof: below demMaxZoom the leaves are
 * imagery-only tiles still wearing the default 4-vertex PlaneGeometry, so a
 * gate that harvests the scene graph proves identity on 2-triangle quads while
 * the martini meshes that actually cost the 4 ms are already unloaded.
 *
 * It keeps the LAST `max`, not the first: a quadtree descent starts at the
 * root, and upstream's own error curve gives z2-z8 a 2-triangle mesh, so a
 * first-N capture yields a 48-triangle corpus and proves nothing about the
 * ~4 000-triangle z13-z16 meshes the defect lives in.
 */
...
/** The two implementations, exported side by side so verify-frame-pace can
 *  prove they agree on real tile index buffers rather than assert it. */
function flyBoundaryEdgesRef(i) {
  const was = FLY_PATCH.skirtEdges;
...
class Ge {
  ...
  // Material-only equivalent of update(). Map loaders override this entry to
  // apply exactly the same coordinate/bounds preprocessing as ordinary loads.
...
gt();
export {
  // R22 A TERRA patch seam (see VENDOR.md). Not upstream API.
  flyGetPatch,
  flySetDemErrorTable as setDemErrorTable,
  flySetPatch as setFlyPatch,
  // R22.1 B STUTTER patch #5 seam — the two boundary-edge implementations,
  // exported so verify-frame-pace can PROVE they agree. Not upstream API.
  flyBoundaryEdgesFast,
  flyBoundaryEdgesRef,
  flySkirtStats,
  flyEdgeCapture,
  // R24 A MOTION HOLD patch #6/#7/#8 seam — the dwell/reject/error receipts, so
  // a gate can prove the patched path was EXERCISED rather than merely armed.
  // Not upstream API.
  flyTileHoldStats,
  V as EARTH_RADIUS,
  v as LoaderFactory,
