...
class T extends oe {
  ...
  get BBox() {
    ...
    if (R24_SWITCHES.bboxCache) {
      ...
      this._r24Box = new ke(new I(-a, -b, 0), new I(a, b, this._maxZ)).applyMatrix4(this.matrixWorld);
      this._r24BoxK0 = this._maxZ;
      this._r24BoxK1 = m[12];
      this._r24BoxK2 = m[13];
      this._r24BoxK3 = m[14];
...
//
// R25 A GROUND — THE FOURTH ARGUMENT, and why this regex had to widen.
// ----------------------------------------------------------------------
// The Codex overhaul re-applied VENDOR.md patch #3 (`demErrorTable`) into the
// LERC worker source string, threading an OPTIONAL `errTable` through
// `ie()` / `le()` / `onmessage`. MEASURED across the two trees:
//
//   0ff2a3f   fe (LERC) tail   le(d.demData,d.z,d.clipBounds)              errTable x0
//   f0cd81e   fe (LERC) tail   le(d.demData,d.z,d.clipBounds,d.errTable)   errTable x2
//             ge (terrain-rgb) Z(o.demData,o.z,o.clipBounds)               unchanged
//
// This regex was NOT widened with it, so it demanded exactly three arguments,
// `r24SpliceWorkerTail(fe)` returned null, `r24MakeWorker` returned null, and
// three-tile built the VERBATIM upstream worker instead. The consequence is the
// nastiest shape a flag can fail in: `TERRA_PACE.skirtWorker` AND
// `TERRAIN_LIGHT.workerNormals` degraded SILENTLY to off on the LERC path —
// which is the ONLY DEM path the live app uses — while the terrain-rgb worker
// (which is exactly what the offline fixture serves) kept matching, so no gate
// in the container and no row in the browser fleet could see it. R25 A's
// `__flyTerrainLightOverride = { workerNormals: true }` console A/B would have
// read "no difference" on the user's machine for THIS reason and not a
// graphics one. Found by E CERT; `scripts/verify-skirt-worker.mjs` gate 2b
// reads this regex OUT of the bundle and tests it against both live tails, so
// the gate and the code it judges cannot drift apart.
//
// The fourth argument is OPTIONAL and CAPTURED (group 5 = its field name), and
// the pattern stays anchored on the exact `demData, z, clipBounds` prefix and
// the exact `self.postMessage(<decoded>)` tail, so it still cannot match the
// IMAGERY worker (`self.onmessage=t=>{const n=c(t.data.imgData);…}`) or any
// other site — verified: 2 of 3 `self.onmessage=` sites in this bundle match,
// and the third is the imagery worker.
const R24_WORKER_TAIL_RE =
  /self\.onmessage=(\w+)=>\{const (\w+)=\1\.data,(\w+)=(\w+)\(\2\.demData,\2\.z,\2\.clipBounds(?:,\2\.(\w+))?\);self\.postMessage\(\3\)\}/;
// The decode CALL as the readable tail spells it. It is replaced by name AND
// by argument list (the tail cannot know whether this worker's decode takes a
// fourth argument), and the replacement is ASSERTED rather than trusted: an
// un-checked String.replace that silently no-ops is exactly the R24 defect
// class that cost that round twice, and here it would produce a worker whose
// decode call still reads `__DECODE__` — a ReferenceError inside a Blob, on
// the user's machine only.
const R24_DECODE_CALL = '__DECODE__(req.demData, req.z, req.clipBounds)';
...
function r24SpliceWorkerTail(src) {
  ...
  // R25 A: forward the tail's OWN extra argument (patch #3's `errTable` on the
  // LERC worker, absent on terrain-rgb) into the decode call. Dropping it would
  // splice a worker that decodes without the per-level error table — a subtler
  // and worse outcome than not splicing at all.
  let body = null;
  if (m) {
    const args = `req.demData, req.z, req.clipBounds${m[5] ? `, req.${m[5]}` : ''}`;
    if (r24Tail.indexOf(R24_DECODE_CALL) >= 0) {
      body = r24Tail
        .replace(R24_DECODE_CALL, `${m[4]}(${args})`)
        .replace(/__DECODE__/g, m[4]); // the two remaining occurrences are comments
    }
    // body stays null when the readable tail no longer spells the call the way
    // R24_DECODE_CALL does — degrade to the verbatim upstream worker, loudly in
    // the stats (skirtWorkers stops incrementing) rather than silently wrong.
  }
  const out = m && body ? src.slice(0, m.index) + body + src.slice(m.index + m[0].length) : null;
  ...
  async update(e, t) {
    ...
    try {
      if (FLY_PATCH.parallelFetch || R24_SWITCHES.parallelLoad) {
        ...
        const [n, s] = await Promise.all([this.updateMaterial(e, t), this.updateGeometry(e, t)]);
    ...
    try {
      for (const l of n) {
        ...
        if (h)
          ...
          o = !0;
          const c = await a(l);
          s.push(c), this._materialClip(c, l, e);
        }
      }
