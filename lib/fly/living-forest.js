...
export function buildForestCluster(detail=true) {
  const p=[],indices=[],colors=[],uv=[],leaf=[];
  const rings=detail?3:1,sides=6;
  for(let crown=0;crown<9;crown++){
    ...
    const cx=Math.cos(angle)*radius,cz=Math.sin(angle)*radius,width=.14+seed*.03,height=.62+seed*.35;
    for(let row=0;row<=rings;row++)for(let col=0;col<sides;col++){
      const a=col*Math.PI*2/sides,phi=row/rings*Math.PI/2,r=Math.cos(phi)*width;
      p.push(cx+Math.cos(a)*r,.28+Math.sin(phi)*height*.72,cz+Math.sin(a)*r);uv.push(cx+Math.cos(a)*r,cz+Math.sin(a)*r);leaf.push(1);
      const v=.74+row*.065+seed*.08;colors.push(v*.88,v,v*.76);
    }
    for(let row=0;row<rings;row++)for(let col=0;col<sides;col++){
...
uniform float uForestRadius;uniform float uForestK;varying vec3 vForestLocal;varying float vForestLeaf;
...
if(vForestLeaf>.5&&leafResolved>.35&&forestNoise(leafUV*1.4)<.13+.24*forestRim)discard;`);
...
if(forestDither>forestKeep)discard;`);
  ...
  update(surface,runtime,now){
    ...
    if(now-this.lastScan>.5||this.stats.sourceCommit!==surface.stats.commits){
      ...
      for(const slot of band.slots){
        ...
        for(let q=0;q<4;q++){
          const key=`${slot.key}:${q}`;keep.add(key);if(this.tiles.has(key))continue;
          const qx=q%2,qz=Math.floor(q/2),source=slot.mask.forest,values=[];
    ...
    for(const t of tiles){
      ...
      const detail=tier!=='low'&&d<(t.mesh.geometry===this.geometry?600:450);
      t.mesh.geometry=detail?this.geometry:this.farGeometry;
  ...
  _place(tile,i,row,k,fade=1){
    const sx=(worldHash(Math.round(row.x),Math.round(row.z),712)-.5)*.035*row.width,sz=(row.seed-.5)*.035*row.width;
    dummy.position.set(row.x-tile.x+sx,row.ground-.8,row.z-tile.z+sz);dummy.rotation.set(0,row.seed*Math.PI*2,0);dummy.scale.set(row.width,row.height*fade,row.width);dummy.updateMatrix();tile.mesh.setMatrixAt(i,dummy.matrix);tile.mesh.instanceMatrix.addUpdateRange(i*16,16);tile.mesh.instanceMatrix.needsUpdate=true;
  ...
  dispose(){for(const t of this.tiles.values()){t.mesh.removeFromParent();t.mesh.dispose();}this.tiles.clear();this.geometry.dispose();this.farGeometry.dispose();this.material.dispose();this.group.removeFromParent();}
}
