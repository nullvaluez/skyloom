...
export function StepSafeRig({ setDpr }) {
  ...
  // `state` is r3f's live RootState — read gl and the CSS size off it INSIDE
  // the callback rather than closing over render-time values, so the rig can
  // never resize to a stale box (and so nothing is captured from the render
  // pass at all; see the orchestrator's dead-instance note).
  useFrame((state) => {
    const d = pendingDpr;
    if (d == null) return;
    pendingDpr = null;
