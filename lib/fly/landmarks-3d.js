...
/**
 * Round 18 (A2) — the SATELLITE painter. Same placement math as part(); the
 * COLOUR is where they diverge. Satellite's taste lock is "no toy tint on Esri
 * imagery", so a monument gets value only: a grey multiplier on the one stone
 * tint LANDMARKS_3D.satStyle.color carries, with
 *   • a ROLE term — the toy palette hex's luma ÷ the body hex's, clamped to
 *     [roleMin, roleMax]: a monumentTrim cornice comes out light, a
 *     monumentDark plinth comes out dark, and the archetypes need no second
 *     set of authored colours. Emissive accents take accentTone flat (the toy
 *     EMIT ×3.2 would blow out against daylight imagery, and there is no bloom
 *     ramp to catch it here).
 *   • a VERTICAL term — baseMul at y = 0 up to topMul at y = 1: contact grime
 *     at the ground, sky bounce at the roofline. It is per-VERTEX and the parts
 *     are boxes/cylinders with verts at both ends, so it lands as a real
 *     gradient (a "banding" term would not — a box has no vertices in between).
 * Zero hue is emitted at any point: r == g == b for every vertex.
 */
function satPart(geom, hex, { x = 0, y = 0, z = 0, rx = 0, ry = 0, rz = 0, emit = 0 } = {}) {
  if (rx) geom.rotateX(rx);
  if (ry) geom.rotateY(ry);
  if (rz) geom.rotateZ(rz);
  geom.translate(x, y, z);
  ...
  const V = LANDMARKS_3D.satStyle.value;
  const role = emit
    ? V.accentTone
    : Math.min(V.roleMax, Math.max(V.roleMin, hexLuma(hex) / hexLuma(BODY)));
  const n = geom.attributes.position.count;
  const colors = new Float32Array(n * 3);
  const posY = geom.attributes.position;
  for (let i = 0; i < n; i++) {
    const yy = Math.min(1, Math.max(0, posY.getY(i)));
    const v = role * (V.baseMul + (V.topMul - V.baseMul) * yy);
    colors[i * 3] = v;
    colors[i * 3 + 1] = v;
    colors[i * 3 + 2] = v;
  }
...
/** Rec. 709 luma of a palette hex, 0..255 (the sat role term's input). */
...
/**
 * Per-variant build context. `toy` hands the builders the ORIGINAL part()
 * function and adds no parts, so every toy geometry is byte-identical to R17
 * (verify-monuments' pixel gate guards that); `sat` swaps the painter and lets
 * the builders append their silhouette parts.
 */
function makeCtx(variant) {
  const sat = variant === 'sat';
  return { part: sat ? satPart : part, sat };
}

// --- The archetypes ---------------------------------------------------------
// Every builder takes the variant ctx and appends its SAT-ONLY silhouette parts
// LAST, so the toy prefix — and therefore the merged toy buffer — never moves.

/** Stepped supertall + antenna (Empire State / Space Needle family). */
function spireGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    P(new BoxGeometry(0.3, 0.44, 0.3), BODY, { y: 0.22 }),
    P(new BoxGeometry(0.21, 0.28, 0.21), BODY, { y: 0.58 }),
    P(new BoxGeometry(0.13, 0.13, 0.13), TRIM, { y: 0.785 }),
    // emissive crown band at the top setback
    P(new BoxGeometry(0.145, 0.035, 0.145), ACCENT, { y: 0.867, emit: EMIT }),
    P(new CylinderGeometry(0.012, 0.02, 0.1, 6), TRIM, { y: 0.935 }),
    P(new OctahedronGeometry(0.02), COOL, { y: 0.99, emit: EMIT }),
  ];
  if (ctx.sat) {
    // SETBACK ARCHITECTURE — the whole Empire State / One WTC read. Three
    // cornice slabs that overhang their block by ~2% turn a stack of plain
    // prisms into a stepped tower whose silhouette has shadow lines in it.
    parts.push(
      P(new BoxGeometry(0.345, 0.03, 0.345), DARK, { y: 0.015 }), // street plinth
      P(new BoxGeometry(0.315, 0.012, 0.315), TRIM, { y: 0.2 }), // mid-base cornice
      P(new BoxGeometry(0.32, 0.018, 0.32), TRIM, { y: 0.431 }), // first setback
      P(new BoxGeometry(0.235, 0.016, 0.235), TRIM, { y: 0.712 }) // second setback
    );
  }
...
function obeliskGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    P(new BoxGeometry(0.18, 0.07, 0.18), TRIM, { y: 0.035 }),
    // 4-seg cylinder rotated 45° = an axis-aligned square taper
    P(new CylinderGeometry(0.05, 0.075, 0.84, 4), BODY, { ry: QUARTER_PI, y: 0.49 }),
    P(new ConeGeometry(0.07, 0.09, 4), COOL, { ry: QUARTER_PI, y: 0.955, emit: EMIT }),
  ];
  if (ctx.sat) {
    parts.push(
      P(new BoxGeometry(0.26, 0.035, 0.26), DARK, { y: 0.0175 }), // wide ground step
      P(new BoxGeometry(0.13, 0.02, 0.13), TRIM, { y: 0.08 }) // cap under the shaft
    );
  }
  return mergeGeometries(parts);
}
...
function statueGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    P(new BoxGeometry(0.36, 0.1, 0.36), DARK, { y: 0.05 }),
    P(new BoxGeometry(0.24, 0.28, 0.24), TRIM, { y: 0.24 }),
    // robe → torso → head: stacked tapers read "figure" at any distance
    P(new ConeGeometry(0.14, 0.34, 8), BODY, { y: 0.55 }),
    P(new CylinderGeometry(0.06, 0.1, 0.16, 8), BODY, { y: 0.8 }),
    P(new SphereGeometry(0.055, 8, 6), TRIM, { y: 0.9 }),
    ...
    P(new CylinderGeometry(0.028, 0.028, 0.18, 6), TRIM, { rz: -0.45, x: 0.09, y: 0.92 }),
    P(new OctahedronGeometry(0.055), ACCENT, { x: 0.135, y: 0.99, emit: EMIT }),
  ];
  if (ctx.sat) {
    // the star-fort pedestal Liberty actually stands on, as two steps
    parts.push(
      P(new BoxGeometry(0.44, 0.05, 0.44), DARK, { y: 0.025 }),
      P(new BoxGeometry(0.28, 0.02, 0.28), TRIM, { y: 0.105 }),
      P(new BoxGeometry(0.26, 0.018, 0.26), TRIM, { y: 0.383 })
    );
  }
  return mergeGeometries(parts);
}
...
function domeGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    P(new BoxGeometry(0.6, 0.08, 0.6), DARK, { y: 0.04 }),
    P(new CylinderGeometry(0.27, 0.27, 0.36, 12), BODY, { y: 0.26 }),
    P(new SphereGeometry(0.3, 12, 6, 0, Math.PI * 2, 0, HALF_PI), TRIM, { y: 0.44 }),
    P(new CylinderGeometry(0.05, 0.06, 0.1, 6), BODY, { y: 0.79 }),
    P(new ConeGeometry(0.04, 0.12, 6), TRIM, { y: 0.9 }),
    P(new OctahedronGeometry(0.025), ACCENT, { y: 0.975, emit: EMIT }),
  ];
  if (ctx.sat) {
    // a DRUM STEP + cornice: without them the hemisphere grows straight out of
    // a cylinder and the whole thing reads as a plain capsule
    parts.push(
      P(new BoxGeometry(0.66, 0.035, 0.66), DARK, { y: 0.018 }),
      P(new CylinderGeometry(0.3, 0.31, 0.035, 12), TRIM, { y: 0.0975 }),
      P(new CylinderGeometry(0.3, 0.29, 0.03, 12), TRIM, { y: 0.428 })
    );
  }
  return mergeGeometries(parts);
}
...
function archGeometry(ctx) {
  ...
  if (ctx.sat) {
    // ground pylons where the legs meet the plaza (the arch's only "base")
    for (const px of [-0.5, 0.5]) {
      parts.push(part(new BoxGeometry(0.11, 0.06, 0.12), DARK, { x: px, y: 0.03 }));
    }
  }
  return mergeGeometries(parts);
}
...
function bridgeGeometry(ctx) {
  ...
  // No sat-only parts here BY DESIGN: the bridge's local x IS the span (the
  // instance stretches it by spanM/height), so any part added in this space is
  // sheared by a different factor than the towers — the one archetype where
  // extra silhouette would cost more than it buys.
  return mergeGeometries(parts);
}

/** Keep + parapet + 4 corner towers with cone roofs (fortress family). */
function castleGeometry(ctx) {
  const { part } = ctx;
  const parts = [
  ...
  if (ctx.sat) {
    // battered base + a string course under the parapet: the two horizontals
    // that stop a fortress reading as one grey lump
    parts.push(
      part(new BoxGeometry(0.44, 0.04, 0.38), DARK, { y: 0.02 }),
      part(new BoxGeometry(0.38, 0.014, 0.32), TRIM, { y: 0.552 })
    );
  }
  return mergeGeometries(parts);
}
 ...
 */
function churchGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    // nave behind the tower (+z = rear), eaves at 0.42
    P(new BoxGeometry(0.34, 0.42, 0.6), BODY, { y: 0.21, z: 0.14 }),
    // steep gabled roof: triangular prism along z, slight eave overhang
    P(new CylinderGeometry(0.22, 0.22, 0.6, 3, 1, false, Math.PI), DARK, {
      rx: HALF_PI,
      y: 0.53,
      z: 0.14,
    }),
    // front bell tower over the entrance
    P(new BoxGeometry(0.17, 0.8, 0.17), BODY, { y: 0.4, z: -0.25 }),
    P(new BoxGeometry(0.2, 0.12, 0.2), TRIM, { y: 0.84, z: -0.25 }),
    // warm belfry opening + entrance door (the parish is awake)
    P(new BoxGeometry(0.05, 0.08, 0.012), ACCENT, { y: 0.84, z: -0.355, emit: EMIT }),
    P(new BoxGeometry(0.05, 0.1, 0.012), ACCENT, { y: 0.08, z: -0.34, emit: EMIT }),
    // pyramidal spire + emissive ice tip
    P(new ConeGeometry(0.14, 0.1, 4), DARK, { ry: QUARTER_PI, y: 0.95, z: -0.25 }),
    P(new OctahedronGeometry(0.018), COOL, { y: 1.0, z: -0.25, emit: EMIT }),
  ];
  if (ctx.sat) {
    // BUTTRESS HINTS — four battered piers on the nave flanks. From above (the
    // only angle satellite ever gives a church) they are what says "cathedral"
    // instead of "shed with a tower".
    for (const bx of [-0.183, 0.183]) {
      for (const bz of [0.0, 0.3]) {
        parts.push(P(new BoxGeometry(0.05, 0.3, 0.07), BODY, { rz: bx > 0 ? -0.05 : 0.05, x: bx, y: 0.15, z: bz }));
      }
    }
  }
  return mergeGeometries(parts);
...
function crownTowerGeometry(ctx) {
  const { part: P } = ctx;
  const parts = [
    // Round 18 (A2): in SATELLITE the shaft is an 8-segment taper (chamfered
    // corners) instead of the 4-segment square. A modern supertall is never a
    // plain box — the chamfer is what reads as glass-and-steel rather than
    // extruded clay, and it is the One-WTC/Petronas silhouette. Toy keeps the
    // square taper (its stepped toon ramp needs the four broad faces).
    ctx.sat
      ? P(new CylinderGeometry(0.1, 0.14, 0.86, 8), BODY, { ry: Math.PI / 8, y: 0.43 })
      : P(new CylinderGeometry(0.105, 0.135, 0.86, 4), BODY, { ry: QUARTER_PI, y: 0.43 }),
    P(new BoxGeometry(0.26, 0.055, 0.26), ACCENT, { y: 0.8875, emit: EMIT }),
    P(new BoxGeometry(0.21, 0.025, 0.21), TRIM, { y: 0.9275 }),
    P(new CylinderGeometry(0.008, 0.008, 0.065, 5), TRIM, { x: -0.06, y: 0.9625 }),
    P(new CylinderGeometry(0.008, 0.008, 0.065, 5), TRIM, { x: 0.06, y: 0.9625 }),
    P(new OctahedronGeometry(0.015), COOL, { x: -0.06, y: 1.0, emit: EMIT }),
    P(new OctahedronGeometry(0.015), COOL, { x: 0.06, y: 1.0, emit: EMIT }),
  ];
  if (ctx.sat) {
    // podium + two setback bands on the taper (radii tracked off the shaft:
    // r(y) = 0.14 − 0.0465·y, each band overhanging by ~6%)
    parts.push(
      P(new BoxGeometry(0.3, 0.028, 0.3), DARK, { y: 0.014 }),
      P(new CylinderGeometry(0.133, 0.133, 0.016, 8), TRIM, { ry: Math.PI / 8, y: 0.3 }),
      P(new CylinderGeometry(0.119, 0.119, 0.014, 8), TRIM, { ry: Math.PI / 8, y: 0.6 })
    );
  }
  return mergeGeometries(parts);
}
 ...
 * `variant` is 'toy' (default — byte-identical to every round before 18) or
 * 'sat', which adds the round-18 silhouette parts and paints value-only grey
 * vertex colours for the satellite MeshToon (which flips vertexColors on).
 * LandmarkMonuments is keyed by mapStyle, so a style flip is a clean rebuild.
 */
export function buildLandmarkGeometries(variant = 'toy') {
  const ctx = makeCtx(variant);
  const out = {};
  for (const a of LANDMARK_ARCHETYPES) out[a] = BUILDERS[a](ctx);
  return out;
}
