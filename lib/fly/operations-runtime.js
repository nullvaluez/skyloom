...
import { distKm, flightPlanOn, freeFlightPlacement, normalizeSetup, resolveDestination, saveLastSetup } from './flight-plan';
...
export function connectFlightOperations({runtime,operations,flight,input,autopilot,chase,engine,rebase,crashSys,crashRef}) {
    ...
    const pollStaging = () => {
      ...
      const r = readinessOf(now - s.since);
      s.polls++; s.progress = r.progress; s.missing = r.missing;
      if (r.ready && !s.ready) { s.ready = true; s.readyAt = now; s.readyMs = Math.round(now - s.since); }
    ...
    runtime.stageDestination = (input) => {
      ...
      if (runtime.staging?.key === dest.id) return true;
      ...
      stopStaging();
      runtime.staging = { key: dest.id, dest, since: performance.now(), ready: false, readyAt: null, readyMs: null, progress: 0, missing: [], warped, polls: 0, pumped: 0 };
    ...
    return () => {
      ...
      delete runtime.launchFreeFlight; delete runtime.stageDestination; delete runtime.launchSetup;
    };
