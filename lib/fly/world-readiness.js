...
// Scenery streams independently of the flight surface. Requiring every local
// tree/pavement support to finish can deadlock against frustum-driven DEM LOD.
// Keep the real terrain, surface materials, aircraft and shaders mandatory;
// give surrounding scenery a bounded head start, then finish it during flight.
export const SCENERY_HOLD_MS=12000;
const STREAMING_PARTS=new Set(['buildings','roads','forest','airports']);
...
export function worldReadiness(runtime,elapsedMs=0){
  ...
  const deferred=elapsedMs>=SCENERY_HOLD_MS?missing.filter(k=>STREAMING_PARTS.has(k)):[];
  const blocking=missing.filter(k=>!deferred.includes(k));
  return{ready:blocking.length===0,detailReady:missing.length===0,missing,deferred,blocking,parts,buildings,roads,progress:(Object.keys(parts).length-blocking.length)/Object.keys(parts).length};
}
