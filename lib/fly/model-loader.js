...
function bakeMeshGeometry(mesh) {
  ...
  // Round 15: a GLB may already carry COLOR_0 — the fleet's textured models
  // are shipped with their base-color texture baked to per-vertex colors
  // offline, because a texture bakes here as its color FACTOR (white) and the
  // hull renders flat white (that is what traffic-military.glb did for seven
  // rounds). glTF COLOR_0 is linear, like material.color, so the two simply
  // multiply. Solid-material models (no COLOR_0) keep the flat-factor path
  // byte-for-byte.
  const srcColor = g.attributes.color;
  _color.set(mesh.material?.color ?? 0xffffff);
  for (let i = 0; i < count; i++) {
    colors[i * 3] = srcColor ? srcColor.getX(i) * _color.r : _color.r;
    colors[i * 3 + 1] = srcColor ? srcColor.getY(i) * _color.g : _color.g;
    colors[i * 3 + 2] = srcColor ? srcColor.getZ(i) * _color.b : _color.b;
  }
...
// instance variety comes from the 8 archetypes × 6 lights all differing).
