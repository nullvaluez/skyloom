 ...
 * space. Unclipped images also pass through a canvas: WebGL ignores flipY
 * for ImageBitmap, whereas the upstream HTMLImageElement honors Texture's
 * default flipY. Uploading the raw bitmap mirrors every tile independently.
 * Keep decoding/cropping north-up and let the canvas upload perform the flip.
 * `needsUpdate` is set explicitly: upstream relies on TileLoader's
...
class FlyCachedImageLoader extends TileImageLoader {
  ...
  async doLoad(url, params) {
    ...
    try {
      ...
      if (cb && cb[2] - cb[0] < 1) {
        image = getSubImage(bitmap, cb);
      } else {
        image = new OffscreenCanvas(bitmap.width, bitmap.height);
      ...
      // The canvas owns its pixels now; retaining the decoder bitmap doubles
      // residency until GC during fast streaming. Also close on crop failure.
      bitmap?.close();
    }
...
class FlyCachedLercLoader extends TerrainLercLoader {
  ...
  async doLoad(url, params) {
    ...
    // R25 D GROUND (R25_GROUND.relief): the cached twin of vendored PATCH 32 —
    // request the relief map only while the app asks for it (Visuals Enhanced,
    // the worker spliced for it); otherwise the message is exactly R22's.
    if (R24_SWITCHES.r25Relief && R24_SWITCHES.r25ReliefPx > 1) msg.r25Relief = R24_SWITCHES.r25ReliefPx;
    const data = (await this._workerPool.postMessage(msg, [demData])).data;
    // A provider's 200-OK all-no-data response is not a reusable raw DEM.
    // Keep partially valid raw tiles: other clipped regions may still use them.
    if (data.demNoDataFull) {
      try { const cache = await openCache(); await cache?.delete(url); } catch { /* cache remains optional */ }
    }
    // Same construction as upstream (setAttributes, not setData — the worker
    // has already run the martini decimation).
    return this.geometryFromData(data, z);
  }
}
