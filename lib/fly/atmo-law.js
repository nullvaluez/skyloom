...
/**
 * R25 (C SKY): the law's Henyey-Greenstein forward lobe as a STANDALONE
 * function — the same four lines `atmoInscatter` inlines above, lifted out so
 * the R25 sky model (lib/fly/sky-model.js) evaluates the SAME phase shape the
 * law does rather than a second copy of it. ATMO_GLSL_FRAGMENT is deliberately
 * left byte-identical (every FINAL key that embeds it stays put);
 * scripts/verify-r25-sky.mjs asserts the two texts carry one expression.
 * Normalised to 1 at mu = 1: lobe = (1-g)^3 / (1 + g^2 - 2 g mu)^1.5.
 */
export const ATMO_MIE_LOBE_GLSL = /* glsl */ `
float atmoMieLobe( float g, float mu ) {
  float og = 1.0 - g;
  float den = max( 1.0e-4, 1.0 + g * g - 2.0 * g * mu );
  return ( og * og * og ) / ( den * sqrt( den ) );
}
`;

/** JS mirror of ATMO_MIE_LOBE_GLSL (and of the lobe inside atmoInscatterJS). */
export function atmoMieLobe(g, mu) {
  const og = 1 - g;
  const den = Math.max(1.0e-4, 1 + g * g - 2 * g * mu);
  return (og * og * og) / (den * Math.sqrt(den));
}
...
/**
 * THE POST PASS'S OWN GATE. The law text declares one `uAtmoStrength`, and the
 * post pass is a SEPARATE program with a separate uniform map — so the same
 * name carries the per-material gate in the materials and the post gate in the
 * effect. That is the tier split, and it is why the two evaluators never
 * double-haze: at high the materials read 0 and the post reads the law, at
 * medium/low the reverse. Everything that DEFINES the law (beta, scale height,
 * eye height, inscatter, sun tint, Mie, sun direction, eye, datum) is the one
 * shared block above, so "identical constants" is structural, not a promise.
 */
let _postStrength = 0;
export function getAtmoPostStrength() {
  return _postStrength;
}
...
export function setAtmoLaw(s) {
  ...
  _postStrength = s.postStrength ?? 0;
}
