...
export const lodStats = {
   ...
   *
   * NUMBERS ONLY, deliberately: E's gate diffs this object key by key across
   * two snapshots, so a non-numeric field here would be a trap for the next
   * reader. The one denial that needs CONTEXT rather than a count gets its own
   * sibling below.
   */
  skip: { disabled: 0, boot: 0, warp: 0, concurrency: 0, noParentMap: 0, shape: 0, unpatched: 0 },
  /**
   * THE FIRST FEW `noParentMap` DENIALS, NAMED.
   *
   * Pass 2b's row denied exactly one refine a blend for `noParentMap`, and the
   * counter could say only "one" — the cause had to be inferred from source
   * across three reachable paths (a parent whose imagery load FAILED and was
   * replaced by three-tile's `_errorMaterial`, which carries no `map`; a source
   * that does not cover the tile, leaving the material array empty; a parent
   * whose model was unloaded during the await). A single field settles it:
   * three-tile names that fallback `"error-material"`, so `matName` alone
   * distinguishes a transient fetch miss from a real ladder gap.
   *
   * Bounded to the first few — this is a diagnosis, not a log — and it is
   * unreachable when the flag is off, because `eligible()` refuses the swap
   * before `onRefine` ever looks at the parent's map. Nothing is written on a
   * run where nothing is denied.
   */
  noParentMapFirst: [],
};

/** How many denials to describe before the array stops growing. */
const NO_PARENT_MAP_SAMPLES = 4;

function resetSkips() {
  for (const k in lodStats.skip) lodStats.skip[k] = 0;
  lodStats.noParentMapFirst.length = 0;
}

/**
 * Describe one `noParentMap` denial. `matName` is the load-bearing field:
 * three-tile's failed-imagery fallback is `MeshBasicMaterial({ name:
 * 'error-material' })` with no `map`, so seeing that name says "a fetch
 * missed" where its absence says "look further".
 */
function recordNoParentMap(parent, pm) {
  if (lodStats.noParentMapFirst.length >= NO_PARENT_MAP_SAMPLES) return;
  const mats = parent?.model?.material;
  lodStats.noParentMapFirst.push({
    z: parent?.z ?? -1,
    x: parent?.x ?? -1,
    y: parent?.y ?? -1,
    hasModel: !!parent?.model,
    matCount: Array.isArray(mats) ? mats.length : mats ? 1 : 0,
    matName: pm?.name ?? null,
  });
}

/**
 * DEV-ONLY OVERRIDE (`__flyTerraPaceOverride` / R16 weather-pin idiom).
 *
 * `window.__flyLodFadeOverride = { enabled: true, fadeSec: 8 }` — a PARTIAL
 * override of the whole block, set before Fly mode mounts (addInitScript, or
 * the pause-menu console). Two reasons it exists:
 *
 *  1. The flag is a module const and the tile CACHE KEY derives from it, so a
 *     harness that wants the ON leg would otherwise have to rewrite
 *     `fly-constants.js` between runs — the live-constant-rewrite hygiene debt
 *     recon HARN-HYG-9 names. `enabled` must be pinned BEFORE the first tile
 *     material is patched; flipping it mid-session would leave already-patched
 *     materials on the other program.
 *  2. `fadeSec` is the ONLY way this container can see the fade at all.
 *     SwiftShader runs the game at ~1 fps (HARN-ENV-3), so the clock advances
 *     ~1000 ms per frame and a 250 ms blend completes inside a single frame —
 *     invisible to a screenshot by construction. Pinning `fadeSec` to a few
 *     seconds spreads the same blend across several frames so a mid-fade
 *     capture can PROVE it: on E's fixture every imagery tile is stamped with
 *     its own "z / x / y", so a child rendering the parent's stamp is the
 *     crossfade, photographed.
 *
 * Production compiles the branch out; the constant is then the only input.
 */
function cfg() {
  if (
    ...
    const p = window.__flyLodFadeOverride;
    return typeof p === 'object' ? { ...LOD_CROSSFADE, ...p } : { ...LOD_CROSSFADE, enabled: !!p };
  }
  return LOD_CROSSFADE;
}
...
export function attachLodFade(material) {
  if (!cfg().enabled || !material) return null;
...
function eligible(kind) {
  const c = cfg();
  if (!c.enabled || c.mode !== 'parentBlend') { lodStats.skip.disabled++; return false; }
  if (kind === 'refine' && !c.onRefine) { lodStats.skip.disabled++; return false; }
  if (kind === 'merge' && !c.onMerge) { lodStats.skip.disabled++; return false; }
  if (now() - _installedAt < c.skipBootMs) { lodStats.skip.boot++; return false; }
  if (c.skipOnWarp && now() < _warpUntil) { lodStats.skip.warp++; return false; }
  if (_active.size >= c.maxConcurrent) { lodStats.skip.concurrency++; return false; }
  return true;
}
...
function onRefine(parent, children, aborted) {
  ...
  const tex = pm?.map;
  if (!tex) {
    lodStats.skip.noParentMap++;
    recordNoParentMap(parent, pm);
    lodStats.hardSwaps++;
    return false;
  }

  const dur = cfg().fadeSec;
  let armed = 0;
...
// A camera turn can cancel a merge after its asynchronous fade. Also release
// timed-out blends before the temporary parent texture is disposed.
function onMergeEnd(parent, parentModel) {
  const tex = parentModel?.material?.[0]?.map;
  if (!tex) return;
  for (const l of leaves(parent)) {
    const material = tileMap(l);
    const entry = _active.get(material);
    if (entry?.map === tex && !entry.owned) finish(material, entry, false);
  }
}

export const lodFadeHook = { onRefine, onMerge, onMergeEnd };
