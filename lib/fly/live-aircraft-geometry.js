...
import { liveEngineStations } from './aircraft-effects.js';
...
export function buildLiveAirframe(f,detail=true){
  ...
  if(f.helicopter){
    ...
    for(const {x,y,z,radius:er,length:el} of liveEngineStations(f)){
      ...
      if(!f.prop){
        ...
        if(detail){
          // Fan hub/blades, intake lip and dark exhaust core are part of the
          // existing merged hull: more depth at inspection range, no new draw.
          parts.push(paint(new SphereGeometry(1,10,6).scale(er*.22,er*.22,er*.38).translate(x,y,z-el*.42),grey,2));
          for(let b=0;b<10;b++)parts.push(paint(new BoxGeometry(er*.62,er*.09,.035)
            .translate(er*.43,0,0).rotateZ(b*Math.PI/5).translate(x,y,z-el*.395),grey,2));
          parts.push(paint(new CylinderGeometry(er*.58,er*.58,.12,12).rotateX(Math.PI/2).translate(x,y,z+el*.47),dark,2));
        }
      }
    }
  }
  ...
  if(!f.helicopter)for(const side of [-1,1])parts.push(navLight(side*f.span*.495,tipY+f.span*.02,f.span*.5*f.sweep-L*.035+.18,'#ffffff',.34,Math.max(.08,R*.055)));
