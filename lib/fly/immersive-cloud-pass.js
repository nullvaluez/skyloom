...
// The Classic cloud ambient ends (cinematicLight = 1), whose LUMINANCE the
// Enhanced ambient keeps — only the chroma follows the model.
...
export class ImmersiveCloudPass extends Pass {
  ...
  ensureR25(){
    ...
    const uniforms={...this.uniforms,
      ...
      uR25SkyR:{value:new Float32Array(SKY_ROWS*3)},uR25SkyM:{value:new Float32Array(SKY_ROWS*3)},
  ...
  r25Frame(rt){
    ...
    const marchOn=air&&!!m;
    if(marchOn){
      q.uR25Key.value.fromArray(m.key);
      const a=m.ambient,ya=Y(a[0],a[1],a[2])||1;
      // Classic cinematic ends: upper (.62,.68,.74), lower (.16,.20,.26).
      q.uR25AmbHi.value.set(a[0]/ya*.6716,a[1]/ya*.6716,a[2]/ya*.6716);
      q.uR25AmbLo.value.set(a[0]/ya*.1961,a[1]/ya*.1961,a[2]/ya*.1961);
