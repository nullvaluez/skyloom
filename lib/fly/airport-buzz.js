...
export class AirportBuzzDetector {
  ...
  update(nowSec, flight, engine, k) {
    ...
    const operations = flight.operations;
    if (operations) {
      const contact = operations.events.find(e => e.type === 'touch-and-go' && e.id > (this._lastContactId || 0));
      if (contact) { this._lastContactId = contact.id; return { type: 'touch-go', airport: { name: contact.airport } }; }
      if (operations.grounded || operations.phase === 'hangar') return null;
    }

    // --- touch-and-go: dip below touchAglM, then climb out promptly -------
    if (!operations && agl < AIRPORT_BUZZ.touchAglM) {
