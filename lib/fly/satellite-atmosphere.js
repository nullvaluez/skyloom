...
/**
 * R25 C: the exposure term of resolveSatelliteAtmosphere, alone and without
 * allocating — r25SkyFrame evaluates it every frame. The arithmetic is the
 * resolver's own line (scripts/verify-r25-sky.mjs asserts equality with
 * `resolveSatelliteAtmosphere(...).exposureStops` across the elevation range).
 */
export function satelliteExposureStops(sun) {
  ...
  return -0.12 * day - 0.18 * night - 0.08 * twilight;
}
...
export function resolveSatelliteAtmosphere(sun = {}, weather = {}) {
  ...
  const exposureStops = -0.12 * day - 0.18 * night - 0.08 * twilight;
  ...
  return {
    ...
    bloomThreshold: mix(1.08, 0.91, night),
    ...
    // R25 C (R25_SKY.exposure): the SAME white balance WITHOUT the exposure
    // gamma. Enhanced applies the exposure PRE-curve (AerialPerspective's
    // uExposure, lib/fly/r25-sky.js) and the grade keeps only the tint.
    // Classic reads `balance` above, untouched.
    balanceTint: [
      1 + 0.032 * warmth - 0.018 * night,
      1 - 0.006 * warmth,
      1 - 0.036 * warmth + 0.023 * night,
    ],
    cloudColor: [
