  ...
  return {
    ...
    step(items, dt, nowSec) {
      ...
      for (const it of items) {
        ...
        if (!p) {
          prev.set(it.hex, { d, rate: 0, seen: frame });
          continue;
        }
        const rate = (p.d - d) / dt; // >0 closing, <=0 opening
        // p.d is the closest sample of the pass: it was still closing into it
        // and this frame is already back out.
        if (
          p.rate > NEARMISS.closingMps &&
          rate <= 0 &&
          p.d < NEARMISS.distM &&
