import { findAirportSurface, projectAirportPoint } from './operations-airports';
...
import { TileMap, LoaderFactory, R24_SWITCHES, setLodFadeHook as setVendorLodFadeHook, setFlyPatch, setDemErrorTable, flyGetPatch, flyBoundaryEdgesFast, flyBoundaryEdgesRef, flySkirtStats, flyEdgeCapture, flyTileHoldStats } from './vendor/three-tile/index.js';
import { HILLSHADE, TERRA_PACE, GLOBE, WORLD_EDGE, TERRAIN_LIGHT, TERRA_SHARP, TERRA_PIPE, TERRA_CACHE, ARRIVAL_GATE, FRAME_PACE, TILE_HOLD } from './fly-constants';
import { TileResidency, DEFAULT_TERRAIN_RESIDENT_BYTES } from './tile-residency';
import { STYLIZED_EARTH, stylizedEarthOn } from './stylized-earth';
import { LIVING_EARTH } from './living-earth';
import { TerrainGroundQuery } from './terrain-ground-query';
import { markPhase } from './frame-stats';
// R25 D GROUND: the relief builder the terrain-rgb path runs on the main
// thread (vendored PATCH 33), the relief/mesh flags, and the mesh latch
// (installed by lib/fly/r25-ground.js; false when nothing installed it).
import { R25_GROUND } from './fly-constants';
import { reliefFromGrid, r25MeshLatched } from './r25-relief';
...
export function applyTerraPaceSwitches(cfg = resolveTerraPace()) {
  ...
  R24_SWITCHES.walkWhileSaturated = on && !!cfg.walkWhileSaturated;
  R24_SWITCHES.bboxCache = on && !!cfg.bboxCache;
  // R24 A PATCH 26 — the DRAW half of keepResident. Only meaningful WITH
  // keepResident: without it upstream collapses the field behind the camera
  // anyway, so there is nothing retained to park, and arming it alone would
  // add a per-tile write for no benefit.
  R24_SWITCHES.parkOffscreen = on && !!cfg.keepResident && !!cfg.parkOffscreen;
  // C LIGHT's worker-side DEM normals (recon T6) live in the same spliced
  // worker tail as A's skirt move, but they are NOT A's switch to own: read
  // C's flag defensively so this file compiles on a branch where C's block
  // does not have the key yet, and never write into C's block.
  R24_SWITCHES.workerNormals =
    TERRAIN_LIGHT?.enabled === true && TERRAIN_LIGHT?.workerNormals === true; // R24 Fable audit: master flag gates the sub-switch
  // R24 A: E CERT's FRAME_STATS attribution hook, installed by inversion so the
  // vendored bundle still imports no app code. markPhase is a no-op until the
  // instrument is live (one string write when it is), and it is what makes the
  // user's "it froze" answerable with "during a skirt build" or "during the
  // LOD walk" instead of a guess.
  R24_SWITCHES.onPhase = markPhase;
...
/**
 * R25 D GROUND — arm the vendored relief patches (VENDOR.md rows 30-33) from
 * the FLAG, never the profile: the DEM workers are created lazily and live for
 * the session, so the grid-capturing splice must be decided before the first
 * one exists; the live Visuals profile only decides whether a request ASKS for
 * a relief map (`setR25ReliefPx`, written by lib/fly/r25-ground.js). Flag off:
 * all three stay off and the vendored bundle is byte-behaviour upstream.
 */
export function applyR25GroundSwitches() {
  const on = !!R25_GROUND.enabled && R25_GROUND.relief?.enabled !== false;
  R24_SWITCHES.r25Relief = on;
  R24_SWITCHES.r25ReliefFn = on ? reliefFromGrid : null;
  if (!on) R24_SWITCHES.r25ReliefPx = 0;
  return on;
}
applyR25GroundSwitches();

/** R25 D (R25_GROUND.mesh, launch-applied): tighter mid-ring Martini error. */
export function r25MeshTable(base) {
  const out = {};
  if (base) for (const k of Object.keys(base)) out[k] = base[k];
  for (const k of Object.keys(R25_GROUND.mesh?.table || {})) out[k] = R25_GROUND.mesh.table[k];
  return out;
}
import {
  prefetchUrl,
  installRasterCacheHandle,
  rasterStats,
  setRasterPolicy,
} from './raster-cache';
...
/**
 * R22.1 (B STUTTER) — FRAME_PACE is armed. Deliberately NOT gated on
 * `terraPinned()`: patch #5's output is identical to upstream's by
 * construction (same edges, same order, same direction — verify-frame-pace
 * gate (2) proves it on live index buffers), so there is nothing for a
 * determinism pin to protect and no fifth fleet pin was added to
 * scripts/_boot.js. `window.__flyPaceForce` is the dev-only A/B lever, same
 * shape as `__flyTerraForce`; production builds never read it.
 */
export function framePaceOn() {
  if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {
    const f = window.__flyPaceForce;
    if (f != null) return !!f;
  }
  return !!FRAME_PACE.enabled;
}
...
/**
 * R24 (A "MOTION HOLD") — TILE_HOLD is armed: vendored patches #6/#7/#8 plus
 * the raster failure policy in raster-cache.js.
 *
 * IT RIDES THE EXISTING `__flyTerraPin`, and that is a deliberate choice with a
 * cost worth stating. Unlike FRAME_PACE — whose patch #5 is output-identical BY
 * CONSTRUCTION and therefore needed no pin — patch #6a changes WHEN a merge
 * happens. At any settled pose it converges on the identical tree (the dwell
 * only delays a merge, never cancels one), so no frozen gate can see it once it
 * has settled; but "converges" is not "identical", and the project rule is that
 * only identity earns a pin exemption. Reusing the terra pin rather than adding
 * a fifth one keeps this family in the same bucket as TERRA_SHARP/PIPE/CACHE,
 * needs no edit to scripts/_boot.js, and gives every frozen-number harness the
 * R21 world for free.
 *
 * THE COST, NAMED: `soak-fly.js` boots through `_boot.js` and is therefore
 * PINNED, so the fleet's only long motion run does not exercise this round's
 * fix. Un-pinning the soak is a certification decision for the round's owner of
 * scripts/_boot.js, not a call this file can make. scripts/r24-a-churn.js
 * un-pins itself through `__flyTileHoldForce` — the `__flyTerraForce` idiom.
 */
export function tileHoldOn() {
  if (process.env.NODE_ENV === 'development' && typeof window !== 'undefined') {
    const f = window.__flyTileHoldForce;
    if (f != null) return !!f;
  }
  return !!TILE_HOLD?.enabled && !terraPinned();
}
...
export class TerrainEngine {
  constructor({
    ...
    this._pace = applyTerraPaceSwitches();
    applyR25GroundSwitches();
    // R25 D (R25_GROUND.mesh): LAUNCH-applied — latched here, at creation, in
    // Enhanced only; a later Visuals toggle never re-meshes resident tiles.
    this._r25Mesh = r25MeshLatched();
    this._armPatches();
    this.map = TileMap.create({ imgSource, demSource, minLevel });
    this.map.rotateX(-Math.PI / 2);
    this.map.updateMatrixWorld(true);
    this._groundQuery = new TerrainGroundQuery(this.map);
    ...
    this.residency = new TileResidency(this.map, {
      ...
      pace,
      budgetBytes: style === 'satellite' && stylizedEarthOn() ? STYLIZED_EARTH.terrainResidentBytes : undefined,
    });
    ...
    this._hooked = false;
    if (terraSharpOn() || terraPipeOn() || (pace.enabled && pace.keepResident)) this._installUpdateHook();
  ...
  _armPatches() {
    ...
    const hold = tileHoldOn();
    setFlyPatch({
      parallelFetch: pipe && TERRA_PIPE.parallelFetch !== false,
      lodBailFix: pipe && TERRA_PIPE.lodBailFix !== false,
      // R22.1 patch #5 — rides FRAME_PACE, not TERRA_PIPE, and therefore not
      // the terra pin either (see framePaceOn's comment).
      skirtEdges: framePaceOn() && FRAME_PACE.skirtEdges !== false,
      // R24 patches #6/#7/#8 — every one of them resolves to the value that
      // makes its site byte-equivalent to upstream when TILE_HOLD is off:
      // 0 ms of dwell, the literal 5, the literal threshold, a re-thrown
      // rejection, an un-stamped error material.
      mergeDwellMs: hold ? (TILE_HOLD.mergeDwellMs ?? 0) : 0,
      frustumPenalty: hold ? (TILE_HOLD.frustumPenalty || 5) : 5,
      lodHysteresis: hold ? (TILE_HOLD.lodHysteresis || 1) : 1,
      unlockOnReject: hold && TILE_HOLD.unlockOnReject !== false,
      rasterMark: hold && !!TILE_HOLD.raster,
    });
    // The R22.1 arm receipt. Published unconditionally — three property writes
    // on a rare event — because verify-frame-pace must PROVE which arm it
    // measured rather than infer it from the constants file.
    if (typeof window !== 'undefined') {
      (window.__flyStats ??= {}).pace = {
        on: framePaceOn(),
        skirtEdges: !!flyGetPatch().skirtEdges,
        skirt: flySkirtStats(), // live object: {fast, bail, upstream, tris}
      };
      // R24 — the same receipt for TILE_HOLD. `hold` is the live object, so a
      // probe reads dwellHeld/dwellFired/reject* without re-fetching.
      (window.__flyStats ??= {}).tileHold = {
        on: hold,
        pinned: terraPinned(),
        mergeDwellMs: flyGetPatch().mergeDwellMs,
        frustumPenalty: flyGetPatch().frustumPenalty,
        lodHysteresis: flyGetPatch().lodHysteresis,
        unlockOnReject: !!flyGetPatch().unlockOnReject,
        hold: flyTileHoldStats(),
      };
      // The raster receipt is the LIVE stats object, so a probe polls one
      // reference. `errorTiles` is the one field owned by the vendored patch #8
      // site rather than by raster-cache; _tick mirrors it in at 2 Hz.
      window.__flyStats.raster = rasterStats;
    }
    // The raster failure policy (TILE_HOLD.raster) lives in raster-cache.js,
    // which owns the only transport this app's imagery and DEM actually use.
    setRasterPolicy(hold ? TILE_HOLD.raster : null);
    ...
    const table = useTable ? f?.errTableValues || TERRA_SHARP.demErrorTable || null : null;
    setDemErrorTable(this._r25Mesh ? r25MeshTable(table) : table);
  }

  // ── R25 D GROUND hooks (lib/fly/r25-ground.js drives them) ─────────────────

  /** The live relief request size in px (0 = no request: Classic / toy). */
  setR25ReliefPx(px) {
    R24_SWITCHES.r25ReliefPx = R24_SWITCHES.r25Relief ? Math.max(0, px | 0) : 0;
  }

  /** Decode a resident tile through the normal cached DEM path, keep only relief.
   * The scratch geometry is never attached, uploaded, or swapped into the scene.
   * LERC's measured parent fallback and crop rules remain those of the loader.
   */
  async loadR25Relief(tile) {
    const source = this.map.demSource;
    if (!source || this._disposed || R24_SWITCHES.r25ReliefPx < 2) return null;
    const n = 2 ** tile.z;
    const geometry = await LoaderFactory.getGeometryLoader(source).load({
      source, x: ((tile.x % n) + n) % n, y: tile.y, z: tile.z,
    });
    try { return geometry.userData.r25Relief || null; }
    finally { geometry.dispose(); }
  }

  isResidentTile(tile, geometry) {
    if (this._disposed || tile?.model?.geometry !== geometry) return false;
    let root = tile;
    while (root && root !== this.map) root = root.parent;
    return root === this.map;
  }

  /** Subscribe to engine disposal without importing its consumers. */
  onDispose(fn) {
    this.map.addEventListener('fly-engine-dispose', fn);
    return () => this.map.removeEventListener('fly-engine-dispose', fn);
  }

  /** Every resident tile material (the Visuals toggle's needsUpdate walk). */
  forEachTileMaterial(cb) {
    this.map.traverse((o) => {
  ...
  /** Every resident tile that carries a loaded model. */
  forEachLoadedTile(cb) {
    this.map.traverse((o) => {
      if (o.isTile && o.model) cb(o);
    });
  }

  /** tile-loaded / tile-unload listeners; returns the unsubscribe. */
  onTileEvents(onLoaded, onUnload) {
    const a = (e) => e.tile && onLoaded?.(e.tile);
    const b = (e) => e.tile && onUnload?.(e.tile);
    this.map.addEventListener('tile-loaded', a);
    this.map.addEventListener('tile-unload', b);
    return () => {
      this.map.removeEventListener('tile-loaded', a);
      this.map.removeEventListener('tile-unload', b);
    };
  }

  /** The imagery source the tiles stream from (the colour reference reuses it). */
  get imagerySource() {
    const s = this.map.imgSource;
    return Array.isArray(s) ? s[0] : s;
  }

  /**
   * R25 D (R25_GROUND.budget): an Enhanced-only terrain residency cap in MiB
   * (null restores the style budget). Only ever set when a measured Enhanced
   * texture peak exceeds 300 MiB; ships null.
   */
  setR25ResidentCapMiB(mib) {
    if (!this.residency) return;
    const base = this._style === 'satellite' && stylizedEarthOn()
      ? STYLIZED_EARTH.terrainResidentBytes : DEFAULT_TERRAIN_RESIDENT_BYTES;
    const cap = Number.isFinite(mib) && mib > 0 ? mib * 1048576 : null;
    this.residency.budgetBytes = cap && base ? Math.min(base, cap) : base;
  }
  ...
  setImagery(source) {
    ...
    this.residency.budgetBytes = this._style === 'satellite' && stylizedEarthOn()
      ? STYLIZED_EARTH.terrainResidentBytes : DEFAULT_TERRAIN_RESIDENT_BYTES;
  ...
  getElevationAt(lon, lat) {
    const xy = projectAirportPoint(lon,lat), surface=findAirportSurface(xy.x,xy.z);
  ...
  getGroundAt(lon, lat) {
    const xy = projectAirportPoint(lon,lat), surface=findAirportSurface(xy.x,xy.z);
    if(surface)return { elev: surface.height, tileZ: 18 };
    if (this._style === 'satellite' && stylizedEarthOn()) {
      const sample = this._groundQuery.sample(lon, lat);
      if (sample !== undefined) {
        const tile = sample?.tile;
        if (tile?._sizeInWorld > 0) this._sizeZ0 = tile._sizeInWorld * 2 ** tile.z;
        return sample ? { elev: sample.elev, tileZ: sample.tileZ } : null;
      }
    }
    const info = this.map.getLocalInfoFromGeo(_geo.set(lon, lat, 0));
    if (!info) return null;
  ...
  _installUpdateHook() {
    ...
    this.map.update = (camera) => {
      const rt=this._runtime,f=rt?.flight;
      if(this._style==='satellite'&&rt?.worldLoading&&f&&f.pos.y-(rt.groundElevVis??f.groundElev??0)<2800){
        const focus=this.map.rootTile.flyArrivalGround??={};
        Object.assign(focus,{x:f.pos.x,z:f.pos.z,radius:LIVING_EARTH.readinessRadiusM/Math.max(.1,Math.cos(f.latDeg*Math.PI/180)),zoom:Math.min(15,this.map.maxLevel)});
      }else this.map.rootTile.flyArrivalGround=null;
      base(camera);
      // One wrapper composes residency policy with the sharpness publisher.
      if (this._pace.enabled && this._pace.keepResident) {
  ...
  _tick(camera) {
    ...
    // R24: keep the one raster receipt whole. `errorTiles` is counted at the
    // vendored patch #8 site (TileLoader.updateMaterial's catch) because that is
    // the only place that knows a tile was handed the black error material;
    // every other field is written here in raster-cache.js.
    rasterStats.errorTiles = flyTileHoldStats().errorTiles;
  ...
  _installDevHandle() {
    ...
    /**
     * R22.1 (B STUTTER) — the FRAME_PACE dev handle. `edges` hands
     * verify-frame-pace both boundary-edge implementations plus a live harvest
     * of real tile index buffers, so gate (2) can prove patch #5's output
     * identity on the meshes this session actually built instead of on a
     * synthetic fixture.
     */
    window.__flyPace = {
      get: () => ({ on: framePaceOn(), patch: { ...flyGetPatch() }, skirt: { ...flySkirtStats() } }),
      set: (v) => {
        window.__flyPaceForce = v;
        this._armPatches();
        return framePaceOn();
      },
      edges: { fast: flyBoundaryEdgesFast, ref: flyBoundaryEdgesRef },
      /** Arm/read the harness capture of the REAL boundary-edge inputs. */
      capture: (max) => flyEdgeCapture(max),
      /**
       * The LIVE engine, resolved at call time and NOT captured.
       *
       * MEASURED (R22.1): FlyScene builds the engine in a `useMemo` with an
       * empty dep list, and React 19 StrictMode double-invokes the component
       * body — so TWO TerrainEngines are constructed and the one React
       * DISCARDS runs its constructor LAST. Every dev handle installed from a
       * constructor therefore ends up bound to the dead engine: on this tree
       * `window.__flyTerra.get().sizeZ0` reads 0 and `.stats()` reads null
       * while `window.__fly.engine.terraStats` is live. That is a pre-existing
       * R22 fault (left alone here — a hotfix wave does not re-point another
       * round's instrument), and this handle simply refuses to inherit it.
       */
      engine: () => (typeof window !== 'undefined' && window.__fly?.engine) || this,
      /**
       * Every resident tile's geometry, keyed by the tile's own z/x/y — the
       * gate's equality corpus AND its content fingerprint. Keyed, because a
       * scene-total triangle count cannot tell "the patch changed a mesh" from
       * "one more chunk happened to be resident this second".
       */
      tiles: (cap = 400) => {
        const eng = window.__flyPace.engine();
        const out = [];
        eng.map.traverse((o) => {
          if (out.length >= cap || !o.isTile || !o.model) return;
          const g = o.model.geometry;
      ...
      /** Back-compat shape for callers that only want the index buffers. */
      indices: (cap = 400) => window.__flyPace.tiles(cap).map((t) => t.idx),
    };
    /**
     * R24 (A "MOTION HOLD") — the live A/B handle, the `__flyDepthArm` idiom.
     * `__flyTileHold.set(false)` restores the R22.1 world without a reload, so
     * the user can fly the identical pose both ways in one session and say which
     * one is the world they recognise. Dev-only: production never defines it.
     *
     * IMMUNE TO R22.1 F4 (the StrictMode dead-engine handle) by construction,
     * and deliberately so: `census()` resolves the LIVE engine at call time via
     * `window.__fly.engine` rather than capturing `this`, and `set()`/`dwell()`
     * only touch process-wide module state (setFlyPatch / setRasterPolicy), so
     * it does not matter which of the two constructed engines owns this closure.
     * `dispose()` does NOT delete this handle for the same reason — an
     * unconditional delete from the corpse's teardown is exactly the F4 bug.
     *
     *   __flyTileHold.get()          → arm state + live dwell/reject counters
     *   __flyTileHold.set(true|null) → force on / off / back to the constants
     *   __flyTileHold.census()       → resident tiles by zoom, and the deepest
     *                                  leaf — the one read that shows a collapse
     *   __flyTileHold.dwell(ms)      → sweep the dwell live (0 = upstream)
     */
    window.__flyTileHold = {
      get: () => ({
        on: tileHoldOn(),
        pinned: terraPinned(),
        patch: { ...flyGetPatch() },
      ...
      set: (v) => {
        if (v == null) delete window.__flyTileHoldForce;
        else window.__flyTileHoldForce = !!v;
        return this._armPatches(), tileHoldOn();
      },
      dwell: (ms) => {
        setFlyPatch({ mergeDwellMs: Math.max(0, ms | 0) });
        return flyGetPatch().mergeDwellMs;
      },
      /**
       * The census that makes a collapse legible in ONE read. `byZ` counts
       * RESIDENT LEAVES (a tile with no Tile children), which is what the
       * renderer actually draws and what `getGroundAt().tileZ` answers from.
       */
      census: () => {
        const eng = (typeof window !== 'undefined' && window.__fly?.engine) || this;
        const byZ = {};
        let leaves = 0;
        let maxLeafZ = 0;
        let nodes = 0;
        eng.map.traverse((o) => {
          if (!o.isTile) return;
          nodes++;
          if (o.children.length > 1) return; // interior: has Tile children
          leaves++;
          byZ[o.z] = (byZ[o.z] ?? 0) + 1;
          if (o.z > maxLeafZ) maxLeafZ = o.z;
        });
        return { nodes, leaves, maxLeafZ, byZ };
      },
    };
  }
  ...
  dispose() {
    this._disposed = true;
    this.map.dispatchEvent({ type: 'fly-engine-dispose' });
    ...
    this.residency = null;
    if (typeof window !== 'undefined' && window.__flyTerra?.__owner === this) delete window.__flyTerra;
    this.map.dispose();
  }
}
