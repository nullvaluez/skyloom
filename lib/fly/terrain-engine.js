...
import { pinned } from './fly-pins'; // R25 A: the __flyTerrainLightOverride A/B
...
export function applyTerraPaceSwitches(cfg = resolveTerraPace()) {
  ...
  // R25 A (GROUND_DETAIL_R25.workerNormals, plan §3 A6): read through `pinned`
  // so the USER can A/B C's worker-side DEM normals on the one machine that has
  // a real LERC path, without editing a constant and rebuilding —
  //   window.__flyTerrainLightOverride = { workerNormals: true }   (then RELOAD)
  // It must be set BEFORE the app mounts (an addInitScript, or pasted and then
  // reloaded): the switch is substituted as a LITERAL into the spliced DEM
  // worker source when the Blob is created, so a post-boot flip cannot reach
  // the worker that is already running. It SHIPS OFF — `pinned` returns the
  // constants object itself when the global is absent, so with no pin this line
  // evaluates to exactly what R24 evaluated to. NOTE FOR E: with the pin ON,
  // `verify-skirt-worker`'s identity leg goes RED BY DESIGN — its contract is
  // that the worker's skirt output is element-for-element the main thread's,
  // and smooth normals deliberately change the normal array.
  const TL = pinned(TERRAIN_LIGHT, '__flyTerrainLightOverride');
  R24_SWITCHES.workerNormals = TL?.enabled === true && TL?.workerNormals === true; // R24 Fable audit: master flag gates the sub-switch
