...
import { LoaderFactory } from './vendor/three-tile/index.js';
...
import { TileMap, R24_SWITCHES, setLodFadeHook as setVendorLodFadeHook, setFlyPatch, setDemErrorTable, flyGetPatch, flyBoundaryEdgesFast, flyBoundaryEdgesRef, flySkirtStats, flyEdgeCapture, flyTileHoldStats } from './vendor/three-tile/index.js';
...
export class TerrainEngine {
  ...
  /** Decode through the existing cache/worker, retaining only relief bytes.
   * The temporary geometry is never attached or uploaded. Visible geometry,
   * drape/collision support and imagery remain owned by the original tile. */
  async loadResidentRelief(tile) {
    if(this.disposed || !R24_SWITCHES.r25ReliefPx || !this.map.demSource)return null;
    const source=this.map.demSource;
    const params=this.map.loader._getTileCoords(tile);
    const decoded=await LoaderFactory.getGeometryLoader(source).load({source,...params});
    try{return decoded.userData.r25Relief??null;}finally{decoded.dispose();}
  }
