...
export function createTerrainSources(style = 'satellite', tier = null) {
  ...
  // (D W1: E's two hunks are hand-applied onto the VENDORED import lines —
  // E branched before A's vendor landed, so a `git checkout r24/e --
  // lib/fly/tile-sources.js` would have reverted A. Same text; flagged to
  // Fable as a merge note.)
  const fx = (typeof window !== 'undefined' && window.__flyTileFixture) || null;
  return {
    // MEASURED (E, fixture rev r24-e.3), and the reason imagery is a SOURCE
    // rather than a Playwright route: fulfilling every imagery tile through
    // context.route kept three-tile's download queue saturated, and its
    // quadtree walk FREEZES wholesale while `downloadingThreads + 4 >=
    // maxThreads` (recon T3) — the tree stalled at z6, so every building drape
    // sample came back below SAT_BUILDINGS.demZ and the whole satellite
    // pipeline retried forever. Fetching straight from 127.0.0.1 removes the
    // interception cost entirely. Toy keeps its solid data-URI tile untouched.
    imgSource:
