// R24 A (W1a): three-tile 0.12.1 is VENDORED at lib/fly/vendor/three-tile/
// (verbatim copy; see that folder's VENDOR.md for the patch ledger). The
// plugin's own `from 'three-tile'` import was rewritten to the vendored core
// so both halves share ONE LoaderFactory singleton — two copies would split
// the loader registry and the DEM/imagery loaders would never be found.
import { TileSource } from './vendor/three-tile/index.js';
...
// R25 A (GROUND_DETAIL_R25.z19) — SHIPS OFF. The imagery CEILING, and why it is
// a ceiling rather than a live k-keyed write, is written out in full at
// `z19MaxZoomFor`; the short version is that `maxLevel` is consumed once at
// source construction and the only public way to move it re-loads the whole
// quadtree, which is the user's own reported symptom on purpose.
import { z19MaxZoomFor } from './ground-detail';
import {
...
export function satMaxZoomFor(tier) {
  // R25 A: the z19 step wraps the R19/R22 resolution rather than replacing it,
  // so with the sub-switch off this function returns its pre-R25 value by
  // construction (z19MaxZoomFor returns its input) and TERRA_SHARP keeps
  // deciding what the high-tier baseline is.
  return z19MaxZoomFor(tier, satMaxZoomBase(tier));
}

function satMaxZoomBase(tier) {
...
export function createTerrainSources(style = 'satellite', tier = null) {
  ...
  // MEASURED, and the reason imagery is here rather than on a Playwright route:
  // fulfilling every imagery tile through context.route kept three-tile's
  // download queue saturated, and its quadtree walk FREEZES wholesale while
  // `downloadingThreads + 4 >= maxThreads` (recon T3) — the tree stalled at
  // z6, so every building drape sample came back below SAT_BUILDINGS.demZ (12)
  // and the whole satellite pipeline retried forever. Fetching imagery
  // straight from 127.0.0.1 removes the interception cost entirely. Toy keeps
  // its solid data-URI tile (zero network) untouched.
  const cached = terraCacheOn() && registerRasterCacheLoaders();
  const DemSrc = cached ? CachedArcGisDemSource : ArcGisDemSource;
  const fx = (typeof window !== 'undefined' && window.__flyTileFixture) || null;
  return {
    imgSource:
      fx?.img && style === 'satellite'
        ? new TileSource({ maxLevel: satMaxZoomFor(tier), ...fx.img })
        : createImagerySource(style, tier),
    demSource: fx?.dem
      ? new TileSource({ maxLevel: demMaxZoomFor(tier), ...fx.dem, dataType: 'terrain-rgb' })
      : new DemSrc({
