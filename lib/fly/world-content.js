...
export function deriveWorldContent(vt, mask, span=2445.98490512564, k=1, frame) {
  ...
  const forest=[];
  const airportExclusion=airportSceneryMask(vt,size,span/k,false,frame);
  ...
  mask.airport=buildAirportSurfaces(span<3000?vt:{layers:{}},span,k,frame);
  return mask;
}
