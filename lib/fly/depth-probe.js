...
export function installDepthProbe({ gl, composer, camera, scene: worldScene }) {
  ...
  /**
   * One corrected candidate for a pixel at a given vertical LIFT, self-checked.
   * Returns null when the ray meets nothing.
   */
  const castAt = (px, py, lift, size, list, k, cx, cz) => {
    ...
    const drop = (dx * dx + dz * dz) * k;
    _p.copy(h.point);
    _p.y -= drop;
    // THE ARBITER. A candidate is only the truth if it lands on the pixel that
    // was asked for. A near object found by the unlifted ray reprojects to ~0;
    // distant terrain found by the unlifted ray gets pushed off the ray by its
    // own drop and reprojects far away, so it is rejected without a special
    // case anywhere.
   ...
   * depth buffer keeps.
   *
   * TWO candidates, and the second one is the fix for a real defect: a rigid
   * vertical lift is the right correction for distant terrain and is fatal in
   * the near field. At bendK 5e-6 a 3 km hit lifts the ray by ~45 m, which
   * sweeps it clean over anything within a few tens of metres — which is
   * exactly how the far pick missed a depth-writing object 34 m in front of the
   * camera and reported 3,144 m of terrain instead. So cast BOTH: unlifted
   * (near field, where the drop is 0.05 m at 100 m and does not matter) and
   * bend-solved (distant terrain), reject whatever does not reproject onto the
   * pixel, and take the nearest survivor.
   */
  const solve = (px, py, size, list, k, cx, cz) => {
    const cands = [];
    const a = castAt(px, py, 0, size, list, k, cx, cz);
    if (a) cands.push({ ...a, via: 'unlifted' });
    let iters = 0;
    if (k) {
      let lift = a ? a.drop : 0;
      for (let i = 0; i < 4; i += 1) {
        const b = castAt(px, py, lift, size, list, k, cx, cz);
        iters = i + 1;
        if (!b) break;
        const done = Math.abs(b.drop - lift) < 0.01; // 1 cm of vertical drop
        lift = b.drop;
        if (done || i === 3) {
          cands.push({ ...b, via: 'bend-solved' });
          break;
        }
      }
    }
    const valid = cands.filter((c) => c.reproj <= 1.5);
    valid.sort((c1, c2) => c1.distance - c2.distance);
    return { pick: valid[0] ?? null, tried: cands.length, rejected: cands.length - valid.length, iters };
  };
  ...
  const truth = (x, y) => {
    ...
    out.object = c.object;
    out.via = c.via;
    out.bendDropM = c.drop;
    out.distance = c.distance;
    out.viewZ = c.viewZ;
    out.reprojectionPx = c.reproj;
    // Residual off the ORIGINAL ray: the bend solve is a fixed point, not an
    // exact inverse, and this says whether it converged at this pose.
