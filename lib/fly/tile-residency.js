 ...
 *
 * Stylized Earth correction: the historical TILES byte key cited above had
 * already been removed. The default now lives here (140 MB); the review
 * profile supplies its smaller allowance explicitly and updates it on style
 * changes. This is a coverage-preserving brake, not an instantaneous hard cap.
 *
...
export const DEFAULT_TERRAIN_RESIDENT_BYTES = TILES.lruBudgetBytes ?? 140e6;
...
export class TileResidency {
  constructor(map, { instrument = false, pace = TERRA_PACE, budgetBytes = DEFAULT_TERRAIN_RESIDENT_BYTES } = {}) {
    this.map = map;
    this.rootTile = map?.rootTile ?? null;
    this.enabled = !!(pace.enabled && pace.keepResident);
    // The former TILES key was removed as unused before this reader landed.
    // An absent key must never silently turn the memory brake into Infinity.
    if (!Number.isFinite(budgetBytes) || budgetBytes <= 0) throw new RangeError('Terrain residency budget must be positive and finite');
    this.budgetBytes = budgetBytes;
    /**
     * R24 A — the TILE-COUNT cap, and why bytes alone were not a cap at all.
     *
     * Pass 2b measured the retained set growing with sweep DURATION on one
     * fixed pose: Owens 62 -> 103 resident tiles and draws 152 -> 279 between a
     * 45 s and a 600 s sweep with identical flags, at 113.7 MB — comfortably
     * under the 140 MB byte budget, so this module elected NOTHING and the byte
     * rule never fired. A budget that a session cannot reach is not a bound.
     * Tiles are also what costs DRAW CALLS, and draw calls are the frozen
     * ceiling, so the count is the honest unit for the second trigger.
     */
    this.maxTiles = pace.residency?.maxResidentTiles ?? Infinity;
    /**
     * R24 A — the cap is a FLOOR plus a multiple of what is actually on
     * screen, not a constant. A constant was wrong twice over: I sized 260
     * against a 240-frame synthetic yaw whose working set saturates at 190,
     * and the venue's 650-frame revolution at z17 holds more than that (its
     * flag-OFF arm alone carries 205 resident / 152 with a model at Owens),
     * so the cap BOUND, the LRU elected off-frustum subtrees, and the round's
     * headline paid 47 merges and 47 parent refetches on one revolution.
     *
     * The retained set is a full revolution's worth of the visible set — that
     * is the geometry, and it is why a multiple of the DRAWN count tracks it
     * at any altitude, on any venue, at any sweep length, while a constant
     * measured on one path cannot. It is still bounded, because the visible
     * count is frustum-bounded.
     *
     * `TILES.lruBudgetBytes` remains the hard memory stop underneath: this
     * lifts the COUNT trigger off the legitimate working set, it does not
     * remove the byte trigger, and pass 2b measured 113.7 MB there — under
     * budget, which is why lifting the count cap does not spend memory the
     * byte rule was going to refuse anyway.
     */
    this.capPerVisible = pace.residency?.maxResidentPerVisible ?? 0;
    ...
    this.stats = {
      ...
      collapseMarks: 0,
      residentTiles: 0,
      // Published so the NEXT log names the cause instead of leaving a merge
      // count ambiguous between "the LOD policy did it" and "the cap did it"
      // — the reading that cost a whole re-take here.
      visibleTiles: 0,
      effectiveCap: 0,
      evictions: 0,
      residentBytes: 0,
  ...
  update(camPos, nowMs = performance.now()) {
    ...
    // The effective cap: never below the floor, and never below a full
    // revolution's worth of what is currently drawn.
    const cap = Math.max(this.maxTiles, this.capPerVisible * visible);
    this.stats.visibleTiles = visible;
    this.stats.effectiveCap = cap;
    const overTiles = count > cap;
    const overBytes = bytes > this.budgetBytes;
    if (!this.enabled || (!overBytes && !overTiles) || !camPos) {
    ...
    cands.length = w;
    // LEAST RECENTLY VISIBLE FIRST. `_r24LastVisible` is stamped by PATCH 26 on
    // every walk that sees a tile in frustum, so this is a real recency reading
    // rather than a distance heuristic standing in for one — a tile just behind
    // the camera after a 180-degree turn is CLOSE but has not been visible for
    // a while, and it is the right thing to shed before something far away that
    // the player is flying toward. Distance breaks ties (farthest first), which
    // is the old ordering and still the right answer among equals.
    cands.sort((a, b) => {
      const la = a._r24LastVisible ?? 0;
      const lb = b._r24LastVisible ?? 0;
      return la !== lb ? la - lb : dist(b) - dist(a);
    });
    ...
    // Elect against WHICHEVER budget is breached. Each candidate frees its four
    // children, so the tile overshoot converts to candidates directly; the byte
    // overshoot is still counted in bytes freed.
    const overshoot = overBytes ? bytes - this.budgetBytes : 0;
    const tileOvershoot = overTiles ? count - cap : 0;
    const needCands = Math.ceil(tileOvershoot / 4);
    let freed = 0;
    let n = 0;
    while (
      n < cands.length &&
      n < this.maxCollapsePerPass &&
      (freed < overshoot || n < needCands)
    ) {
      const t = cands[n];
