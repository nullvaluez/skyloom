...
function flattenScene(scene) {
  ...
  scene.traverse((o) => {
    ...
    for (const name of ['_model_surface', '_model_light']) {
      if (g.attributes[name]) clean.setAttribute(name, g.attributes[name]);
    }
...
// One semaphore across style lifetimes, far models and nearby detail. A style flip
// may leave old requests in flight; it must not double the network budget.
let activeDownloads = 0;
const downloadQueue = [];
async function downloadModel(file) {
  if (activeDownloads >= 2) await new Promise(resolve => downloadQueue.push(resolve));
  else activeDownloads++;
  try { return await _loader.loadAsync(file); }
  finally {
...
export async function loadOne(entry, variant) {
  ...
  const gltf = await downloadModel(entry.file);
  try { return normalizeAndGrade(flattenScene(gltf.scene), entry, variant); }
  finally { gltf.scene.traverse(o => {
    o.geometry?.dispose();
...
export async function loadMonumentGeometries(variant = 'toy') {
  const out = new Map();
  const entries = MONUMENT_MANIFEST.filter(e => e.enabled !== false);
  let next = 0;
  await Promise.all(Array.from({length: 2}, async () => {
    while (next < entries.length) {
      const entry = entries[next++];
      try { out.set(entry.poi, {entry, geometry: await loadOne(entry,variant)}); }
      catch (err) { console.warn(`[fly-monuments] ${entry.poi}: keeping procedural fallback`, err); }
    }
  }));
  return out;
}

export function loadMonumentDetail(entry, level, variant) {
  const asset = entry.detail[level];
  return loadOne({...entry, ...asset, build:undefined}, variant);
}
