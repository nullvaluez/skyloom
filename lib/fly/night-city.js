...
export function nightCityOn(sub) {
  let on = NIGHT_CITY_R23.enabled === true || (satelliteVisualsOn('lighting') && sub !== 'windows');
...
/** The old sweep compensated for terrain overwriting the road overlay. With
 * correct reverse-Z order, use restrained continuous glow and distinct lamp pools.
 * Shared by prewarm and runtime so warming cannot restore the old global gains.
 */
export function satelliteRoadLighting() {
  if (!nightCityOn('roads')) return null;
  const r = NIGHT_CITY_R23.roads;
  if (!satelliteVisualsOn('lighting')) return r;
  const l = SATELLITE_VISUALS.lighting;
  if (nearGroundOn('lighting')) {
    const light = GROUND_LIGHTING.road;
    return { ...r, cinematic: true, intensity: light.glow,
      lamp: { ...r.lamp, gain: light.lampGain, sharp: light.lampSharp },
      stream: { ...r.stream, boost: light.streamGain },
      traffic: { ...r.traffic, boost: light.trafficGain, dashDuty: 0.045 } };
  }
  return { ...r, cinematic: true, intensity: l.roadGlow, lamp: { ...r.lamp, gain: l.streetPoolGain },
    stream: { ...r.stream, boost: l.streamGain }, traffic: { ...r.traffic, boost: l.trafficGain, dashDuty: 0.035 } };
}
...
export function installNightCityDevHandle() {
  ...
  const cells = {
    ...
    neutralUV: SAT_BUILDINGS.facade.neutralUV, // the roof sentinel
  };
  const applyRoads = (r) => {
    setNightCityRoadGain(r.intensity, r.streetGain.c5, r.streetGain.c6, r.stream?.boost);
    setNightCityRoads(r);
  };
  /* A leg names ONE knob and means "everything else as it was" — so the merge
   * has to be per-sub-object, not shallow. A shallow spread of
   * `{ traffic: { boost: 0 } }` deletes the dash triple, and the road material
   * then uploads an undefined vec3 every frame. */
  const merge = (base, patch) => {
    const out = { ...base, ...patch };
    for (const k of ['streetGain', 'lamp', 'stream', 'traffic', 'gain']) {
      if (base?.[k] && patch?.[k]) out[k] = { ...base[k], ...patch[k] };
    }
    return out;
  };
  /* The swept leg keeps the R23 traffic GEOMETRY (wavelength/duty/speed) and
   * only zeroes its boost: the control must differ from the test leg in the
   * knob under test and in nothing else. */
  const sweptRoads = merge(NIGHT_CITY_R23.roads, NIGHT_CITY_SWEPT.roads);
  window.__flyNightCity = {
    armed: nightCityOn(),
    swept(patch) {
      applyRoads(merge(sweptRoads, patch?.roads));
      setNightCityWindows(merge(NIGHT_CITY_SWEPT.windows, patch?.windows), cells);
      return this.read();
    },
    r23(patch) {
      applyRoads(merge(NIGHT_CITY_R23.roads, patch?.roads));
      setNightCityWindows(merge(NIGHT_CITY_R23.windows, patch?.windows), cells);
      return this.read();
    },
    set(patch) {
      if (patch?.roads) applyRoads(merge(NIGHT_CITY_R23.roads, patch.roads));
      if (patch?.windows) setNightCityWindows(merge(NIGHT_CITY_R23.windows, patch.windows), cells);
      return this.read();
    },
