...
export function installShadowKernel() {
  ...
  try {
    let src = ShaderChunk.shadowmap_pars_fragment;
    if (SHADOW_CALM.biasSignFix && src.includes(PCF_BIAS_FROM)) {
      src = src.replace(PCF_BIAS_FROM, PCF_BIAS_TO);
      _state.biasSign = true;
    }
    const mode = SHADOW_CALM.kernel;
    if (mode && mode !== 'three' && src.includes(PHI_FROM)) {
      src = src.replace(PHI_FROM, PHI_TO(mode));
      _state.kernel = mode;
    }
    ShaderChunk.shadowmap_pars_fragment = src;
  } catch {
