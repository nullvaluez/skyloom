...
/**
 * The bounded relief texture pool. `lastVisible(entry)` returns the recency
 * stamp the LRU orders by (bigger = more recent).
 */
export class ReliefPool {
  constructor({ maxTiles = R25_GROUND.relief.poolTiles, mapPx = R25_GROUND.relief.mapPx } = {}) {
    this.max = maxTiles;
    this.N = mapPx;
    this.entries = new Map(); // key -> { key, tex, holders:Set, data, tile, stamp }
    this.free = [];
    this.allocated = 0;
    this.stats = { binds: 0, evictions: 0, stitches: 0, peak: 0 };
    this._clock = 0;
  }
  ...
  _takeTexture(exceptKey) {
    ...
    // Evict the least recently VISIBLE entry.
    let worst = null;
    let ws = Infinity;
  ...
  bind(key, data, holders, tile = null) {
    ...
    if (!e) {
      ...
      const tex = this._takeTexture(key);
      if (!tex) return null;
