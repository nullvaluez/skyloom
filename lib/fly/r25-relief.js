...
export function reliefFromGrid(dem, w, h, z, N = R25_GROUND.relief.mapPx) {
  ...
  const inv = 1 / (N - 1);
  // A grid denser than the map (LERC 257 -> 128) is PREFILTERED: each texel
  // averages the gradient at a 2x2 set of taps a quarter-footprint apart (a box
  // over its footprint), so the finest DEM detail cannot alias into the map.
  // A grid no denser than the map (terrain-rgb's 33) is sampled once.
  const q = w - 1 > N - 1 ? 0.25 * ((w - 1) / (N - 1)) : 0;
  const qz = h - 1 > N - 1 ? 0.25 * ((h - 1) / (N - 1)) : 0;
  const taps = q > 0 || qz > 0 ? 4 : 1;
  let hx = 0;
  let hz = 0;
  const tap = (fx, fy) => {
    if (fx < 0) fx = 0;
