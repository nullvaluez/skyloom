...
export const LOD_CROSSFADE = {
  // R24 CLOSE — SHIP STATE: OFF, pending the pass-1 `verify-lod-fade` ON row.
  // The RED is clean and measured twice: D's Powell altitude ladder on the
  // offline fixture (20 refines, 20 of them single-frame hard swaps, 0
  // blended, `skip.disabled` 20 — every one un-faded because the flag is off
  // rather than because a guard fired), and E's 40 s pure-yaw sweep on the
  // integrated tree (8 hard refines + 3 hard merges, crossfade window 0).
  // There is no GREEN: the flag-ON leg was never booted on any tree — the run
  // that looked like a load timeout was the signature of D's own
  // AerialPerspective import defect, fixed in 1620e32. A feature whose entire
  // claim is visual does not ship on a RED alone.
  //
