...
// ===========================================================================
// ROUND 19 "Grounded & Alive" — pre-seeded blocks (Fable scaffolding commit).
// Same discipline as the R18 banner above: each block is OWNED by exactly one
// R19 charter agent, named in its header; agents edit ONLY the interior of
// blocks they own (disjoint ranges = clean merges). Every block lands
// `enabled: false` and every read site must treat that as a byte-noop —
// flipping a flag back off after the round must restore pre-R19
// pixels/behavior exactly. Values below are the plan's defaults; the owning
// agent measures, tunes, and documents deltas inline.
// ===========================================================================

/**
 * A1 HOMESTEAD — procedural house infill for generalized satellite tiles.
 * OpenFreeMap ships ~3 building footprints/tile outside dense cores
 * (Naperville-class; see scripts/verify-roof-variety.js tile table) — the
 * suburbs the user actually flies have nothing for ROOFS_SAT to operate on.
 * Worker-side synthesis along residential road setbacks, merged into the
 * existing sat-building chunk meshes (+0 draws). HONESTY RAILS (the R18
 * area-hatch lesson — never invent mass): fires only where real data is
 * sparse, houses only (5–8 m), residential landuse only, deterministic
 * per-tile seed, and the whole thing dies with enabled:false.
 */
export const INFILL_SAT = {
  enabled: false,
  // Scarcity gate: a tile with MORE parsed real footprints than this never
  // synthesizes (Manhattan/Chicago can't qualify by construction).
  maxRealBuildings: 40,
  maxPerTile: 180,
  setbackM: [12, 18], // house front from minor-road centerline
  spacingM: [28, 45], // along-road lot pitch (hash-jittered)
  footprintM2: [90, 220],
  heightM: [5, 8], // the honesty rail — never taller than a house
};

/**
 * A1 HOMESTEAD — suburban vegetation sources. The sat-veg scatter reads only
 * park / landcover=wood / landcover=grass today (vector-tile.worker.js
 * scatter table) — yards, farmland and street trees are structurally absent.
 * New sources feed the SAME pooled InstancedMesh (+0 draws; pool re-tunes
 * documented in SAT_VEG).
 */
export const VEG_SUBURB = {
  enabled: false,
  yard: { m2PerTree: 700, minR: 3, maxR: 6 }, // landuse=residential sprinkle
  farm: { edgeSpacingM: 55, minR: 4, maxR: 8 }, // farmland/orchard field edges
  streetTree: { spacingM: 38, offsetM: 7, jitterM: 6 }, // minor roads in residential
  hedgeFrac: 0.15, // fraction rendered as kind-3 hedges (scale/tint only)
};

/**
 * A2 SUNCAST — satellite contact shadows. Nothing casts a shadow in
 * satellite today (castShadow is toy-only), so buildings/trees sit ON the
 * photo instead of IN it. Two pooled InstancedMesh gradient blobs
 * (MultiplyBlending, depthWrite off), placed on a slow cadence from
 * runtime.shadowAnchors / runtime.satVeg, offset along the live sun azimuth.
 * +2 draws high / +1 medium; visible=false at count 0; opacity is EXACT 0
 * below the horizon and when disabled.
 */
export const SAT_SHADOWS = {
  enabled: false,
  maxByTier: { high: { bldg: 1500, veg: 1500 }, medium: { bldg: 800, veg: 0 } },
  cadenceSec: 2, // placement/sun re-aim cadence — never per frame
  lengthMaxM: 60, // shadow offset clamp at low sun
  opacityMax: 0.32,
  minSunElevDeg: 2, // below this, opacity is exact 0
};

/**
 * A3 DAYLIGHT — daytime road presence. The satellite road material renders
 * ZERO pixels in daylight by design (night-only shader; day = motorway glint
 * at 5% duty) — the whole suburban grid streams and shows nothing. Two new
 * fragment terms in the EXISTING road variant (key -r16 → -r19), both exact
 * 0.0 at these defaults: class-weighted asphalt lift via the uClsW LUT
 * (structure, not level, is what reads as a grid — cap ~0.15 per the R16
 * "steady glow reads as haze" warning) and slow sparse day dash trains.
 */
export const ROAD_DAYLIGHT = {
  enabled: false,
  lift: 0.0, // owner tunes toward ~0.10; hard cap 0.15
  liftClsFloor: 0.25, // residential share of the class-weighted lift
  traffic: { intensity: 0.0, dashLenM: 26, dashDuty: 0.12, speedMps: 14, minCls: 3 },
};

/**
 * A3 DAYLIGHT — content-aware ground micro v2, inside applyHillshade (key
 * -r13 → -r19) and the existing uMicroStrength envelope (toy pinned 0 stays
 * byte-identical). Second octave modulated by the tile texel's own chroma —
 * field vs urban grain from the imagery itself, no invented parcel lines —
 * plus a sun-azimuth gradient-luma term so grain shades like relief.
 */
export const GROUND_MICRO2 = {
  enabled: false,
  octave2: { amp: 0.0, scaleM: 90 }, // exact-0 default
  chromaMod: 0.0, // 0 = content-blind (v1 behavior)
  sunGrad: 0.0,
  inAglM: 2200, // band re-tune reaching the dead-band floor (v1: 1500)
  outAglM: 3200, // (v1: 2500)
};

/**
 * A3 DAYLIGHT — satellite water rework (key -r13 → -r19 + material flags read
 * in sat-building-engine's ONE guarded creation line — implemented by A1 in
 * W1, spec'd here; named cross-charter seam). Normal-blended deep tint
 * finally able to DARKEN the matte Esri slab, grazing-angle brightening,
 * distance-feathered shore. Existing sun glint rides on top. Stays
 * high-tier, maxWaterChunks unmoved.
 */
export const SAT_WATER2 = {
  enabled: false,
  deepTint: { color: 0x0a2436, strength: 0.0 }, // exact-0 default
  grazing: 0.0,
  shoreFeatherM: 0.0,
};

/**
 * A4 SKYFALL — overcast-dusk sky retention. The overcast lid currently
 * collapses dusk to a featureless tan dome (R18 §5b #3 — the tracer half was
 * fixed in R18, the sky half is this block). Vertical gradient + sun-azimuth
 * warm bruise, terms scaled by overcastT × duskT so baseline/noon stay
 * byte-stable.
 */
export const SKY_DUSK = {
  enabled: false,
  gradKeep: 0.0, // vertical gradient retention under the lid (exact-0 default)
  bruise: { strength: 0.0, widthDeg: 70, color: 0x8a5a3a },
};

/**
 * A4 SKYFALL — THE ALTITUDE DEAD BAND. Veg dies by 2.4 km AGL, buildings by
 * 3.0–3.2 km, SAT_SKYLINE catches nothing in a suburb (minH 35) → from
 * ~2.4 km to cruise a suburb is pure photo on ~30 m-relief DEM, exactly the
 * band the user flies. Fade EXTENSION, not a new layer (rejected:
 * settlement-mass tint = invented mass; skyline minH lowering = nothing to
 * mass). When enabled, read sites prefer these over the legacy
 * SAT_BLDG_FADE / SAT_VEG.altFade+cull values; roads already carry to
 * 5.2 km and bridge the rest.
 */
export const ALT_BAND = {
  enabled: false,
  bldgFade: { startM: 3200, holeEndM: 4000, evictM: 4200 }, // legacy 2400/3000/3200
  vegFade: { startM: 2400, endM: 3200 }, // legacy 1800/2400
  vegCull: { onM: 2800, offM: 3400 }, // legacy 2000/2600
};

/**
 * A4 SKYFALL — satMaxZoom 16→17 EVALUATION (tier-gated, NOT default-on;
 * R11 dropped z17 for stream churn and it does not return un-measured).
 * Read site goes through a selector in tile-sources.js; ships enabled at
 * high only if the measured tile-draw delta, texture bytes vs the LRU law,
 * and stream churn at 300–800 m all hold. Otherwise stays a checkpoint-row
 * knob for the user's machine.
 */
export const TILES_Z17 = {
  enabled: false,
  satMaxZoomByTier: { high: 17 },
};

/**
 * A5 NEONTRUE — the toy/Neon winding fix (the filed R19 headline; R18 §1).
 * classifyRings hard-codes the wrong winding sign for this tileset — ~99% of
 * toy polygons drop at the three frozen call sites (polygonPass, toy
 * buildings, toy scatter). ONE flag, mirroring ROOFS_SAT.enabled semantics:
 * dispatch classifyRingsSat at those sites when enabled; false ⇒ the R18
 * code paths byte-identically. Shared classify function BODIES untouched.
 * Cap re-tunes (TOY_WORLD / ROOFS / WORLD_EDGE interiors) are pre-authorized
 * under the R19 checkpoint ruling — every move documented with old values.
 */
export const TOY_COVERAGE = {
  enabled: false,
  // Frozen reference tiles for verify-toy-coverage's before/after counts
  // (Midtown Manhattan / Naperville-class suburb — same z14 ids the roof
  // variety gate documents).
  refTiles: [
    { z: 14, x: 4824, y: 6157 }, // Manhattan Midtown
    { z: 14, x: 4188, y: 6115 }, // Naperville-class suburb
  ],
};

/**
 * A6 AIRSENSE — sense-of-speed wind streaks. ONE camera-space InstancedMesh
 * of thin quads (+1 draw), opacity ∝ airspeed above threshold AND
 * AGL-weighted (speed FEELS fastest low). visible=false below threshold ⇒
 * 0 draws and zero pixels at every frozen probe pose by construction.
 * (FOV ramp rejected: the harness fleet flies at held boost under
 * __flyBoostInfinite — it would move every frozen screenshot gate.)
 */
export const SPEED_CUES = {
  enabled: false,
  count: 90,
  minMps: 95, // below this the layer is invisible AND visible=false
  fullMps: 220,
  aglBoostBelowM: 900, // streaks intensify near ground/cloud
  lengthM: [30, 90],
  opacityMax: 0.35,
};

/**
 * A6 AIRSENSE — ambient day life: bird flocks on closed orbits around
 * park/water anchors from runtime.satVeg (the SAT_AMBIENT leash discipline —
 * closed paths, never free integration; +1 draw, high tier, day only).
 */
export const AMBIENT_DAY = {
  enabled: false,
  flocks: { max: 6, birds: 12, radiusM: [120, 360], aglM: [60, 220] },
  dayFracMin: 0.25, // birds land at night
};

/**
 * A6 AIRSENSE — wind audio feel on the EXISTING wind bus (zero new nodes at
 * enabled:false): AGL-proximity gain boost + slow gust LFO.
 */
export const WIND_FEEL = {
  enabled: false,
  aglGain: { belowM: 700, boost: 0.0 }, // exact-0 default
  gust: { depth: 0.0, periodSec: 9 },
};
