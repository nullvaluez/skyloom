...
/** C: directional + hillshade + dome lobe + water specular all from runtime.sun at every tier (recon L3). */
export const ONE_SUN = { enabled: false };
/** C: tone → grade → vignette → SMAA last (+ output dither) (recon L6). */
export const POST_ORDER = { enabled: false, smaaPreset: 'high', dither: true };
/** C: reversed-depth double-conversion fix for postprocessing depth readers (recon L2, FL-07). */
export const DEPTH_FIX = { enabled: false };
/** C: shadow catcher gate, stable PCF kernel, texel snap, sane normalBias (recon L5, FL-12). */
export const SHADOW_CALM = { enabled: false };
/** C: fragment-stage hillshade, elevation-keyed hillshade weight, smooth DEM normals (recon L8, T6, T7). */
export const TERRAIN_LIGHT = { enabled: false };
/** C: lit cloud sprites — fake sphere normal, sun side / rim / moon tint (recon L7). */
export const CLOUD_LIT = { enabled: false };
/** C: Lambert env multiply tamed (reflectivity / combine) on buildings, skyline, canopy, homes (recon WB-7). */
