...
export const TERRA_PACE = {
  enabled: false,
  skirtFast: false, // O(V) boundary scan replaces the allocate-and-sort edge finder (output-identical)
  skirtWorker: false, // skirt built inside the LERC worker; main thread only wraps attributes
  timerFix: false, // Timer.reset zeroes _elapsed so updateInterval actually gates the quadtree walk
  mergeHysteresis: false, // merge only when ratio > threshold*hysteresis, in-frustum formula both ways
  keepResident: false, // never merge on frustum exit alone; residency bounded by a byte LRU
  parallelLoad: false, // imagery and DEM fetched concurrently per tile
  imageBitmap: false, // fetch + createImageBitmap decode off the main thread
  preUpload: false, // renderer.initTexture / attribute upload before the LOD swap frame
  lodOutsideRender: false, // map.autoUpdate=false; the walk runs from a scheduled useFrame
 ...
 * believes it applied.
 */
export const STEP_SAFE = { enabled: false, valveMs: 1000 };
 ...
 */
export const HUD_SYNC = { enabled: false, framePriority: true };
/**
 ...
 */
export const REBASE_CALM = { enabled: false, quantM: 704 };
