...
export const STEP_SAFE = { enabled: false };
...
export const HUD_SYNC = { enabled: false };
/** A: wall-clock finalize brake on every engine incl. the first chunk; veg commit cap (recon WB-3, A9). */
export const FINALIZE_PACE = { enabled: false, budgetMs: 3 };
/** A: sub-native DPR rungs so a DPR-1 display has render-scale steps before a tier step (recon A4). */
export const LADDER_FIX = { enabled: false, dprMin: 0.75, subStep: 0.125, nativeRefresh: false };
/** A: floating-origin rebase — drop the dead store bump, targeted matrix update, quantised anchor (recon FL-09, T12). */
export const REBASE_CALM = { enabled: false };

// ----- B WORLD — chunk pipeline, births/evicts, recompile storms ------------
/** B: zero-area triangle filter at drape finalize, engine-side, no worker change (recon WB-1, A1/A1b). */
export const FLASH_GUARD = { enabled: false, minArea2: 0 };
/** B: worker ring dedupe (drop the loadGeometry closing clone) — flag-off byte-identical bundles (recon WB-1). */
export const RING_DEDUPE = { enabled: false };
/** B: per-mesh birth ramp + deferred fade-out on every streamed chunk, zero new keys where possible (recon WB-2, A6). */
export const CHUNK_FADE = { enabled: false, birthSec: 0.4, evictSec: 0.3, maxConcurrent: 8 };
/** B: heal = re-drape in place, never evict+refetch (recon WB-8, T8). */
export const HEAL_IN_PLACE = { enabled: false };
/** B: constant PMREM height across sky buckets + two-state prewarm + re-warm on style flip (recon WB-4, A5). */
export const ENV_UNIFORM = { enabled: false };
/** B: bend-margin pad includes the lookahead lead (recon WB-6). */
export const BEND_LEAD = { enabled: false };
/** B: slew-limited visual ground elevation; flight/crash keep RAW (recon A6, T8). */
...
export const FRAME_STATS = { enabled: false, ringSize: 600 };
