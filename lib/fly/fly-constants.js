...
export const SAT_BUILDINGS = {
  ...
  wallBaseMul: [0.5, 0.8], // [short building, tall building] bottom-vertex multiplier
  ...
  roofTones: {
    ...
    mid: ['#4e4b47', '#5f5a52', '#6e6a60', '#8e897c', '#4f6153', '#7b7166'],
    tall: ['#454341', '#565350', '#68645c', '#9a968b', '#736f68'],
  },
...
export const MOBILE_UI = {
  ...
  zones: {
    ...
    // label + callsign + type + tier chip, so the stack goes full-width and
    // the card itself wraps to two lines (that is card markup, not a zone).
    toasts: {
      desktop: 'absolute right-4 top-16 z-10',
      phonePort:
        'phone-port:inset-x-2 phone-port:right-auto phone-port:top-[calc(env(safe-area-inset-top)+3.4rem)]',
      // Landscape has width but almost no height: centre a bounded stack just
      // under the compressed HUD strip instead of hugging the right edge.
      phoneLand:
        'phone-land:left-1/2 phone-land:right-auto phone-land:top-[calc(env(safe-area-inset-top)+2.6rem)] phone-land:-translate-x-1/2 phone-land:items-center',
    },

    // Contracts.jsx:456 — `pointer-events-none absolute left-4 top-24 z-10`
    // plus the R16 phone offsets, which are RETAINED verbatim as the portrait
    // rule (they are already correct; only their spelling moves).
    contracts: {
      desktop: 'absolute left-4 top-24 z-10',
      phonePort: 'phone-port:left-2 phone-port:top-[calc(env(safe-area-inset-top)+8.25rem)]',
    ...
    // plus the R7 phone offsets, retained verbatim as the portrait rule.
    minimap: {
      desktop: 'absolute bottom-10 right-4 z-10',
      phonePort:
        'phone-port:bottom-auto phone-port:right-2 phone-port:top-[calc(env(safe-area-inset-top)+3.25rem)]',
      phoneLand:
    ...
    // The phone bottom offset is the load-bearing one: `infoChip.dockBottomRem`
    // is derived from the joystick anchor below, so the chip and the stick are
    // disjoint BY CONSTRUCTION rather than by eyeball (verify-mobile-layout
    // asserts it, but the geometry is what makes it true).
    'info-dock': {
      desktop: 'absolute bottom-10 left-4 z-10',
      phonePort:
        'phone-port:inset-x-2 phone-port:bottom-[calc(env(safe-area-inset-bottom)+11.75rem)]',
      // Landscape: centred between the stick (left) and the cluster (right).
      phoneLand:
        'phone-land:left-1/2 phone-land:right-auto phone-land:bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] phone-land:-translate-x-1/2',
    },
  ...
  infoChip: {
    ...
    // DERIVED, not guessed: the thumbstick is 8rem tall (h-32) and anchored at
    // `env(safe-area-inset-bottom) + 3.25rem`, so its top edge is at
    // 11.25rem + inset. The chip docks 0.5rem above that => 11.75rem, which is
    // exactly the `phone-port:bottom-[...]` above. Keep the two in step.
    dockBottomRem: 11.75,
  },
