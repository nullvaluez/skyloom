...
export const SAT_POLY_COVER = {
  ...
  skyline: { minAreaM2: 900, maxPerChunk: 300 }, // per-polygon admission for the far-mass ring
  perPolyDrape: true,
};
...
export const TOY_MID_SUBURB = {
  ...
  maxPerChunkMid: 240, // legacy 180 rules when flag off
};

/**
 * R20 B PARCEL-HOMES — procedural suburban buildings where OpenFreeMap ships
 * no footprints. Consumes the EXISTING satVeg cls-4 residential parcel
 * anchors (worker:2665-2671, already building-avoided + DEM-draped) via the
 * SatHouseLights hash-stable cluster idiom (SatHouseLights.jsx:209-241 —
 * 2,128 lights at Powell from zero footprints). Owens Valley has no
 * landuse=residential, so the Owens lock holds BY CONSTRUCTION. Day+night,
 * satellite medium+high tiers, ONE InstancedMesh (+1 draw active, 0 mounted-low).
 */
export const PARCEL_HOMES = {
  enabled: false,
  perAnchor: 10, // homes attempted per parcel anchor (hash-thinned)
  hM: [5, 9], // procedural house height band — must stay OUT of (25,35) (verify-suburbia gate G)
  footprintM: [9, 15],
  spreadM: 110, // cluster radius around the anchor (SUBURB_NIGHT parity)
  houseAvoidM: 14, // never inside a real streamed footprint
  pool: 3000,
  rangeM: 2600, // engine-side distance gate (frustumCulled=false idiom)
  minTier: 'medium',
};
...
export const MONUMENT_MODELS = {
  enabled: false,
  maxPlaced: 12, // marquee instances on screen (pool)
  targetTopM: null, // per-model targetHeightM lives in lib/fly/monument-models.js
  satAlbedo: { saturationCap: 0.45, valueLock: false }, // muted real albedo in satellite (R20 sanctioned taste evolution)
  toy: { accentEmit: 3.2, floodBaseBoost: 1.35, floodTopMul: 0.82 }, // bake-time night read parity with landmarks-3d
};
