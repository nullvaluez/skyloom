...
export const MOBILE_UI = {
  ...
  zones: {
    ...
    toasts: {
      ...
      // 3.9rem, not 3.4: the phone stats strip MEASURES 52 px tall and ends at
      // 58 px, which 3.4rem (54.4) ran into. Numbers like this are exactly why
      // the harness measures instead of trusting the arithmetic.
      phonePort:
        'phone-port:left-2 phone-port:right-[7.5rem] phone-port:top-[calc(env(safe-area-inset-top)+3.9rem)]',
      // Landscape has width but almost no height: centre a bounded stack under
      // the compressed HUD strip instead of hugging the right edge. 3.6rem
      // clears that strip's measured 44 px bottom edge — 2.6rem did not, and
      // the harness caught the 3 px of overlap.
      phoneLand:
        'phone-land:left-1/2 phone-land:right-auto phone-land:top-[calc(env(safe-area-inset-top)+3.6rem)] phone-land:-translate-x-1/2 phone-land:items-center',
    },
    ...
    contracts: {
      ...
      phonePort: 'phone-port:left-2 phone-port:top-[calc(env(safe-area-inset-top)+11rem)]',
    ...
    minimap: {
      ...
      phonePort:
        'phone-port:bottom-auto phone-port:right-2 phone-port:top-[calc(env(safe-area-inset-top)+3.9rem)]',
      phoneLand:
    ...
    'info-dock': {
      ...
      phoneLand:
        'phone-land:left-1/2 phone-land:right-auto phone-land:w-[min(46vw,340px)] phone-land:bottom-[calc(env(safe-area-inset-bottom)+0.5rem)] phone-land:-translate-x-1/2',
    },
