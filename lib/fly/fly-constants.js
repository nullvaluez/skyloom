...
export const LOD_CROSSFADE = {
  ...
  // The RED is clean and measured (Powell altitude ladder on the offline
  // fixture: 20 refines, 20 of them single-frame hard swaps, 0 blended, with
  // `skip.disabled` 20 proving every one was un-faded because the flag is off
  // rather than because a guard fired). There is no GREEN: the flag-ON leg was
  // never booted on any tree — the run that looked like a load timeout was the
  // signature of D's own AerialPerspective import defect, fixed in 1620e32.
  // A feature whose entire claim is visual does not ship on a RED alone.
  enabled: false,
