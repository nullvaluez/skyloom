...
export const MOBILE_UI = {
  ...
  zones: {
    ...
    'info-dock': {
      ...
      phonePort:
        'phone-port:inset-x-2 phone-port:bottom-[calc(env(safe-area-inset-bottom)+15.5rem)]',
  ...
  infoChip: {
    ...
    // 390x844 screen.
    //
    // A5 IS ADDING CLUSTER BUTTONS (logbook, photo, three contextual). If that
    // row wraps to two lines the cluster grows ~3.75rem taller and this number
    // must grow with it; verify-mobile-layout's controls gate will say so.
    dockBottomRem: 15.5,
  },
