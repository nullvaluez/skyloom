 ...
 * out.satRoadPaths = { pts: Float32Array [x,z,...] per polyline, cls:
 * Uint8Array, offsets: Uint32Array, junctions: Float32Array } for cls-3..6
 * centerlines; 'sat-buildings' additionally emits out.satParking =
 * Float32Array anchor points (parking polygons + parcel driveway mouths).
 ...
 * client-side at the road shader's streetSpacingM 42 phase so lamps land ON
 * the existing shader light pools. OWENS DISCIPLINE: cars key to
 * residential/parking anchors, poles to a cls-5/6 density floor — Owens
 * places 0 instances AND +0 draws BY CONSTRUCTION (empty pools issue no
 * draw — the R18 empty-chunk idiom), gate-asserted with bit-identical-totals
 * flips. All clutter ships castShadow:false — D flips casters under
 * DEPTH_PASS with measured gpuFrameMs (cross-charter seam, D owns it).
 * Budgets (plan §5.9): parked <= +1 draw / <= 48k tris; moving <= +1 /
 * <= 12k; poles <= +1 / <= 20k; trees2 same 1 draw / pool <= 320k tris.
 */
export const CLUTTER = {
  enabled: false,
  trees2: { enabled: true, maxTrisPerInstance: 96, trunkFrac: 0.28 },
  cars: {
    parked: { enabled: true, pool: 1500, minAglM: 0, maxAglM: 1400 },
    moving: { enabled: true, pool: 300, maxAglM: 1200, speedMps: [8, 17], tierMin: 'high' },
  },
  poles: { enabled: true, pool: 900, maxAglM: 900, clsDensityFloor: 0.15 },
};
