 ...
 */
export const STEP_SAFE = {
  enabled: true,
  /**
   * useFrame priority for the apply rig. Must sit BETWEEN PerfGovernor (-100,
   * which decides the step) and the EffectComposer (+1, which draws), so a dpr
   * decided this frame is applied this frame and drawn into this frame. -99 is
   * the first slot after the governor; FlyScene's -50 flight step and the -45
   * traffic update both run after it and are indifferent to the buffer size.
   */
  framePriority: -99,
  /**
   * Safety valve. If the rig has not consumed a pending dpr this long after it
   * was requested, apply it the legacy way. rAF does not run in a hidden tab
   * (nothing is presented there either, so the wait is harmless), but a rig
   * that failed to mount must never silently swallow the ladder's only cheap
   * rung. 0 disables the valve.
   */
  maxDeferMs: 1000,
};
 ...
 */
export const FLASH_GUARD = {
  enabled: true,
  /** Strip zero-area triangles from streamed satellite-building chunks. */
  dropDegenerateTris: true,
  /**
   * Twice the triangle area (the cross-product magnitude), below which a
   * triangle is dropped, in square metres.
   *
   * 0 drops ONLY exactly-degenerate triangles and is the measured-safe value:
   * every degenerate found in the census was EXACTLY zero (the `tiny` bucket,
   * 0 < area2 < 1e-6, was empty in every chunk), so 0 removes the whole
   * measured population without touching one real triangle. A positive value
   * would start deleting real slivers on a guess; if a future pose produces
   * near-zero-but-not-zero painters, raise this against a measurement and say
   * so here.
   */
  minArea2: 0,
  /**
   * The cloud half of this round is NOT shipped, and this flag records why.
   * Agent A measured a strong stochastic A/B at the NYC spawn (pale ~1/1,600
   * with clouds on, 0 in 30,499 with `__flyClouds.visible = false`) and
   * recommended a near-camera puff fade. The same-frame hide-bisection refutes
   * it 7/7: hiding `__flyClouds` IN the pale frame, with every matrix frozen,
   * leaves the pale ratio unchanged, and of 36 top-level children only the
   * HemisphereLight and the origin-offset Group clear it. Clouds do not paint
   * these pixels. Why parking them drove A's RATE to zero is still unexplained
   * (see the ledger §6) — a plausible reading is that 55 fewer lit billboards
   * shifts frame cost and therefore which poses get sampled, but that is a
   * hypothesis and it is not measured. Shipping a cloud fade on an unexplained
   * rate correlation would move cloud placement, which several frozen pixel
