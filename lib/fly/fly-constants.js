...
export const TRACERS = {
  ...
  spot: {
    ...
    // ---- length in TRUE metres: L(D) = clamp(4000·(D/8000)^0.7, 4000, 22000)
    // × clamp(speed/220, 0.45, 1). Record spacing (world) = min(1500, L/32·k).
    // Jet: 4.0 km ≤8 km, 6.5 @16, 9.1 @26, 12.3 @40, 16.4 @60, 20.0 @80 km —
    // about 240–490 px broadside at 1080p, so far craft stay long lines.
    length: { nearM: 4000, refDistM: 8000, exp: 0.7, maxM: 22000, speedRefMps: 220, speedMinK: 0.45,
    ...
    width: {
      ...
      floorHeadPx: 2.0, floorTailPx: 1.4, minHeadPx: 1.5, minTailPx: 1.2, refHeightPx: 1080,
      energyMin: 0.65, // floored points: alpha & emission × max(0.65, sqrt(worldW / floorW))
    ...
    // ---- body ----
    body: {
      vaporAlpha: 0.6, // day vapour opacity at the head end (× (1 − night))
      nearVapor: { minK: 0.35, startM: 4000, endM: 12000 }, // near: hull + engine plume carry the read
      emitDay: 0.12, emitNight: 0.75, // additive emission at the head end
      whiteDay: 0.5, // day emission colour = mix(band, white, 0.5·day)
      headFrac: 0.15, headBoostNight: 0.9, coreWhiteNight: 0.55, // night comet head 1.65 (blooms)
    ...
    glint: {
      radiusPx: 5.0, minRadiusPx: 3.0, // r = max(3, 5·H/1080) buffer px; tips at 1.6 r
      farScale: 0.7, farMinPx: 2.5, bandNearM: 20000, bandFarM: 120000,
      dayGain: 2.4, nightGain: 3.0, // luma of the white core (tints are luma-normalised)
      satDay: 0.3, goldMix: 0.5, // halo tint: white → band colour with night, → gold at dusk
      halo: 0.55, spikeNight: 0.35,
      nearFadeM: [4000, 12000], // WORLD distance: the hull carries up close (chase/cinema see none)
      pullM: 90, pullFrac: 0.25, // view-space pull so it composites over its own hull
      hazeDay: 0.4, hazeNearM: 40000, hazeFarM: 160000,
      pulse: { depth: 0.12, hz: 0.4 }, // gentle breathing, per-craft phase — never a blink (R8 BEACONS)
      lockBoost: 1.3,
    },
