...
export const TRACERS = {
  ...
  headMinBrightness: 0.62, // × headBoost 1.5 = 0.93 (legacy look; satellite bloom now 1.08/0.91 — see spot)
  ...
  // SPOTTER TRAILS (September 2026) — SATELLITE ONLY. The user: "we used to see
  // contrails of other planes as indicators with long lines … spot other
  // crafts in the distance". Four independent erasers had removed them in the
  // default satellite view: a ×0.18 brightness / ×0.25 width presentation
  // squeeze (be711f2), the cloud composite painting the analytic sky over every
  // depth-less mark (f0cd81e, fixed by lib/fly/sky-overlay-pass.js), world-metre
  // widths that go sub-pixel past a few km, and the ground rim fade killing
  // everything past ~91 km. TrafficTracers mounts <SpotTracers> (one draw:
  // ribbons + head glints) when spot.enabled && spot.styles[style]; toy,
  // streak mode and enabled:false keep the legacy RibbonTracers byte-for-byte.
  //
  // Still read here from the legacy keys above: max, alphaFloor, speedOn/Off,
  // sun.lerpPerSec, ribbon.warpResetM/vertCutM/nearFadeStartM/nearFadeEndM/
  // sweepFrames. NOT read by the spot path: styleGain, headSectionFrac,
  // sun.dayGain/nightGain/dayWidthK and SATELLITE_VISUALS.presentation.trail*
  // (they remain the rollback look).
  //
  // Every brightness is LINEAR in the post-cloud HDR buffer (after the
  // Enhanced pre-curve exposure, before bloom), so it compares directly with
  // the satellite bloom thresholds in force: 1.08 day → 0.91 night, smoothing
  // 0.2, and NO bloom on tier low. Visibility never depends on bloom.
  //
  // R16 taste rule (full neon ribbons over daylight photography read as a toy
  // overlay): by day the body is near-white VAPOUR drawn with a premultiplied
  // optical blend (out = vapour·a + dst·(1−a) + emission), so a day pixel is
  // ≤ max(sky, 0.95) + 0.12 and can never bloom into a tube; only the head
  // glint (a small white sun glint) blooms by day. Night is the airloom neon.
  spot: {
    // false = the legacy satellite ribbon: still drawn after the cloud
    // composite but WITHOUT the cloud gate (it paints over a deck in front) —
    // a rollback, not a look.
    enabled: true,
    styles: { satellite: true, toy: false },

    // ---- ONE draw, two fixed regions filled strictly by RANK (A before B,
    // so the draw range is contiguous). 96·70 + 416·30 = 19,200 verts: Uint16.
    nearSlots: 96, // ranks 0..95: 33 pairs (32 recorded + live head) + 4 glint verts
    farSlots: 416, // ranks 96..511: 13 pairs + 4 glint verts; 96 + 416 === TRACERS.max
    points: 32, // absolute float64 ring per ARMED track (slotted or not)
    farStride: 3, // far tier: recorded points with absSeq % 3 === 0, + the oldest, + the live head
    farPairs: 13, // must be ≥ 2 + ceil(points / farStride)

    // ---- priority (lower score wins): in view first, then distance. The
    // inspected/locked target scores 0. keep* = hysteresis so ranks don't flap.
    priority: { viewPadDeg: 6, offViewMul: 3.5, keepSlotMul: 0.85, keepNearMul: 0.8 },

    // ---- length in TRUE metres: L(D) = clamp(4000·(D/8000)^0.55, 4000, 14000)
    // × clamp(speed/220, 0.45, 1). Record spacing (world) = min(1500, L/32·k).
    // Jet: 4.0 km ≤8 km, 5.9 @16, 7.6 @26, 9.7 @40, 12.1 @60, 14 km ≥78 km —
    // long lines near you, shorter (and fainter — see prominence) far away.
    length: { nearM: 4000, refDistM: 8000, exp: 0.55, maxM: 14000, speedRefMps: 220, speedMinK: 0.45,
      maxSpacingWorldM: 1500 }, // every step < 0.6·warpResetM, even at 70°N

    vertSlope: 2.5, // cut when vStep > ribbon.vertCutM (400) AND vStep > 2.5·hStep
    backfillVertM: 600, // synthetic tail slope clamp = min(0.12, 600 / L_true)

    // ---- width (FULL widths). World metres near; a drawing-buffer PIXEL
    // floor far away (× H/1080, never below min*), corrected for the air
    // bend's vertical far-lift, so lines never break into dots.
    width: {
      headM: 18, tailDayM: 48, tailNightM: 5, // day vapour spreads with age, night neon tapers
      floorHeadPx: 1.6, floorTailPx: 1.1, minHeadPx: 1.3, minTailPx: 1.0, refHeightPx: 1080,
      energyMin: 0.5, // floored points: alpha & emission × max(0.5, sqrt(worldW / floorW)) — thin light, not tubes
      profileEdge: 0.3, // GPU soft edge on WIDE ribbons only (hairlines stay flat)
      coreK: 10, // white core exp(−10u²) on wide ribbons
      vScaleProbeM: 30, vScaleClamp: [0.4, 4], distScaleClamp: [0.5, 4],
    },

    // ---- body ----
    body: {
      vaporAlpha: 0.42, // day vapour opacity at the head end (× (1 − night))
      nearVapor: { minK: 0.35, startM: 4000, endM: 12000 }, // near: hull + engine plume carry the read
      emitDay: 0.06, emitNight: 0.55, // additive emission at the head end
      whiteDay: 0.5, // day emission colour = mix(band, white, 0.5·day)
      headFrac: 0.15, headBoostNight: 0.45, coreWhiteNight: 0.35, // night head 1.0: a soft glow, not a flare
      tailFloor: 0.18, tailFadeTT: 0.12, // taper = smoothstep(0,.12,tt)·(0.18 + 0.82·tt)
      goldGlow: 0.3, goldAltM: [4000, 9000], // dusk: sunlit high contrails catch gold
      hazeDay: 0.45, hazeNearM: 35000, hazeFarM: 140000, // deferred marks skip the aerial pass
      lockPresence: 1.15,
    },

    // ---- head glint: 4 verts in the SAME draw, a post-projection pixel
    // diamond (exempt from the edge-on collapse, so head-on traffic pops).
    // A SOFT DOT, not a star (user, live review: far glints read as a
    // starfield). Only traffic within reach — pickable/lockable — carries it;
    // it is gone by prominence.glintFarM.
    glint: {
      radiusPx: 3.5, minRadiusPx: 2.5, // r = max(2.5, 3.5·H/1080) buffer px; tips at 1.6 r
      farScale: 0.75, farMinPx: 2.0, bandNearM: 20000, bandFarM: 120000,
      dayGain: 0.8, nightGain: 1.3, // luma of the dot (tints are luma-normalised): barely blooms
      satDay: 0.3, goldMix: 0.5, // tint: white → band colour with night, → gold at dusk
      halo: 0.4, spikeNight: 0, // no 4-point spikes (the star read)
      nearFadeM: [3000, 8000], // WORLD distance: the hull carries up close (chase/cinema see none)
      pullM: 90, pullFrac: 0.25, // view-space pull so it composites over its own hull
      hazeDay: 0.4, hazeNearM: 40000, hazeFarM: 160000,
      pulse: { depth: 0, hz: 0.4 }, // still: a steady mark, never a blink (R8 BEACONS)
      lockBoost: 1.4,
    },

    // ---- prominence follows REACH (TRUE metres). Traffic you can act on —
    // lock-on acquires inside 10 km, hover picks the nearest — keeps the full
    // read; beyond, trails recede to faint hairlines and lose their glint, so
    // a busy sky reads as depth, not as a starfield of equal marks.
    // NIGHT: additive light over a black sky reads at any level, so past
    // reach every emissive mark (body, head, glint) also dims to nightFloor
    // by nightFarM (measured on the offline fixture, isolated targets: the
    // day law alone left 32 km at Δ120 and 48–64 km at Δ50–65 — a sky of
    // equal lines). Day keeps the gentle ramp (vapour recedes into a bright
    // sky on its own).
    prominence: { fullM: 15000, farM: 36000, bodyFar: 0.25, glintFullM: 15000, glintFarM: 30000, nightFarM: 32000, nightFloor: 0.22 },

    // ---- look: TRUE sun elevation, damped with TRACERS.sun.lerpPerSec ----
    look: {
      nightElDeg: -6, dayElDeg: 3, // night n = 1 − smoothstep(−6, 3, el)
      vapor: [0.78, 0.8, 0.84], // lum 0.80 (headroom for the forward sheen under the cap)
      gold: [1.0, 0.64, 0.38],
      goldVapor: 0.75, // vapour → gold by immersiveLighting().golden
      vaporMaxLum: 0.95, // < 1.08: optical vapour never blooms
      sunForward: 0.45, // vapour × (1 + 0.45·day·max(0, view·sun)^6)
      overcastVaporDim: 0.2, overcastGlow: 0.2, // R18: overcast dims vapour, ADDS white emission
    },

    // ---- tracer-owned GPU range fade (TRUE m × mercatorScale → world XZ).
    // Replaces the ground rim's ~45–91 km kill for sky-backed traffic only:
    // traffic well below the eye keeps the ground band (R11: no ghosts over
    // farmland). uEdgeFade itself is untouched.
    fade: { skyStartM: 150000, skyEndM: 185000, groundBelowEyeM: [2000, 500] },
  },
};

// Depth-less airborne overlays drawn after the cloud composite
// (lib/fly/sky-overlay-pass.js). cloudGate = analytic cloud-slab occlusion:
// this frame's cloud-march transmittance raised to the fraction of the
// marched slab in front of the mark, so trails never paint over cloud
// undersides. Point marks (glints, billboards) keep markFloor — a light
// glowing through cloud; lines (trail bodies, plumes) are fully occluded.
// enabled:false → billboards/wakes keep their pre-gate program keys.
export const SKY_OVERLAYS = {
  cloudGate: { enabled: true, markFloor: 0.2, billboards: true, wakes: true },
};
...
// ===========================================================================
// ROUND 25 "Front Door & Clear Sky" — W0 owner blocks (orchestrator scaffold).
// Plan: FLY_ROUND25_PLAN.md. Every master flag ships `enabled:false` here and
// is byte-identity when off; each owner flips ONLY its own block on its own
// branch once its gates are green. Edit ONLY your own block.
//   A FRONT DOOR   — FRONT_DOOR, VISUALS
//   B FLIGHT PLAN  — FLIGHT_PLAN
//   C SKY          — R25_SKY
//   D GROUND       — R25_GROUND
//   E CERT         — R25_CERT
// Visuals contract: every C/D decision point (shader text, program cache key,
// CPU uniform write, allocation) goes through r25On(block, sub) in
// lib/fly/visuals-profile.js = block.enabled && sub.enabled !== false &&
// the live profile is 'enhanced'. Classic therefore IS the flag-off tree.
// ===========================================================================

/** A: the title screen (DOM at z-45 over BootScreen z-40, under hangar z-60) + title flyby orbit. */
export const FRONT_DOOR = {
  enabled: true, // A: flipped on r25/a once verify-r25-front-door + verify-r25-title read green
  titleZ: 45,
  bootCompact: true,
  exitToTitle: true,
  // pitchDeg (A): the look-down angle toward the orbit centre (the look point
  // never sinks below the centre's ground) — 16° keeps the horizon in frame.
  orbit: { periodSec: 240, reducedMotionPeriodSec: 900, radiusM: 2600, aglM: 900, minAglM: 350, fovDeg: 52, easeSec: 2.5, pitchDeg: 16 },
}; // owner A

/** A: the Visuals Enhanced/Classic profile (persisted key; default applies only when an R25 visual block ships ON). */
export const VISUALS = {
  key: 'fly-visuals',
  // User-requested default on every device (2026-09-25). Quality tier is
  // resolved separately; explicit saved visuals preferences still win.
  defaultProfile: 'enhanced',
}; // owner A

/** B: Free Flight vs Takeoff & Landing, featured destinations, last setup + Continue, hangar destination staging. */
export const FLIGHT_PLAN = {
  enabled: true,
  lastSetupKey: 'fly-last-setup-v1',
  stage: { enabled: true, hzDesktop: 10, hzPhone: 4, debounceMs: 400 },
  freeFlight: { minAglM: 450, cityOffsetM: 3000, fallbackAltMslM: 800 },
  titleSpot: { preferDaylight: true, minSunElDeg: 8, fallbackId: 'grand-canyon', toyId: 'manhattan' },
}; // owner B

/** C: one analytic atmosphere feeding sky pixels, haze, fog, rim, void, clouds and IBL alignment. */
export const R25_SKY = {
  // Every sub-block is read through r25On(R25_SKY, sub) (lib/fly/visuals-profile.js):
  // Classic and flag-off never read a number below. Ledger: scripts/r25-c-sky.md.
  enabled: true,
  // lib/fly/sky-model.js — single-scattering Rayleigh + HG Mie over a spherical
  // exponential atmosphere, azimuth-separable (plane-parallel sun path via the
  // Chapman grazing function), tabulated at 8 view elevations. sunE is the
  // ONE radiance scale (calibrated: noon horizon luminance at 1500 m MSL =
  // the certified Classic rim #c6d7e8, Y 0.667); turbidity scales Mie.
  model: {
    enabled: true,
    sunE: 11.17,
    turbidity: 0.6,
    ...
    mieHM: 1200,
    ozone: 1,
    msK: 1, // first-order multiple scattering (sky-lit view path); 0 = pure single scattering
    steps: 24,
    voidK: 0.35, // below-rim void = horizon x this (only visible while day < 1)
    discK: 4, // sun disc gain (the Classic composite's `disc*4.`)
    recomputeAltM: 10, // re-integrate the table when the eye moves this far vertically
    recomputeSinEl: 0.0002, // ... or the sun this far
  },
  skyDip: { enabled: true },
  aerialSun: { enabled: true },
  // Clouds: hazed at slab-entry distance (livingAirTransmission), model
  // ambient/key chroma, and a fadeStart..fadeEnd alpha ramp replacing the
  // hard IMMERSIVE.clouds.rangeM (22 km) cut.
  cloudAir: { enabled: true, fadeStartM: 16000, fadeEndM: 22000 },
  // The haze stack retires BY THE AERIAL PASS'S AUTHORITY (strength/maxMix):
  // where the post pass hazes, the 16-55 km tile band goes to 0 and fog falls
  // to fogFloor (SKY.altAtmo's cruise floor); where it does not (night ramp,
  // fleet pin) the Classic band stays, so Enhanced never loses haze.
  retireStack: { enabled: true, fogFloor: 0.000003 },
  // scripts/hdr-sun.mjs --r25: each HDRI's sun azimuth in the app hour-angle
  // convention; environment/background rotate by (hdriSunAz - sun.az) about +Y.
  iblAlign: {
    enabled: true,
    hdriSunAz: { day: -0.972783, dawn: -0.952488, dusk: -0.979601, night: -0.803958 },
  },
  // Pre-curve exposure 2^(satelliteExposureStops + biasStops) in the aerial /
  // cloud composite; WhiteBalance then carries the balance WITHOUT the
  // exposure gamma. biasStops is C's single luminance knob (plan contract).
  // Integrated high-altitude C-only ground measured +8.96% luminance against
  // Classic. A small scene-linear bias restrains that lift before tone mapping.
  exposure: { enabled: true, biasStops: -0.05 },
}; // owner C

/** D: DEM relief normal maps, one bounded sun term, low-frequency imagery colour transfer, magnification sharpening. */
export const R25_GROUND = {
  // R25 D: ON on r25/d once the node gate (scripts/verify-r25-ground.mjs) went
  // green; the fixture browser row is recorded in scripts/r25-d-ground.md.
  enabled: true,
  // Relief: a mapPx^2 RG8 world-frame normal map per DEM tile from central
  // differences over the FULL decoded grid (LERC worker splice / terrain-rgb
  // main-thread decode), pooled (<= poolTiles, LRU by last-visible) and
  // same-LOD edge-stitched. Enhanced requests only (lib/fly/r25-relief.js).
  relief: { enabled: true, mapPx: 128, poolTiles: 96, stitch: true, rebindEveryFrames: 30, rebindPerFrame: 4 },
  // albedo *= clamp((N.L + a) / (Ncap.L + a), lo, hi), Ncap = up; relief is at
  // full weight when the tier's hill strength reaches fullStrength (0.55 =
  // HILLSHADE.strengthByTier.high). groundValueK scales the earth surface's
  // 0.70 ground value (the D-only luminance normaliser; 1 = neutral).
  oneSun: { enabled: true, lo: 0.6, hi: 1.45, a: 0.2, fullStrength: 0.55, groundValueK: 1 },
  // Rolling atlas of z11 imagery slots (the colour-balanced low zoom) that the
  // tile's own low mip is divided by: hi *= clamp(ref / lod(hi), clampLo, clampHi).
  colorRef: { enabled: true, zoom: 11, slotPx: 64, atlasPx: 512, clampLo: 0.5, clampHi: 2, edgeFadeTiles: 0.5, maxFetches: 4 },
  // r25-ground suppresses this photographic term while painterly materials
  // own the surface. The saved ground component keeps its original contract.
  sharpen: { enabled: true, minTier: 'high', k: 0.3 },
  retireQuilt: { enabled: true },
  // Launch-applied (NOT live): tighter mid-ring Martini error, Enhanced only;
  // ships only if tris <= 2.0M at fixed poses and Owens draws do not move.
  mesh: { enabled: false, table: { 13: 15, 14: 6, 15: 5, 16: 2 } },
  // Measured 304.69 MiB on a long warp; the 64 MiB trial reached 299.90 MiB.
  // Reserve 24 MiB from Classic's 80 MiB cache for maps and upload overlap.
  // Residency sheds only hidden tiles; visible geometry keeps its coverage.
  budget: { enhancedTerrainResidentMiB: 56 },
}; // owner D

/** E: certification bounds shared by C/D/E instruments (scripts/_r25-luma.js). */
export const R25_CERT = {
  enabled: false,
  luma: { cOnly: [0.92, 1.08], dOnly: [0.95, 1.05], both: [0.90, 1.10], clipPts: 1 },
  horizon: { maxDeltaE: 8, improveK: 0.6 },
  relief: { minGain: 1.15 },
}; // owner E
