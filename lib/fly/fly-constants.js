...
export const TILES = {
  ...
  // (lruBudgetBytes — R22 CORRECTION: that key was already DEAD when this was
  // written; nothing reads it, three-tile 0.12.1 has no such budget, and what
  // actually bounds resident texture memory is the quadtree's own unload of
  // non-visible subtrees. verify-aerial's texture-bytes gate is the real
  // instrument — see the R22 deletion note below),
  ...
  // Round 22 (A TERRA, debris): the comment here read "Terrarium data ceiling"
  // — a leftover from the AWS Terrarium provider that R7 replaced with Esri
  // WorldElevation3D/Terrain3D LERC (lib/fly/tile-sources.js has been Esri-only
  // for fifteen rounds). The ceiling is now a MEASURED property of Esri's
  // service: probed 2026-08-07 at all six R22 poses
  // (scripts/r22-a-esri-probe.json), Terrain3D serves real LERC through z16
  // (20–68 KB) and a degenerate 67-byte constant-surface tile at z17. 15 stays
  // the shipped value for every tier; TERRA_SHARP.demMaxZoom raises the HIGH
  // tier to 16 behind the flag (plan §5.5) via tile-sources.demMaxZoomFor().
  demMaxZoom: 15,
  // Round 22 (A TERRA, debris, 2026-08-07): `lruBudgetBytes: 140MB` and
  // `viewDistanceM: 250000` were DELETED. Both were dead — grep-proven: the
  // only surviving references in the whole tree were the R19 comment above and
  // these two lines. Neither was ever read by three-tile, terrain-engine or
  // tile-sources, so neither ever bounded anything; keeping them invited the
  // exact mistake R19 made, which was to argue z17 was affordable because "the
  // LRU is unchanged".
...
export const MOBILE_UI = {
  ...
  zones: {
    ...
    'info-dock': {
      ...
      // R25 (D MOBILE) — a GETTER, and BOTH strings are written out as
      // literals, for one reason: Tailwind v4 emits an arbitrary-value utility
      // only when the candidate appears VERBATIM in a scanned source file. A
      // template literal built from `dockBottomRem` would compile to no rule
      // at all and the chip would dock nowhere. So the flag-off branch stays
      // CHARACTER-IDENTICAL to the pre-R25 string (verify-mobile-fan asserts
      // that equality), and arming MOBILE_FAN_R25 swaps in the shorter
      // column's 21.75rem — see `infoChip.dockBottomRem` for the derivation.
      // A PIN that moves `MOBILE_FAN_R25.dockBottomRem` moves the number but
      // NOT this class (Tailwind cannot generate what is not in the source);
      // verify-mobile-fan asserts the two agree so the pair cannot drift.
      get phonePort() {
        return mobileFanOn()
          ? 'phone-port:inset-x-2 phone-port:bottom-[calc(env(safe-area-inset-bottom)+21.75rem)]'
   ...
   *
   * R25 (D MOBILE) appends `hangar`, and it carries `fanOnly: true` because it
   * is NOT a row button: the row is the flag-off surface and has to stay
   * byte-for-byte what it was, so TouchControls filters `fanOnly` out of
   * PERSISTENT and only MOBILE_FAN_R25.order (the petal contract) names it.
   * The hangar had NO key and NO touch button before this round — PauseMenu
   * was its only door (PauseMenu.jsx:201-211), which is the whole reason the
   * fan exists.
   */
  cluster: [
    ...
    { id: 'hangar', label: 'Hangar', testid: 'touch-hangar', fanOnly: true }, // R25 (D)
  ...
  infoChip: {
    ...
    //
    // R25 (D MOBILE) — THE COLUMN GOT SHORTER, so this got smaller, and it is
    // now a SWITCH rather than a number. With MOBILE_FAN_R25 armed the
    // persistent row and BOTH contextual rows are gone: one 3.5rem FAB stands
    // where the 3rem row was, the petals are absolutely positioned (they take
    // no column height at all), and the contextual row can no longer appear —
    // so the worst case and the resting case are the same column.
    //   armed, bottom-up: anchor 3.25 + BOOST 2.75 + gap 0.75 + rail 8.375 +
    //   gap 0.75 + FAB 3.5 = 19.375 rem. MEASURED, not derived: on a 390x844
    //   screen the pad reads [306,748-374,792], the rail [306,602-374,736]
    //   (134 px: three min-h-11 detents plus the two hairline borders, which
    //   is why the rail is 8.375 rem and not the 8.25 the A5 block quotes),
    //   and the FAB [318,534-374,590] — a column top of 534 px, i.e. 310 px
    //   = 19.375 rem above the bottom.
    // `MOBILE_FAN_R25.dockBottomRem` ships 21.75, which docks the chip's bottom
    // 2.5 rem above that top rather than the 1 rem the pre-R25 value used. The
    // slack is DELIBERATE and it is the plan's own number: FLY_ROUND25_PLAN.md
    // §3 D derives 20.75 rem by counting the BOOST pad at 4.25 rem, which is
    // its 68 px WIDTH (`clusterSize.boostPx`), not its 44 px height — the same
    // slip the A5 block above carries. Rather than move a plan number on a
    // SwiftShader measurement, the extra 2.375 rem is kept as headroom: it is
    // the conservative direction (the chip sits HIGHER, never lower, and the
    // one thing it must never do is land on a control), and every disjointness
    // gate passes with it.
    // FLAG-OFF IS THE LITERAL 24.5 — see `zones['info-dock'].phonePort`, whose
    // two Tailwind strings are the pixels this number only describes.
    get dockBottomRem() {
      return mobileFanOn() ? mobileFanPinned().dockBottomRem : 24.5;
    },
  },
   ...
   *
   * R25 (D MOBILE) adds `fabPx` / `petalPx`. They are a documented MIRROR of
   * `MOBILE_FAN_R25.fabPx.portrait` / `.petalPx`, in the `toast.maxStack`
   * idiom: the FAN reads the pinnable block (so a console pin can re-shape the
   * geometry live), while anything doing LAYOUT arithmetic against the control
   * column — the way BoostBar already reads `boostPx` — finds the numbers
   * where every other control size lives. Change one and change the other in
   * the same commit; verify-mobile-fan asserts they are equal, so they cannot
   * drift silently. `buttonPx` 48 is UNTOUCHED: it sizes the flag-off row.
   */
  clusterSize: { buttonPx: 48, contextualPx: 44, boostPx: 68, fabPx: 56, petalPx: 44 },
};
...
export const AERIAL_PERSPECTIVE = {
   ...
   */
  content: {
    enabled: true, // R22 W3 FLIP (sanction 5.4: the W2 arbitration gate makes minTier real; verify-sat-mobile re-run MANDATORY in cert)
    minTier: 'medium', // R22 SANCTIONED 5.4: the R19 5b-named right fix for medium/low; high stays excluded (double-haze)
...
export const SAT_QUILT = {
  ...
  minTier: 'high',
  desatMax: 0.22, // R22 SANCTIONED 5.7 (A, measured): 0.35 -> 0.22 - shipped strength removed 53% of ground colour at the P-DUBLIN arrival; checkpoint #6 reviews
  lumaFlatten: 0.25,
  inAglM: 4000, // starts blending in here...
  outAglM: 12000, // R22 SANCTIONED 5.7 (A, measured): 9000 -> 12000 - the FL300 arrival (~8870 AGL) sat at 97% of the ramp; now ~61%, full strength reserved for genuine high cruise
};
...
export const PREWARM = {
  ...
  // --- R22 (B SETTLE) additions ---------------------------------------------
  // warmExtra: the variants R21 left cold, every one of which first draws AFTER
  // the boot reveal and therefore compiles in front of the player — troika's
  // POI letter material (PoiLetters first draws ~t+2 s), drei <Clouds>, the
  // PrecipLayer ShaderMaterial, the player hull rim, the warp burst, and the
  // SHADOW DEPTH variants (a caster's depth material is a SECOND program that
  // three mints on the first shadow pass, not on the first colour draw).
  warmExtra: true,
  // SMAA ships two lookup textures decoded from base64 data URLs on an
  // HTMLImageElement — an async decode + upload that lands mid-flight the first
  // time an SMAA pass renders. Pre-decoding is not a compile, it is the same
  // class of first-draw stall.
  warmSmaaLuts: true,
  // The OTHER map style's whole chain. Measured OFF by default: a style flip is
  // a deliberate, rare, already-expensive action, and warming both styles
  // doubles the boot compile train on the machines least able to afford it.
  warmAltStyle: false,
};
...
// ---------------------------------------------------------------------------
// ROUND 24 "SMOOTH WORLD" — pre-seeded owner blocks (W0 scaffolding, Fable).
// Every block ships `enabled:false` and is byte-identity when off. Owners
// fill in their own knobs; nobody edits another owner's block (the R18–R21
// idiom that has produced zero constants conflicts four rounds running).
// Plan: FLY_ROUND24_PLAN.md §3. Evidence: scripts/r24-recon.md (ids in
// each comment are that ledger's pain-point ids).
// ---------------------------------------------------------------------------

// ----- A PACE — frame loop, terrain pipeline (vendored three-tile), pacing ---
/** A: vendored three-tile patches, each its own switch (recon T1–T5, FL-02/03, A2). */
export const TERRA_PACE = {
  // R24 CLOSE (A PACE): SHIPPED ON. The six ON switches below are the ones with
  // a measured before/after in scripts/r24-a-pace.md; the six OFF ones each
  // carry the reason they are off, and NONE of them is off merely by omission.
  enabled: true,
  skirtFast: true, // O(V) boundary scan replaces the allocate-and-sort edge finder (output-identical)
  timerFix: true, // Timer.reset zeroes _elapsed so updateInterval actually gates the quadtree walk
  mergeHysteresis: true, // merge only when ratio > threshold*hysteresis, in-frustum formula both ways
  keepResident: true, // never merge on frustum exit alone; residency bounded by a byte LRU
  // --- OFF, each for a stated reason ---------------------------------------
  // BUILT and proven output-identical in node (verify-skirt-worker 8/8), but
  // the path it patches is UNREACHABLE in the fixture: three-tile's
  // terrain-rgb loader builds geometry on the main thread, so only Esri LERC
  // tiles reach the geometry worker. Needs one real-hardware run.
  skirtWorker: false,
  // BUILT (TerrainEngine._armBendSpheres). Inflating the tile bounding sphere
  // fixes the bend-blind cull but necessarily SUBMITS far tiles that are culled
  // today — a draw-count change against Owens <= 261 / sat <= 375. Needs one
  // real-hardware run at the canonical poses.
  bendSphere: false,
  // NOT IMPLEMENTED this round. These four are W0 scaffolding switches that no
  // patch reads: turning any of them on changes NOTHING. Left in place so the
  // pain-point ids keep a home (T5, T4a, FL-03), and named here so a future
  // reader does not flip one and wait for an effect.
  parallelLoad: false, // T5 — imagery and DEM fetched concurrently per tile
  imageBitmap: false, // T5 — fetch + createImageBitmap decode off the main thread
  preUpload: false, // T4a — renderer.initTexture before the LOD swap frame
  lodOutsideRender: false, // FL-03 — map.autoUpdate=false; the walk from a scheduled useFrame
  // recon T3, second half — MEASURED by E CERT on the R24 fixture: upstream
  // freezes the WHOLE quadtree walk while `downloadingThreads + 4 >=
  // maxThreads` (six loads of ten), so a busy queue pins the tree at whatever
  // depth it reached — dl 9/10, maxZ stuck at 6, ground samples answering
  // 193 m where the true elevation is 276 m, and every building drape
  // restarting because tileZ 6 is below SAT_BUILDINGS.demZ. The fix keeps the
  // walk running and withholds only the LOAD STARTS, per tile.
  walkWhileSaturated: true,
  // recon T3, allocation half. `Tile.BBox` allocates a Box3 + two Vector3s per
  // tile per visit; the box only moves on a floating-origin rebase or when the
  // tile's own max height changes. Ships WITH walkWhileSaturated, which
  // roughly doubles the visits (MEASURED: 36 -> 233 nodes per walk on the
  // serpentine fixture).
  bboxCache: true,
  // mergeHysteresis: merge only past threshold x this. Upstream uses the SAME
  // threshold both ways, so a tile sitting at the boundary flips
  // refine<->merge and discards four downloads per cycle (recon T1).
  mergeHysteresisK: 1.6,
  // keepResident's byte bound. The residency budget itself is the long-dead
  // TILES.lruBudgetBytes (140 MB, zero consumers at 3592656) — wired by
  // lib/fly/tile-residency.js. These are only its cadence and its per-pass
  // collapse cap: a pass elects at most `maxCollapsePerPass` farthest
  // subtrees and marks them collapsible for `collapseHoldMs`, so a budget
  // that comes back under does not keep collapsing.
  // MEASURED (scripts/verify-terra-residency.mjs H): a fixed 4-subtree brake at
  // 2 Hz cannot keep up with a streaming serpentine — the resident peak still
  // grew. The pass now elects enough OUT-OF-FRUSTUM subtrees to cover the
  // overshoot (in-frustum collapses are undone by the next walk), capped per
  // pass so one pass can never stall a frame.
  // `maxResidentTiles` is the SECOND eviction trigger, and the one that binds.
  // TILES.lruBudgetBytes (140 MB) is the first, and pass 2b proved a byte
  // budget alone is not a bound: on ONE fixed Owens pose the retained set grew
  // with sweep DURATION — 62 -> 103 resident tiles and draws 152 -> 279 between
  // a 45 s and a 600 s sweep with identical flags — at 113.7 MB, i.e. under
  // budget the whole time, so nothing was ever elected. A budget a session
  // cannot reach is not a budget. Tiles are also what cost DRAW CALLS, and draw
  // calls are the frozen ceiling, so the count is the honest second unit.
  //
  // The value must clear one full 360-degree yaw's working set, or the round's
  // headline (zero refetches on yaw) dies to its own memory brake. MEASURED in
  // scripts/verify-terra-residency.mjs: a 720-frame yaw settles at the number
  // recorded there, and the gate asserts BOTH that the set stays bounded and
  // that refetches stay 0 at this cap.
  residency: {
    passIntervalMs: 250,
    collapseHoldMs: 2000,
    maxCollapsePerPass: 32,
    // The FLOOR. Sized against a 240-frame synthetic yaw, whose working set
    // saturates at 190 — which is why it is a floor and not the cap.
    maxResidentTiles: 260,
    // The cap that actually binds: this many resident tiles per DRAWN tile.
    // The retained set is a full revolution's worth of the visible set, so a
    // multiple of the drawn count tracks it at any altitude, on any venue and
    // at any sweep length, where a constant measured on one path cannot — the
    // re-take's 650-frame revolution at z17 exceeded 260 and cost 47 merges
    // and 47 parent refetches, all of them off-screen (replacedOnScreen held
    // at 0, so the user-visible contract never broke — the cost was bandwidth,
    // not a swap the player could see).
    //
    // MEASURED resident/drawn across three paths and two sweep lengths, because
    // the yaw case alone would have under-sized it again:
    //   yaw 240f 3.96 · yaw 650f 4.87 · bob 240f 9.20 · bob 650f 5.75
    //   serpentine 240f 5.34 · serpentine 650f 4.94
    // The worst is the vertical bob at 9.20 — a steep look-down draws few tiles
    // while the retained set spans altitudes — so the ratio must be sized on
    // THAT, not on the yaw the round happens to be about. 14 is ~1.5x margin
    // over the worst measured.
    //
    // TILES.lruBudgetBytes (140 MB) is still the hard memory stop underneath,
    // and after this change it is the trigger that actually guards memory —
    // which is right, because bytes are the real resource and a tile count is
    // only ever a proxy for them. The division of labour: the count cap stops
    // a pathological count at a BOUNDED view, the byte budget stops real
    // memory growth (a serpentine measured 283 -> 316 resident from 240 to 650
    // frames — translation genuinely grows the set, and only bytes can bound
    // that). Pass 2b measured 113.7 MB at Owens, under budget, so lifting the
    // count trigger spends no memory the byte rule was going to refuse.
    maxResidentPerVisible: 14,
  },
  // T14, OFF pending a draw measurement. Three culls tile meshes on their
  // UNBENT bounding sphere while the shader drops each vertex by d^2*k. With
  // k = 1/(2*100000) the drop VARIES across one tile by ~2*d*k*r, i.e. ~30% of
  // the tile radius at 30 km and ~120% at the 120 km fade end, so a far
  // resident tile whose bent vertices are on screen can be culled. Inflating
  // the sphere fixes that — and SUBMITS far tiles that are culled today,
  // which is a draw-count change against frozen ceilings (Owens <= 261,
  // sat <= 375). It therefore ships OFF until it is measured at the canonical
  // poses on E's fixture. keepResident makes it MORE relevant (more far tiles
  // stay resident) but not more dangerous: a merged parent has a bigger
  // sphere and is culled LESS often, so today's coarse field hides the defect
  // rather than avoiding it.
  bendSphere: false,
  bendSpherePad: 1.15, // matches STREAM_KEEPER.bendMargin.pad
  // R24 A — the DRAW half of keepResident (recon T1). Retention must not cost
  // draw calls: a tile out of frustum keeps its model and its place in the tree
  // (nothing disposed, nothing re-downloaded) but its model is parked
  // invisible, so three skips the whole subtree. Pass 2b measured the retained
  // set growing with sweep DURATION on one fixed pose — Owens draws 185 after a
  // 45 s sweep and 279 after 600 s with identical flags — which is residency
  // charging rent in draw calls forever.
  parkOffscreen: true,
  // lodOutsideRender: how often the scheduled walk runs (ms). Upstream's own
  // TileMap.updateInterval is 50 and is INERT (the Timer bug, recon T3).
  lodIntervalMs: 50,
};
/**
 * A: apply a governor DPR step INSIDE the frame that draws it (recon FL-05, A3).
 * valveMs is the safety valve: if the -99 rig has not run within this long
 * (hidden tab, canvas unmounted between the park and the next frame) the value
 * is applied the legacy R21 way so the governor is never stranded on a rung it
 * believes it applied.
 */
export const STEP_SAFE = { enabled: true, valveMs: 1000, framePriority: -99, maxDeferMs: 1000 }; // R24 CLOSE: SHIPPED ON
/** A: fixed-timestep flight/camera sim + interpolated render pose (recon FL-04). */
// R24 CLOSE (A PACE): SHIPPED OFF, and not because it is unfinished on its own
// terms — the accumulator and the interpolated pose are built and proven
// (verify-frame-step 10/10, including the substep-boundary identity). The
// CONSUMER OPT-IN is what did not land: PlayerPlane, the chase camera, Contrail
// and PlayerGroundShadow still read `flight.pos`, and pointing them at
// `renderPos` is the half that can move a harness pose. It cannot be certified
// in the R24 venue, because the fleet pins its poses THROUGH `flight.pos`. See
// scripts/r24-a-pace.md §8c for the four call sites and the certification list.
// Turning this on today changes the SIM integration without any consumer
// reading the smoother pose — all of the risk, none of the benefit.
export const FRAME_STEP = { enabled: false, hz: 120, maxSubsteps: 4 };
/**
 * A: HUD LabelCanvas drawn from the GL frame, not its own rAF (recon FL-01).
 *
 * rAF callbacks run in REGISTRATION order and both loops re-register as their
 * first statement, so whichever registered first at boot stays first forever.
 * LabelCanvas mounts before the GL canvas does (FlyMode renders it
 * unconditionally; FlyCanvas waits on geolocation), so every frame the overlay
 * projects tracks through the camera matrices computed by the PREVIOUS
 * renderer.render — a consistent picture of frame N-1 composited over GL frame
 * N. At 60 fps and ~60 deg/s of yaw that is ~1 deg, about 30 px on a 1920-wide
 * canvas: labels, the lock reticle and the hover ring visibly slide off their
 * planes in every turn, which is one of the things "tearing" can mean.
 *
 * framePriority: the two FL-06 companions — CloudField's useFrame gets an
 * explicit negative priority so it runs BEFORE drei's default-0 child (which
 * decomposes each puff's matrixWorld), and `camera.updateMatrixWorld()` runs
 * unconditionally at the end of the -50 block so every priority-0 reader sees
 * THIS frame's camera rather than only the satellite+high+aerial branch.
 */
export const HUD_SYNC = { enabled: true, framePriority: true }; // R24 CLOSE: SHIPPED ON
/**
 * A: wall-clock finalize brake on every engine incl. the first chunk; veg
 * commit cap (recon WB-3, A9).
 *
 * budgetMs is SHARED across all four chunk engines and counted from the start
 * of the frame, not from each engine's own entry — today each engine may spend
 * its own allowance in the same frame and none of them can see the others.
 * longFrameMs is rule 1: if the PREVIOUS frame took longer than this, no
 * engine finalizes anything this frame. A frame that is already late is the
 * worst moment to add a multi-megabyte upload, and "always let the first one
 * through" (the toy engine's `done > 0 &&` guard) is how a hitch train
 * sustains itself. vegPerFrame caps SatVegEngine._commitPending, which today
 * flips EVERY finished chunk ready on one tick (recon A9) while every other
 * engine budgets its finalize.
 *
 * spikeK / maxRefuseFrames — ADDED AFTER THE FLIP, and the reason is a real
 * defect, not a tune. As first shipped, rule 1 compared the previous frame to
 * a FIXED 24 ms, which is a test for a LEVEL, not for a spike. On any machine
 * running steadily below ~41 fps every frame is "long", so rule 1 refused the
 * first finalize of every frame FOREVER and no chunk ever landed — buildings
 * simply never appear. 30 fps is a 33 ms frame; 20 fps is 50 ms. Those are
 * laptops and integrated GPUs: exactly the machines that report the symptoms
 * this round exists to fix. The build container (300-1000 ms frames) only made
 * it total and therefore visible.
 *
 *   spikeK: refuse only when the frame ALSO exceeds this multiple of a running
 *     EMA of recent frame times. A 40 ms hitch amid 16 ms frames is a spike and
 *     is refused; a steady 33 ms machine never is, because its own baseline is
 *     33 ms. This is what makes rule 1 a hitch guard instead of a slow-machine
 *     embargo — and on a steady 1 fps venue it is a no-op BY CONSTRUCTION,
 *     which is the proof that the fix belongs to the product and not to a
 *     harness seam.
 *   maxRefuseFrames: the hard starvation cap. Even a genuine hitch TRAIN may
 *     not defer more than this many consecutive frames; the next one is
 *     admitted whatever the EMA says. No streaming rule may be able to stall
 *     the world indefinitely, however good its reason on any single frame.
 */
export const FINALIZE_PACE = {
  enabled: true, // R24 CLOSE: SHIPPED ON
  budgetMs: 3,
  longFrameMs: 24, // ~1.5 frames at 60 Hz
  // A frame must beat BOTH longFrameMs and this multiple of the running EMA to
  // count as a spike. 2 is deliberately generous: rule 1 is an optimisation,
  // and the cost of missing a hitch is one late upload, while the cost of a
  // false positive is a world that never streams in.
  spikeK: 2,
  // Never defer more than three consecutive frames, then admit one regardless.
  maxRefuseFrames: 3,
  vegPerFrame: 1,
};
/**
 * A: sub-native DPR rungs so a DPR-1 display has render-scale steps before a
 * tier step (recon A4), a governor target that follows the display's own
 * refresh (recon FL-04), and a long-frame term so STUTTER can step the ladder
 * (recon FL-13).
 *
 * dprMin/subStep are the GOVERNOR's floor and step BELOW native — CANVAS.dprMin
 * is untouched, because that is the canvas's own boot arithmetic. At the
 * shipped values a devicePixelRatio-1 display gains two render-scale rungs
 * (0.875, 0.75) in front of the first tier rung, which is the one that rebuilds
 * the post chain and unmounts the high-tier layers.
 *
 * longFrameFrac: step down when more than this fraction of frames in a window
 * exceed longFrameK / targetFps. It is a FRACTION over a window, so one hitch
 * can never trigger it; it exists because the EMA drops outliers by design and
 * is therefore blind to exactly the pattern users call "not smooth".
 */
export const LADDER_FIX = {
  enabled: true, // R24 CLOSE: SHIPPED ON
  dprMin: 0.75,
  subStep: 0.125,
  // Plan §0 ruling 6 ("Governor targets native refresh"). NOTE, because it is
  // the one flipped value with no measurement behind it: on a 60 Hz display
  // this is an EXACT no-op (min(60, 60) === 60), so it changes nothing on the
  // machine the round assumed. On 120/144 Hz it raises the target and the
  // governor steps down more readily — and recon FL-04's own question to the
  // user ("native refresh, or is a 60 Hz cap preferred?") is still unanswered.
  nativeRefresh: true,
  longFrameFrac: 0.08,
  longFrameK: 1.5,
  longFrameWindow: 240,
...
/**
 * A: floating-origin rebase — drop the dead store bump, targeted matrix update,
 * quantised anchor (recon FL-09, T12).
 *
 * quantM: the anchor is snapped to a multiple of this. The low-AGL micro-detail
 * noise is hashed on REBASED world metres (world-bend.js: `vWorldXZ =
 ...
 * re-phases in one frame, every 10 km of travel. 704 m = HILLSHADE.micro.scaleM
 * (5.5) × 128 keeps the field continuous with no shader change and no new
 * uniform — C LIGHT declined a `uMicroAnchor` uniform and this is the route
 * that needs neither. The cost is that the anchor can sit up to 704 m from the
 * aircraft instead of exactly on it; nothing reads the anchor as "the player's
 * position" (terrain-engine's conversions are pure `_anchor` arithmetic), so a
 * quantised anchor is simply a different — and stable — origin.
 */
export const REBASE_CALM = { enabled: true, quantM: 704 }; // R24 CLOSE: SHIPPED ON

// ----- B WORLD — chunk pipeline, births/evicts, recompile storms ------------
/**
 * B: zero-area triangle filter at drape finalize, engine-side, no worker
 * change (recon WB-1, A1/A1b). THE ROUND'S HEADLINE FIX — the one-frame
 * near-full-screen pale flash. `@mapbox/vector-tile` closes every polygon ring
 * with a clone of its first point and every wall extruder walks the ring as if
 * it were OPEN, so the wrap-around edge emits two exactly-degenerate triangles
 * per ring AND per hole, on DoubleSide materials, under a per-vertex float32
 * bend. Measured on the R24 node fixture (scripts/r24-b-worker-proof.js):
 * 14.2% of a dense sat-building chunk's triangles, 100% of the coincident
 * kind; the skyline is clean (simplifyRing already drops the clone — A1b).
 *
 * minArea2 0 = EXACTLY-degenerate only. The archived R22.1 census found the
 * 0 < area² < 1e-6 bucket empty everywhere, so 0 removes the whole measured
 * population and can never touch a real triangle. Raising it is a taste
 * decision with a real risk of eating thin geometry — don't, without a census.
 *
 * Shading-neutral by construction (a degenerate's face normal is (0,0,0)), so
 * the filter runs BEFORE computeVertexNormals. Runtime pin
 * `window.__flyFlashPin = 'off'` disables it for the rest of the session
 * without a reload — that is how a harness measures RED and GREEN in ONE page.
 * One-line revert: enabled:false (index buffers are then passed through by
 * reference, byte-identical).
 */
// SHIPPED ON (R24 close). Certified at the SATELLITE BUILDING site — E's RED
// legs measured Powell 8.28% (2,616 of 31,576 tris) and Manhattan 13.98% worst
// chunk (5,820 of 42,364), all coincident-vertex, going to 0. The SKYLINE site
// is insurance only: simplifyRing already drops the closing clone there, so
// there is nothing for the filter to remove (scripts/r24-b-world.md §15.1).
// The TOY site is wired and proven at node level (14.84% → 0) but was NOT
// exercised by a browser leg this round — no toy chunk is resident in
// satellite, which is where both certification poses run.
// `window.__flyFlashPin = 'off'` remains the same-session RED leg.
export const FLASH_GUARD = { enabled: true, minArea2: 0, dropDegenerateTris: true, cloudNearFade: false };
/**
 * B: worker ring dedupe (drop the loadGeometry closing clone) — flag-off
 * byte-identical bundles (recon WB-1, A1b). FLASH_GUARD removes the degenerate
 * triangles on the main thread, which is the correctness fix and moves no
 * bundle bytes; this is the SOURCE fix, so they are never built, transferred or
 * draped either. Measured on the node fixture: sat-building verts −15.3%, toy
 * verts −15.7%, degenerate 0 at the source.
 *
 * IT ALSO FIXES A1b's SKYLINE DEFECT FOR FREE, and that is not a side effect —
 * it is the same bug. `simplifyRing` walks the ring cyclically and drops any
 * point whose two edges are collinear; for the closing clone `bc` is (0,0) and
 * for ring[0] `ab` is (0,0), so BOTH go and every z14 block-mass ring silently
 * loses its genuine first corner. With the clone gone ring[0] survives on its
 * own merits, with no change to simplifyRing at all. Measured: skyline verts
 * +33% on the fixture (a 4-corner footprint stops rendering as 3).
 *
 * BUNDLE BYTES MOVE, so verify-neon-cover's five frozen R18 hashes and
 * verify-seam's determinism hashes are re-baseline candidates under a
 * controlled A/B with a FIXTURE column — the flag-off/flag-on pair and the
 * kept-count identity are in scripts/r24-b-world.md §7. Cache API stores RAW
 * tile bytes, so nothing needs invalidating.
 *
 * minEdgeTile: a wall edge shorter than this (tile units at extent 4096 — 1e-3
 * is ~0.15 mm on a z14 tile) is skipped at the extruders. Dedupe already
 * removes the exact duplicates; this catches the clip-intersection case where
 * two points land a few float ulps apart, which is the SLIVER half of the
 * WB-1 hypothesis rather than the coincident half.
 */
// SHIPPED OFF (R24 close), BUILT AND MEASURED. It moves WORKER_PROTOCOL and
// verify-skyline's frozen numbers, and — measured on E's live-winding tiles —
// it moves ROOF-FORM DISPATCH, because restoring ring[0] changes each
// footprint's corner count and the roof bands test for an exact 4-corner ring
// (§7.2b: Powell toy +17.4%). FLASH_GUARD already removes the whole degenerate
// population from the GPU with zero bundle bytes moved, which is the cheap 95%
// of the win. The other half of this flag — the SKYLINE'S LOST FIRST CORNER,
// a genuinely different bug that no gate measures (§15.1) — is the reason to
// come back to it: next round, under the roof re-certification it needs.
export const RING_DEDUPE = { enabled: false, minEdgeTile: 1e-3 };
/**
 * B: per-mesh birth ramp + deferred fade-out on every streamed chunk (recon
 * WB-2, A6) — the user's "buildings appearing and disappearing" in its most
 * common form. No streamed chunk has a birth or death transition today: every
 * engine's finalize adds the finished Mesh the frame it completes and `_evict`
 * removes+disposes it the frame the desired set changes, so a city block web
 * appears fully opaque at the ring edge IN VIEW and vanishes behind. The only
 * dissolve that exists is ALTITUDE-keyed, driven by ONE module-shared uniform
 * (`uSatBldgFade`), which by construction cannot express a per-mesh ramp.
 *
 * A fading chunk therefore wears a POOLED TWIN of its engine's shared material
 * carrying its OWN fade uniform (three re-uploads a material's uniforms only
 * when the MATERIAL changes between draws). Same constructor parameters, same
 * map/emissiveMap state, same customProgramCacheKey ⇒ three's program cache
 * returns the SAME program and refcounts it: ZERO new cache keys and ZERO
 * compiles, which is `programs.length`-flat and E's proof obligation.
 * Steady state is ONE shared material per engine exactly as today — a mesh
 * wears a twin only while it is ramping.
 *
 * births NEVER change a count: a fading-in chunk is `ready` from the frame it
 * finalizes, so verify-sat-buildings / verify-roof-variety / verify-skyline
 * see exactly what they see today, and only pixels move for ~0.4 s.
 *
 * maxDying is the only knob that can add DRAWS (an evicted mesh keeps drawing
 * while it dims). It is deliberately small, and an empty scene has nothing to
 * fade — OWENS TAKES EXACTLY 0 EXTRA DRAWS BY CONSTRUCTION. A warp, a tier arm
 * (a twin's program key follows USE_MAP / USE_EMISSIVEMAP), a style flip and
 * dispose() all land every ramp immediately: a cut is never a crossfade.
 *
 * Runtime pin `window.__flyFadePin = 'off'` for a same-session RED leg.
 * DELIBERATELY NOT APPLIED TO sat-roads: verify-sat-night's frozen gate
 * asserts ONE material instance carries every road mesh and that
 * meshes === ready === visible, and turning that into a load-decided coin is
 * the exact R21 anti-pattern. See scripts/r24-b-world.md §3.
 */
export const CHUNK_FADE = {
  // SHIPPED ON (R24 close): single-frame pops 92 → 2 on a 2,700-frame headless
  // serpentine, and BOTH residuals are `fadeBudgetMiss` — i.e. attributable to
  // a spent budget, the documented degradation, not an unexplained remainder.
  enabled: true,
  birthSec: 0.4,
  evictSec: 0.3,
  maxConcurrent: 8, // simultaneous BIRTHS (twin materials only — no new draws)
  maxDying: 4, // simultaneous fade-OUTS — the only term that adds draws
  // Parcel homes: the provisional→confirmed grow was a DISCRETE step (R22's
  // RED: 1,874 Melton homes in one 100 ms sample at 100% of settled scale).
  // Eased in place by a uniform-scale matrix ratio — zero keys, zero draws,
  // and the buffer is only walked while an ease is actually running.
  parcelGrowSec: 0.6,
  // R24 B (W3) — THE FRAME-COUNT FLOOR. The ramps above are in SECONDS, which
  // is right on a machine that renders in milliseconds and useless on one that
  // does not: at the cloud venue's measured 2.84 s/frame a 0.3 s eviction ramp
  // begins and ends between two samples, so a fade that is genuinely running
  // is indistinguishable from no fade at all — and, worse, a HITCH on a real
  // machine does the same thing for one frame, which is exactly the moment a
  // pop is most visible. So progress is
  //     min(elapsed / sec, framesSince / minFrames)
  // and a ramp always spans at least `minFrames` frames however long they are.
  //
  // IT CHANGES NOTHING ON A NORMAL MACHINE, by construction: at 60 Hz the
  // 0.3 s eviction ramp is 18 frames and the 0.4 s birth ramp is 24, both far
  // above 4, so `elapsed / sec` is the smaller term and governs alone. The
  // floor only binds when a frame is longer than sec/minFrames (75 ms here) —
  // i.e. only when the seconds term would have skipped the fade.
  //
  // WHY 4 AND NOT 3. A BIRTH starts at presence 0, so it is already partial on
  // its first frame and N frames give N partial samples. A DEATH starts at
  // full presence by definition, so N frames give only N-1 partial samples.
  // Three partial samples on BOTH ramps therefore needs 4, and the death case
  // is the one an observer has to catch.
  minFrames: 4,
};
/**
 * B: heal = re-drape in place, never evict+refetch (recon WB-8, T8).
 *
 * THE BLINK. When a finer DEM tile lands under a streamed sat-building chunk,
 * the heal loop calls `_evict` and re-queues the tile — so the chunk is GONE
 * for the whole worker roundtrip + drape + finalize latency. R21 bounded how
 * MANY times that can happen per key (healCap 3); it never stopped a single
 * heal from being a HOLE. Each one also takes the chunk's collision columns and
 * house anchors with it, which is what replays the R21 parcel-home placement
 * race on every DEM refinement.
 *
 * With the per-run drape record retained at finalize (one entry per BUILDING:
 * its local anchor, its vertex span and the ground it was draped on), a heal
 * re-samples the DEM per building on the SAME budget as the first drape and
 * patches the RESIDENT position buffer with a RANGED upload — plus the
 * collision-column tops and the porch-light anchors, each by the same delta.
 * No refetch, no hole, no re-place, no worker traffic.
 *
 * minDeltaM: a run whose ground moved less than this is left alone (a ranged
 * upload over a metre of noise is pure churn). budgetMs is its OWN slice, kept
 * below SAT_BUILDINGS.drapeBudgetMs so a heal can never cost more than the
 * arrival it replaces. maxConcurrent bounds the sampling work per frame.
 *
 * Scope this round: the SATELLITE BUILDING ring only — the user's symptom, and
 * the only engine whose drape is per-anchor (roads and toy drape on bilinear
 * grids, which need a different patch and are documented as follow-ups in
 * scripts/r24-b-world.md §4).
 */
export const HEAL_IN_PLACE = {
  // SHIPPED ON (R24 close): heals 16 (0 patched in place) → 21 (21 patched in
  // place) on the same serpentine, with evictions falling 40 → 24 because a
  // heal is no longer an eviction at all.
  enabled: true,
  budgetMs: 0.6,
  minDeltaM: 0.25,
  maxConcurrent: 2,
  // R24 B (W3) — THE SAME LESSON AS CHUNK_FADE.minFrames. `budgetMs` is a
  // per-FRAME time budget, i.e. a frame-rate-dependent THROUGHPUT: at 60 Hz it
  // is 36 ms of sampling per second, at the venue's 0.35 fps it is 0.2 ms per
  // second. A job that cannot finish before its chunk is evicted under it never
  // finishes at all, and with maxConcurrent 2 those stalled slots then starve
  // every later heal into `healsQueueFull` — the exact shape the re-take
  // measured (heals 9 = queueFull 4 + aborted 4 + coalesced 1, in-place 0).
  // This floor guarantees FORWARD PROGRESS per frame however long the frame is;
  // at 60 Hz the ms budget is reached first and governs alone, so a normal
  // machine is unchanged.
  minRunsPerFrame: 64,
};
/**
 * B: constant PMREM height across sky buckets + two-state prewarm + re-warm on
 * a late HDRI (recon WB-4, A5, FL-10).
 *
 * THE RECOMPILE STORM. three folds `envMapCubeUVHeight` into the program cache
 * key of EVERY MeshStandard/Lambert/Phong material, and PMREMGenerator sizes
 * its output from `texture.image.width / 4`. The day sky ships as a 2k file,
 * the three twilight files are 1k and the blend scratch is 1k-class — so the
 * FIRST dusk blend step re-keys every lit material in the scene (tiles,
 * buildings, canopy, homes, traffic) and dawn's return to the raw day file
 * re-keys them all back. A multi-hundred-millisecond compile stall, landing
 * exactly when frames are already slow, that no harness can see because
 * `prewarm` runs once per session and the fleet pins the other half
 * (SAT_SHADOWS) off.
 *
 * equirectWidth is THE DAY FILE'S WIDTH on purpose: the day endpoint then
 * bypasses the resize entirely and noon stays bit-identical BY CONSTRUCTION.
 * Only the twilight endpoints are upsampled (through the existing blend pass
 * at mix 0 — no new shader, no new render path). The alternative, shipping the
 * day sky at 1k, is cheaper per bake and moves every certified NOON pixel;
 * this direction costs ~4x the bake area at twilight instead, which is work
 * FL-10 wants sliced off the frame loop anyway. Twilight pixels DO shift
 * slightly (an upsample before the PMREM blur), so verify-dusk /
 * verify-sat-night need a fixture A/B at the twilight poses before this flips
 * ON — a re-baseline CANDIDATE, not a taken one.
 *
 * warmBothShadowStates: the satellite shadow rig is gated on tier 'high', so a
 * governor step flips `shadows.length` 1↔0 and re-keys every lit material a
 * second way. The warm therefore seeds BOTH keys — but NOT by flipping the live
 * light. That obvious version is wrong twice over (adversarial review of r24/b,
 * two independent verifiers): it sat inside the boot gate, where it could
 * extend the reveal up to PREWARM.maxMs against a FROZEN rule; and
 * `frameloop="always"` keeps rendering across the awaited compile, so
 * production frames in that window re-key every lit material on
 * `shadowMapEnabled` and render a shadow pass at tiers that have none — and
 * with a slow HDRI (envWaitMs 4000 > maxMs 3000) that window can land AFTER
 * the reveal, where the user SEES shadows snap. The live scene is never
 * touched: the second pass compiles against a STAND-IN target holding CLONES
 * of the live lights with castShadow inverted, carrying the live `environment`
 * and `fog` (both are in three's program key — a stand-in missing either would
 * mint a THIRD program instead of the shadow twin), and it runs AFTER
 * `_state.done` on the same idle drain as the re-queue below. Gate:
 * scripts/r24-b-prewarm-proof.mjs, RED-calibrated on the defective revision.
 * requeueForEnvironment: PREWARM.envWaitMs caps the wait for the HDRI, so a
 * slow network makes the warm mint the NO-ENVIRONMENT permutation of every lit
 * material and the real ones compile after the reveal (A5 measured 179–714 ms
 * worst frames). A re-queue re-warms when `scene.environment` actually
 * arrives, drained at idleBudget compiles per IDLE frame — never in bulk, and
 * never inside the boot gate, so BOOT.maxBootMs and the reveal timing cannot
 * move.
 */
export const ENV_UNIFORM = {
  // SHIPPED OFF (R24 close). The stand-in-target two-state warm is BUILT and
  // its source contract is gated (scripts/r24-b-prewarm-proof.mjs, 9/9, RED on
  // the pre-fix revision), but its BROWSER proof — verify-env-uniform:
  // programsDelta flat across a dusk crossing AND a forced high↔medium step
  // with `__flyGovPin` AND `__flySatShadowOverride` released, plus a twilight
  // pixel A/B at the verify-dusk / verify-sat-night poses — HAS NOT BEEN RUN.
  // Noon is bit-identical by construction (day-width normalisation + endpoint
  // bypass); twilight is an upsample and is exactly what that A/B must judge.
  enabled: false,
  equirectWidth: 2048, // = the day HDRI's width; the day endpoint bypasses
  warmBothShadowStates: true,
  requeueForEnvironment: true,
  idleBudget: 1, // compileAsync calls per idle frame while draining the re-queue
  idleFrameMs: 12, // a frame slower than this is not idle; skip it
};
/**
 * B: bend-margin pad includes the lookahead lead (recon WB-6).
 *
 * THE RESIDUAL R21 LEFT. `bendMarginM(ringAliveR, halfDiag)` models the worst
 * vertex distance from the player as ring radius + tile half-diagonal, but
 * R21's own lookahead deliberately keeps chunks alive out to
 * (1 + STREAM_KEEPER.lookahead.maxLeadFrac) · ringR from the player because
 * the desired set is centred on the LEAD point, not on the aircraft. The drop
 * the shader applies is quadratic in that distance, so a 35% radius excess is
 * ~82% more drop than the sphere was padded for — and the result is a chunk
 * that is ON SCREEN being frustum-culled while the camera turns AT SPEED.
 * R21's false-cull census could not see it: it ran on the ORBIT phase, at
 * speed 0, where the lead is exactly 0 (FLY_ROUND21.md:112-113).
 *
 * That is the most literal form of the user's "buildings appearing and
 * disappearing" report, so it is fixed at all four engines with one line each:
 * the pad is computed from `ringAliveR * (1 + maxLeadFrac)`. Culling stays
 * effective (the pad grows ~55%, a handful of ring-edge draws), no cache keys,
 * no worker change, and the margin can still only ever KEEP geometry.
 *
 * poolLeadM: the pooled instanced layers (canopy, parcel homes, landcover
 * tint) pad with maxD measured at the LAST placement pass, which is up to one
 * 2 s cadence stale — at the fleet's fastest airframe (750 m/s boost) the
 * player can be 1,500 m further from those instances before the next pass.
 * Stated here rather than read from FLIGHT.bars (a hangar-UI normalizer that
 * happens to hold the same number) so the coupling is deliberate.
 */
// SHIPPED ON (R24 close): the bend pad was short on 7 of 7 rings (deficit 54 /
// 558 / 747 / 241 / 1,205 / 3,405 / 31,412 m at the lead edge) and padON >=
// padOFF everywhere, so the flag can only ever KEEP geometry, never drop it.
export const BEND_LEAD = { enabled: true, poolLeadM: 1500 };
/**
 * B: slew-limited visual ground elevation; flight/crash keep RAW (recon A6,
 * T8). `flight.groundElev` is a raw raycast sampled every 3rd frame and given
 * to everyone at once, so when a finer DEM tile lands under the aircraft it
 * STEPS (R22 measured ~384 m in one frame) and every AGL-keyed band in the
 * world steps with it — the bend datum, the building / skyline / road fades,
 * the POI letter horizon, the micro-detail and quilt ramps, the altitude
 * atmosphere. `runtime.groundElevVis` is the damped twin those consumers read;
 * the flight model, the crash floor, the ground shadow, the cameras and every
 * placement/drape sample keep the RAW value. A visual ramp may lag reality by
 * a metre; a collision floor may not.
 *
 * maxStepM is per FRAME, not per second, on purpose: the quantity it protects
 * is a per-frame visual delta, and a frame-rate-independent rate would let a
 * hitch deliver the whole step at once — precisely the event being damped.
 * A warp SNAPS (a cut is not a ramp), and so does the first sample.
 */
// SHIPPED ON (R24 close): worst single-frame visual step 384.0 m → 4.000 m,
// still converging to the raw value in 95 frames, snapping on warpEpoch, with
// the flight model and crash floor provably still reading the RAW value.
export const GROUND_VIS = { enabled: true, maxStepM: 4 };

// ----- C LIGHT — one sun, correct color space, post order, shadows, terrain --
/** C: haze/fade/aerial targets decoded to linear at the setters (recon L1). */
export const LINEAR_HAZE = { enabled: true };
/**
 * C: directional + hillshade + dome lobe + water specular all from runtime.sun
 * at every tier (recon L3). Satellite only — toy has no live `runtime.sun`
 * (the R17 rule), so MOODS.toy.lightDir (TOY.moonDirection) is untouched and
 * every toy program, uniform and pixel is byte-identical with this on.
 */
export const ONE_SUN = {
  enabled: true,
  /**
   * INSTRUMENT PRECISION, because it was mistaken for a lighting result once.
   * `__flyStats.sun`'s key/hill/dome/water carry NINE decimals per component:
   * a unit component rounded to 1e-6 moves the azimuth derived from it by up to
   * 4.05e-5 / cos(el) DEGREES, which is larger than the 1e-6 deg agreement the
   * audit exists to show — so a gate differencing two SIX-decimal vectors reads
   * its own rounding as a disagreement. The rig itself carries no float32 on
   * that path (Object3D.position and the hillshade dir are float64 JS numbers;
   * only the GPU copy is float32, and nothing reads that back), and the two
   * azimuths are bit-identical as doubles at every leg.
   */
  /**
   * MOON KEY (recon L3 fix 5). Below the horizon the R21 rig kept the key at
   * the SOLAR azimuth with `el` floored at SAT_SHADOWS.minElRad (8.6°) and
   * merely re-COLOURED it — so the night key came from the side of the sky
   * the sun set on, while SkyDome draws the moon disc ANTI-solar
   * (moonDirFromSun). ONE_SUN blends the DIRECTION across this band, in TRUE
   * elevation degrees (trueElevationDeg(sinEl) — `runtime.sun.el` is CLAMPED
   * into the hillshade band and can never express a negative elevation; the
   * R19 lesson). Colour and intensity are deliberately NOT touched:
   * SKY.hdriCycle.keyColor.night is already moonlight-cool and verify-sun's
   * noon/midnight intensity contract then holds by construction.
   */
  moon: { fadeStartDeg: 0, fadeFullDeg: -8 },
  /**
   * HILLSHADE WEIGHT ON TRUE SUN ELEVATION (recon L3 fix 4 / L8 fix c). Esri
   * imagery already carries its capture-date sun, so a full-strength second
   * directional term at midday is a literal second sun. Weight is dayK at and
   * above dayDeg, 1.0 at and below lowDeg (where the baked shading is wrong
   * anyway and the live term should carry the relief), smoothstepped between.
   * dayK is a USER-CHECKPOINT knob: it is the one value here that trades a
   * frozen gate margin (verify-sat-depth's hillshade A/B > 2/255) for honesty.
   */
  // SHIPS AT 1.0, i.e. the demotion is OFF. The value that would have demoted
  // the daytime hillshade is a USER-CHECKPOINT knob whose only justification
  // was going to be a measurement, and that measurement never landed: the
  // Sierra A/B is unrun (its v1 result was a drift signature, see
  // scripts/r24-c-light.md). Shipping 0.65 would spend up to 35% of
  // verify-sat-depth's hillshade A/B margin on an argument. At 1.0 the weight
  // is exactly 1 at every elevation, so `uHillElev` is the identity, the tile
  // fragment's `uHillStrength * uHillElev` is bit-identical to R21's
  // `uHillStrength`, and that frozen margin does not move at all. The term is
  // BUILT and one edit away the moment there is a number.
  hill: { dayDeg: 40, dayK: 1.0, lowDeg: 15 },
  /**
   * Satellite monuments MeshToon -> MeshLambert vertexColors: SAME draw count
   * and the same 3-step value palette, but Lambert is one of the three
   * materials three r185 hands `scene.environment` to (WebGLPrograms.js:60-63),
   * which is what closes the R20 §5b.4 Taj night residual — a Toon monument
   * takes no envMap, so the raw cool night key was its ONLY light and the
   * marquee read +25.0 blue-minus-red against a +18.4 baseline. Toy monuments
   * keep their toon ramp (they are the toy look).
   */
  monumentsLambert: true,
};
/**
 * C: bloom → speed → aerial → DoF → ACES → grade → vignette → SMAA last,
 * with an output dither on the final pass (recon L6).
 *
 * Expressed as a PERMUTATION of R21's own descriptor list in
 * `buildPassList` — never a second list — so the el()/raw() twins that the
 * pre-warm depends on cannot drift. Merged EffectPass count can only FALL
 * (satellite high 4→3, toy high 6→5): the tone descriptor stops being a
 * trailing pass of its own and merges into the grade group. Proven by
 * `scripts/r24-c-post-order-proof.mjs`, which reads the descriptor order out
 * of Effects.jsx itself.
 *
 * NO GRADE CONSTANT MOVES. The charter budgeted a post-curve re-tune of
 * SKY.grade / TOY.saturation / TOY.contrast; a least-squares fit over the
 * midtones (0.004–0.6 linear) says the best post-curve satellite contrast is
 * 0.037 against the shipped 0.05, and that leaving the knob alone costs
 * 1.33/255 rms versus 1.10/255 for the re-fit. 0.23/255 is not worth moving a
 * constant this agent does not own.
 *
 * smaaPreset: 'low' | 'medium' | 'high' | 'ultra' (SMAAPreset key). HIGH plus
 * LUMA edge detection is only defensible AFTER the reorder — on the R21 chain
 * the detector was reading unbounded scene-linear values.
 * dither: three's <dithering_fragment> on the LAST EffectPass — a PER-PIXEL
 * gl_FragCoord hash at ±0.5 LSB applied after the color-space encode, never an
 * ordered/Bayer matrix (an ordered matrix is a fixed spatial texture that can
 * itself crawl at reduced DPR — recon A7). Applied through
 * `lib/fly/post-policy.js` so the live composer and the pre-warm set the same
 * DITHERING define and therefore compile the same program.
 */
export const POST_ORDER = { enabled: true, smaaPreset: 'high', dither: true };
/**
 * C: reversed-depth double-conversion fix for postprocessing's depth readers
 * (recon L2, FL-07). three r185 injects USE_REVERSED_DEPTH_BUFFER into every
 * non-raw ShaderMaterial AND switches perspectiveDepthToViewZ to the
 * reversed-input formula; postprocessing 6.39.2 ALSO un-reverses in its own
 * readDepth. Two conversions where the formula wanted zero, so every fragment
 * reconstructs at ~-2.5 m (= -cameraNear) and toy-high DoF is a flat ~0.177
 * blur over the whole frame instead of a 700 m sharp band. Numerically proven
 * in `scripts/r24-c-depth-roundtrip-proof.mjs` (GREEN error 0.000000 m from
 * 5 m to 300 km). Also fixes AerialPerspective's sky early-out, which the same
 * library line had made unreachable.
 */
export const DEPTH_FIX = { enabled: true };
/**
 * C: shadow catcher gate, stable PCF kernel, texel snap, sane normalBias
 * (recon L5, FL-12) — plus the reversed-depth BIAS SIGN three r185 flips in
 * its VSM and BASIC shadow branches and NOT in the PCF branch this app runs
 * (see lib/fly/shadow-kernel.js for the derivation; it is the R21 P8
 * polygonOffset defect one layer down, and it is what `normalBias: 4` was
 * silently paying for).
 */
export const SHADOW_CALM = {
  enabled: true,
  // ShaderChunk.shadowmap_pars_fragment edits (installed once, pre-compile, so
  // no material, cache key or prewarm entry moves — three's CSM.js was refused
  // for exactly the opposite property).
  biasSignFix: true,
  // ALLOW-LISTED in shadow-kernel.js: 'world' = hash the shadow texel
  // (camera-motion invariant) | 'fixed' = phi 0 (deterministic, mild banding).
  // ANY other value — 'three', a typo, undefined — leaves three's screen-space
  // rotation alone. That is deliberate: verify-shadow-calm caught the first
  // version defaulting an unrecognised name to 'world', which would have
  // shipped a kernel nobody asked for from a one-character mistake.
  kernel: 'world',
  texelSnap: true, // quantise light + target to 2*orthoRadius/mapSize in LIGHT space
  // With the sign fixed, the peter-panning offset can come down. 4 m on the
  // toy rig's 0.78 m/texel map is 5.1 texels of visible float.
  toyNormalBias: 1,
  satNormalBias: 1,
  /**
   * The ground catcher (SAT_SHADOWS.catcher, BUILT and shipped OFF in R19).
   * Armed only where a caster actually exists and only below `aglM`, so it is
   * +1 draw in cities and EXACTLY 0 draws over Owens BY CONSTRUCTION —
   * `queryColumns` answers with an empty array when nothing has streamed.
   */
  catcher: { aglM: 1500, casterRangeM: 1200, everyNFrames: 10 },
  /**
   * Shadow-map update cadence for the satellite rig. CONSIDERED AND LEFT AT 0
   * (= every frame): the ortho follows the AIRCRAFT, so skipping updates
   * strands the shadow map behind the world it is shadowing and trades a
   * measured-nowhere GPU saving for a visible lag. Re-open only with a number
   * from the user's machine.
   */
  satCadence: 0,
};
/**
 * C: fragment-stage hillshade, smooth DEM normals, error-bounded skirts,
 * fwidth-attenuated micro grain (recon L8, T6, T7, T13).
 *
 * TWO HALVES with different venues. The TILE half (`fragmentHill`,
 * `microFwidth`) is shader-only, lands here, and E's fixture can A/B it. The
 * WORKER half (`workerNormals`) has to live inside three-tile's LERC worker,
 * which only runs on the Esri LERC path — 403 in this container, and the
 * fixture's terrain-rgb loader builds geometry on the MAIN thread — so it is
 * node-proven and honestly "could not measure here" for pixels.
 */
export const TERRAIN_LIGHT = {
  enabled: true,
  /**
   * Move the hillshade's N·L out of the vertex stage. R21 computes `vHill` in
   * `<defaultnormal_vertex>` and Gouraud-interpolates it, and three-tile's
   * Martini mesh carries LAST-WRITER face normals (recon T6: `t[n] = t[s] =
   * t[o] = g * w`, every shared vertex taking one arbitrary triangle's normal),
   * so the relief FACETS by construction and the facets are visible wherever
   * Martini leaves long slivers. Interpolating the NORMAL and taking the dot
   * per fragment removes the faceting for ~4 ALU. It does NOT fix the normals
   * themselves — that is `workerNormals` — but it is the half that does not
   * need a worker, and a smooth normal field interpolated per fragment is what
   * makes the worker half worth having.
   */
  fragmentHill: true,
  /**
   * T13: fade the 5.5 m luma grain out where a noise cell is under ~2 px.
   * SMAA is a geometric-edge filter and cannot touch sub-pixel procedural
   * frequency, so at grazing angles the grain aliases into exactly the crawling
   * "tearing" the user reported. `fwidth` of the noise coordinate is cells per
   * pixel; above `cellPx` the term is faded to nothing.
   */
  microFwidth: true,
  microFadeLo: 0.35, // cells/px where the fade starts
  microFadeHi: 0.9, // …and where the grain is fully gone
  /** The worker half. See the block header for why it cannot be measured here. */
  workerNormals: false,
};
/**
 * C: lit cloud sprites on drei's EXISTING instancer (recon L7) — fake sphere
 * normal from the sprite quad, sun side / shadow side, a Henyey-Greenstein
 * back-scatter rim toward `runtime.sun`, moon tint after dark, overcast grey
 * from the weather scalars. SAME draw count, ~+20 ALU on the ~5-10% of pixels
 * a deck covers, exact early-out at mix 0. Satellite only: the toy deck is
 * MeshBasic (`style.lit` false) and never reaches the lit class.
 */
export const CLOUD_LIT = {
  enabled: true,
  mix: 1, // master shaping weight; 0 = plain Lambert, bit-identical
  // Henyey-Greenstein asymmetry. Real water droplets are strongly forward
  // scattering (g ~ 0.85 in Mie theory); 0.6 is a deliberate compromise —
  // higher g concentrates the silver lining into a few degrees around the sun,
  // which on a 54-puff deck reads as a handful of sprites flaring rather than
  // as a rim. A user-checkpoint knob.
  g: 0.6,
  rim: 0.55, // back-scatter gain
  sun: '#fff6e2', // lit side (warm daylight)
  shade: '#8fa3bf', // shadow side (sky-bounced blue, never black)
  moon: '#c8d6ee', // lit side after dark
  moonShade: '#3d4a60',
  // Below this TRUE sun elevation the key is the moon (the ONE_SUN band).
  moonElDeg: -2,
  overcastGrey: '#b9c0c8', // weather greys BOTH sides toward this
};
/**
 * C: Lambert env multiply tamed (recon WB-7). three r185 applies
 * `scene.environment` to Lambert/Phong (WebGLPrograms.js:60-63) and Lambert's
 * defaults are `combine = MultiplyOperation`, `reflectivity = 1` — a
 * FULL-STRENGTH mirror-reflection lookup of the HDRI on every wall, roof,
 * canopy and house. Roofs take the zenith colour, facades take the horizon
 * band, and the twilight HDRI's bright azimuth band can light one facade
 * direction at night. Uniform-only (no shader text, no key move): this is a
 * material PARAMETER, so the program is unchanged and only the ENVMAP
 * intensity term moves.
 */
export const LAMBERT_ENV = { enabled: true, reflectivity: 0.15 };

// ----- D ATMOS — one atmosphere law, LOD crossfade, sky (Level B, measured) --
/**
 * D: single analytic aerial-perspective law shared by the post pass and the
 * per-material term (recon L4 + A8). The law itself lives in
 * `lib/fly/atmo-law.js` (ONE GLSL string + ONE JS mirror, proven equal by
 * `scripts/verify-atmo-law.mjs`); these are only its coefficients.
 *
 * WHY THESE NUMBERS: `beta.base` is calibrated so that at LOW flight a ground
 * fragment 14 km out keeps transmittance 0.45 — i.e. it lands on exactly the
 * mix the R19 post pass produced at the END of its 800-14,000 m band, so the
 * near/mid field a user has already signed off does not move. Everything the
 * old stack could not do falls out of the same coefficient: the 14-16 km
 * plateau disappears (one continuous function), the 16-55 km tile band gains a
 * height term, the 60-120 km rim melt becomes extinction → 1 (T(120 km) <
 * 0.02), and from cruise the SAME law thins out on its own because the
 * exponential-atmosphere integral knows the eye is above most of the air
 * (eye 9 km, ground at 14 km: mix 0.14, not the old altitude-blind 0.55).
 *
 * SCOPE IS SATELLITE. Toy/Neon keeps its certified R8 haze (TOY.haze, a single
 * 4-13 km law that is NOT the defect L4 names — every L4 symptom is a
 * satellite one) and writes `strength` 0, which is the IEEE-exact identity
 * path, so the Neon palette is pixel-identical with the flag ON.
 */
export const AERIAL_LAW = {
  // R24 CLOSE — SHIP STATE: the LAW ships OFF; `nightRamp` (A8) ships ON.
  //
  // The law's evidence is structural and complete — verify-atmo-law parses the
  // shipping GLSL and proves it equals the JS mirror at 4,160 points, flag-off
  // is byte-identical in generated text / uniforms / key, and the two
  // evaluators provably never both run. What does NOT exist is a single pixel
  // of it: no fixture A/B was captured at any canonical pose, and every horizon
  // pixel gate moves by construction. Turning it on would be shipping a look
  // nobody has seen, so it waits for the horizon re-baseline batch to run WITH
  // a fixture column (plan §0 ruling 4, one batch, C's to execute).
  enabled: false,
  styles: { satellite: true, toy: false },
  // Per-channel extinction at the datum, 1/m: base × tilt. The tilt is a MILD
  // Rayleigh lean (real near-ground aerial perspective is Mie-dominated and
  // close to grey; a full 1/λ⁴ lean reads as a blue filter, not as distance).
  beta: { base: 0.0000645, tilt: [0.84, 1.0, 1.225] },
  scaleHM: 1500, // e-fold height of the density profile
  strength: 1, // master multiplier; 0 is the exact-identity kill switch
  // Mie forward lobe toward runtime.sun (the term A8 says the post pass never
  // had). k = the peak fraction of the way from the rim colour to `sunTint`;
  // g = Henyey-Greenstein asymmetry (0.76 = a broad haze lobe).
  mie: { k: 0.35, g: 0.76 },
  sunTint: '#ffd09a', // raw sRGB; atmo-law decodes it to linear at the setter
  // A8: retire the post pass's fixed 0.55 at night. Multiplier =
  // 1 - clamp01(1 - frac/dayFrac)^gamma — the EXACT SAT_BUILDINGS.night ramp
  // shape, so the atmosphere fades out on the same curve the city lights come
  // up on. Exactly 1 at frac >= dayFrac (noon keeps 0.55 bit-for-bit) and
  // exactly 0 at frac 0 (deep night reads 0).
  //
  // R24 CLOSE: this ships ON with `enabled` still FALSE, and it can, because
  // the ramp is gated on THIS field alone — FlyScene reads
  // `if (AERIAL_LAW.nightRamp)` independently of `lawOn`, and applies the
  // multiplier to the LEGACY post strength on the `lawOn === false` branch. So
  // with the law off, A8 is exactly what recon A8 asked for: a CPU multiply on
  // an existing uniform, no shader text, no cache key, no program.
  //
  // It moves no frozen number. verify-aerial's `aerial strength resolves to
  // maxMix` assertion (0.55 ± 0.001) is taken at a NOON pose, where
  // `frac >= dayFrac` makes the multiplier EXACTLY 1 — that identity is the
  // property the ramp was shaped around, not a coincidence. Below the band the
  // legacy shader's own `uMaxMix <= 0.0` early-out returns the input
  // unmodified, so deep night is bit-identical to no pass at all rather than
  // merely dark.
  nightRamp: true,
  night: { dayFrac: 0.3, gamma: 1.5 },
  // The in-shader CONTENT slot (sat buildings + skyline) evaluates the law
  // instead of AERIAL_PERSPECTIVE.content's separate smoothstep — which is
  // what finally lets it ship at medium/low without double-hazing high.
  content: true,
  // The air/anchor variants (traffic, monuments, veg, parcel homes, town glow,
  // road network, tracers). Additive materials get extinction only
  // (`atmoExtinct`): inscatter for a layer that ADDS light is already in the
  // background behind it.
  airAnchor: true,
};
/**
 * D: parent→child tile crossfade so LOD refines never hard-pop (recon T4).
 *
 * The user's own report (2026-09-06): "terrain tiles swapping for other ones".
 * three-tile has no transition state — `_loadSubTiles` awaits four children
 * then `add(...children); parent.unloadModel()` in ONE synchronous block, and
 * `_removeSubTiles` merges the same way, so every refine and every merge is a
 * single-frame cut in which BOTH the texture sharpness and the Martini relief
 * (91 m error at z13 vs 11 m at z15) change at once.
 *
 * `parentBlend` is the cheap fix: the children are added holding the PARENT's
 * texture, sampled through a clip-UV transform (each child covers one quadrant
 * of the parent's [0,1] map), and cross-dissolve to their own map over
 * `fadeSec`. Zero extra draws, one extra sampler on the tile program. The
 * alternative — keeping the parent drawn under the children and dithering it
 * out — costs a transient draw per in-flight quad AND (archived R22.1 B3) an
 * ordered dither under SMAA-only AA reads as shimmer, which is the class of
 * artifact this round exists to remove.
 *
 * Boot is deliberately fade-free (`skipBootMs`): the reveal timing is frozen,
 * and a world that assembles itself out of blurry parents is a different
 * (worse) first impression than the certified hold-until-sharp mask.
 */
export const LOD_CROSSFADE = {
  // R24 CLOSE — SHIP STATE: ON, on a measured standalone row.
  //
  // RED (flag-off arm, same run): a 360° pure yaw at Powell over 424 rendered
  // frames produced 72 refines, ALL 72 of them single-frame hard swaps, 0
  // blended, `skip.disabled` 72 — every one un-faded because the flag is off
  // rather than because a guard fired. That is the user's reported "terrain
  // tiles swapping for other ones", counted.
  //
  // GREEN (flag-on arm, same pose, same arc, one browser, TERRA_PACE shipped
  // on both legs): 72 refines again — ratio 1.00 over 434 → 433 frames, so the
  // crossfade did not move the streamer — of which 69 blended and 3 did not.
  // Those 3 are exactly the 3 `noParentMap` denials: every swap the ladder
  // COULD blend, it blended, and it declined only where the parent had no
  // imagery to blend from (vendored index.js:1236 hands back an
  // `_errorMaterial` clone after a transient miss — the world, not the
  // ladder). Blend set live on 246 of 433 frames, longest run 19 (a complete
  // blend is ≥ 5 clamped frames), peak 20 concurrent against a cap of 32 with
  // zero denials, every blend drained to `active` 0 / `retained` 0 at the
  // drain and again 12 frames later — no parent-texture leak — and zero page
  // errors on either arm.
  //
  // WHAT IS NOT PROVEN, so it is written here rather than left to be assumed:
  // (a) the MERGE ramp is unexercised — `merges` is 0 at every pose measured
  //     because `keepResident` removed the frustum-exit merges by design, so
  //     the merge direction is structurally gated (hunk placement, the
  //     `_loadState` hold across the await, flag-off byte identity) and
  //     inferred from shared machinery, never measured;
  // (b) the Owens arm-equality could not be read — both arms settled at maxZ
  //     17 but with 405 vs 473 tiles, so the columns describe two different
  //     scenes. Read for PER-TILE cost instead they are decisive: draws/tile
  //     FELL 0.652 → 0.603 and the delta is 0.31 draws per extra tile, where a
  //     per-tile added draw would need +473; and the ON arm's co-display run
  //     is 0, i.e. no parent mesh was ever retained beside its children, which
  //     is the transient second draw the dithered alternative would have cost.
  // (c) every ms, and whether 250 ms reads as smooth at 60 or 144 Hz, is the
  //     user's machine (§9).
  //
  // FLIP CRITERIA (scripts/r24-d-atmos.md §4.10 is authoritative; an earlier
  // draft said "hardSwaps flat at 20" and that was WRONG — `hardSwaps` is not
  // an event count, it is the UN-BLENDED SHARE of the events, and the 20 came
  // from a different pose). ON vs OFF on the SAME pose, sweep and TERRA_PACE:
  //   invariant  refines + merges === hardSwaps + faded, both legs
  //   1  refines + merges FLAT OFF->ON (the feature must not change how often
  //      the LOD refines, only whether the swap is blended) — and this is a
  //      COMPARISON, so it is only valid when both arms got the same pre-sweep
  //      settle and rendered comparable frame counts; otherwise NOT CALIBRATED,
  //      not red (plan §4: a load-decided instrument gets a control, never a
  //      new bound)
  //   2  hardSwaps DROPS toward 0; faded RISES
  //   3  active === 0 AND retained === 0, reached by POLLING until active === 0
  //      with a cap of 90 rendered frames and the count REPORTED — NOT a fixed
  //      frame count. A drain must cover
  //      fadeSec of FADE CLOCK, which advances min(delta, 0.05) per rendered
  //      frame: 250 fade-ms is 5 frames at this venue's ~50 ms frames but 15
  //      at 60 fps, so any fixed count is a venue constant and fails on a
  //      healthy GPU. (retained > 0 at rest is a texture leak — the best catch
  //      here.) Same arithmetic governs the warp settle: 900 fade-ms is 18
  //      frames here, 54 at 60 fps, ~130 at 144 Hz — poll until skip.warp is
  //      unchanged across two consecutive reads instead of counting.
  //   4  0 < peakActive <= 32
  //   5  skip.concurrency read WITH peakActive: > 0 at peakActive 32 is the
  //      bound working (report); > 0 below 32 is an accounting bug (fail)
  //   6  skip.shape / noParentMap / unpatched all 0
  //   7  Owens EQUAL to the OFF leg's fixture draws/tris, not merely <= 261
  //   8  zero pageerrors
  // Pin the leg { enabled: true, skipBootMs: 0 } with a >= 20 rendered-frame
  // post-warp settle: skipBootMs is 6000 FADE-CLOCK ms and the clock advances
  // <= 50 ms per RENDERED frame, i.e. 120 frames, which a 40 s sweep at 1-3
  // fps cannot reach; unpinned, the leg reports "boot never expired" as a red.
  // If refines + merges is 0 in the window the leg must print NOT CALIBRATED.
  enabled: true,
  // The blend window, in SIMULATION seconds. FlyScene's -50 block computes
  // `const dt = Math.min(delta, 0.05)`, and the fade clock rides that dt — so
  // this is 0.25 s of UN-HITCHED frame time, not of wall time. At 60 fps it is
  // 250 ms wall and 15 frames. Through a hitch it stretches in wall time but
  // never drops below 5 RENDERED frames, which is the property that matters: a
  // crossfade wants a frame count, and a "250 ms" fade that renders twice is a
  // two-step pop, i.e. the defect. MEASURED consequence in the other
  // direction: on SwiftShader at ~1 fps the clock advances 50 ms per frame, so
  // a probe pin of `fadeSec: 6` is 120 frames — two minutes per blend, which
  // is why the first fixture ON leg read `active` stuck at the concurrency cap.
  fadeSec: 0.25, // ≤ 300 ms per the charter
  mode: 'parentBlend', // 'parentBlend' | 'off'
  onRefine: true,
  onMerge: true,
  skipBootMs: 6000, // no fades before this age (boot reveal timing is frozen)
  skipOnWarp: true, // a warp is a CUT; WARP.flashMs 250 already masks it
  // In-flight fades, counted in MATERIALS (a refine arms four). The bound is
  // NOT about draws or samplers — the sampler is on the tile program either
  // way and an idle fade skips its branch. It bounds RETAINED PARENT TEXTURES:
  // a refine keeps its parent's map alive past `unloadModel()` until the last
  // child finishes, so 32 materials = 8 concurrent refines = 8 retained tile
  // textures, ~2.7 MB against the 300 MB texture gate. MEASURED consequence of
  // getting this wrong: at 12 the fixture ladder denied 9 of 19 refines a fade
  // with `skip.concurrency`, because 12 materials is only THREE refines.
  maxConcurrent: 32,
};
/** D: procedural analytic sky in the dome — built-but-off pending user checkpoint (recon L10). */
// R24 CLOSE — SHIP STATE: OFF, and NOT BUILT. The design is in
// scripts/r24-d-atmos.md §4.5 so a later round does not re-derive it. It is
// the only D item with no measured symptom behind it (recon L10 is a [low]
// hypothesis), and its cost — sky pixels are ~30-50% of the frame at altitude
// — is a GPU number this venue cannot produce.
export const SKY_PROCEDURAL = { enabled: false };

// ----- E CERT — instruments (fixture is harness-only, see plan §1) -----------
/** E: in-app frame ring buffer (dt, long frames/min, worst, longtasks) at __flyStats.frame (recon HARN-GAP-4). */
export const FRAME_STATS = {
  // R24 CLOSE — SHIPPED ON. This is an INSTRUMENT, not a feature: it adds no
  // draw, no triangle, no shader program and no material state, so it cannot
  // move a frozen pixel, draw or triangle number. What it costs is one
  // subtraction, one ring store, ~6 comparisons and two counter increments per
  // frame (~0.01 ms), plus one 600-element sort amortised over 30 frames.
  //
  // It ships ON because the alternative is shipping blind: the round's own
  // certification could not answer "is it smoother" — the pacing legs of
  // verify-frame-pace read "instrument absent — unmeasurable, not a renderer
  // failure" all round — and neither could the user, whose report of stutter
  // and tearing had no number attached to it in three previous rounds. With it
  // on, `copy(__flyStats.frame.sample())` in the console is the whole
  // diagnosis, and soak-fly and verify-stability report frame pace beside
  // every other number they already print.
  //
  // Turning it back off is a one-line revert and restores byte identity: the
  // rig is simply not mounted (FlyCanvas guards it), no ring is allocated, no
  // PerformanceObserver is registered, and window.__flyStats.frame never
  // exists.
  enabled: true,
  ringSize: 600,
  // Percentiles are recomputed every N frames (and on demand via
  // `__flyStats.frame.sample()`), never per frame — one 600-element sort
  // amortised over 30 frames is far below the 0.01 ms/frame budget, and a
  // gate that needs a fresh number asks for one instead of waiting.
  publishEveryFrames: 30,
};

// The earlier immersion lineage supplies these additional subsystem controls.
// ============================================================================
 ...
 *
 * R22 A, MEASURED CORRECTION to the `sharp` term. The scaffolding defined it
 * as `camTileZ >= min(targetZ - 1, sharpZCap) && settled`. That can NEVER be
 * true at cruise: three-tile refuses to subdivide an out-of-frustum tile, and
 * at FL300 the ground directly below the aeroplane is off screen, so camTileZ
 * saturates at ~10 with the loader completely idle while targetZ (a pure
 * function of AGL) reads 13 (scripts/r22-a-dublin-*.json). A target-only
 * `sharp` would therefore have made B's content gate degrade to the time cap
 * on exactly the arrival it exists for. SHIPPED: `sharp` is true when the
 * descent is settled AND (it reached the target OR camTileZ has stopped
 * improving for `settle.stallMs`) — "as sharp as it is going to get here".
 * The additive `sharpReason` field ('target' | 'stalled' | null) says which
 * term fired, so a stalled-but-shallow arrival stays legible to gates.
 *
 * PUBLISH PATH: the engine writes the object to `engine.terraStats` always,
 * to `window.__flyStats.terra` always, and to `runtime.terraStats` when it has
 * the runtime object. In dev it finds it through `window.__fly`; in PRODUCTION
 * it needs `engine.attachRuntime(runtime)` — one line beside
 * `runtime.engine = engine` in FlyScene's publish effect (A does not own that
 * file; Fable arbitrates at merge). Until that line exists, consumers should
 * read `runtime.terraStats ?? runtime.engine?.terraStats`, which is correct in
 * every build.
 *
...
export const TERRA_SHARP = {
  enabled: true, // R22 W3 FLIP (sanction 5.5 consumed: z18 high + demMaxZoom 16, probe-backed; Owens 235<=261 at the shipped lodCurve)
  // R22 PROBE (2026-08-07, scripts/r22-a-esri-probe.json — the §5.5 condition):
  // Esri World_Imagery serves REAL z18 jpegs (10–27 KB) at all six R22 poses,
  // and Terrain3D serves REAL z16 LERC (20–68 KB) at all six while z17 is a
  // 67-byte degenerate constant-surface tile. So 18/16 are both genuine data
  // ceilings, not stretched parents, and 17 is where the DEM actually ends.
  maxZoomHigh: 18, // consumed at engine mount; falls back to TILES.satMaxZoomByTier when off
  demMaxZoom: 16, // probe-confirmed above; high tier only, via demMaxZoomFor()
  lodCurve: [
    // [aglM, threshold] — interpolated, clamped at the ends. Toy style never
    // reads this (the curve is armed for satellite + HIGH tier only — see
    // TerrainEngine._lodCurveArmed; toy keeps three-tile's default 1, frozen).
    //
    // R22 MEASURED at the verify-sat-depth Owens pose (AGL 65–121 m, high
    // tier, z18 + demMaxZoom 16 + the error table all armed, re-warping to the
    // identical pose before every row — scripts/r22-a-owens-sharp.json):
    //   threshold 1.00 …… 291 draws   ← BREAKS the 261 gate (R19 saw 270 at z17)
    //   threshold 0.92 …… 280 draws   ← breaks it too
    //   threshold 0.86 …… 235 draws   ← SHIPPED low-AGL value (control: 215)
    //   threshold 0.82 …… 217 draws
    //   threshold 0.78 …… 189 draws
    // So the plan's proposed 1.0 at low AGL was not affordable, and it was not
    // needed: the low-AGL sharpness win comes from the z18 LEVEL (texels), not
    // from a looser threshold (geometry). Holding 0.86 — the R19 shipped value
    // — keeps the low-AGL end byte-comparable to R21 while z18 doubles the
    // texel resolution under the aeroplane (P-LEWIS leaf zoom 17 → 18).
    //
    // The high end goes DOWN, and the reason is measured too: at FL300 the
    // ground in view is 10–120 km away (scripts/r22-a-dublin-prof-*.json), the
    // leaf under the aircraft is frustum-capped at z10 with the loader IDLE,
    // and raising the threshold to 1.2 bought exactly one extra level at the
    // 10 km sample while nearly doubling Esri requests (306 → 564) and adding
    // ~72 draws. Cruise tessellation nobody can resolve is the right thing to
    // shed; draws at cruise measured 146–155, so this is headroom, not a save.
    [600, 0.86],
    [3000, 0.82],
    [9000, 0.78],
  ],
  // Vendored martini decimation targets (meters of tolerated vertical error),
  // consumed by the R22 patch #3 seam. Upstream's curve is
  // round(7000*(1-z/17)^3): z13 = 91 m, z14 = 38 m, z15 = 11 m, z16 = 1 m.
  // R22 MEASURED at the Owens pose, one lever at a time (draws / triangles):
  //   control (flags off, z17, dem15, upstream table) …… 215 / 220k
  //   + the curve alone (z17, dem15, no table) ………………… 223 / 221k
  //   + z18 …………………………………………………………………………………………… 235 / 223k
  //   + demMaxZoom 16 …………………………………………………………………… 232 / 402k
  //   + this table ……………………………………………………………………………… 235 / 588k
  // i.e. z18 is +12 draws and ~zero triangles (it is texels, not geometry),
  // while the DEM levers are ~zero draws and +368k triangles. All four stay
  // inside the 261 Owens ceiling; P-LEWIS peaked at 806k triangles against the
  // 2.0M fixed-pose gate, Manhattan at 1.12M against the 2.2M soak p95.
  demErrorTable: { 13: 45, 14: 18, 15: 5, 16: 2 },
  statsHz: 2, // runtime.terraStats publish cadence
  // What "downloads settled" means for terraStats.sharp (B's reveal gate reads
  // the boolean, not these): the loader must have been at or below
  // maxDownloading continuously for quietMs. stallMs is the second term — how
  // long camTileZ must stop improving before the descent counts as finished
  // (see the long note at TerrainEngine._tick: at cruise the leaf under the
  // aircraft is frustum-capped, so a target-only definition never fires).
  settle: { maxDownloading: 2, quietMs: 400, stallMs: 1200 },
};
...
export const TERRA_PIPE = {
  enabled: true, // R22 W3 FLIP (vendored patches + notifyWarp prefetch; flag-off = verbatim upstream, ledgered in VENDOR.md)
  parallelFetch: true,
  lodBailFix: true,
...
export const TERRA_CACHE = {
  enabled: true, // R22 W3 FLIP (second-visit 0.026x/0.00x vs the 0.40 target)
...
export const ARRIVAL_GATE = {
  enabled: true, // R22 W3 FLIP (sanction 5.1 consumed via this block: holdMax 6500; time cap always wins; bootTerms stays false - escalated, not spent)
  ...
  // --- THE BOOT LEG, AND WHY IT SHIPS OFF ----------------------------------
  // The same content terms are BUILT into BootScreen's satellite world gate
  // and are RESOLVED AND RECORDED on every boot (flag or no flag — that record
  // is the control), but they do not hold the reveal unless `bootTerms` is
  // true. Measured on the calibration machine at Powell OH, 4 alternating
  // boots with the HDRI held back 9 s:
  //
  //   flags off   boot 9549 / 7864 ms   terms at reveal: parcel FALSE in one
  //                                     of the two (the legacy boot really
  //                                     does reveal over an unresolved world)
  //   terms on    boot 10565 / 11969 ms content held 1343 / 6303 ms
  //
  // i.e. +2.6 s of boot on average. Plan §4 freezes boot-reveal timing ("the
  // WARP hold may lengthen — §5.1; the boot may not"), and §5 sanctions no
  // boot-hold move, so the terms cannot be armed inside this round's budget.
  // They are one flag away and the evidence for the trade is above; the
  // pop-in the boot leg would have bought is covered instead by the birth
  // fades, which cost no boot time at all.
  bootTerms: false,
  // Ceiling on that hold if it is ever armed, measured from boot start (never
  // from the moment the terms are first consulted) so the boot envelope can be
  // bounded by a single number.
  bootContentMaxMs: 8000,
};
...
export const SETTLE_CALM = {
  enabled: true, // R22 W3 FLIP (sanction 5.8 consumed: ladder dprMin 0.75 + subStep 0.125; births/parcel-ease/groundElevVis/arrival-calm)
  // births — the shared birth-envelope contract (lib/fly/settle.js). rampSec is
  // the INSTANCED ramp (parcel homes, house lights, and C's veg/clutter, which
  // read this same value so the whole ground stack fades on ONE curve);
  // bayerSec is the chunked-mesh dissolve (sat-buildings' existing
  // SAT_BLDG_FADE Bayer path, the skyline's uSkyFade, the road network's
  // additive opacity). tintStepSec: the landcover tint's multiplier lives in a
  // CPU vertex-colour array (multiply blending has no alpha of its own), so its
  // birth is a stepped re-fill on this interval rather than a per-frame ramp —
  // a 10% wash stepped in 4 increments is indistinguishable from a continuous
  // one and costs 3 extra pool fills instead of 36.
  births: { rampSec: 0.6, bayerSec: 0.5, tintStepSec: 0.15 },
  parcel: { growEaseSec: 0.6, deleteFadeSec: 0.5 },
  prewarmSlice: { perIdleRaf: 1 },
  groundElevVis: { slewMps: 80 },
  // ladderFix — CANVAS.dprMin 1 → 0.75 for the GOVERNOR LADDER ONLY (plan §5.8;
  // the canvas' own boot DPR arithmetic is untouched). subStep is the rung
  // spacing BELOW native: at devicePixelRatio 1 the native dprStep of 0.25
  // would buy exactly one sub-native rung, and the gate wants at least two
  // render-scale rungs standing between the boot state and the first tier step.
  // 0.125 = a 12% pixel-count cut per rung, which is a real lever and still
  // invisible on a 1× display.
  ladderFix: { dprMin: 0.75, subStep: 0.125 },
  // arrivalCalm: graceSec = the post-reveal window in which the weather blend
  // is held at the value it arrived with and the HDRI bucket may not re-pick;
  // hdriHysteresis = the fraction of a bucket's own width a NEW bucket must
  // clear before the 5 s re-pick is allowed to act on it (the sun crawls, so a
  // bucket edge crossed at 0.001°/s otherwise flaps the whole sky pair).
  arrivalCalm: { graceSec: 6, hdriHysteresis: 0.15 },
};

/**
 * ROUND 22.1 (B "STUTTER") — FRAME PACING. The post-ship hotfix for the live
 * defect the user recorded on the PRODUCTION build: banked low-AGL flight over
 * Powell OH suburbs freezes for 33-50 ms and snaps, roughly once every two
 * seconds while manoeuvring (ffmpeg signalstats over 872 frames: five runs of
 * >= 2 duplicated frames, 22 near-duplicates).
 *
 * WHAT IT IS, MEASURED — not guessed. A CDP main-thread sampling profile of a
 * scripted 350 kt / 233 m-AGL serpentine over the user's exact pose
 * (scripts/r22p1-b-probe.js; full table in scripts/r22p1-b-stutter.md) put
 * **67% of every stalled millisecond inside three-tile's skirt builder** —
 * `getBoundaryEdges`, which each DEM tile runs ON THE MAIN THREAD inside
 * `TerrainLercLoader.doLoad` after the worker hands back the decimated mesh.
 * It allocates 3·T two-element arrays, sorts them with a JS comparator running
 * four Math.min/max per comparison, then de-duplicates. R22 made it hurt: z18
 * imagery + demMaxZoom 16 + the altitude-keyed LOD curve stream far more DEM
 * tiles at low AGL, and `Tile._loadSubTiles` resolves FOUR children in one
 * microtask drain, so four of those builds land in one frame.
 *
 * The fix is arithmetic, not policy: count each undirected edge in a hash and
 * keep the ones seen once. Vendored patch #5 (`skirtEdges`), whose output is
 * IDENTICAL to upstream's by construction — same edges, same order, same
 * direction — and which returns null (deferring to the verbatim upstream body)
 * for every input shape it does not claim. That identity is why this block
 * needs NO fleet pin in scripts/_boot.js and why no frozen gate moves: the
 * scene content, draw calls and triangle totals are bit-for-bit what they were.
 * It repaces work; it does not remove content.
 *
 * enabled:false restores the R22 behaviour byte-for-byte (the patch switch is
 * armed only from here, and OFF is the upstream expression).
 *
 * Dev/harness A/B: `window.__flyPaceForce = true|false` (the __flyTerraForce
 * idiom) overrides this flag live; undefined in production builds.
 */
export const FRAME_PACE = {
  enabled: true,
  /**
   * Vendored patch #5. The only sub-switch, kept separate from `enabled` so a
   * future pacing measure can land in this block without re-arming the skirt
   * fix or vice versa.
   */
  skirtEdges: true,
};
...
export const CLUTTER = {
  enabled: true, // R22 W3 FLIP (sanction 5.9 budgets hold by construction; Owens 0/0 bit-identical proven)
 ...
 * when casters are inside the ortho frustum and AGL < maxAglM.
 * ⚠️ R22 W2 MEASURED CORRECTION (Fable): the W0 scaffold claimed "Owens pays
 * +0 BY CONSTRUCTION" — FALSE. The Owens gate pose sits on Lone Pine, which
 * has 949 building collision columns inside the ortho frustum, so the
 * catcher MOUNTS there: +1 draw when armed (measured 202 total ≤ 261 with
 * everything on; the ceiling holds with margin). Fable ruling: accepted —
 * DEPTH_PASS ships OFF this round, and if the user opts in at checkpoint #3
 * the +1 is inside every ceiling. Raise a minCasters floor only if a future
 * round needs Owens-exact-zero with shadows armed. nearReceive: near-ring
 * LEAF terrain tiles
...
export const DEPTH_PASS = {
  enabled: false,
  /**
   * The ground catcher. `maxAglM` is the AGL band above which the disc is
   * pointless (its 900 m rim subtends nothing at cruise and the shadows that
   * would land on it are already sub-pixel); `minCasters` is the OTHER half of
   * the header's demand — the disc only mounts when at least this many shadow
   * CASTERS are actually inside the 1500 m ortho frustum. Owens has zero
   * casters at every altitude (no buildings, no canopy in the near ring), so
   * `minCasters: 1` makes Owens +0 draws BY CONSTRUCTION rather than by
   * winning a count race. `pollHz` is the caster census cadence — a mount is a
   * discrete React transition (it allocates a mesh), never a per-frame call.
   */
  catcher: { enabled: true, maxAglM: 2200, minCasters: 1, pollHz: 2 },
  /**
   * Near-ring terrain receive set. R13 rejected this on TWO grounds; only one
   * survives r185. (a) "a recompile on the hot tile path" — DEAD: three r185
   * carries `receiveShadow` as a UNIFORM (lights_pars_begin.glsl.js:2
   * `uniform bool receiveShadow`), set per-object in WebGLRenderer.js:2687.
   * Flipping it costs one uniform write, never a program. (b) fill rate — the
   * real objection, and the one this flag is measured against (gpuFrameMs A/B,
   * plan §7 budget +1.0 ms). Scope: LEAF tiles only (a subdivided parent draws
   * nothing), inside `SAT_SHADOWS.orthoRadiusM` + `padM` of the player, and
   * only while the shadow rig is actually armed. `maxTiles` is a hard stop so
   * a pathological quadtree can never enlist the whole world.
   */
  nearReceive: {
    enabled: true,
    padM: 200, // frustum radius + this: a tile straddling the rim still receives
    maxTiles: 48,
    pollHz: 6, // sweep cadence; leaves change far slower than frames
    parcelHomes: true, // R20 parcel instancer joins as a RECEIVER (never a caster)
  },
  /**
   * N8AO (n8ao@1.10.3, already a transitive dep — no package.json move).
   * Satellite + HIGH TIER ONLY, half-res, its own Pass at the HEAD of the
   * chain (AO must darken the scene colour before bloom reads luminance).
   *
   * REVERSED DEPTH — see components/fly/N8AO.jsx for the full derivation. Two
   * upstream defects had to be closed: n8ao's auto-detect reads
   * `renderer.capabilities.reverseDepthBuffer`, a property that does not exist
   * in three r185 (it is `reversedDepthBuffer`, and the string `reverseDepth`
   * appears ZERO times in every three build file), so the detect never fires;
   * and `configureHalfResTargets()` never injects `#define REVERSEDEPTH` into
   * the depth-downsample material even when the rest of the chain has it.
   *
   * DRAW COST is 6-7 calls, not the plan's "+3" estimate — n8ao's pass is
   * SIX internal fullscreen quads (downsample, AO, blur x denoiseIterations,
   * accumulate, composite, copy). `denoiseIterations: 1` buys the cheapest
   * honest configuration. Measured deltas + the ceiling arithmetic are in the
   * D report; the frozen ceilings (Owens 261 / sat 375) are what bind.
   */
  n8ao: {
    enabled: true,
    halfRes: true,
    aoRadius: 24, // world metres
    // n8ao's own default. MEASURED at P-LEWIS/Manhattan the raw AO buffer runs
    // 240-255/255 (0-6% occlusion) at these altitudes, so intensity is what
    // decides whether the effect reads at all: pow(0.95, 2.2) darkens a crease
    // 11%, pow(0.95, 5) darkens it 23%. Pure taste, and it is exactly what
    // user checkpoint #3 is for — the value is a one-line move.
    intensity: 5.0,
    distanceFalloff: 1.0,
    aoSamples: 16,
    denoiseSamples: 8,
    denoiseIterations: 1, // 2 upstream; each iteration is a whole extra draw
    denoiseRadius: 12,
    colorMultiply: true,
    screenSpaceRadius: false,
    depthAwareUpsampling: true,
    selfWarm: true, // compile the four quads at construction (prewarm can't reach a Pass)
  },
  /**
   * The 0-800 m band has ZERO distance attenuation today: AERIAL_PERSPECTIVE's
   * smoothstep starts at `startM` 800, so everything nearer is mathematically
   * untouched. This adds a SECOND, much weaker band under it. Implemented
   * inside AerialPerspective.jsx (module setters), so FlyScene's -50 feed is
   * untouched — and the near term is compiled into the fragment shader ONLY
   * when this flag is armed at module load, which makes the flag-off program
   * TEXT-IDENTICAL to R21's rather than merely arithmetically identical.
   */
  aerialNear: { enabled: true, nearStartM: 420, nearMaxMix: 0.1 },
  /**
   * The cross-charter seam (plan §6 C, last bullet): C ships ALL clutter
   * `castShadow:false`, and D owns the flip with per-flip gpuFrameMs. C's
   * meshes are not merged in W1, so the mechanism is MARKER-DRIVEN rather than
   * import-coupled: any Object3D carrying `userData.r22Caster = <kind>` is
   * enlisted when its kind's flag is on. W1 numbers are measured against
   * stand-in casters (`__flyCasterStandIn`); W2 re-measures post-C merge.
   */
  casters: { trees: false, carsParked: false, carsMoving: false, poles: false },
};

/**
 * ROUND 22.1 (A "FLASH") — STEP_SAFE: the quality ladder must never present a
 * frame the frame loop did not draw.
 *
 * THE DEFECT (user report, production build, one frame at 60 fps). A DPR step
 * replaced the whole 3D world with a uniform pale field for exactly one
 * presented frame, with the right 1/6 of the canvas holding the PREVIOUS
 * frame's pixels. 1.25/1.5 — the ladder's first rung ratio at
 * devicePixelRatio 1.5 — is exactly the fraction of the canvas width that was
 * updated.
 *
 * THE MEASURED ORDERING (scripts/r22p1-a-probe.js, trace rows verbatim; every
 * row carries inRaf:false):
 *
 *   t+0.0 ms  PerfGovernor.useFrame -> applyDpr -> React setDpr   [INSIDE rAF]
 *   t+6.0 ms  gl.setPixelRatio(1.25)            inRaf FALSE   <- new task
 *   t+6.7 ms  canvas.width  1920 -> 1600        inRaf FALSE   <- REALLOC+CLEAR
 *   t+7.1 ms  canvas.height  990 ->  825        inRaf FALSE   <- REALLOC+CLEAR
 *   t+9.3 ms  composer.render()                               <- 1 frame drawn
 *             with composer buffers 1920 wide into a 1600-wide drawing buffer
 *   t+22.5 ms composer.setSize(1280,660)        inRaf FALSE   <- 15 ms LATER
 *
 * So r3f's DPR path reallocates (and therefore CLEARS) the drawing buffer in a
 * plain task, between animation frames, and the composer catches up a whole
 * frame later in a passive effect. That leaves TWO windows in which the
 * compositor can present something the frame loop never drew at the current
 * size: the cleared buffer itself, and the one composed frame per step whose
 * post-chain buffers disagree with the drawing buffer (measured: exactly 1 per
 * step, 12/12 forced steps).
 *
 * THE FIX. `applyDpr` no longer writes React state directly — it records a
 * PENDING dpr, and a useFrame inside the canvas applies renderer.setPixelRatio
 * + renderer.setSize + composer.setSize and only then lets the SAME animation
 * frame draw (the composer renders at priority 1, this rig at framePriority).
 * The React catch-up write happens in the same tick; when r3f's own store
 * subscriber re-applies the identical numbers it is a genuine no-op —
 * assigning an UNCHANGED value to canvas.width does not reallocate (measured
 * in Chromium: same-value assignment preserves the buffer, different-value
 * clears it to opaque black).
 *
 * enabled:false ⇒ the governor calls setDpr exactly as R22 did and the rig is
 * not mounted: the one-line revert contract. The legacy PerformanceMonitor
 * branch (PERF_GOVERNOR.enabled:false) is untouched in BOTH states — it steps
 * DPR with a functional setState that has no pending-value seam.
 */


/**
 * ROUND 22.1 (C "FLASH") — FLASH_GUARD: the one-frame white flash.
 *
 * THE DEFECT. On the shipped build, roughly one composed frame in 1,600 is
 * replaced by a near-full-screen uniform pale field with a dead-straight
 * vertical boundary and the live world beyond it. The user caught it on video
 * at Powell OH, 233 m AGL. The next frame is normal.
 *
 * THE CAUSE, measured (scripts/r22p1-c-flash.md §3). It is ONE zero-area
 * triangle inside a streamed satellite-building chunk. A drawRange binary
 * search over a chunk's 109,503 indices found that indices [0, 61410) render
 * black and the single triangle at [61410, 61413) paints the entire pale
 * field. Its three vertices are collinear — x and y byte-identical, z spanning
 * 5 m. A census (probe 12) then found that EVERY large chunk carries 6–9 %
 * exactly-zero-area triangles, in two kinds: ~99.9 % coincident (two vertices
 * byte-identical — a wall quad extruded over a zero-length footprint edge, so
 * duplicate consecutive OpenFreeMap footprint points) and ~2 per chunk
 * collinear-but-distinct, which is the kind that was caught painting.
 *
 * WHY IT DRAWS. A zero-area triangle is mathematically invisible, but its
 * rasterization is decided by the sign and magnitude of an area determinant
 * that is ~0 — i.e. by the last bits of the projected coordinates. The
 * building material is `side: DoubleSide` (sat buildings are open shells), so
 * backface culling never removes the half a winding test would. The anchor
 * bend adds a per-vertex float32 offset (`bendD²·uBendK`) that perturbs the
 * projection just enough to tip that determinant: setting `uBendK = 0` or
 * moving `uBendCenter` onto the camera collapses the pale to zero, while the
 * material colour, both lights, the scene fog, the environment, the cull fade
 * and the content haze are all inert.
 *
 * NOT A RACE. The pale reproduces on demand from a frozen frame — same
 * matrices, same uniforms. It is a pose-dependent numerical condition: bendD
 * changes every frame as the aircraft moves, and only at some poses does the
 * determinant tip. That is why it is rare, why it lasts exactly one frame, and
 * why no fixed-pose gate in the fleet has ever caught it.
 *
 * THE FIX. Drop zero-area triangles when a chunk's geometry is assembled. A
 * triangle with no area contributes no pixels when it behaves, so this is
 * content-neutral by construction — but it is NOT triangle-count-neutral, and
 * that is deliberate and measured, not an oversight: index counts fall ~6–9 %
 * on dense chunks. Nothing in the fleet asserts an exact sat-building index
 * count; the tris budgets are ceilings and this only ever lowers them.
 *
 * WHY AN AREA TEST AND NOT A POINT DEDUPE. A consecutive-duplicate dedupe on
 * the footprint ring would remove the coincident 99.9 % and MISS the
 * collinear-but-distinct triangle that was actually caught painting the
 * screen. The test has to be on area.
 *
 * WHY POST-DRAPE. The filter runs engine-side on the final draped positions,
 * so it also catches any degeneracy the drape itself introduces. The worker is
 * untouched — WORKER_PROTOCOL does not move.
 *
 * Revert contract: `enabled: false` ⇒ the index buffer is used verbatim, one
 * branch, byte-identical to R22.
 */


/* ============================== ROUND 23 =================================
 * "Night Alive" — blocks pre-seeded by Fable at plan time (FLY_ROUND23_PLAN
 * .md), filled by their owners: A NIGHT-TRUTH below, B CITY-LIGHT after it.
 * C (cert) owns neither block and touches neither.
 * ========================================================================= */

/**
 * ROUND 23 (A NIGHT-TRUTH) — the night-regression block.
 *
 * Scope: FIXES ONLY. New night VISUALS belong to B's `NIGHT_CITY_R23`; nothing
 * here adds a light, a draw or a mesh. Everything in this block exists to undo
 * a mechanism that is measurably removing light that R21 certified as present.
 *
 * --- hazeNight: the content haze had no night term -------------------------
 *
 * THE DEFECT (R23 A, measured — scripts/r23-a-tiernight.json).
 * `AERIAL_PERSPECTIVE.content` shipped `enabled:false` from R19 through R21 and
 * was flipped ON at `minTier:'medium'`, `max:0.55` in R22 W3 (sanction §5.4).
 * Its strength is a pure function of DISTANCE — it reads no sun, no elevation,
 * no `nightFrac`. Measured at a pinned deep-night clock on the user's world:
 *
 *     tier medium, sunFrac 0 (deep night) -> contentHaze 0.55
 *     tier medium, sunFrac 1 (noon)       -> contentHaze 0.55   <- identical
 *
 * By day that is correct aerial perspective: the mix target is the bright rim,
 * so distant content recedes into haze. At night `_atmoRim` is the deep-night
 * keyframe `#101a30`, so the SAME 0.55 mixes the city toward dark navy — and
 * the injection site (`world-bend.js`, after the lighting chunks, on
 * `gl_FragColor`) means it pulls EMISSIVE WINDOW LIGHT down with everything
 * else, on precisely the two variants that carry the city's night read
 * (sat-building + sat-skyline).
 *
 * WHY NO GATE CAUGHT IT (the R19 §7 lesson, third occurrence — and this time
 * proven by a paired leg, not argued): `scripts/_boot.js` pins
 * `__flyAerialOverride = 0` fleet-wide and FlyScene multiplies the content gate
 * by it, so every harness forces the identity path. `verify-aerial` is the ONE
 * un-pinner — and it runs `setQualityTier('high')`, where the `!highTier` gate
 * excludes the term by construction. The single un-pinner runs at the one tier
 * where the feature cannot exist. Control, same build/pose/clock: user
 * condition 0.55 in every night cell, fleet condition 0.00 in every cell.
 *
 * THE FIX. Retire the haze as the city lights arrive, on the SAME ramp shape
 * the windows and the road network already use (`SAT_BUILDINGS.night.dayFrac` /
 * `.gamma`), so the two hand off to each other instead of fighting:
 *
 *     nightT = clamp01(1 - sunFrac/dayFrac) ** gamma      // 0 by day, 1 deep night
 *     haze  *= 1 - retire * nightT
 *
 * At `retire: 1` the night haze is exactly 0 — which is R21's certified night
 * behaviour restored EXACTLY (the term was `enabled:false` then) — while noon
 * keeps R22's shipped 0.55 bit-for-bit. It is a night-only change by
 * construction, so every daylight gate is untouched by arithmetic, not by luck.
 *
 * `retire` is a knob rather than a hard revert because "should a night city
 * veil at all?" is a taste question B/Fable may want to answer differently
 * (some real veiling at 8 km is defensible). It is NOT a taste question whether
 * a 55% dark-navy wash should sit on a city nobody ever measured — that is the
 * defect.
 *
 * ROLLBACK: `enabled:false` restores the shipped R22 behaviour exactly (the
 * multiplier is never computed and never applied), which is the flag-off
 * byte-identity contract. Owens: this fix can only ever REMOVE a mix term, so
 * it adds no draw, no tri and no lit pixel anywhere — the desert stays dark by
 * construction.
 */
export const NIGHT_TRUTH_R23 = {
  enabled: true,
  hazeNight: {
    enabled: true,
    // Retire fraction at full night. 1 = the haze is exactly 0 at deep night
    // (R21's certified state). Lower it to leave some night veiling.
    retire: 1.0,
    // The ramp shape. Deliberately the SAME numbers as SAT_BUILDINGS.night so
    // the haze retires on exactly the curve the windows come up on; they are
    // read as defaults here rather than imported so a future divergence is a
    // visible edit, not a spooky action at a distance.
    dayFrac: 0.3,
    gamma: 1.5,
  },
  /**
   * Read-only night telemetry on `__flyStats.night` (dev builds only). No
   * product behaviour, no draws — this is the instrument that makes the ONE
   * thing R23 A could not measure (which quality tier a real machine resolves
   * to, and what that does to the night chain) answerable in a single read on
   * the user's own hardware. `hz` is the publish cadence.
   */
  telemetry: { enabled: true, hz: 2 },
};

// ---------------------------------------------------------------------------
// ROUND 23 (B "CITY-LIGHT") — NIGHT_CITY_R23. The enhancement half of "Night
// Alive": the FLY_ROUND22_HANDOFF §1 deltas that address LIGHTS (delta 3 road
// life, delta 2b window variance) plus the medium-tier night PROPOSAL. It is
// deliberately ONE block with sub-flags — every knob a night look is allowed
// to move lives here, so a revert is one boolean and a re-sweep never edits an
// R16/R19-swept value in place.
//
// SHIPS `enabled: false`. Flag-off is the REVERT CONTRACT and it is
// STRUCTURAL, not arithmetic: every consumer takes the R19 code path verbatim
// (same generated GLSL, same program cache key, same uniform values), so the
// disarmed tree is byte-identical to base by construction rather than by
// measurement. Armed, the road and building night variants compile their own
// '-r23' keys (see the world-bend.js registry header).
//
// ARMING (lib/fly/night-city.js — the R22 DEPTH_PASS `__flyDepthArm` idiom):
//   window.__flyNightCityArm = 1 → on, 0 → off, undefined → this constant.
// The arm is read at material-APPLY time (that is where the shader source is
// chosen), so an A/B leg that changes the SOURCE re-boots; every knob below is
// a live uniform, so a knob A/B is one page.evaluate (window.__flyNightCity).
export const NIGHT_CITY_R23 = {
  enabled: false, // ships OFF — Fable flips after review (plan §4B)

  /**
   * DELTA 3 — ROAD LIFE AT NIGHT. The R16 sweep calibrated `night.intensity`
   * 1.15 → 2.4 and R19 (C) added the cls 5-6 streetlight envelope; both are
   * load-bearing history and are NOT edited. These knobs SHADOW them while
   * armed — applyBendRoadSat writes these into the same uniforms instead — so
   * the knob A/B against the swept baseline is literally "set these back to
   * {2.4, 0.24, 0.34, 1.6, 260} and re-shoot" (scripts/r23-b-roadsweep-*.json).
   */
  roads: {
    enabled: true,
    // THE RE-SWEEP (measured — see the report's per-knob table). The R16 value
    // was swept at a Manhattan pose where building occlusion hides most of the
    // street web; the handoff's complaint ("nearly invisible at this pose") and
    // the user's are the same complaint.
    intensity: 3.5, // steady network glow at full night (R16 swept: 2.4)
    // Suburban parity, re-measured on top of the new intensity. R19 sized these
    // so cls 5/6 land at ~0.70 effective weight; the same target against the
    // brighter network puts them a touch higher — a suburb at night is mostly
    // streetlights and almost nothing else.
    streetGain: { c5: 0.3, c6: 0.42 }, // R19 swept: { c5: 0.24, c6: 0.34 }
    // Lamp pools: R16 baked these as compile-time constants (STREET_DOT_GAIN /
    // STREET_DOT_SHARP). Armed they become live uniforms, so the pool can be
    // swept without a source edit; the disarmed path still emits the literals,
    // so nothing about the R19 program changes.
    lamp: { gain: 1.9, sharp: 210 }, // R16 literals: 1.6 / 260 (sharp ↓ = wider pool)
    // Headlight dash trains (cls 1-3) — the R16 term, re-swept.
    stream: { boost: 2.9, dashLenM: 380, dashDuty: 0.34, dashSpeed: 0.12 },
    /**
     * THE TWO-COLOUR TRAFFIC TERM (handoff delta 3's "warm headlights one
     * direction, red tails the other"). It rides INSIDE the one road material —
     * an ADDITIVE colour written after the class-weighted gain — so it costs
     * zero draws and zero geometry. `boost: 0` is the exact identity.
     *
     * LANES: the ribbon carries no side attribute from the worker, but
     * `pushSatRoadQuads` writes every quad as (a+n, a−n, b+n, b−n), so global
     * vertex-index PARITY is the side — SatRoadEngine derives `aRoadSide`
     * client-side (+1/−1) with no worker payload change and no protocol bump.
     * A missing attribute reads 0 ⇒ both streams at half weight ⇒ fail-blended,
     * never fail-wrong.
     */
    traffic: {
      head: '#ffe9c0', // headlights: warm white, running with +arc
      tail: '#ff4530', // tail lights: red, running against it
      boost: 1.35, // 0 = IEEE identity (the term contributes literally nothing)
      dashLenM: 300, // wavelength of ONE vehicle packet
      dashDuty: 0.22, // lit fraction of the wavelength (tighter than the glow dash)
      speed: 0.16, // wavelengths/sec (head runs +, tail runs −)
      lanes: 1, // 0 = both colours across the whole ribbon, 1 = split by side
    },
  },

  /**
   * DELTA 2b — WINDOW VARIANCE. The R15 night atlas is ONE 512² texture tiled
   * in facade metres across every building in the city, so its 8×8 lit/dark
   * pattern repeats on every wall — "density is plausible… now uniform across
   * the whole frame" (handoff §0). The fix is per-BUILDING, and the
   * per-building identity already exists as an attribute: `aBendAnchor`, the
   * footprint centroid. Hashing it in the fragment gives every building its own
   * stable random with no new attribute, no worker change, no second texture.
   *
   * DAYLIGHT IS UNTOUCHED BY CONSTRUCTION: every term below lands inside
   * `<emissivemap_fragment>`, and the layer holds `emissiveIntensity` at 0
   * whenever the sun is up (SatBuildingEngine.setNightMix) — anything
   * multiplied into a zero emissive is zero. No daylight gate can move.
   */
  windows: {
    enabled: true,
    // (a) DE-REPEAT: shift the emissive lookup by a per-building whole number of
    // atlas CELLS. Quantising to cells is what keeps the R15 roof invariant
    // exact — neutralUV 0.25 × cols 8 = 2.0 is a cell boundary, and boundary + k
    // cells is still a boundary, i.e. still the solid background texel that makes
    // roofs textureless at every distance. 0 = no shift.
    phase: 1,
    // (b) BRIGHTNESS SPREAD: a per-building multiplier on the night emissive, so
    // the city stops being one uniform sheet of light. { min: 1, max: 1 } is the
    // identity.
    gain: { min: 0.45, max: 1.55 },
    // (c) DARK BUILDINGS: the handoff's "dark floors between bright clusters".
    // A hash-stable fraction of buildings sit at `darkGain` — the office block at
    // 2 a.m. with the cleaners gone. 0 = every building lit (the R15 read).
    darkFrac: 0.22,
    darkGain: 0.08,
    // (d) TINT SPREAD: per-building warm↔cool jitter around the atlas tint, so
    // one block reads sodium-warm and its neighbour fluorescent-cool. 0 = the
    // single SAT_BUILDINGS.night.color across the whole city.
    tintJitter: 0.18,
  },

  /**
   * THE TIER QUESTION (plan §4B — "may be the single highest-leverage change of
   * the round"). Night windows are `SAT_BUILDINGS.night.minTier: 'high'`, a
   * 2026-07 perf call made for a much lighter world. If the live session settles
   * at medium — which R22's heavier terrain makes likely and which NO night gate
   * has ever measured — the city has no window lights at all: the user's "very
   * few show lights in windows" is what the design DOES at medium.
   *
   * A PROPOSAL, measured and shipped OFF inside an already-off block: armed, it
   * lowers the night gate to `nightMinTier`. Never flipped silently — the
   * measured cost is in the B report and the default is Fable's call.
   */
  tier: {
    enabled: true,
    nightMinTier: 'medium', // SAT_BUILDINGS.night.minTier stays 'high', untouched
  },
};

// ===========================================================================
// ROUND 24 "MOTION HOLD" — the scaffold (D).
//
// THE DEFECT THIS ROUND CLOSES, in the user's words (2026-08-15, live
// machine): in SATELLITE, "buildings and shrubs/vegetation appear/disappear,
// the satellite ground plane itself glitches, worst when moving fast" — and
// "we've tried to solve this multiple times".
//
// WHY IT SURVIVED R21, R22, R22.1 AND R23 (Wave-1 D, §5): every certification
// pass was blind to it BY CONSTRUCTION, not by bad luck.
//   · scripts/_boot.js:86 sets `__flyTerraPin = 1` FLEET-WIDE — ~30 of ~35
//     harnesses AND BOTH SOAKS measure the R21 terrain, not the shipped one
//     (soak-fly.js:62 un-pins only __flyGovPin).
//   · verify-flicker.js:1 is titled "does a FROZEN scene hold still?" and
//     imports bootFly WITHOUT unpinPins (:83) — the one visual-stability gate
//     is pose-frozen and fully pinned.
//   · verify-stability.js:655 orbits at a FIXED point: rotation only, so no
//     ring translation, no keep-set churn, no heal-counter reset — and, per
//     R21's own merge note (88587f7), "speed 0 = bit-identical centre", i.e.
//     the velocity lookahead is EXACTLY ZERO in every harness that asserts.
//   · verify-terra.js:287 says it out loud: "A 350 kt aeroplane crosses a z17
//     tile in under a second, so an unfrozen 'settle' measures travel" — and
//     then FREEZES the aeroplane (:289-295).
// The invisible class is therefore: any defect needing speed > 0 on the
// SHIPPED R22 terrain whose observable is appearance/disappearance rather
// than frame time, triangles, draws or heap. That is the whole user report.
//
// Every block below is one-flag revertible and every flag-off path must be the
// VERBATIM prior expression (the R22 byte-noop discipline). Owners tune values
// only inside their own block; D owns this scaffold region.
// ===========================================================================

/**
 * R24 A TERRA — TILE_HOLD. Closes A-R1/A-R2 (= Wave-1 D theory T1), the
 * confirmed root of the ground-plane glitch and a large share of the building
 * blinks.
 *
 * MECHANISM (vendored three-tile 0.12.1, upstream behavior, unprotected since
 * the library was adopted — R21 never touched this layer; its last pre-R22
 * commit is R19's 751625c):
 *   vendor/three-tile/index.js:276  `_getDistRatio` returns dist/size × 0.8 in
 *     frustum and × 5 out of it — a 6.25× DISCONTINUITY the instant a tile
 *     leaves the view.
 *   :265  `_LODEvaluate` subdivides at `ratio <= T` and merges at `ratio > T`
 *     — the SAME threshold both ways, i.e. ZERO hysteresis at the boundary.
 *   :211/:242  a subdivided parent released its own model at :321, so the
 *     out-of-frustum guard does not stop it: LOD() runs and returns MERGE.
 *   :331  `_removeSubTiles` then DOWNLOADS the parent before committing, and
 *     :190-204 `unloadSubTiles`/`unloadModel` DISPOSE every fine descendant.
 ...
 * ESSENTIALLY EVERY SUBDIVIDED TILE THAT LEAVES THE FRUSTUM IS DESTROYED, and
 * turning back re-descends from the coarse leaf. R22 made it worse on both
 * axes: +1 imagery level (z17→18) and +1 DEM level (15→16) = one more level to
 * destroy per turn, and `lodBailFix` removed upstream's DFS bail, which had
 * been an accidental throttle on merge/subdivide churn while the loader was
 * saturated (which, at speed, is permanently).
 *
 * This is also the mechanism behind R22.1 close ledger F11 ("a second visit to
 * a pose does not re-refine, with the loader IDLE") and behind the verify-terra
 * gate (2) camTileZ 10↔13↔18 swing that F10 dismissed as an instrument: the
 * tile really merged, so `getGroundAt().tileZ` really answers coarse.
 *
 *   mergeDwellMs    — a merge must be WANTED continuously for this long before
 *                     it destroys anything; a tile that comes back inside the
 *                     window keeps its children. The release is patch #6a's
 *                     own: `Tile.LOD` clears `_mergeWantSince` on ANY non-merge
 *                     evaluation (`if (s !== 2)`), so re-entering the frustum
 *                     re-arms the clock from zero.
 *                     (CORRECTED post-review — this line previously credited
 *                     the release to `unlockOnReject`, which is patch #7 and
 *                     touches the stamp only on the `_removeSubTiles`
 *                     REJECTION path, never on frustum re-entry. Caught by
 *                     A TERRA reading the scaffold against the vendored source.)
 *   frustumPenalty  — the ×5 above, made a constant instead of a literal so it
 *                     can be swept. NOT swept this round (deferred, plan §7).
 *   lodHysteresis   — subdivide/merge threshold split. 1.0 = IDENTITY, i.e.
 *                     the shipped state is dwell-only; arming it is deferred to
 *                     R25 so this round moves one mechanism at a time.
 *   raster          — A's timeout/retry/backoff for the Esri raster path
 *                     (terrain-engine.js / raster-cache.js), the R21 P2
 *                     reason-coded-retry idiom applied to imagery+DEM.
 */
export const TILE_HOLD = {
  enabled: true,
  mergeDwellMs: 2000,
  frustumPenalty: 5,
  lodHysteresis: 1.0,
  unlockOnReject: true,
  raster: { timeoutMs: 12000, retries: 2, backoffBaseMs: 800 },
}; // owner A

/**
 * R24 B STREAM — RING_HOLD. Closes B-F-B1 (= Wave-1 D theory T2): the actor
 * ring's nearest-N membership has NO hysteresis and NO fade.
 *
 * MECHANISM: sat-building-engine.js:874-893 sorts candidate z14 tiles by
 * squared distance from the (lookahead-shifted) centre, `slice(0, maxChunks)`,
 * and then `if (!keep.has(key)) this._evict(key, chunk)` — a chunk that
 * crosses the rank-12/13 boundary is destroyed instantly and re-fetched from
 * the worker on the way back, i.e. a 2.7 km block of buildings blinks.
 * SAT_BLDG_FADE cannot help: it is wired ONLY to the ALTITUDE cull
 * (sat-building-engine.js:826, `offAglM`), never to the horizontal ring.
 *
 *   keepHysteresis  — rank slack: a resident chunk survives to rank
 *                     maxChunks + this before it is dropped.
 *   minResidencySec — floor on how briefly a chunk can live, so a centre that
 *                     jitters across a bisector cannot strobe it.
 *
 * DEFERRED THIS ROUND (plan §7): the DEATH FADE. Fading an evicting chunk out
 * would need a shader term on the streamed materials, and a cache-key move is
 * exactly what this round must not spend (five R18 hashes + the R19/R20 keys
 * are frozen). Hysteresis + residency reduce the EVENT RATE without touching
 * one byte of GLSL; the fade is an R25 seed.
 */
export const RING_HOLD = {
  enabled: true,
  keepHysteresis: 2,
  minResidencySec: 4,
}; // owner B
...
/**
 * R24 B STREAM — LEAD_SAFE. Closes B-F-B2: R21's velocity lookahead
 * (STREAM_KEEPER.lookahead, sat-building-engine.js:514-521) offsets each
 * engine's ring centre by min(speed × 2.5 s, 0.35 × ringR) along a 0.6 s
 * velocity EMA. For the BUILDING ring that is a tuned trade; for the SHORT
 * rings it silently breaks the coverage guarantee — a lead comparable to the
 * ring radius walks the ring off the player, so veg/clutter thin out or vanish
 * ahead of the aircraft exactly when it is moving fastest.
 *
 * And the fleet is structurally blind to it: R21's own merge note (88587f7)
 * records "speed 0 = bit-identical centre", so a feature that only exists at
 * speed has never been exercised by a gate that asserts appearance.
 *
 * capByEngine — per-engine absolute metre cap on the lead, re-derived by B's
 * bisection sim against each ring's own radius. Units are metres; an engine
 * absent from the map keeps the STREAM_KEEPER fraction unchanged.
 */
export const LEAD_SAFE = {
  enabled: true,
  // THE DERIVATION (B, W2). The scaffold carried B's Wave-1 numbers, which were
  // single-point reads off a 20 u radial scan and were flagged as such. These
  // are the EXACT bisection's answers — `scripts/r24-b-ringhold.mjs --derive`,
  // which computes the guaranteed radius as the closest-point distance to the
  // nearest NON-KEPT grid cell (exact, no scan resolution) over 17×17 player
  // positions inside one tile × 72 lead bearings, then bisects the lead:
  //
  //   ring                     G(lead 0)   G(0.35·r)   must cover        CAP
  //   satVeg    z14 r3600 /9      2417        1164      2400 distFade      48
  //   satClutter z13 r3600 /6     2885        1646      2200 moving.range 746
  //
  // The G(lead 0) column is the load-bearing validation: it reproduces both
  // shipped invariants — SAT_VEG's "endM is inside the guaranteed-coverage
  // radius" (the veg one to the unit: 2417 ≥ 2400, true by 17 u) and CLUTTER's
  // "guaranteed coverage 2446 m >= every rangeM" (2885 ≥ 2200). A model that
  // did not reproduce the un-led world would not be evidence about the led one.
  //
  // THE CLUTTER FIGURE DOES NOT MATCH TO THE UNIT AND SHOULD NOT (D's review,
  // B3): the model reads 2885 where that comment says 2446, because 2446 is a
  // z14 tile span (WORLD_SIZE / 2^14) and CLUTTER.ring.z is 13, whose span is
  // 4892. The shipped comment therefore UNDERSTATES its own ring's guarantee by
  // half a tile. The discrepancy is in the conservative direction — every
  // decision ever sized against 2446 assumed less coverage than the ring
  // actually has — so nothing built on it was unsafe, and the cap below is
  // bisected against the corrected geometry, not against the prose. Fixing the
  // CLUTTER comment itself is R25's (it is C CLUTTER's block, and this round
  // does not edit another owner's constants).
  capByEngine: {
    // 48: this ring's fade constant sits 17 u inside its own lead-free
    // guarantee, so there is almost no slack to spend and the lookahead is
    // effectively retired here. That is the correct trade THIS round — the
    // lookahead buys stream latency, the guarantee buys "a tree never vanishes
    // at full scale" — but the better fix is more coverage, not less lead:
    // raising SAT_VEG.maxChunksByTier.high 9 → 12 would buy the lead back and
    // costs stream bandwidth, not draws (the canopy is one pooled draw either
    // way). Named for R25 rather than spent unmeasured here.
    satVeg: 48,
    satClutter: 746,
    // The three long rings have NO per-instance consumer range — nothing fades
    // on distance-from-player — so there is no invariant for a lead to break
    // and each cap is set to EXACTLY its present effective maximum
    // (0.35 × ringR). LEAD_SAFE is therefore a PROVEN NO-OP on these three, and
    // the entries exist so that a future change to a ring radius cannot
    // silently re-open the hole this block closed for veg and clutter.
    satBuildings: 1540, // 0.35 × 4400 (SAT_COVERAGE.high.ringM)
    satRoads: 4200, // 0.35 × 12000 (SAT_ROADS.ring.r)
    // 4900, not the scaffold's 4200: 0.35 × 14000 is 4900, and 4200 would have
    // been a silent 14 % tightening derived from nothing. The bisection does
    // NOT model this ring — its chunks are groupN×groupN BLOCKS, not tiles — so
    // a tighter number here would be a guess dressed as a measurement.
    satSkyline: 4900, // 0.35 × 14000 (SAT_SKYLINE.ring.r)
  },
}; // owner B

/**
 * R24 B STREAM — POOL_FAIR. Closes B-F-B3: the pooled instancers allocate
 * their slot tables nearest-first from a table that is re-selected whenever the
 * resident-chunk signature moves, so at speed the far half of a pool can be
 * starved by whichever chunks happen to resolve first — the visible symptom is
 * a pool that fills in unevenly and re-shuffles as the ring translates.
 * Fair-share slot allocation across resident chunks; no new pools, no new
 * draws, and the Owens invariant (nothing placed ⇒ no draw) is preserved.
 */
export const POOL_FAIR = { enabled: true }; // owner B

/**
 * R24 B STREAM — COMMIT_BUDGET. Closes R22.1 close-ledger F3 = Wave-1 D theory
 * T3 = C's M-item on the same code: `SatVegEngine._commitPending` and
 * `SatClutterEngine._commitPending` are the only streaming engines with NO
 * per-frame finalize cap, so they can flip many chunks ready in ONE frame and
 * force every downstream pooled layer's signature to change on the same tick.
 * B found it, C found it independently, and it is ONE fix — B owns it.
 *
 * finalizePerFrame — the budget every other engine already has.
 */
export const COMMIT_BUDGET = {
  enabled: true,
  finalizePerFrame: 1,
}; // owner B

/**
 * R24 C MOTION — MOTION_R24. Closes C-M1/M2/M3: the surface layers key their
 * cadences and their grades off quantities that are wrong under motion.
 *
 *   aglTruth  — veg/clutter read a RAW AGL that R22's own SETTLE_CALM already
 *               had to damp for the visible ground (groundElevVis ~384 →
 *               ≤4 m/frame). Same truth, one source, so a DEM refinement step
 *               can no longer teleport an AGL-gated layer.
 *   grades    — SAT_QUILT / micro-detail grade on raw AGL too, so the same
 *               step re-grades the whole frame; they move onto the damped
 *               value. Uniform writes only — NO shader text, NO cache key.
 *
 * BUILT-BUT-OFF, on C's own measure-first ruling and the DEPTH_PASS precedent
 * (R22 shipped that pass off pending a user checkpoint rather than flipping an
 * unmeasured default):
 *   elevGate    — refuse to commit an elevation from a tile shallower than
 *                 minTileZ. Correct in principle; the risk is refusing forever
 *                 over genuinely coarse coverage, which is the R19 Owens class
 *                 of mistake. OFF until measured at a controlled pose pair.
 *   elevSlew    — rate-limit ground elevation (slewMps) with a snap escape
 *                 (stepSnapM) and a dt floor (dtFloorSec). OFF: a slew that is
 *                 too slow reads as the aircraft sinking into terrain.
 *   paceBySpeed — scale the 2 s wall-clock refresh cadences by ground speed
 *                 (refMps reference, minCadenceSec floor). This is the honest
 *                 fix for "a 2 s cadence is a 500 m cadence at 250 m/s", but it
 *                 raises worker load exactly when the loader is most saturated,
 *                 so it does not ship on an argument.
 */
export const MOTION_R24 = {
  enabled: true,
  aglTruth: { enabled: true },
  grades: { enabled: true },
  elevGate: { enabled: false, minTileZ: 11 },
  elevSlew: { enabled: false, slewMps: 80, stepSnapM: 120, dtFloorSec: 0.05 },
  paceBySpeed: { enabled: false, refMps: 120, minCadenceSec: 0.35 },
}; // owner C

// ---------------------------------------------------------------------------
// ROUND 25 "GROUND & NIGHT" — pre-seeded owner blocks (W0 scaffolding, Fable).
// Every block ships `enabled:false` and is byte-identity when off. Owners fill
// in their own knobs; nobody edits another owner's block (the R18–R24 idiom
// that has produced zero constants conflicts five rounds running). Every block
// is read through `r25On()` / `r25Block()` in lib/fly/r25-pins.js, which merges
// a `window.__fly<Name>Override` pin over it (lib/fly/fly-pins.js), so a
// harness or the user arms a feature from the console, never by editing this
// file. Plan: FLY_ROUND25_PLAN.md §3. Evidence: scripts/r25-recon.md.
//
// The user's report this round: "on the ground the immersive feel still
// lacks", "at night the ground just appears as dark with copper striping
// (roads) and the lighting is really lacking", "things feel flat", and the
// phone HUD should be "a single button that pans out all of our
// functionalities aside from the joystick". Rulings (asked and answered):
// ground = LOW SKIMMING 50–500 ft, not touchdown; night = a REAL CITY FROM A
// SMALL PLANE; the fan is a RADIAL ARC and throttle + boost stay beside the
// stick; every frozen ceiling stays and new content is gated so the Owens
// desert control stays 0 by construction.
// ---------------------------------------------------------------------------

// ----- Fable (shared substrate) — the one "how low are we" signal -----------
/**
 * GROUND_BUBBLE — ONE writer (components/fly/GroundBubbleRig.jsx, mounted from
 * FlyCanvas) publishes `runtime.groundBubble = { k, aglM, aglVisM }` once per
 * frame at useFrame priority −49, i.e. AFTER FlyScene's −50 block has stepped
 * `runtime.groundElevVis` (GROUND_VIS). `k` is 1 inside the bubble (AGL ≤
 * aglInM), 0 above aglOutM, smoothstep between, with a DEADBAND of
 * `hysteresisM` on the AGL input (a DEM refinement step smaller than the band
 * cannot move k at all — the MOTION_R24 lesson) and an exponential settle of
 * `smoothSec`. A warp SNAPS (a cut is not a ramp). Readers: A GROUND (detail
 * overlay strength, scrub/hedge placement), C LIGHT (shadow radius, AO radius),
 * F FEEL (ground rush, DoF, chase settle, audio). Every reader spells it
 * `runtime.groundBubble?.k ?? 0`, so with this block off — the rig is not
 * mounted and the field is undefined — every consumer reads exactly 0 and the
 * R24 behaviour is byte-identical. Pure math in lib/fly/ground-bubble.js
 * (node-testable: scripts/verify-ground-bubble-k.mjs).
 */
export const GROUND_BUBBLE = {
  enabled: true, // R25 W2 (Fable): ON for the user's test build — gates green on the fixture, one-line revert
  aglInM: 500, // k = 1 at or below this visual AGL
  aglOutM: 700, // k = 0 at or above this visual AGL
  hysteresisM: 60, // input deadband: AGL must move this far before k moves
  smoothSec: 0.6, // exponential settle time constant toward the target k
}; // owner Fable (shared)

// ----- A GROUND — the low-AGL detail bubble ---------------------------------
/**
 * GROUND_DETAIL_R25 — what turns ON as you descend (today every AGL band is a
 * cull band; nothing exists finer than an 8.5 m lamp post). `overlay` is a
 * procedural detail term injected into the tile fragment inside
 * applyHillshade (world-bend.js) as a NEW FINAL hill-key token 'd': two
 * octaves of the existing value noise at metre scale giving a fine luma grain
 * and an analytic normal perturbation that feeds the fragment-stage N·L, so
 * relief moves with the key light — classed by the sampled imagery colour
 * (grass / asphalt / soil). `scrub` and `hedges` are ONE InstancedMesh each in
 * components/fly/SatGroundDetailLayer.jsx (crossed leaf-atlas cards on pooled
 * SatTint landcover triangles; box segments 6 m outboard of cls 5/6 parcel
 * roads), hash-stable, content-gated so an empty landcover set parks the mesh
 * at count 0 / visible false ⇒ 0 draws at Owens by construction. `tint` lifts
 * the SatTint drape alpha inside the bubble (CPU multiplier, no shader text).
 * `z19` and `workerNormals` ship OFF inside the block: each wants a measured
 * row (fixture Owens draws; a real LERC machine) before it flips. Byte-noop
 * revert: `enabled:false` ⇒ no token, no layer mount, alpha as R24.
 */
export const GROUND_DETAIL_R25 = {
  enabled: true, // R25 W2 (Fable): ON for the user's test build — the one red row is the venue's A/B control; one-line revert
  overlay: {
    enabled: true,
    strengthByTier: { high: 1, medium: 0.7, low: 0 },
    scaleM: [1.4, 0.45], // the two noise octaves' cell periods (m)
    normalAmp: 0.35, // slope perturbation fed to the fragment N·L
    lumaAmp: 0.06, // fine luma grain amplitude (±)
    classify: {
      grassHue: [70, 160], // degrees
      grassSatMin: 0.12,
      asphaltSatMax: 0.08,
      asphaltLuma: [0.18, 0.55],
    },
  },
  scrub: {
    enabled: true,
    poolByTier: { high: 2000, medium: 800, low: 0 },
    radiusM: 300,
    cardM: [0.6, 1.4], // card height range
    urbanAvoidM: 20, // no scrub within this of a building column
    roadAvoidM: 6,
    waterAvoidM: 4,
    landcover: ['grass', 'farmland', 'wood'],
    cadenceSec: 2,
  },
  hedges: {
    enabled: true,
    poolByTier: { high: 600, medium: 0, low: 0 },
    offsetM: 6, // outboard of the cls 5/6 road edge
    segM: 12,
    heightM: 1.6,
    columnAvoidM: 10, // skip segments with a building column this close (driveways)
  },
  tint: { enabled: true, lowAglAlpha: 0.18 }, // SAT_TINT.alpha 0.1 → this at k = 1
  // BUILT OFF (R25 W1). `levels` is how many zoom levels the satellite + high
  // imagery CEILING rises by when armed; `aglM` is the altitude at which that
  // ceiling starts to BIND, which is three-tile's own distance descent and NOT
  // a second mechanism — see lib/fly/ground-detail.js `z19MaxZoomFor` for why a
  // literally-k-keyed `maxLevel` is either a vendor-private write or a
  // whole-tree reload at every crossing (i.e. the user's own reported symptom,
  // on purpose, once per crossing). Measured at the fixture Owens pose before
  // any flip — scripts/r25-a-ground.md §z19.
  z19: { enabled: false, aglM: 150, levels: 1 },
  workerNormals: { enabled: false }, // built OFF: LERC path unreachable here
}; // owner A

// ----- B NIGHT — the night ground irradiance ------------------------------
/**
 * NIGHT_GROUND_R25 — nothing lights the ground at night today: one 0.09
 * directional, and a road layer that adds light to its OWN ribbon only. This
 * block owns a top-down NIGHT GROUND IRRADIANCE render target
 * (components/fly/NightGroundRig.jsx, mounted from FlyCanvas): an ortho camera
 * straight down over `spanM`, texel-snapped, into which the resident road
 * meshes are re-rendered (main scene through layer bit 7 — never a bare Group,
 * which would drop `fog` from the program key and compile a twin) plus one
 * instanced additive splat mesh for lamp posts (from the pole instance
 * matrices SatClutterLayer already writes), lit windows (from queryColumns)
 * and porch lights; separably blurred; updated ONLY on ≥ updateMoveM of
 * movement or a source change, so a settled pose never updates and every
 * frozen draw census sees +0. Every ground receiver — terrain (hill-key token
 * 'n'), building bases, canopy, parcel homes, clutter — samples it once via
 * world XZ → UV inside `if (uNGGain > 0.0)`, and uNGGain is 0 by day, so a
 * pinned-noon program executes none of it. `roadResweep` SHADOWS
 * SATELLITE_VISUALS.lighting (never edits it): the copper dot-train is
 * roadGlow 0.035 / streetPoolGain 12 / streamGain 0, a reaction to "broad
 * white ribbons" that was never re-swept with any pavement spill present.
 * `windows` ports NIGHT_CITY_R23.windows' three knobs onto the CINEMATIC
 * architecture material (the R23 atlas path is dead under it). `halo` is a
 * SkyDome term (not a remounted SatCityGlow). Byte-noop revert:
 * `enabled:false` ⇒ no rig, no RT, no token, no key move, road gains as R24.
 */
export const NIGHT_GROUND_R25 = {
  enabled: false,
  rt: {
    sizeByTier: { high: 1024, medium: 512, low: 0 },
    spanM: 3000,
    updateMoveM: 150,
    minUpdateSec: 1,
    blurPx: 2,
  ...
  sources: {
    roads: { enabled: true, gain: 1, spillM: 16 },
    lamps: { enabled: true, gain: 1, radiusM: 14, color: '#ffb066', pool: 900 },
    windows: { enabled: true, gain: 0.6, radiusPadM: 8, pool: 2048 },
    houses: { enabled: true, gain: 0.5 },
  },
  receivers: {
    terrain: true,
    buildings: { enabled: true, heightFalloffM: 6 },
    canopy: true,
    homes: true,
    clutter: true,
  },
  roadResweep: { enabled: true, roadGlow: 0.022, streetPoolGain: 8, streamGain: 0.6, trafficGain: 0.32 },
  windows: { enabled: true, darkFrac: 0.22, gain: [0.45, 1.55], tintJitter: 0.18 },
  canopyNight: { enabled: true, lift: 0.06 },
  halo: {
    enabled: true,
    cities: 4,
    rangeM: 90000,
...
// ----- C LIGHT — a source for the light, a moon that is a light ----------
/**
 * LIGHT_BUBBLE_R25 — zero shader text. `shadow`: the ONE shadow cascade's
 * ortho radius follows the bubble (1500 m → 350 m, 1.46 → 0.34 m/texel at
 * 2048) in `steps` quantised rungs with `hysteresisM`, written imperatively on
 * the light's shadow camera by components/fly/LightBubbleRig.jsx (the JSX boot
 * props stay); a SECOND shadow light was rejected because a light count is
 * part of every program's cache key and would recompile every lit material.
 * `ao`: N8AO radius/intensity follow the bubble (high tier only). `moon`: the
 * night key is today a constant 0.09 with the moon only steering its
 * DIRECTION (ONE_SUN); this adds a synodic phase (`illum`) and an anti-solar
 * elevation weight (`up`) into the key, env and fill night terms of
 * immersiveLighting() — written so that `day === 1` yields the R24 expression
 * bit-for-bit. `hemi`: the hemisphere GROUND colour (daytime olive, never
 * re-written today) follows the time-of-day bucket. `grade`: night bloom
 * threshold, exposure and a HAZE FLOOR (AERIAL_LAW.nightRamp drives aerial
 * perspective to exactly 0 at night; a night city with no depth cue reads as a
 * black photo) — all multiplied under THIS flag, never editing NIGHT_TRUTH_R23
 * or AERIAL_LAW. Byte-noop revert: `enabled:false` ⇒ no rig, R24 lighting.
 */
export const LIGHT_BUBBLE_R25 = {
  enabled: true, // R25 W2 (Fable): ON for the user's test build — gates green on the fixture, one-line revert
  shadow: { enabled: true, radiusLowM: 350, radiusHighM: 1500, steps: 8, hysteresisM: 40 },
  ao: { enabled: true, radiusLowM: 5, intensityLow: 3.5 },
  moon: {
    enabled: true,
    keyNew: 0.025, // night key at new moon (R24: constant 0.09)
    keyFull: 0.16, // night key at full moon, moon up
    envNew: 0.05,
    envFull: 0.1,
    fillGain: 0.04,
    epochNewMoonMs: Date.UTC(2000, 0, 6, 18, 14),
    synodicDays: 29.530588,
    discBrightness: [0.12, 0.5], // SkyDome disc, new → full (B's setSkyMoonPhase)
  },
  hemi: {
    enabled: true,
    ground: { day: '#5a6b53', dusk: '#5a4a3e', night: '#1a2030' },
    skyNight: '#3d4c6e',
  },
  grade: { enabled: true, nightBloomThreshold: 0.78, nightExposureStops: -0.1, hazeNightFloor: 0.12 },
}; // owner C

// ----- D MOBILE — one floating button, a quarter-arc fan --------------------
/**
 * MOBILE_FAN_R25 — components/fly/hud/TouchFan.jsx replaces the persistent
 * row and both contextual rows inside the `controls-right` zone (NO new zone:
 * verify-mobile-layout asserts exactly 8). The joystick, throttle rail and
 * boost pad stay where they are. One FAB (`touch-fab`) fans `order` petals
 * out on an arc from `arc.fromDeg` to `arc.toDeg` at `radiusPx`; the
 * contextual petals take the inner slots so the persistent set never shifts
 * when a lock appears; every existing `touch-*` testid is preserved and
 * `touch-hangar` is NEW (the hangar had no key and no touch button). Closes on
 * FAB tap, tap outside, any petal tap and any `covered` transition. While open
 * the zones in `hideWhileOpen` are hidden (the 180° petal lands on the info
 * chip's row in both orientations). `dockBottomRem` re-derives
 * MOBILE_UI.infoChip.dockBottomRem for the shorter column (anchor 3.25 + boost
 * 2.75 + 0.75 + rail 8.375 + 0.75 + FAB 3.5 = 19.375 rem MEASURED; 21.75 keeps
 * the plan's 20.75 + 1 as conservative headroom — see the derivation comment
 * at `MOBILE_UI.infoChip.dockBottomRem`). Reduced motion
 * (`fly-reduced-motion`) ⇒ petals land on the first frame. Byte-noop revert:
 * `enabled:false` ⇒ TouchFan is not imported and TouchControls renders today's
 * rows byte-for-byte, dock string character-identical.
 */
export const MOBILE_FAN_R25 = {
  enabled: true, // R25 W2 (Fable): ON for the user's test build — gates green on the fixture, one-line revert
  fabPx: { portrait: 56, landscape: 48 },
  petalPx: 44,
  radiusPx: { portrait: 168, landscape: 150 },
  // CLEARANCE between ring 1 and ring 2, not the ring offset: ring 2 sits at
  // `radiusPx + petalPx + ringGapPx` = 168 + 44 + 8 = 220 px portrait, i.e.
  // the +52 px the plan (§3 D) quotes, with the two 44 px petals edge-to-edge
  // 8 px apart. Spelling it as clearance is what keeps that true if `petalPx`
  // ever moves.
  ringGapPx: 8,
  arc: { fromDeg: 180, toDeg: 90 },
  spring: { stiffness: 520, damping: 34, mass: 0.8, staggerMs: 22 },
  hideWhileOpen: ['info-dock', 'toasts'],
  closeOnCover: true,
  order: ['look', 'atlas', 'logbook', 'photo', 'hangar', 'pause'],
  contextual: ['inspect', 'intercept', 'cinema'],
  dockBottomRem: 21.75,
}; // owner D

/**
 * R25 (D MOBILE) — the pin read for MOBILE_FAN_R25, spelled HERE instead of
 * imported from `lib/fly/r25-pins.js`, and that is not laziness:
 * `r25-pins.js` imports this file, so importing it back would close a module
 * cycle whose `const BLOCKS = { … }` initializer reads six of this file's
 * exports at evaluation time — a TDZ ReferenceError in one of the two possible
 * load orders. Components keep using `r25On('MobileFan')`; only the two
 * MOBILE_UI getters (which run INSIDE this module) use these.
 *
 * The merge semantics are `fly-pins.js`'s `pinned()` exactly — a shallow
 * `{ ...block, ...window.__flyMobileFanOverride }` — so the two readings of
 * "is the fan armed" can never disagree. verify-mobile-fan asserts that in the
 * browser, on both legs.
 *
 * Hoisted function declarations on purpose: MOBILE_UI is 4,300 lines ABOVE
 * this block, and its getters call these at render time, long after module
 * evaluation. Nothing here runs at import.
 */
export function mobileFanPinned() {
  const pin = typeof window === 'undefined' ? null : window.__flyMobileFanOverride;
  return pin ? { ...MOBILE_FAN_R25, ...pin } : MOBILE_FAN_R25;
}

/** Is the touch fan armed (block + pin)? The `r25On('MobileFan')` reading. */
export function mobileFanOn() {
  return mobileFanPinned().enabled === true;
}

// ----- F FEEL — the ground rushes, the lens finds it, the air changes -------
/**
 * FEEL_R25 — 0 draws except `dof`. `groundRush`: SPEED_FEEL's ground rush is
 * forced off in satellite immersive (Effects.jsx `speedOn`); this re-arms it
 * and multiplies its strength by the bubble so cruise stays exactly 0.
 * `dof`: a 'sat-dof' pass descriptor beside 'toy-dof' on the DEPTH_FIX
 * reversed-depth path, focus = camera→aircraft distance, bokeh 0 above the
 * bubble; ≈ +4 fullscreen draws at satellite high WHENEVER MOUNTED (Owens
 * post-overhaul reads 249–252 against 261 — measured on the fixture before any
 * flip; medium/low never mount). `chase`: less lag, less bank inheritance, a
 * touch less FOV and a higher eye near the deck, via a setter the bubble rig
 * calls (chase-camera.js, no FlyScene touch; IEEE-identical at k = 0).
 * `audio`: wind and engine keyed to the bubble in immersive-audio.js.
 * `proximity`: a filtered hiss rising below `aglM`. Byte-noop revert:
 * `enabled:false` ⇒ pass list, camera and audio graph as R24.
 */
export const FEEL_R25 = {
  enabled: false,
  groundRush: { enabled: true, aglBandM: 120, boost: 1.2 },
  dof: { enabled: true, tier: 'high', focusRangeM: [60, 220], bokeh: 0.9 },
  chase: { enabled: true, posLambdaMul: 1.25, bankShareMul: 0.6, fovDeltaDeg: -2, heightBiasM: 3 },
  audio: { enabled: true, windGain: 0.35, groundBounceGain: 0.12, bounceAglM: 60 },
  proximity: { enabled: true, aglM: 30, gain: 0.08 },
}; // owner F
