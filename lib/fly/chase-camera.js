...
export class ChaseCamera {
  ...
  update(dt, flight, camera, freeLook, k, groundSignal) {
    ...
    const back = CAMERA.offset.z * followScale * k * camScale * (earth ? STYLIZED_EARTH.camera.distance : cinematic ? SATELLITE_VISUALS.scale.chaseDistance : 1);
    const up = CAMERA.offset.y * followScale * camScale * (earth ? STYLIZED_EARTH.camera.height : cinematic ? SATELLITE_VISUALS.scale.chaseHeight : 1);
    ...
    flight.forward(_fwd);
    if (cinematic) _lookTarget.copy(flight.pos).add(metricDirection(_fwd, CAMERA.lookAheadM * (earth ? STYLIZED_EARTH.camera.lookAhead : 1), k));
    ...
    const fovTarget = (earth ? STYLIZED_EARTH.camera.fov : immersive ? 62 : cinematic ? SATELLITE_VISUALS.scale.fov : CANVAS.fov) + CAMERA.fovBoost * Math.pow(speedFrac, 1.5) * (reduced ? 0 : immersive ? 0.35 : 1) + groundCue * 0.85 * (1 - this._freeAmt);
