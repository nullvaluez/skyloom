import { satelliteVisualsOn, SATELLITE_VISUALS } from './satellite-visuals';
import { immersiveOn, readReducedMotion } from './immersive';
import { STYLIZED_EARTH, stylizedEarthOn, cinematicFlightOn } from './stylized-earth';
import { metricDirection } from './render-scale';
import { useFlyStore } from '../../stores/fly-store';
import { Vector3, Quaternion, Matrix4, Euler, MathUtils } from 'three';
import { CAMERA, CANVAS, FLIGHT, SHAKE, SPEED_FEEL } from './fly-constants';
import { expApproach } from './coords';
import { getTrauma } from './juice';
import { groundCameraCue, nearGroundOn } from './near-ground';
import { cameraModelSize } from './camera-framing';
...
export class ChaseCamera {
  constructor() {
    ...
    this._lastFlightPos = new Vector3();
    this._orbOff = new Vector3(); // damped plane-relative orbit offset
    this._initialized = false;
    this._shakeT = 0; // round 18: shake noise phase (seconds)
    // Round 19 (E): boost-ENGAGE FOV punch. `_punchDeg` is the live transient
    // and `_wasBoosting` is the edge detector. Both start at rest, and rest is
    // exactly the pre-R19 rig — see the FOV block at the end of update().
    this._punchDeg = 0;
   ...
   */
  update(dt, flight, camera, freeLook, k, groundSignal) {
    // --- Free-look offsets -------------------------------------------------
    ...
    const cinematic = useFlyStore.getState().mapStyle === 'satellite' && satelliteVisualsOn('scale');
    const immersive = cinematic && immersiveOn('camera');
    const reduced = immersive && readReducedMotion();
    const groundCue = cinematic && nearGroundOn('camera')
      ? groundCameraCue(groundSignal, readReducedMotion()) : 0;
    const earth = cinematic && stylizedEarthOn();
    const framed = earth && cinematicFlightOn('camera');
    let back = CAMERA.offset.z * followScale * k * camScale * (earth ? STYLIZED_EARTH.camera.distance : cinematic ? SATELLITE_VISUALS.scale.chaseDistance : 1);
    let up = CAMERA.offset.y * followScale * camScale * (earth ? framed ? STYLIZED_EARTH.camera.height : 1.45 : cinematic ? SATELLITE_VISUALS.scale.chaseHeight : 1);
    this.modelSize=cameraModelSize(flight);
    if(framed&&this.modelSize){
      const size=this.modelSize,tan=Math.tan(STYLIZED_EARTH.camera.fov*Math.PI/360);
      const fitWidth=size.width/(2*tan*Math.max(.5,camera.aspect)*STYLIZED_EARTH.camera.targetWidth);
      const fitHeight=size.height/(2*tan*.20);
      back=Math.max(size.length*1.6,Math.sqrt(Math.max(1,Math.max(fitWidth,fitHeight)**2-up*up)));
    }
    const ops=flight.operations,opsGround=ops?.grounded;
    const groundBlend=ops?.profile&&(opsGround||ops.lowSpeed)?Math.max(0,Math.min(1,1-(flight.agl-ops.profile.clearance)/100)):0;
    this._opsBlend=this._initialized?expApproach(this._opsBlend,groundBlend,3,dt):groundBlend;
    if(this._opsBlend>0.001){const len=ops?.profile?.length??20;
      back+=(Math.max(24,len*1.8)*k-back)*this._opsBlend;up+=(Math.max(6,len*.4)-up)*this._opsBlend;
    }
    const verticalBack = cinematic ? back / k : back;

    // --- Chase pose: behind + above in the plane's heading frame -----------
    const yaw = flight.heading;
    _anchor.set(
      flight.pos.x - Math.sin(yaw) * back,
      flight.pos.y + up + Math.sin(-flight.pitch) * verticalBack * 0.35,
      flight.pos.z + Math.cos(yaw) * back
    );
    flight.forward(_fwd);
    if (cinematic) _lookTarget.copy(flight.pos).add(metricDirection(_fwd, CAMERA.lookAheadM * (earth ? framed ? STYLIZED_EARTH.camera.lookAhead : .55 : 1), k));
    else _lookTarget.copy(flight.pos).addScaledVector(_fwd, CAMERA.lookAheadM * k);

    if(this._opsBlend>0.001){
      const ahead=(ops?.profile?.length??20)*.4;
      _fwd.set(flight.pos.x+Math.sin(yaw)*ahead*k,flight.pos.y,flight.pos.z-Math.cos(yaw)*ahead*k);
      _lookTarget.lerp(_fwd,this._opsBlend);
    }
    ...
    _orbOffIdeal.set(-Math.sin(a) * back * cp, up + verticalBack * sp, Math.cos(a) * back * cp);

    const snapPose = !this._initialized;
    const l = 1 - Math.exp(-CAMERA.posLambda * (immersive ? 0.85 : 1) * dt);
    if (snapPose) {
      ...
      if(framed){
        // Transport most translation with the aircraft. Retain only a bounded
        // 7-metre trailing cue, instead of speed/lambda (50+ metres at cruise).
        _fwd.copy(flight.pos).sub(this._lastFlightPos);
        const retained=Math.min(1,STYLIZED_EARTH.camera.lagMaxM*CAMERA.posLambda/Math.max(1,flight.speed));
        this._chasePos.addScaledVector(_fwd,1-retained);
      }
      this._chasePos.lerp(_anchor, l); // world-space lag (sells speed)
      this._orbOff.lerp(_orbOffIdeal, l); // offset-space (orbits cleanly)
    }
    this._lastFlightPos.copy(flight.pos);
    ...
    const bankShare = flight.bank * CAMERA.bankShare * (immersive ? 0.7 : 1) * (1 - fa);
    ...
    if (SHAKE.enabled && !reduced) {
      ...
      const ampDeg = (SHAKE.maxDeg * speedShake + SHAKE.traumaMaxDeg * trauma * trauma) * (immersive ? 0.18 : 1);
      if (ampDeg > 1e-4) {
    ...
    // --- FOV kick with speed ------------------------------------------------
    // Round 19 (E): a PUNCH on top of the damped kick. The kick alone is the
    // whole of R18's speed sensation and it is a slow exponential — by the time
    // it arrives you have stopped noticing you accelerated. The punch is the
    // event: a few degrees thrown in on the ENGAGE EDGE and decayed away in
    // half a second, so hitting boost shoves the world at you and then settles
    // into the steady kick.
    //
    // Zero at every probe speed BY CONSTRUCTION, no pin: `_punchDeg` only ever
    // leaves 0 on a rising boost edge, and a harness that never boosts never
    // creates one. When it is 0 the expression below is `fovTarget + 0`, which
    // is IEEE-identical to the R18 line, so verify-chase-cam / verify-photo /
    // every projection gate measure the numbers they always did. It is also
    // deliberately NOT tier-gated: decision 2's "medium/low byte-identical" is
    // about GPU spend, and camera feel is not spend — a phone that can boost
    // gets the punch.
    const boosting = !!flight.boosting;
    if (SPEED_FEEL.enabled) {
      if (boosting && !this._wasBoosting) this._punchDeg = SPEED_FEEL.fovPunch.deg;
      this._wasBoosting = boosting;
      if (this._punchDeg > 1e-4) {
        // Exponential decay with a ~decaySec time constant (3τ ≈ the quoted
        // 0.5 s to visually nothing), then a hard zero so the transient can
        // never leave a permanent sub-threshold offset on the FOV.
        this._punchDeg *= Math.exp((-3 * dt) / Math.max(0.05, SPEED_FEEL.fovPunch.decaySec));
        if (this._punchDeg <= 1e-4) this._punchDeg = 0;
      }
    }
    const fovTarget = (earth ? framed ? STYLIZED_EARTH.camera.fov : 60 : immersive ? 62 : cinematic ? SATELLITE_VISUALS.scale.fov : CANVAS.fov) + CAMERA.fovBoost * Math.pow(speedFrac, 1.5) * (reduced ? 0 : immersive ? 0.35 : 1) + groundCue * 0.85 * (1 - this._freeAmt);
    this.fov = expApproach(this.fov, fovTarget, CAMERA.fovLambda, dt);
    // The punch rides OUTSIDE the damped state on purpose. Added to the target
    // instead, CAMERA.fovLambda (2 ⇒ τ 0.5 s) would swallow a 0.5 s transient
    // almost entirely — the rig would chase ~1° of a 4° event and the punch
    // would be a rounding error. Applied here it lands whole, and `this.fov`
    // stays the clean damped speed curve every other frame's arithmetic uses.
    const fovOut = this.fov + this._punchDeg * (reduced ? 0 : immersive ? 0.15 : 1);
    if (Math.abs(camera.fov - fovOut) > 0.05) {
      camera.fov = fovOut;
