...
export function buildCinematicTreeGeometry(pool = 1) {
  if (immersiveOn('materials') && !stylizedEarthOn()) return buildImmersiveFoliage(pool);
  const pos = [], col = [], idx = [];
  ...
  for (let h = 0; h <= H; h++) {
    ...
    for (let w = 0; w <= W; w++) {
      ...
      const lobes = 1 + (stylizedEarthOn() ? 0.17 : 0.075) * Math.cos(a * 3) * r;
...
export function createCinematicTreeMaterial(bend, { depth = false } = {}) {
  const m = depth
    ...
    : new MeshStandardMaterial({ color: stylizedEarthOn() ? '#92bb65' : '#ffffff', vertexColors: true, roughness: 0.94, metalness: 0, envMapIntensity: 0.4 });
  bend(m);
  const immersive = immersiveOn('materials') && !stylizedEarthOn();
