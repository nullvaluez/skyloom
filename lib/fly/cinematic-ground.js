...
import { applyImmersiveFoliage, buildImmersiveFoliage } from './immersive-foliage';
import { applyNightGroundReceiver } from './night-ground';
import { applyNearGroundMaterial } from './near-ground-material';
import { stylizedEarthOn, cinematicFlightOn } from './stylized-earth';
...
export const CINEMATIC_TREE_TRIANGLES = 34;
const leafCanopiesOn = () => immersiveOn('materials') && (!stylizedEarthOn() || cinematicFlightOn('materials'));
...
/** Leaf clusters share one atlas and draw. The solid crown remains the review fallback. */
export function buildCinematicTreeGeometry(pool = 1) {
  if (leafCanopiesOn()) return buildImmersiveFoliage(pool);
  const pos = [], col = [], idx = [];
  ...
  for (let h = 0; h <= H; h++) {
    ...
    for (let w = 0; w <= W; w++) {
      ...
      const lobes = 1 + (earth ? 0.27 : 0.075) * Math.cos(a * 3+(earth?v*.85:0)) * r;
      const shade = 0.72 + 0.34 * (v + 1) / 2 + (earth ? Math.cos(a*3)*r*.055 : 0);
      // Offset crown tiers suggest branching instead of a symmetric oval. Same
      // 58 triangles and topology, shared by visible and shadow geometry.
      push(Math.cos(a) * r * lobes+(earth?.10*r*v:0), 0.67 + v * 0.33+(earth?.035*Math.cos(a*3)*r:0), Math.sin(a) * r * lobes+(earth?.07*r:0), [shade, shade, shade]);
    }
  }
...
export function createCinematicTreeMaterial(bend, { depth = false } = {}) {
  const m = depth
    ...
    : new MeshStandardMaterial({ color: leafCanopiesOn() ? '#d8dfbd' : stylizedEarthOn() ? '#94ad76' : '#ffffff', vertexColors: true, roughness: 0.94, metalness: 0, envMapIntensity: 0.4 });
  bend(m);
  const immersive = leafCanopiesOn();
  if (immersive) applyImmersiveFoliage(m);
  const prev = m.onBeforeCompile, key = m.customProgramCacheKey();
  m.onBeforeCompile = (shader, renderer) => {
    prev(shader, renderer);
    if (immersive && !depth) {
      // Both sides of a leaf cluster share its rounded outward lighting normal.
      // DoubleSide's automatic flip otherwise turns half the crown nearly black.
      shader.vertexShader=shader.vertexShader.replace('#include <common>', '#include <common>\nvarying float vLeafCard;')
        .replace('#include <begin_vertex>', '#include <begin_vertex>\nvLeafCard=1.-step(.51,uv.x);');
      shader.fragmentShader=shader.fragmentShader.replace('#include <common>', '#include <common>\nvarying float vLeafCard;')
        .replace('#include <normal_fragment_begin>', '#include <normal_fragment_begin>\nif(vLeafCard>.5 && !gl_FrontFacing) normal=-normal;');
    }
...
transformed.z += cos(uCanopyTime * 0.53 + aCanopyPhase * 1.37) * leafK * 0.009;`);
  ...
  m.customProgramCacheKey = () => `${key}-cinematic-canopy-v1${depth ? '-depth' : ''}${immersive ? '-immersive-leaves-v1' : ''}`;
  if (!depth) applyNightGroundReceiver(m, 'canopy', { heightM: 10, coolFill: 0.018 });
  if (!depth) applyDaylightSurface(m, 'canopy');
  return m;
}
...
export function createCinematicHomeMaterial(bend) {
  ...
  applyNightGroundReceiver(m, 'homes', { heightM: 3.5, coolFill: 0.015 });
  applyNearGroundMaterial(m, { surface: 'building' });
  applyDaylightSurface(m, 'building');
  return m;
}

/** The visible pool owns placement, support repairs and birth/grow matrices.
 * Its shadow uses those same instances and the identical rigid anchor bend;
 * no second pool can lag behind a heal, a warp or an origin rebase.
 */
export function createCinematicHomeDepthMaterial(bend) {
  const m = new MeshDepthMaterial({ depthPacking: RGBADepthPacking });
  bend(m);
  const key = m.customProgramCacheKey();
  m.customProgramCacheKey = () => `${key}-cinematic-parcel-depth-v1`;
  return m;
}
