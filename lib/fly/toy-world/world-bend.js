...
function hazeC(c) {
  return LINEAR_HAZE.enabled ? srgbToLinear(c) : c;
}
...
export function applyHillshade(material, cfg) {
  ...
  // R24 C: the FINAL TILE key moves ONLY when a C flag actually injects new
  // GLSL, derived here from the flags so a flag-off tree keeps the R19 key AND
  // the R19 text — the world-bend idiom (shared rules, cache-key section).
  material.customProgramCacheKey = () =>
    hillElevOn() ? 'world-bend-fade-hill-r19-c24' : 'world-bend-fade-hill-r19';
  material.needsUpdate = true;
}
