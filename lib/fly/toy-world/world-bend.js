 ...
 */
export function applyHillshade(material, cfg, lodFade) {
  ...
  // Round 24 (D): '-r19' → '-r19-d24' when (and ONLY when) the LOD crossfade
  // slot is present — the map-chunk blend is generated GLSL and a second
  // sampler, so it is a program the R19 key must not be served for. With
  // LOD_CROSSFADE off the ternary yields the R19 key and the R19 text.
  material.customProgramCacheKey = () =>
    lod ? 'world-bend-fade-hill-r19-d24' : 'world-bend-fade-hill-r19';
  material.needsUpdate = true;
}
