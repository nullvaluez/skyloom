...
uniform float uNCTint;${satBldgHashGLSL}`
    ...
    if (r23) {
      ...
      //   (a) a whole-CELL uv phase, which is what keeps the R15 roof invariant
      //       EXACT. Roof/detail verts all carry the constant neutralUV 0.25,
      //       and 0.25 × 8 cells = 2.0 is a cell BOUNDARY (solid background in
      //       both atlas paintings); boundary + k cells is still a boundary, so
      //       roofs still sample background at mip 0 and never grow windows.
      //       uNCPhase is deliberately a SWITCH, not a ramp — a fractional phase
      //       would land the roof texel inside a lit pane.
      //   (b) a per-building brightness multiplier, (c) a hash-stable DARK share
      //       (the office block at 2 a.m.), (d) a warm↔cool tint jitter so one
      //       block reads sodium and its neighbour fluorescent.
  ...
  vec4 emissiveColor = texture2D( emissiveMap, wbUv );
  float wbG = mix( uNCGain.x, uNCGain.y, wbH3 );
  if ( wbH1 < uNCDark.x ) wbG = uNCDark.y;
  float wbT = uNCTint * ( wbH2 - 0.5 );
  totalEmissiveRadiance *= emissiveColor.rgb * wbG * vec3( 1.0 + wbT, 1.0, 1.0 - wbT );
#endif
`
      );
    }
...
export function setNightCityRoads(cfg) {
  ...
  if (cfg.lamp) {
    U.uRoadLamp.value.x = cfg.lamp.gain;
    U.uRoadLamp.value.y = cfg.lamp.sharp;
  }
  const t = cfg.traffic;
  if (t) {
    U.uTrafDash.value.x = t.dashLenM;
    U.uTrafDash.value.y = t.dashDuty;
    U.uTrafDash.value.z = t.speed;
    if (typeof t.head === 'string') _hexToVec3(t.head, U.uTrafHead.value);
    if (typeof t.tail === 'string') _hexToVec3(t.tail, U.uTrafTail.value);
    U.uTrafBoost.value = t.boost;
    U.uLaneK.value = t.lanes;
  }
}
