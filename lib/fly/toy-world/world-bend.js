 ...
 *   'world-bend-anchor-satbldg-r16' (round 13, P3; -r16 in round 16 A2)
 ...
 *   'world-bend-anchor-satskyline-r18' (round 18, A2) applyBendAnchorSatSkyline
 ...
 *                            moves. FINAL SAT-SKYLINE key.
 *   'world-bend-road-satnight-r16' (round 16, A2) applyBendRoadSat — SATELLITE
...
export function applyBendAnchorSat(material) {
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace(
        '#include <common>',
        `#include <common>\nuniform float uSatBldgFade;${bayerDitherGLSL}`
      )
      .replace(
  ...
  material.customProgramCacheKey = () => 'world-bend-anchor-satbldg-r16';
  material.needsUpdate = true;
}
...
export function applyBendAnchorSatSkyline(material) {
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace(
        '#include <common>',
        `#include <common>\nvarying float vSkyDist;\nuniform vec2 uSkyHole;\nuniform float uSkyFade;${bayerDitherGLSL}`
      )
      .replace(
        '#include <clipping_planes_fragment>',
        '#include <clipping_planes_fragment>\n' +
  ...
  material.customProgramCacheKey = () => 'world-bend-anchor-satskyline-r18';
  material.needsUpdate = true;
}
 ...
 */
export function applyBendRoadSat(material, cfg) {
  if (!material || material.userData.__worldBend) return;
        ...
        `#include <color_fragment>
{
  ...
  diffuseColor.rgb *= rw * gain;
}`
      );
  };
  material.customProgramCacheKey = () => 'world-bend-road-satnight-r16';
  material.needsUpdate = true;
}
