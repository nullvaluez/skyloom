 ...
 *                            (albedo *= clamp((N.L+a)/(up.L+a), lo, hi) x the
 *                            Classic flat-ground hill value; direct diffuse on
 *                            the FLAT normal so relief enters once), c = z11
...
function r25PatchTile(shader, t, lod) {
  ...
  if (t.s) {
    const a = fs.indexOf('  float slope = 1.0 - clamp( vUp, 0.0, 1.0 );\n');
    const endKey = '  diffuseColor.rgb = mix( diffuseColor.rgb, shaded, uHillStrength';
    const b = a < 0 ? -1 : fs.indexOf(endKey, a);
    const e = b < 0 ? -1 : fs.indexOf('\n', b);
    if (e < 0) {
      r25PatchStats.misses++;
      r25PatchStats.lastMiss = 'fs-oneSun';
    } else fs = fs.slice(0, a) + r25OneSunGLSL() + fs.slice(e + 1);
    fs = r25Replace(
      fs,
...
/** The ONE bounded sun term (replaces the Classic lit / slope-AO / slope-sat
 * body). On flat ground it is EXACTLY the Classic flat-ground hill value, so
 * the D-only luminance is neutral by construction; relief enters once. */
function r25OneSunGLSL() {
  ...
  return (
    ...
    '    float r25Up = max( r25L.y, 0.0 );\n' +
    '    float r25K = clamp( ( dot( nW, r25L ) + uR25OneSun.x ) / ( r25Up + uR25OneSun.x ), uR25OneSun.y, uR25OneSun.z );\n' +
    `    float r25W = uHillStrength${elev};\n` +
    '    float r25Flat = 1.0 + ( uHillAmbient + ( 1.0 + uHillLift - uHillAmbient ) * clamp( r25L.y, 0.0, 1.0 ) - 1.0 ) * r25W;\n' +
    `    diffuseColor.rgb *= r25Flat * mix( 1.0, r25K, clamp( r25W / ${r25f(o.fullStrength)}, 0.0, 1.0 ) );\n` +
    '  }\n'
  );
}
