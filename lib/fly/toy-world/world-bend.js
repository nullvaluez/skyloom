 ...
 * commit.
 *   ── PRE-EXISTING KEYS REGISTERED AT R25 W2 (Fable). E CERT's
 *      verify-registry-inventory swept every customProgramCacheKey the tree
 *      can emit and found ELEVEN that this header never listed — ten from the
 *      Codex satellite-graphics overhaul (be711f2 … f0cd81e), which kept its
 *      knobs outside this file, and one R17-era player key. None is new text
 *      this round; they are inventoried here so the header is again the whole
 *      truth, and the gate now refuses any key it cannot find here.
 *   '|cinematic-architecture-v3-' (Codex overhaul) satellite-architecture-
 *                            material.js — the building material's key BASE;
 *                            the near/far coverage arm and the surface variant
 *                            below are appended to it.
 *   '|immersive-surface-v1'  (Codex overhaul) satellite-architecture-material
 *                            — the procedural facade surface variant. B NIGHT's
 *                            reserved '-v2' below supersedes it when armed.
 *   '-cinematic-canopy-v1'   (Codex overhaul) lib/fly/cinematic-ground.js — the
 *                            canopy (tree) material, derived from the bend key.
 *   '-immersive-leaves-v1'   (Codex overhaul) cinematic-ground.js — the
 *                            immersive leaf-atlas variant suffix on the canopy.
 *   '-cinematic-parcel-v1'   (Codex overhaul) cinematic-ground.js — the
 *                            parcel-home material (emissive window atlas).
 *   '-cinematic-model-v1'    (Codex overhaul) lib/fly/cinematic-models.js —
 *                            the monument / player model material.
 *   '-merged'                (Codex overhaul) cinematic-models.js — the
 *                            merged-geometry suffix on the model key.
 *   'world-bend-road-satnight-cinematic-v1' (Codex overhaul) applyBendRoadSat
 *                            below — the cinematic road branch (lane gaussians).
 *   'world-bend-water-cinematic-v1' (Codex overhaul) lib/fly/satellite-water.js
 *                            SATELLITE_WATER_KEY — the bounded sky/sun/moon
 *                            water approximation.
 *   '|immersive-landcover-v1' (Codex overhaul) components/fly/SatTintLayer.jsx
 *                            — the SatTint drape with the immersive sine grain.
 *   'player-hull-rim'        (R17) lib/fly/prewarm.js + PlayerPlane.jsx — the
 *                            player hull's clearcoat/fresnel rim material.
 *   ── R25 "GROUND & NIGHT" INVENTORY STUBS (Fable, W0). Each entry below is
 *      RESERVED for the owner named; the owner fills in the description in
 *      the same change that ships the shader text, and verify-registry-
 *      inventory (E) checks that every customProgramCacheKey literal in the
 *      tree appears here or is listed as "(no key)". None of these keys exists
 *      in the tree yet; every one is gated by its owner's pre-seeded block and
 *      the flag-off key set is exactly the R24 set above.
 *   hill token 'd'          (A GROUND, GROUND_DETAIL_R25.overlay) — BUILT, R25
 *                            W1. The fifth token on hillKey, in fixed order
 *                            after R24's 'e' 'f' 'a' 'l', so the FINAL TILE key
 *                            with the block off is character-for-character the
 *                            R24 one and nothing recompiles. ARMED it adds, at
 *                            the END of the <color_fragment> replacement (after
 *                            SAT_QUILT, before B's 'n'):
 *                              (a) two live uniforms, uGroundDetail (= the
 *                                  ground-bubble k x strengthByTier, written
 *                                  per frame by setGroundDetail) and uGroundAmp
 *                                  (lumaAmp, normalAmp — live so the LOOK can
 *                                  be swept in a console on the one machine
 *                                  that can judge it);
 *                              (b) two octaves of the EXISTING hillVNoise on
 *                                  vWorldXZ at 1.4 m / 0.45 m cells, each
 *                                  fwidth-faded against R24 C's own cells/px
 *                                  thresholds (microFadeLo/Hi) — per OCTAVE,
 *                                  because the two reach the sub-2px limit at
 *                                  very different altitudes;
 *                              (c) an imagery CLASSIFIER (hue/sat/luma of the
 *                                  sampled pixel) weighting grass / asphalt /
 *                                  soil differently;
 *                              (d) a fine luma grain, and an analytic normal
 *                                  perturbation (finite differences of the
 *                                  COARSE octave only) fed through the SAME
 *                                  N.L the fragment computes from vHillNW —
 *                                  applied as the DIFFERENCE of the perturbed
 *                                  and unperturbed shade, enveloped by
 *                                  uHillStrength * uHillElev so it is exactly
 *                                  0 off satellite and exactly as strong as the
 *                                  hillshade it belongs to.
 *                            0 draws, 0 geometry, 0 texture bytes. The warm set
 *                            needs no entry: prewarm builds the tile material
 *                            through applyHillshade itself (prewarm.js:380-388),
 *                            so the armed program is the one it compiles.
 *   hill token 'n'          (B NIGHT, NIGHT_GROUND_R25.receivers.terrain) —
 *                            the sixth token, after A's 'd': the night ground
 *                            irradiance sample (uNGMap at world XZ → UV).
 *   'world-bend-anchor-scrub-r25' (A GROUND, conditional) — NOT TAKEN, R25 W1.
 *                            The reserved key was for a per-instance sway
 *                            attribute on the scrub cards. Sway was NOT built:
 *                            it needs a new instanced attribute (a new warm-set
 *                            entry and a new program) to animate a 0.6-1.4 m
 *                            card that is under two pixels above ~200 m AGL,
 *                            and the round's ask is "the ground has detail",
 *                            not "the grass moves". SatGroundDetailLayer's two
 *                            instancers therefore share the EXISTING
 *                            'world-bend-anchor-r8' variant (applyBendAnchor)
 *                            verbatim — no new shader text, no key move. The
 *                            stub stays reserved for whoever builds sway.
 *   'night-ground-splat-v1'  (B NIGHT) — the instanced additive splat quads
 *                            rendered INTO the irradiance target (private
 *                            scene; never in the main frame).
 *   'night-ground-blur-v1'   (B NIGHT) — the separable blur over the target.
 *   'clutter-night-ground-v1' (B NIGHT) — SatClutterLayer's Lambert with the
 *                            irradiance sample (onBeforeCompile).
 *   '…|immersive-surface-v2' (B NIGHT) — satellite-architecture-material's key
 *                            moves ONCE for both the irradiance sample and the
 *                            three window-variance uniforms.
 *   '…|night-ground-v1'      (B NIGHT) — the canopy and parcel-home materials
 *                            in cinematic-ground.js, each with the sample.
 *   (no key) SkyDome         (B NIGHT, NIGHT_GROUND_R25.halo) — the light-
 *                            pollution term and setSkyMoonPhase; SkyDome is a
 *                            ShaderMaterial with no registry key, listed here
 *                            like the shadow kernel so the inventory is whole.
 *   (no key) C LIGHT         — LIGHT_BUBBLE_R25 moves NO shader text: the
 *                            shadow radius, AO radius, moon terms, hemisphere
 *                            colours and grade are uniform/property writes.
 *   (no key) F FEEL          — the 'sat-dof' pass descriptor lives in
 *                            Effects.jsx's pass list (postprocessing keys its
 *                            own programs); no world-bend text moves.
 */
...
// R25 A (GROUND_DETAIL_R25) — the ONE predicate + policy accessor for the
// low-AGL detail overlay. It imports nothing from this file (no cycle); the
// 'd' GLSL below, its uniform declarations and the FINAL hill key are all
// derived from `groundOverlayOn()`, so they cannot disagree.
import {
  groundDetailCfg,
  groundDetailLive,
  groundOverlayOn,
  installGroundDetailHandle,
  resolveGroundDetail,
} from '@/lib/fly/ground-detail';
...
const hillUniforms = {
  ...
  uHillElev: { value: 1 },
  // R25 A (GROUND_DETAIL_R25.overlay) — the low-AGL DETAIL overlay's two live
  // uniforms. `uGroundDetail` is `bubble k × strengthByTier`, written every
  // frame by setGroundDetail from FlyScene's −50 block: exactly 0 outside
  // satellite, at low tier, above the bubble, and whenever the flag is off, and
  // the whole block is skipped by its own `> 0.001` branch there. `uGroundAmp`
  // carries (lumaAmp, normalAmp) LIVE rather than as generated literals for one
  // reason: the LOOK of this term is a user checkpoint on a machine this
  // container cannot be, and two numbers a person can sweep in a console is a
  // measurement protocol where "edit a constant and rebuild" is not.
  // (the duck-typed vec2 idiom this file uses throughout — it imports no three)
  uGroundDetail: { value: 0 },
  uGroundAmp: { value: { x: 0.06, y: 0.35, isVector2: true } },
};
...
export function applyHillshade(material, cfg, lodFade = null) {
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    // R25 A (GROUND_DETAIL_R25.overlay): by REFERENCE, like every holder above
    // — one uniform object patched into every tile material, written once per
    // frame rather than once per material.
    if (groundOverlayOn()) {
      shader.uniforms.uGroundDetail = hillUniforms.uGroundDetail;
      shader.uniforms.uGroundAmp = hillUniforms.uGroundAmp;
    }
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace(
        ...
        '#include <common>\n' +
          ...
          // R25 A (GROUND_DETAIL_R25.overlay) — declared only when armed, so
          // the flag-off fragment string is byte-for-byte the R24 one.
          (groundOverlayOn() ? 'uniform float uGroundDetail;\nuniform vec2 uGroundAmp;\n' : '') +
      ...
      .replace(
        ...
        '#include <color_fragment>\n' +
          ...
          '}' +
          // R25 A (GROUND_DETAIL_R25.overlay) — the 'd' block. Empty string
          // when the sub-switch is off, which is what keeps the flag-off
          // fragment byte-for-byte the R24 one.
          groundDetailGLSL() +
          // ▼▼ R25 SEAM — B NIGHT's 'n' block (the night-ground irradiance
          // sample, NIGHT_GROUND_R25.receivers.terrain) GOES HERE, after A's
          // 'd' and last in this replacement, per plan §4 ("applyHillshade
          // GLSL order = A 'd' then B 'n'"). Append, do not interleave: A's
          // term ends with a write to diffuseColor.rgb and B's reads it.
          ''
      );
...
/**
 * ROUND 25 (A GROUND) — THE 'd' BLOCK: a bubble of procedural ground detail.
 *
 * THE DEFECT (recon G1–G4, measured): every AGL band in this game is a CULL
 * band — the micro grain fades OUT below 1.5 km, the poles cull at 900 m, the
 * movers at 1200 — so nothing whatever turns ON as you descend, and the finest
 * thing in the world is an 8.5 m lamp post. At 50–500 ft the tile material is a
 * `MeshStandardMaterial` carrying ONLY `map`: no normal map, no detail map, one
 * ±10 % luma grain at a 5.5 m cell which R24 then correctly `fwidth`-faded out
 * at exactly the grazing angles a low pass is made of. The ground is a photo.
 *
 * WHAT THIS IS. Two octaves of the hillshade's OWN value noise (`hillVNoise`,
 * already compiled into this fragment) on `vWorldXZ` at metre scale, giving
 *   (a) a fine LUMA grain, and
 *   (b) an analytic NORMAL perturbation — finite differences of the coarse
 *       octave — fed through the same N·L the fragment already computes from
 *       `vHillNW`, so the relief moves with the key light instead of being a
 *       painted-on texture that stays put when the sun does not.
 * Zero draws, zero geometry, zero texture bytes: it is more ALU on the tile
 * fragment and nothing else.
 *
 * CLASSED BY THE IMAGERY, because the ground is not one material. Esri's own
 * pixel says what is underfoot better than any tag we have at this scale: a
 * green, saturated pixel is vegetation and takes the full fine high-amplitude
 * grain; a desaturated mid-luma pixel is asphalt/concrete and takes a smooth,
 * sparse speckle (a parking lot with grass texture on it is worse than a flat
 * parking lot); everything else is soil/roof/mixed and takes the medium term.
 * The thresholds are `GROUND_DETAIL_R25.overlay.classify` and they are the
 * knobs the user's §6 checkpoint sweeps.
 *
 * WHY THE NORMAL TERM IS AN ADDITIVE DELTA rather than a change to the
 * hillshade's own `lit`. The hillshade block above has already resolved and
 * mixed; re-deriving its exact multiplicative form here would mean reproducing
 * `mix(rgb, rgb·lit·ao, w)` to divide it out. Instead the perturbed and
 * unperturbed N·L are both evaluated and only their DIFFERENCE is applied,
 * enveloped by the same `uHillStrength · uHillElev` the hillshade itself uses —
 * so the relief is exactly as strong as the shading it belongs to, and it is
 * exactly 0 off satellite where that envelope is 0.
 *
 * ANTI-ALIASING, PER OCTAVE. R24's lesson (recon T13) was that a procedural
 * cell smaller than ~2 px aliases into the crawling shimmer the user reported
 * as tearing, and that SMAA — a geometric-edge filter — cannot touch it. A
 * 1.4 m and a 0.45 m octave reach that limit at very different altitudes, so
 * each is faded by ITS OWN `fwidth` against the SAME calibrated cells/px
 * thresholds rather than by one shared attenuation. Consequence worth stating:
 * the 0.45 m octave is fully gone by ~500 m AGL on its own, before the bubble
 * retires it — the term self-retires with altitude and the bubble is the belt.
 *
 * NOT A NORMAL MAP, and not a detail map: both are texture bytes against a
 * 300 MB ceiling and a `map`-only vendored tile material, and both would need a
 * second UV set to tile at metre scale over a tile that is kilometres wide.
 */
function groundDetailGLSL() {
  if (!groundOverlayOn()) return '';
  const O = groundDetailCfg().overlay ?? {};
  const num = (v, d) => (Number.isFinite(Number(v)) ? Number(v) : d);
  const f = (v, d) => num(v, d).toFixed(5);
  // Cell PERIODS in metres -> the reciprocal is what multiplies world XZ.
  const inv0 = (1 / Math.max(1e-3, num(O.scaleM?.[0], 1.4))).toFixed(6);
  const inv1 = (1 / Math.max(1e-3, num(O.scaleM?.[1], 0.45))).toFixed(6);
  const C = O.classify ?? {};
  const hLo = f(C.grassHue?.[0], 70);
  const hHi = f(C.grassHue?.[1], 160);
  const sMin = f(C.grassSatMin, 0.12);
  const sMax = f(C.asphaltSatMax, 0.08);
  const lLo = f(C.asphaltLuma?.[0], 0.18);
  const lHi = f(C.asphaltLuma?.[1], 0.55);
  // The SAME cells/px thresholds R24 C calibrated for the 5.5 m grain: the
  // quantity is cells per pixel, which is scale-free, so one calibration covers
  // every octave (that is the point of measuring it in cells rather than
  // metres).
  const fadeLo = f(TERRAIN_LIGHT.microFadeLo, 0.35);
  const fadeHi = f(TERRAIN_LIGHT.microFadeHi, 0.9);
  // The hillshade's own envelope, spelled exactly as the block above spells it.
  const env = hillElevOn() ? 'uHillStrength * uHillElev' : 'uHillStrength';
  const relief = hillFragOn()
    ? '  vec3 gdNW = normalize( vHillNW );\n' +
      '  float gdGx = hillVNoise( gdP0 + vec2( 0.5, 0.0 ) ) - gdC;\n' +
      '  float gdGz = hillVNoise( gdP0 + vec2( 0.0, 0.5 ) ) - gdC;\n' +
      '  vec3 gdPN = normalize( gdNW + vec3( -gdGx, 0.0, -gdGz ) * uGroundAmp.y * gdNrmK * gdA0 * uGroundDetail );\n' +
      '  float gdRange = 1.0 + uHillLift - uHillAmbient;\n' +
      '  float gdL0 = clamp( dot( gdNW, normalize( uHillDir ) ), 0.0, 1.0 );\n' +
      '  float gdL1 = clamp( dot( gdPN, normalize( uHillDir ) ), 0.0, 1.0 );\n' +
      `  diffuseColor.rgb = max( vec3( 0.0 ), diffuseColor.rgb * ( 1.0 + ( gdL1 - gdL0 ) * gdRange * ${env} ) );\n`
    : // TERRAIN_LIGHT.fragmentHill off ⇒ there is no world normal in this
      // fragment to perturb (R21 interpolated the DOT), so the overlay ships
      // its luma half alone rather than inventing a normal. It ships true.
      '';
  return (
    '\n// Round 25 (A GROUND) — GROUND_DETAIL_R25.overlay: the low-AGL detail\n' +
    '// bubble. uGroundDetail = bubble k x strengthByTier, 0 everywhere else,\n' +
    '// so this whole block is skipped by its own branch off satellite, at low\n' +
    '// tier, above the bubble and on every toy tile.\n' +
    'if ( uGroundDetail > 0.001 ) {\n' +
    `  vec2 gdP0 = vWorldXZ * ${inv0};\n` +
    `  vec2 gdP1 = vWorldXZ * ${inv1};\n` +
    `  float gdA0 = 1.0 - smoothstep( ${fadeLo}, ${fadeHi}, max( fwidth( gdP0.x ), fwidth( gdP0.y ) ) );\n` +
    `  float gdA1 = 1.0 - smoothstep( ${fadeLo}, ${fadeHi}, max( fwidth( gdP1.x ), fwidth( gdP1.y ) ) );\n` +
    '  float gdC = hillVNoise( gdP0 );\n' +
    '  float gdN = ( gdC - 0.5 ) * 0.65 * gdA0 + ( hillVNoise( gdP1 ) - 0.5 ) * 0.35 * gdA1;\n' +
    '  vec3 gdCol = diffuseColor.rgb;\n' +
    '  float gdMx = max( gdCol.r, max( gdCol.g, gdCol.b ) );\n' +
    '  float gdMn = min( gdCol.r, min( gdCol.g, gdCol.b ) );\n' +
    '  float gdD = gdMx - gdMn;\n' +
    '  float gdSat = gdMx > 1e-5 ? gdD / gdMx : 0.0;\n' +
    '  float gdLum = dot( gdCol, vec3( 0.2126, 0.7152, 0.0722 ) );\n' +
    '  float gdHue = 0.0;\n' +
    '  if ( gdD > 1e-5 ) {\n' +
    '    if ( gdMx == gdCol.r ) gdHue = mod( ( gdCol.g - gdCol.b ) / gdD, 6.0 );\n' +
    '    else if ( gdMx == gdCol.g ) gdHue = ( gdCol.b - gdCol.r ) / gdD + 2.0;\n' +
    '    else gdHue = ( gdCol.r - gdCol.g ) / gdD + 4.0;\n' +
    '    gdHue *= 60.0;\n' +
    '  }\n' +
    `  float gdGrass = step( ${hLo}, gdHue ) * step( gdHue, ${hHi} ) * step( ${sMin}, gdSat );\n` +
    `  float gdAsph = step( gdSat, ${sMax} ) * step( ${lLo}, gdLum ) * step( gdLum, ${lHi} );\n` +
    '  float gdSoil = clamp( 1.0 - gdGrass - gdAsph, 0.0, 1.0 );\n' +
    '  float gdLumaK = gdGrass + gdAsph * 0.35 + gdSoil * 0.70;\n' +
    '  float gdNrmK = gdGrass + gdAsph * 0.25 + gdSoil * 0.60;\n' +
    '  diffuseColor.rgb = max( vec3( 0.0 ), diffuseColor.rgb * ( 1.0 + gdN * 2.0 * uGroundAmp.x * gdLumaK * uGroundDetail ) );\n' +
    relief +
    '}'
  );
}
...
function hillKey(lodFade) {
  return r24VariantKey('world-bend-fade-hill-r19', [
    ...
    // R25 A -- the low-AGL procedural detail overlay. APPENDED after R24's
    // four, so with GROUND_DETAIL_R25 off the FINAL key is character-for-
    // character the R24 one and no tile recompiles. Read through the same
    // predicate the GLSL is (groundOverlayOn), never off the constant.
    [groundOverlayOn(), 'd'],
    // ▼ R25 SEAM: B NIGHT's 'n' goes HERE, after A's 'd' (plan §4).
  ]);
}
...
/**
 * ROUND 25 (A GROUND) — the detail overlay's live drive. The `setMicroDetail`
 * idiom one line below it in FlyScene's −50 block, with three differences that
 * are all deliberate:
 *
 *  - it takes the RAW inputs (bubble k, tier, style) rather than a resolved
 *    scalar, so FlyScene's permitted touch is exactly ONE line and the policy
 *    (tier ladder, the dev k pin, the style gate) lives with the rest of the
 *    R25 policy in lib/fly/ground-detail.js;
 *  - it also refreshes the two LIVE amplitudes from the pinned block, so
 *    `window.__flyGroundDetailOverride = { overlay: { …, lumaAmp: 0.12 } }`
 *    set in a console takes effect on the next frame with no rebuild — the
 *    sweep protocol for a look that can only be judged on the user's machine;
 *  - it installs the `window.__flyGroundDetail` handle (dev only) from here,
 *    because this is the one call site that runs every frame in BOTH styles,
 *    so `read()` answers before any layer has mounted.
 *
 * 0 is the identity: the branch in the fragment is skipped, and with the block
 * disabled the branch does not exist at all.
 */
export function setGroundDetail(k, tier, isSatellite) {
  const s = resolveGroundDetail(k, tier, isSatellite);
  hillUniforms.uGroundDetail.value = s;
  if (s > 0) {
    const O = groundDetailCfg().overlay ?? {};
    hillUniforms.uGroundAmp.value.x = Number.isFinite(O.lumaAmp) ? O.lumaAmp : 0.06;
    hillUniforms.uGroundAmp.value.y = Number.isFinite(O.normalAmp) ? O.normalAmp : 0.35;
  }
  if (process.env.NODE_ENV === 'development') {
    installGroundDetailHandle();
    // The FINAL TILE key as this frame's predicates resolve it. Published from
    // HERE because this file owns both the key expression and the predicate,
    // and a gate that reads the key off a live material instead would be
    // reading whichever material it happened to find.
    groundDetailLive().hillKey = hillKey(null);
  }
}

/** Dev/harness introspection — the live overlay uniform (verify-ground-bubble). */
export function getGroundDetail() {
  return {
    strength: hillUniforms.uGroundDetail.value,
    lumaAmp: hillUniforms.uGroundAmp.value.x,
    normalAmp: hillUniforms.uGroundAmp.value.y,
    key: hillKey(null),
  };
}
