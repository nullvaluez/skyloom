 ...
 *
 * Ground immersion composition (September 2026): the tile caller and its
 * prewarm twin append '-near-ground-terrain-v1' then '|ground-light-v2-terrain'
 * to the FINAL hillshade key. Cinematic receiver factories append their
 * ground-light kind; scrub uses 'world-bend-anchor-r8-ground-scrub-v1' and its
 * '-depth' variant. These outer hooks MUST follow bend/animation hooks; they
 * capture unbent source coordinates and never replace a predecessor's key.
 * Daylight surfaces append '-daylight-<kind>-v1'. That outer hook also fixes
 * receiver shadow coordinates to bent wPos only during shadow projection.
 * Parcel shadow depth is 'world-bend-anchor-r8-cinematic-parcel-depth-v1';
 * its prewarm twin uses the same instance matrices and bend as visible homes.
 *
 ...
 *   'world-bend-air|optical-wake-v1' — aircraft-wake wraps the air patch
 *                            with feathered optical density and alpha edge fade.
 ...
 *   'world-bend-air-tracer-v1' applyBendAirTracer — the satellite SPOTTER
 *                            TRAILS program (TrafficTracers' SpotTracers): the
 *                            verbatim airProject + a post-projection pixel
 *                            glint, premultiplied vapour + additive emission,
 *                            a tracer-owned range band and the overlay cloud
 *                            gate. Starts with 'world-bend-air' on purpose
 *                            (depth-probe isAirBent, verify-c-flagoff).
 *   '<prev>|ovgate-v1'       applyOverlayCloudGate — alpha × the overlay cloud
 *                            gate (lib/fly/overlay-gate.js), identity outside
 *                            SkyOverlayPass. FINAL keys:
 *                            'world-bend-air-anchor|ovgate-v1' (far billboards)
 *                            and 'world-bend-air|ovgate-v1|optical-wake-v1'
 *                            (traffic plumes + player contrail).
 ...
 *   'world-bend-anchor-satbldg-r23' (round 23, B CITY-LIGHT) — the SAME variant
 *                            with NIGHT_CITY_R23.windows ARMED. It is a
 *                            SEPARATE key rather than a bump because the block
 *                            ships OFF: disarmed, applyBendAnchorSat emits the
 *                            '-r19' strings byte-for-byte, so the shipped tree
 *                            keeps compiling the certified R19 program and
 *                            nothing in the fleet moves. Armed it adds ONE
 *                            varying (vSatAnchor — the bend's own aBendAnchor,
 *                            re-read, no new attribute) and replaces
 *                            <emissivemap_fragment> with a per-BUILDING version:
 *                            whole-CELL uv phase (de-repeat), brightness spread,
 *                            a hash-stable dark share and a warm↔cool tint
 *                            jitter, all off one "hash without sine" of the
 *                            quantised anchor. DAYLIGHT IS UNREACHABLE from it
 *                            by construction — the layer holds emissiveIntensity
 *                            at 0 whenever the sun is up, and every term here is
 *                            multiplied into that. Uniform identity (phase 0,
 *                            gain (1,1), dark (0,0), tint 0) reproduces the R15
 *                            atlas read exactly. Warm-set membership is
 *                            automatic: prewarm calls this same function.
 ...
 *   'world-bend-road-satnight-r23' (round 23, B CITY-LIGHT) — the SAME variant
 *                            with NIGHT_CITY_R23.roads ARMED. A SEPARATE key,
 *                            not a bump, for the same reason as the building
 *                            twin: the block ships OFF and the disarmed patch
 *                            emits the '-r19' strings byte-for-byte (which is
 *                            what verify-groundlife's frozen ROAD_CACHE_KEY
 *                            reads). Armed it adds (a) the ribbon-SIDE attribute
 *                            aRoadSide + varying — derived client-side by
 *                            SatRoadEngine from the worker's fixed
 *                            (a+n, a−n, b+n, b−n) quad order, so WORKER_PROTOCOL
 *                            stays 18 — (b) the two R16 lamp-pool LITERALS
 *                            promoted to uRoadLamp so the pool is sweepable, and
 *                            (c) term (H), the TWO-COLOUR TRAFFIC stream: warm
 *                            headlights running with the arc, red tails against
 *                            it, added as a COLOUR after the class-weighted gain
 *                            (every R16 term is a gain on the ribbon's own hue,
 *                            so no amount of tuning could make one). Zero draws,
 *                            zero geometry — it is one more fragment term on the
 *                            one road material. 0-identity: uTrafBoost 0 (branch
 *                            never runs) and uRoadLamp at the R16 literals.
 *                            Warm-set membership is automatic (prewarm calls the
 *                            same function with the same arm).
 ...
 *   '…-hill-r19…' + '-{n}{s}{c}{k}25' [+ '-r25g']  (round 25, D GROUND) — the
 *                            Visuals ENHANCED tile variant. One token per D
 *                            sub-flag, FIXED order: n = DEM relief normal map
 *                            (uR25Relief / uR25HasRelief per-material holder,
 *                            vertex fallback), s = ONE bounded sun term
 *                            (albedo *= clamp((N.L+a)/(up.L+a), lo, hi); the
 *                            Classic hill block keeps its slope AO/saturation
 *                            but its sun term reads flat ground, so flat ground
 *                            is EXACTLY Classic; direct diffuse on the FLAT
 *                            normal so the sun direction enters once), c = z11
 *                            colour-reference transfer in map_fragment (stock
 *                            AND LOD-crossfade paths), k = high-tier Catmull-Rom
 *                            magnification + mip-difference unsharp. '-r25g' =
 *                            the earth surface's groundValue literal becomes the
 *                            uR25GroundValue normaliser (s only, earth tiles
 *                            only). WHY THE TOKENS RIDE THE OUTERMOST HOOK
 *                            (lib/fly/r25-ground.js applyR25Terrain) AND NOT
 *                            hillKey(): every outer terrain hook (near-ground,
 *                            night receiver, daylight, earth) FREEZES its
 *                            predecessor's key STRING at apply time, while the
 *                            Visuals profile is LIVE. A hill-level token would
 *                            be baked into those strings by whichever profile
 *                            was current when the tile streamed, and a later
 *                            toggle would compile Classic text under an Enhanced
 *                            key (the R4 collision). `r25GroundKeySuffix()`
 *                            below is the ONE token source; the text in
 *                            applyHillshade / applyEarthSurface and the suffix
 *                            both read `r25GroundOn(sub)`. Classic / flag-off:
 *                            every token is empty, so the key and the text are
 *                            the r25-w0 ones byte for byte (verify-r25-flagoff
 *                            [2], verify-r25-ground). Toy compiles the same
 *                            program in Enhanced and stays pixel-identical: every
 *                            term is gated on uHillStrength > 0 (0 off-satellite)
 *                            or uR25Sat (0 off-satellite). PREWARM: the twin at
 *                            prewarm.js compiles whichever profile is live; the
 *                            Classic alternate is warmed lazily while Enhanced.
...
// Traffic trails: the cloud-slab occlusion shared by every deferred overlay.
import { OVERLAY_GATE_GLSL, bindOverlayGate } from '@/lib/fly/overlay-gate';
...
// --- Traffic trails (September 2026): spotter tracer + overlay cloud gate ----

/**
 * Checked text patch: every needle must be present exactly where expected.
 * A miss logs (dev) and returns null so the caller leaves the shader
 * UNPATCHED — never a throw mid-compile (the R24 un-asserted-replace lesson,
 * inverted: fail visible, not fatal).
 */
function patchText(src, pairs, label) {
  let out = src;
  for (const [needle, repl] of pairs) {
    if (!out.includes(needle)) {
      if (process.env.NODE_ENV !== 'production') console.warn(`[world-bend] ${label}: needle missing: ${needle}`);
      return null;
    }
...
function wireAirUniforms(u) {
  u.uBendCenter = uniforms.uBendCenter;
  u.uBendK = uniforms.uBendK;
  u.uEyeY = uniforms.uEyeY;
  u.uRefGroundY = uniforms.uRefGroundY;
  u.uAirAgl = uniforms.uAirAgl;
  u.uAirCapFrac = uniforms.uAirCapFrac;
  u.uAirLiftRange = uniforms.uAirLiftRange;
  u.uAirCapFar = uniforms.uAirCapFar;
  u.uEdgeFade = uniforms.uEdgeFade;
}

const AIR_DECL =
  'uniform vec2 uBendCenter;\nuniform float uBendK;\nuniform float uEyeY;\nuniform float uRefGroundY;\nuniform vec2 uAirAgl;\nuniform float uAirCapFrac;\nuniform vec2 uAirLiftRange;\nuniform float uAirCapFar;\nvarying float vAirDist;';

// Vertex tail after the verbatim airProject (wPos is bent, mvPosition and
// gl_Position are set). Glint vertices (aTracer.z = 1) are pulled toward the
// camera IN VIEW SPACE — same screen point, nearer depth, so the mark
// composites over its own hull — and then offset by an exact number of
// drawing-buffer pixels. The four verts share one w, so a head behind the
// camera clips the whole diamond; the axes are the screen's, never the
// trail's, so head-on traffic keeps its glint (exempt from the edge-on
// collapse by construction).
const TRACER_VERT_TAIL = /* glsl */ `
vTracer = aTracer;
vOvW = wPos.xyz;
vTrRel = wPos.y - uEyeY;
if ( aTracer.z > 0.5 ) {
  float gL = length( mvPosition.xyz );
  mvPosition.xyz *= max( gL - min( uTrGlint.z, uTrGlint.w * gL ), 1.0 ) / max( gL, 1.0 );
  gl_Position = projectionMatrix * mvPosition;
  float gR = uTrPx.x * mix( 1.0, uTrPx.w, smoothstep( uTrGlint.x, uTrGlint.y, gL ) );
  gl_Position.xy += aTracer.xy * gR * uTrPx.yz * gl_Position.w;
}
`;

const TRACER_FRAG_DECL = /* glsl */ `
varying float vAirDist; varying vec3 vTracer; varying vec3 vOvW; varying float vTrRel;
uniform vec2 uEdgeFade;
uniform vec4 uTrFade;   // x,y sky-backed band (world m); z,w ground-backed ramp on (y - eye)
uniform vec4 uTrVapor;  // rgb vapour colour (linear), w luminance cap
uniform vec4 uTrSun;    // xyz sun direction, w forward-sheen gain
uniform vec4 uTrShape;  // x profile edge, y core k, z halo, w spike gain
uniform float uTrMarkFloor;
`;

// Vertex colour: rgb = ADDITIVE light (emission / glint tint × gain), a =
// VAPOUR opacity. The material blends premultiplied (ONE, ONE_MINUS_SRC_ALPHA)
// so alpha 0 is exactly additive (all of night, every glint).
const TRACER_FRAG_BODY = /* glsl */ `
vec3 trE = gl_FragColor.rgb;
float trA = gl_FragColor.a;
float trLum = dot( trE, vec3( 0.2126, 0.7152, 0.0722 ) );
float trLow = 1.0 - smoothstep( uTrFade.z, uTrFade.w, vTrRel );
vec2 trBand = mix( uTrFade.xy, uEdgeFade, trLow );
float trK = 1.0 - smoothstep( trBand.x, trBand.y, vAirDist );
float trGate = ovCloudGate( vOvW );
float trFw = fwidth( vTracer.x );
if ( vTracer.z > 0.5 ) {
  // A soft tinted dot with a slightly paler centre — deliberately NOT a star
  // (no white-hot core, spikes only if uTrShape.w > 0).
  vec2 q = vTracer.xy; float r2 = dot( q, q );
  float halo = max( 0.0, 1.0 - r2 ); halo *= halo;
  float core = exp( -6.0 * r2 );
  float spk = ( exp( -2.4 * abs( q.x ) - 70.0 * q.y * q.y ) + exp( -2.4 * abs( q.y ) - 70.0 * q.x * q.x ) ) * uTrShape.w;
  vec3 dotC = mix( trE, vec3( trLum ), 0.35 * core );
  gl_FragColor = vec4( dotC * ( core + uTrShape.z * halo ) + trE * spk, 0.0 ) * ( trK * max( trGate, uTrMarkFloor ) );
} else {
  float u = abs( vTracer.x );
  float wpx = 2.0 / max( trFw, 1e-4 );
  float prof = 1.0 - smoothstep( 2.0, 6.0, wpx ) * smoothstep( uTrShape.x, 1.0, u );
  float core = exp( -uTrShape.y * u * u ) * smoothstep( 3.0, 8.0, wpx );
  vec3 vc = uTrVapor.rgb * ( 1.0 + uTrSun.w * pow( max( dot( normalize( vOvW - cameraPosition ), uTrSun.xyz ), 0.0 ), 6.0 ) );
  vc *= min( 1.0, uTrVapor.w / max( dot( vc, vec3( 0.2126, 0.7152, 0.0722 ) ), 1e-4 ) );
  float a = trA * prof;
  gl_FragColor = vec4( vc * a + mix( trE, vec3( trLum ), 0.6 * core ) * prof, a ) * ( trK * trGate );
}
`;

/**
 * The satellite SPOTTER TRAILS program (lib/fly/tracer-spot.js builds the
 * material; TrafficTracers' SpotTracers drives it). `tu` are the per-program
 * uniforms (uTrPx, uTrGlint, uTrFade, uTrVapor, uTrSun, uTrShape,
 * uTrMarkFloor), shared by reference. The air projection is the module-local
 * airProject text VERBATIM, so the CPU mirror airDrop() — labels, reticle,
 * harness aim — stays exact for ribbon and glint alike. The GROUND rim band
 * (uEdgeFade) applies only to traffic well below the eye; sky-backed traffic
 * reads the tracer's own band (uTrFade.xy, the data radius).
 */
export function applyBendAirTracer(material, cfg, tu) {
  if (!material || material.userData.__worldBend) return;
  material.userData.__worldBend = 'air-tracer';
  seedAirCfg(cfg);
  material.onBeforeCompile = (shader) => {
    wireAirUniforms(shader.uniforms);
    for (const k in tu) shader.uniforms[k] = tu[k];
    bindOverlayGate(shader.uniforms);
    const vs = patchText(
      shader.vertexShader,
      [
        [
          '#include <common>',
          '#include <common>\n' + AIR_DECL + '\nattribute vec3 aTracer;\nuniform vec4 uTrPx;\nuniform vec4 uTrGlint;\nvarying vec3 vTracer;\nvarying vec3 vOvW;\nvarying float vTrRel;',
        ],
        ['#include <project_vertex>', airProject + TRACER_VERT_TAIL],
      ],
      'air-tracer vertex'
    );
    const fs = patchText(
      ...
      [
        ['#include <common>', '#include <common>\n' + TRACER_FRAG_DECL + OVERLAY_GATE_GLSL],
  ...
  material.customProgramCacheKey = () => 'world-bend-air-tracer-v1';
  material.needsUpdate = true;
}

/** Present exactly once in both airProject and airAnchorProject. */
export const OV_NEEDLE = 'vec4 mvPosition = viewMatrix * wPos;';

/**
 * Wrap an air- or air-anchor-patched overlay material with the overlay cloud
 * gate: alpha × max(gate, floor). Identity (× 1.0) wherever SkyOverlayPass is
 * not drawing, including every toy frame. Must be applied AFTER the air patch
 * (its onBeforeCompile runs the predecessor first, which inserts OV_NEEDLE).
 */
export function applyOverlayCloudGate(material, { floor = 0 } = {}) {
  if (!material || material.userData.__ovGate) return;
  material.userData.__ovGate = true;
  const prev = material.onBeforeCompile;
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    const fs = patchText(
      shader.fragmentShader,
      [
        ['#include <common>', '#include <common>\nvarying vec3 vOvW;\nuniform float uOvFloor;\n' + OVERLAY_GATE_GLSL],
        ['#include <fog_fragment>', '#include <fog_fragment>\ngl_FragColor.a *= max( ovCloudGate( vOvW ), uOvFloor );'],
      ],
      'overlay-gate fragment'
    );
    if (!vs || !fs) return;
    bindOverlayGate(shader.uniforms);
    shader.uniforms.uOvFloor = floorU;
    shader.vertexShader = vs;
    shader.fragmentShader = fs;
  };
  material.customProgramCacheKey = () => `${key}|ovgate-v1`;
  material.needsUpdate = true;
}
...
export function applyHillshade(material, cfg, lodFade = null) {
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    // R25 D GROUND (Visuals ENHANCED). Read at COMPILE time — the profile is
    // live and a toggle re-compiles through needsUpdate. Everything above this
    // line is the Classic program, untouched; with the flag off, the profile
    // Classic, or no applyR25Terrain holder, `r25` is null and nothing below
    // runs, so the text is byte-identical to r25-w0 (verify-r25-flagoff [2]).
    const r25 = r25GroundText(material);
    if (r25) r25PatchTile(shader, r25, lod);
  };
...
// ===========================================================================
// R25 D GROUND — the Visuals ENHANCED terrain variant (see the registry entry
// at the top of this file). EVERY decision below reads `r25GroundOn(sub)`, so
// the injected text and `r25GroundKeySuffix()` can never disagree, and with
// the flag off or the profile Classic nothing here emits a byte.
// ===========================================================================

/** Shared (all tiles) R25 uniforms. Written per frame by lib/fly/r25-ground.js
 * (Enhanced only); the resting values are the identity. Samplers start null —
 * three binds its empty texture for a null sampler value. */
export const r25Uniforms = {
  uR25Sat: { value: 0 }, // 1 = satellite (map terms run); 0 = toy/off (exact identity)
  uR25Sharp: { value: 0 }, // unsharp k on the high tier; 0 = off
  uR25OneSun: { value: { x: 0.2, y: 0.6, z: 1.45, isVector3: true } }, // a, lo, hi
  uR25Ref: { value: null }, // the rolling z11 colour-reference atlas
  uR25RefWin: { value: { x: 0, y: 0, z: 8, w: 0.5, isVector4: true } }, // x0, y0, span, edge fade
  uR25RefGain: { value: 0 }, // 0 until the atlas holds data
  uR25GroundValue: { value: 0.7 }, // the earth surface's ground value (normaliser)
};

// The Visuals predicate — R25_GROUND on, the sub-flag on, the live profile
// 'enhanced' (visuals-profile.js r25On) — is INSTALLED by lib/fly/r25-ground.js
// rather than imported here: world-bend stays outside the profile / zustand
// module graph, which several R24 node gates load it without (they resolve
// only this file's own imports). Nothing installed = Classic = flag-off: the
// app always imports r25-ground.js (FlyScene, prewarm.js) before any material
// can compile, so the installed predicate is what every live compile reads.
let _r25Pred = null;

/** r25-ground.js installs `(sub) => r25On(R25_GROUND, sub)`. */
export function setR25GroundPredicate(fn) {
  _r25Pred = typeof fn === 'function' ? fn : null;
}

// A compile-time override for the lazy Classic warm (r25-ground.js): while >0
// every predicate reads OFF, so the program compiled is exactly the Classic one
// and its key is exactly the Classic key (both read this same function).
let _r25ForceClassic = 0;

/** Run `fn` with every D ground predicate forced OFF (the Classic program). */
export function withR25GroundClassic(fn) {
  _r25ForceClassic++;
  try {
    return fn();
  } finally {
...
/**
 * THE D ground predicate: R25_GROUND on, the sub-flag on, profile Enhanced
 * (r25On), and — for the two terms that live inside C's fragment-stage hill
 * block — that block exists (TERRAIN_LIGHT.fragmentHill; it ships ON).
 */
export function r25GroundOn(sub) {
  if (_r25ForceClassic > 0 || !_r25Pred || !R25_GROUND.enabled) return false;
  if (!_r25Pred(sub)) return false;
  if ((sub === 'relief' || sub === 'oneSun') && !hillFragOn()) return false;
  return true;
}

/** Which Enhanced terms this material compiles, or null (Classic text). The
 * per-material holder is created by applyR25Terrain; a material that never
 * went through it keeps the Classic text AND the Classic key. */
function r25GroundText(material) {
  const h = material?.userData?.__r25Ground;
  if (!h) return null;
  const t = {
    h,
    n: r25GroundOn('relief'),
    s: r25GroundOn('oneSun'),
    c: r25GroundOn('colorRef'),
    k: r25GroundOn('sharpen'),
  };
  return t.n || t.s || t.c || t.k ? t : null;
}

/** The ONE token source for the Enhanced terrain key (appended LIVE by the
 * outermost hook, applyR25Terrain — see the registry entry for why). */
export function r25GroundKeySuffix(material) {
  const t = r25GroundText(material);
  if (!t) return '';
  const tok = (t.n ? 'n' : '') + (t.s ? 's' : '') + (t.c ? 'c' : '') + (t.k ? 'k' : '');
  return `-${tok}25` + (t.s && material.userData.earthSurface ? '-r25g' : '');
}

/** Does this material compile the earth surface's '-r25g' ground value slot? */
export function r25EarthGroundOn(material) {
  return !!material?.userData?.__r25Ground && r25GroundOn('oneSun');
}

const r25f = (v) => (Number.isInteger(v) ? v.toFixed(1) : String(v));

/** The relief normal decode. The map is CORNER-ALIGNED (texel i sits at
 * uv = i/(N-1), so a tile's edge texels are the physical edge samples its
 * same-LOD neighbour shares) and holds the WORLD-frame normal's x (east) and
 * z (south) in RG; y is reconstructed (terrain normals always face up). */
function r25ReliefGLSL() {
  const N = R25_GROUND.relief.mapPx;
  return (
    'uniform sampler2D uR25Relief;\nuniform float uR25HasRelief;\n' +
    'vec3 r25ReliefN( vec3 nV ) {\n' +
    '  if ( uR25HasRelief < 0.5 ) return nV;\n' +
    `  vec2 e = texture2D( uR25Relief, vR25Uv * ${r25f(1 - 1 / N)} + ${r25f(0.5 / N)} ).rg * 2.0 - 1.0;\n` +
    '  return normalize( vec3( e.x, sqrt( max( 0.0, 1.0 - dot( e, e ) ) ), e.y ) );\n' +
    '}\n'
  );
}

/** The map-chunk terms: Catmull-Rom (5 bilinear taps) when magnified plus the
 * mip-difference unsharp (k), and the z11 colour-reference ratio (c). The
 * sampler is a PARAMETER because `map` is declared after <common>. */
function r25MapGLSL(t) {
  let s = 'uniform float uR25Sat;\n';
  if (t.k) {
    s +=
      'uniform float uR25Sharp;\n' +
      'vec3 r25CatRom( sampler2D tex, vec2 uv, vec2 ts ) {\n' +
      '  vec2 sp = uv * ts;\n' +
      ...
      '  vec2 p0 = ( t1 - 1.0 ) / ts;\n' +
      '  vec2 p3 = ( t1 + 2.0 ) / ts;\n' +
      '  vec2 p12 = ( t1 + w2 / w12 ) / ts;\n' +
      '  vec3 r = textureLod( tex, vec2( p12.x, p0.y ), 0.0 ).rgb * ( w12.x * w0.y )\n' +
      '    + textureLod( tex, vec2( p0.x, p12.y ), 0.0 ).rgb * ( w0.x * w12.y )\n' +
      '    + textureLod( tex, p12, 0.0 ).rgb * ( w12.x * w12.y )\n' +
      '    + textureLod( tex, vec2( p3.x, p12.y ), 0.0 ).rgb * ( w3.x * w12.y )\n' +
      '    + textureLod( tex, vec2( p12.x, p3.y ), 0.0 ).rgb * ( w12.x * w3.y );\n' +
      '  float ws = w12.x * w0.y + w0.x * w12.y + w12.x * w12.y + w3.x * w12.y + w12.x * w3.y;\n' +
      '  return max( r / ws, vec3( 0.0 ) );\n' +
      '}\n' +
      // lambda = the mip level the hardware picks (texels per pixel, log2, the
      // MAJOR axis — the unsharp's low-pass must be at least as filtered as the
      // sample, or it aliases). Magnified (lambda < 0): blend to Catmull-Rom.
      // Minified: c + k (c - mip(lambda + 1)), faded in over lambda [-1, 0].
      'vec3 r25Sharpen( sampler2D tex, vec2 uv, vec3 c ) {\n' +
      '  vec2 ts = vec2( textureSize( tex, 0 ) );\n' +
      '  vec2 dx = dFdx( uv * ts ), dy = dFdy( uv * ts );\n' +
      '  float lam = 0.5 * log2( max( max( dot( dx, dx ), dot( dy, dy ) ), 1e-8 ) );\n' +
      '  float mag = 1.0 - smoothstep( -0.5, 0.0, lam );\n' +
      '  if ( mag > 0.0 ) c = mix( c, r25CatRom( tex, uv, ts ), mag );\n' +
      '  vec3 lo = textureLod( tex, uv, max( lam, 0.0 ) + 1.0 ).rgb;\n' +
      '  return max( c + uR25Sharp * smoothstep( -1.0, 0.0, lam ) * ( c - lo ), vec3( 0.0 ) );\n' +
      '}\n';
  }
  if (t.c) {
    const cr = R25_GROUND.colorRef;
    s +=
      'uniform sampler2D uR25Ref;\nuniform vec4 uR25RefWin;\nuniform float uR25RefGain;\nuniform vec4 uR25TileXf;\n' +
      // uR25TileXf = ( 2^(zoom - z), x * s, y * s, lodK ): the tile's own UV to
      // FRACTIONAL reference-zoom tile coordinates, and the mip level of this
      // tile's map whose texel footprint matches one reference texel.
      'vec3 r25RefRatio( sampler2D tex, vec2 uv ) {\n' +
      '  if ( uR25RefGain <= 0.0 || uR25TileXf.x <= 0.0 ) return vec3( 1.0 );\n' +
      '  vec2 t = vec2( vR25Uv.x, 1.0 - vR25Uv.y ) * uR25TileXf.x + uR25TileXf.yz;\n' +
      '  vec2 w = t - uR25RefWin.xy;\n' +
      '  float edge = smoothstep( 0.0, uR25RefWin.w, min( w.x, w.y ) ) * smoothstep( 0.0, uR25RefWin.w, uR25RefWin.z - max( w.x, w.y ) );\n' +
      '  if ( edge <= 0.0 ) return vec3( 1.0 );\n' +
      // Toroidal slots: reference tile (X, Y) lives in slot (X mod span, Y mod
      // span), so a recentre only rewrites the entering row/column.
      '  vec4 ref = textureLod( uR25Ref, fract( t / uR25RefWin.z ), 0.0 );\n' +
      '  vec3 lo = textureLod( tex, uv, uR25TileXf.w ).rgb;\n' +
      `  vec3 k = clamp( ( ref.rgb + 0.004 ) / ( lo + 0.004 ), vec3( ${r25f(cr.clampLo)} ), vec3( ${r25f(cr.clampHi)} ) );\n` +
      '  return mix( vec3( 1.0 ), k, clamp( ref.a * edge * uR25RefGain, 0.0, 1.0 ) );\n' +
      '}\n';
  }
  return s;
}

/** The Enhanced map-chunk insertion, spliced BEFORE `diffuseColor *=
 * sampledDiffuseColor;` in whichever map chunk the material compiles (stock or
 * LOD crossfade), so the material's colour/vertex factors never divide out. */
function r25MapInsert(t, lod) {
  let s = '\tif ( uR25Sat > 0.5 ) {\n';
  if (t.k)
    s +=
      '\t\tif ( uR25Sharp > 0.0 ) {\n' +
      (lod
        ? // The crossfade already mixed toward the parent: sharpen the CHILD
          // sample only and carry the delta in at the child's weight.
          '\t\t\tvec3 r25C = texture2D( map, vMapUv ).rgb;\n' +
          '\t\t\tsampledDiffuseColor.rgb += ( r25Sharpen( map, vMapUv, r25C ) - r25C ) * ( 1.0 - uLodFadeMix );\n'
        : '\t\t\tsampledDiffuseColor.rgb = r25Sharpen( map, vMapUv, sampledDiffuseColor.rgb );\n') +
      '\t\t}\n';
  if (t.c) s += '\t\tsampledDiffuseColor.rgb *= r25RefRatio( map, vMapUv );\n';
  return s + '\t}\n';
}

/** Instrument: Enhanced patch sites that did not find their Classic anchor
 * (a shape change upstream of this function). verify-r25-ground asserts 0. */
export const r25PatchStats = { misses: 0, lastMiss: null };

function r25Replace(text, from, to, site) {
  const i = text.indexOf(from);
  if (i < 0) {
    r25PatchStats.misses++;
    r25PatchStats.lastMiss = site;
    return text;
  }
  return text.slice(0, i) + to + text.slice(i + from.length);
}

/** Apply the Enhanced terrain terms `t` to an already-Classic tile shader. */
function r25PatchTile(shader, t, lod) {
  const h = t.h;
  const uv = t.n || t.c;
  shader.uniforms.uR25Sat = r25Uniforms.uR25Sat;
  if (t.n) {
    shader.uniforms.uR25Relief = h.uR25Relief;
    shader.uniforms.uR25HasRelief = h.uR25HasRelief;
  }
  if (t.s) shader.uniforms.uR25OneSun = r25Uniforms.uR25OneSun;
  if (t.k) shader.uniforms.uR25Sharp = r25Uniforms.uR25Sharp;
  if (t.c) {
    shader.uniforms.uR25Ref = r25Uniforms.uR25Ref;
    shader.uniforms.uR25RefWin = r25Uniforms.uR25RefWin;
    shader.uniforms.uR25RefGain = r25Uniforms.uR25RefGain;
    shader.uniforms.uR25TileXf = h.uR25TileXf;
  }
  if (uv) {
    let vs = r25Replace(shader.vertexShader, '#include <common>', '#include <common>\nvarying vec2 vR25Uv;', 'vs-common');
    vs = r25Replace(vs, '#include <defaultnormal_vertex>', '#include <defaultnormal_vertex>\nvR25Uv = uv;', 'vs-uv');
    shader.vertexShader = vs;
  }
  ...
  fs = r25Replace(
    ...
    '#include <common>',
    '#include <common>\n' +
      (uv ? 'varying vec2 vR25Uv;\n' : '') +
      (t.n ? r25ReliefGLSL() : '') +
      (t.s ? 'uniform vec3 uR25OneSun;\n' : '') +
      r25MapGLSL(t),
    'fs-common'
  );
  // n: the relief normal replaces the interpolated vertex normal wherever C's
  // fragment-stage hill block reads it (vHill, vUp — or the one-sun term).
  if (t.n) fs = r25Replace(fs, '  vec3 nW = normalize( vHillNW );\n', '  vec3 nW = r25ReliefN( normalize( vHillNW ) );\n', 'fs-nW');
  // s: the Classic lit / slope-AO / slope-saturation body -> the one bounded
  // term, and direct + indirect diffuse on the FLAT normal (relief once).
  if (t.s) {
    fs = r25Replace(
      fs,
      '  float lit = uHillAmbient + ( 1.0 + uHillLift - uHillAmbient ) * vHill;\n',
      '  float lit = uHillAmbient + ( 1.0 + uHillLift - uHillAmbient ) * clamp( normalize( uHillDir ).y, 0.0, 1.0 );\n',
      'fs-oneSun-lit'
    );
    const endKey = '  diffuseColor.rgb = mix( diffuseColor.rgb, shaded, uHillStrength';
    const b = fs.indexOf(endKey);
    const e = b < 0 ? -1 : fs.indexOf('\n', b);
    if (e < 0) {
      r25PatchStats.misses++;
      r25PatchStats.lastMiss = 'fs-oneSun';
    } else fs = fs.slice(0, e + 1) + r25OneSunGLSL() + fs.slice(e + 1);
    fs = r25Replace(
      fs,
      '#include <normal_fragment_begin>',
      '#include <normal_fragment_begin>\n' +
        'if ( uHillStrength > 0.0 ) normal = normalize( ( viewMatrix * vec4( 0.0, 1.0, 0.0, 0.0 ) ).xyz );',
      'fs-flatNormal'
    );
  }
  // c / k: inside the map chunk, before the sample multiplies diffuseColor.
  if (t.c || t.k) {
    const ins = r25MapInsert(t, !!lod);
    if (lod) fs = r25Replace(fs, 'diffuseColor *= sampledDiffuseColor;', ins + '\tdiffuseColor *= sampledDiffuseColor;', 'fs-map-lod');
    else if (h.chunk)
      fs = r25Replace(
        fs,
        '#include <map_fragment>',
        h.chunk.replace('diffuseColor *= sampledDiffuseColor;', ins + '\tdiffuseColor *= sampledDiffuseColor;'),
        'fs-map'
      );
    else {
      r25PatchStats.misses++;
      r25PatchStats.lastMiss = 'fs-map-nochunk';
    }
  }
  shader.fragmentShader = fs;
}

/** The ONE bounded sun term. Classic's hill block stays in place with its
 * sun-DIRECTION term moved to flat ground (`lit` reads up.L — its flat-ground
 * value, so flat ground is EXACTLY Classic); the slope AO / slope saturation
 * (sunless, from the relief normal via vUp) keep their Classic envelope; and
 * the sun's direction enters ONCE, here, as the bounded ratio at full weight
 * once the tier's hill strength reaches `fullStrength`. Direct light uses the
 * flat normal (r25PatchTile), so the Standard N.L no longer doubles it. */
function r25OneSunGLSL() {
  const o = R25_GROUND.oneSun;
  const elev = hillElevOn() ? ' * uHillElev' : '';
  return (
    '  if ( uHillStrength > 0.0 ) {\n' +
    '    vec3 r25L = normalize( uHillDir );\n' +
    '    float r25K = clamp( ( dot( nW, r25L ) + uR25OneSun.x ) / ( max( r25L.y, 0.0 ) + uR25OneSun.x ), uR25OneSun.y, uR25OneSun.z );\n' +
    `    diffuseColor.rgb *= mix( 1.0, r25K, clamp( uHillStrength${elev} / ${r25f(o.fullStrength)}, 0.0, 1.0 ) );\n` +
    '  }\n'
  );
}
...
// Round 23 (B CITY-LIGHT) — NIGHT_CITY_R23.windows. Per-BUILDING night-window
// variance, and the whole point is that it needs NO new attribute: the building
// ring already stamps every vertex with `aBendAnchor` (its footprint centroid),
// which IS a per-building identity. Hashing it in the fragment gives each
// building a stable random for free.
//
// These uniforms exist ONLY in the armed '-r23' program (the disarmed patch
// never declares them), and every term lands inside <emissivemap_fragment> —
// which the layer multiplies by an emissiveIntensity that is 0 whenever the sun
// is up. DAYLIGHT CANNOT MOVE, by construction, armed or not.
const satBldgR23Uniforms = {
  uNCCells: { value: { x: 8, y: 8, isVector2: true } }, // atlas (cols, rows)
  uNCPhase: { value: 0 }, // 0 = no de-repeat, ≥0.5 = whole-CELL uv phase per building
  uNCGain: { value: { x: 1, y: 1, isVector2: true } }, // per-building brightness (min,max)
  uNCDark: { value: { x: 0, y: 0, isVector2: true } }, // (dark share, its gain)
  uNCTint: { value: 0 }, // per-building warm↔cool jitter
  // The ROOF SENTINEL — SAT_BUILDINGS.facade.neutralUV, the uv every roof and
  // roof-detail vertex carries. See the roof-invariant note in the patch.
  uNCNeutral: { value: 0.25 },
};

// "Hash without sine" (Dave Hoskins, public domain) — sin-based hashes lose
// precision at the couple-of-thousand-unit magnitudes a chunk-local anchor
// reaches. The anchor is quantised to 0.5 m first so the value a building hashes
// to is exactly stable, and identical for every vertex of that building.
const satBldgHashGLSL = /* glsl */ `
float wbHash21( vec2 p ) {
 ...
 *
 * r23 = NIGHT_CITY_R23.windows (round 23, OPTIONAL — pass it only when the block
 * is ARMED, `nightCityOn('windows')`): the per-building night variance. Omitting
 * it compiles the R19 program verbatim under the R19 key.
 */
/*
 ...
 */
export function applyBendAnchorSat(material, r23, fadeUniform) {
  // Main's pooled-fade caller and the cinematic night-config caller coexist.
  if (r23 && typeof r23.value === 'number' && fadeUniform === undefined) {
    fadeUniform = r23;
    r23 = undefined;
  }
  if (!material || material.userData.__worldBend) return;
  material.userData.__worldBend = 'anchor-sat';
  if (r23) setNightCityWindows(r23);
  const prev = material.onBeforeCompile;
  material.onBeforeCompile = (shader, renderer) => {
    prev?.(shader, renderer);
    shader.uniforms.uBendCenter = uniforms.uBendCenter;
    ...
    if (r23) {
      for (const name of Object.keys(satBldgR23Uniforms)) {
        shader.uniforms[name] = satBldgR23Uniforms[name];
      }
    }
    shader.vertexShader = shader.vertexShader
      .replace(
        ...
        '#include <common>\nuniform vec2 uBendCenter;\nuniform float uBendK;\nattribute vec2 aBendAnchor;\nvarying float vSatDist;' +
          // R23: the anchor itself, forwarded so the fragment can hash it. No
          // new attribute — this is the bend's own attribute, read twice.
          (r23 ? '\nvarying vec2 vSatAnchor;' : '')
      )
      .replace(
        '#include <project_vertex>',
        r23 ? `${satBldgAnchorProject}\nvSatAnchor = aBendAnchor;` : satBldgAnchorProject
      );
    shader.fragmentShader = shader.fragmentShader
      .replace(
        '#include <common>',
        `#include <common>\nuniform float uSatBldgFade;\nvarying float vSatDist;${satHazeDecl}${bayerDitherGLSL}${
          r23
            ? `varying vec2 vSatAnchor;
uniform vec2 uNCCells;
uniform float uNCPhase;
uniform vec2 uNCGain;
uniform vec2 uNCDark;
uniform float uNCTint;
uniform float uNCNeutral;${satBldgHashGLSL}`
            : ''
        }`
      )
      .replace(
    ...
    if (r23) {
      // Round 23 (B) — WINDOW VARIANCE. Replaces three's emissivemap chunk with
      // the same fetch, per building:
      //   (a) a whole-CELL uv phase (de-repeat), (b) a per-building brightness
      //   multiplier, (c) a hash-stable DARK share (the office block at 2 a.m.),
      //   (d) a warm↔cool tint jitter so one block reads sodium and its
      //   neighbour fluorescent.
      // Identity: phase 0, gain (1,1), dark (0,0), tint 0 ⇒ this computes
      // exactly `totalEmissiveRadiance *= emissiveColor.rgb`.
      //
      // THE ROOF INVARIANT, and why it is a BRANCH rather than an argument.
      // R15's roofs are textureless because every roof/detail vertex carries the
      // constant uv (neutralUV, neutralUV) — zero screen-space derivative ⇒ mip
      // 0 ⇒ always the same background texel. The de-repeat was designed to
      // preserve that arithmetically: neutralUV 0.25 × 8 cells = 2.0 is a cell
      // BOUNDARY, and boundary + k cells is still a boundary. MEASURED, it is
      // not enough — the shader bench (scripts/r23-b-shaderbench.js, roof leg)
      // photographed roof quads getting BRIGHTER by ~1.9 luma under phase alone,
      // i.e. the sampler does not honour the boundary argument here (filtering /
      // LOD selection at a constant-uv primitive is a driver behaviour, not a
      // guarantee). So the fragment tests the sentinel EXACTLY and leaves those
      // fragments on the R15 path untouched. The invariant is now structural:
      // no roof fragment executes one instruction of this block.
      shader.fragmentShader = shader.fragmentShader.replace(
        '#include <emissivemap_fragment>',
        /* glsl */ `
#ifdef USE_EMISSIVEMAP
  vec4 emissiveColor = texture2D( emissiveMap, vEmissiveMapUv );
  vec2 wbDN = abs( vEmissiveMapUv - vec2( uNCNeutral ) );
  if ( max( wbDN.x, wbDN.y ) > 1e-4 ) {
    vec2 wbA = floor( vSatAnchor * 2.0 );
    float wbH1 = wbHash21( wbA );
    float wbH2 = wbHash21( wbA + vec2( 37.7, 91.3 ) );
    float wbH3 = wbHash21( wbA + vec2( -13.1, 57.9 ) );
    vec2 wbUv = vEmissiveMapUv
      + step( 0.5, uNCPhase ) * floor( vec2( wbH1, wbH2 ) * uNCCells ) / uNCCells;
    emissiveColor = texture2D( emissiveMap, wbUv );
    float wbG = mix( uNCGain.x, uNCGain.y, wbH3 );
    if ( wbH1 < uNCDark.x ) wbG = uNCDark.y;
    float wbT = uNCTint * ( wbH2 - 0.5 );
    emissiveColor.rgb *= wbG * vec3( 1.0 + wbT, 1.0, 1.0 - wbT );
  }
...
`
  ...
  // Round 23 (B): the ARMED program adds a varying + the per-building emissive
  // block, so it takes its own '-r23' key; disarmed, every string above is R19's
  // verbatim and this stays '-r19'.
  material.customProgramCacheKey = () =>
    r23 ? 'world-bend-anchor-satbldg-r23' : 'world-bend-anchor-satbldg-r19';
  material.needsUpdate = true;
}

/**
 * Round 23 (B): the per-building night-window knobs. Live uniforms, so an A/B
 * leg is a page.evaluate away (window.__flyNightCity) and the control leg is
 * literally "phase 0, gain (1,1), dark (0,0), tint 0" = the R15 atlas read.
 * `cells` comes from SAT_BUILDINGS.facade (cols, rows) and MUST divide the
 * neutral uv exactly — see the roof invariant note in the patch.
 */
export function setNightCityWindows(cfg, cells) {
  if (!cfg) return;
  const U = satBldgR23Uniforms;
  if (cells) {
    U.uNCCells.value.x = cells.cols;
    U.uNCCells.value.y = cells.rows;
    if (typeof cells.neutralUV === 'number') U.uNCNeutral.value = cells.neutralUV;
  }
  U.uNCPhase.value = cfg.phase;
  U.uNCGain.value.x = cfg.gain.min;
  U.uNCGain.value.y = cfg.gain.max;
  U.uNCDark.value.x = cfg.darkFrac;
  U.uNCDark.value.y = cfg.darkGain;
  U.uNCTint.value = cfg.tintJitter;
}

/** Round 23 (B): read the window-variance uniforms back off the GPU's copy. */
export function getNightCityWindows() {
  const U = satBldgR23Uniforms;
  return {
    cells: [U.uNCCells.value.x, U.uNCCells.value.y],
    phase: U.uNCPhase.value,
    gain: [U.uNCGain.value.x, U.uNCGain.value.y],
    dark: [U.uNCDark.value.x, U.uNCDark.value.y],
    tint: U.uNCTint.value,
  };
}
...
/**
 * R22 (D DEPTH, at E CERT's request — Fable arbitrates this cross-owner touch
 * at merge): the FULL live content-haze state, colour included.
 * `getSatContentHaze` above returns only the band and the max, which cannot
 * distinguish "the haze is off" from "the haze is on and mixing toward black";
 * verify-depth2's medium/low scoping legs for the plan §5.4 flip need the
 * triple. Read-only, additive, no behavior change.
 */
export function getSatHaze() {
  return {
    startM: satHazeUniforms.uSatHaze.value.x,
    endM: satHazeUniforms.uSatHaze.value.y,
    max: satHazeUniforms.uSatHazeMax.value,
    color: [
      satHazeUniforms.uSatHazeColor.value.r,
      satHazeUniforms.uSatHazeColor.value.g,
      satHazeUniforms.uSatHazeColor.value.b,
    ],
  };
}
...
// R23 (B): the same two numbers, named for the armed path's uniform DEFAULT so
// an armed program with NIGHT_CITY_R23.roads.lamp left at the R16 values is
// value-identical to the disarmed literals (this is what the knob A/B's control
// leg restores). The disarmed program still emits the literals above.
const STREET_DOT_GAIN_R16 = STREET_DOT_GAIN;
const STREET_DOT_SHARP_R16 = STREET_DOT_SHARP;

// Round 23 (B CITY-LIGHT) — NIGHT_CITY_R23.roads. These uniforms exist ONLY in
// the armed '-r23' program: the disarmed patch never declares them, never binds
// them and never emits an instruction that reads them, which is what makes
// flag-off byte-identity STRUCTURAL rather than "arithmetically zero".
// uRoadLamp carries the two values R16 baked as GLSL literals, so the lamp pool
// becomes sweepable without another source edit, and its DEFAULT is those exact
// literals (the knob A/B's control leg restores them).
const roadSatR23Uniforms = {
  uRoadLamp: { value: { x: STREET_DOT_GAIN_R16, y: STREET_DOT_SHARP_R16, isVector2: true } },
  uTrafDash: { value: { x: 300, y: 0.22, z: 0.16, isVector3: true } }, // (lenM, duty, speed)
  uTrafHead: { value: { x: 1, y: 0.914, z: 0.753, isVector3: true } }, // warm headlight
  uTrafTail: { value: { x: 1, y: 0.27, z: 0.188, isVector3: true } }, // red tail light
  uTrafBoost: { value: 0 }, // 0 = the term contributes literally nothing
  uLaneK: { value: 0 }, // 0 = both streams across the whole ribbon, 1 = split by side
};
 ...
 * r23 = NIGHT_CITY_R23.roads (round 23, OPTIONAL — pass it only when the block
 * is ARMED, `nightCityOn('roads')`): the night re-sweep + the two-colour
 * traffic term. Omitting it compiles the R19 program, verbatim, under the R19
 * key; passing it compiles '-r23'.
 * Reaches NO existing variant, so no other FINAL keys move.
 */
export function applyBendRoadSat(material, cfg, night, r23) {
  if (!material || material.userData.__worldBend) return;
  ...
  // Round 23 (B CITY-LIGHT) — the re-sweep + the two-colour traffic term.
  // `r23` is NIGHT_CITY_R23.roads or null, resolved by the CALLER against the
  // arm (this module imports nothing — the cfg-passed-in precedent above). It
  // selects the shader SOURCE: null ⇒ every string below is byte-identical to
  // R19's and the key stays '-r19', so the shipped tree cannot tell this round
  // happened. Non-null ⇒ the R23 knobs SHADOW the R16/R19-swept uniforms (which
  // stay in their own blocks, unedited) and the program becomes '-r23'.
  if (r23) {
    roadSatUniforms.uRoadGlow.value = r23.intensity;
    roadSatUniforms.uStreet56.value.x = r23.streetGain.c5;
    roadSatUniforms.uStreet56.value.y = r23.streetGain.c6;
    roadSatUniforms.uStreamBoost.value = r23.stream.boost;
    roadSatUniforms.uRoadDash.value.x = r23.stream.dashLenM;
    roadSatUniforms.uRoadDash.value.y = r23.stream.dashDuty;
    roadSatUniforms.uRoadDash.value.z = r23.stream.dashSpeed;
    setNightCityRoads(r23);
  }
  const prev = material.onBeforeCompile;
  material.onBeforeCompile = (shader, renderer) => {
    prev?.(shader, renderer);
    ...
    if (r23) {
      for (const name of Object.keys(roadSatR23Uniforms)) {
        shader.uniforms[name] = roadSatR23Uniforms[name];
      }
    }
    shader.vertexShader = shader.vertexShader
      .replace(
        ...
        '#include <common>\nuniform vec2 uBendCenter;\nuniform float uBendK;\nattribute float aRoadArc;\nattribute float aRoadCls;\nvarying float vRoadArc;\nvarying float vRoadCls;' +
          // R23: the ribbon SIDE (+1/−1), derived client-side by SatRoadEngine
          // from the worker's fixed (a+n, a−n, b+n, b−n) quad order — no worker
          // payload change, no protocol bump. Absent ⇒ 0 ⇒ both light streams at
          // half weight (fail-blended, never fail-wrong).
          (r23 ? '\nattribute float aRoadSide;\nvarying float vRoadSide;' : '')
      )
      .replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\nvRoadArc = aRoadArc;\nvRoadCls = aRoadCls;' +
...
uniform float uRoadDaySeam;${
          r23
            ? `
varying float vRoadSide;
uniform vec2 uRoadLamp;
uniform vec3 uTrafDash;
uniform vec3 uTrafHead;
uniform vec3 uTrafTail;
uniform float uTrafBoost;
        ...
        `#include <color_fragment>
{
  ...
  gain += mStreet * uRoadNight * uRoadGlow * ${
    r23 ? 'uRoadLamp.x' : STREET_DOT_GAIN.toFixed(2)
  } * exp( -sd * sd * ${r23 ? 'uRoadLamp.y' : STREET_DOT_SHARP.toFixed(1)} );
  ...
  diffuseColor.rgb *= rwEff * gain;${
    r23
      ? `
  // (H) round 23 - TWO-COLOUR TRAFFIC (NIGHT_CITY_R23.roads.traffic). Every
  //     term above is a GAIN on the ribbon's baked class hue, so the R16 dash
  //     train can only ever be a brighter version of the road's own colour.
  //     Real traffic is two colours moving in opposite directions, so this one
  //     is a COLOUR, added after the gain: warm headlights running WITH the arc,
  //     red tails running against it. Same material, same draw, no geometry.
  //     Lanes: vRoadSide is +1/-1 across the ribbon (SatRoadEngine derives it
  //     from the worker's fixed quad order); at uLaneK 0 - or with the attribute
  //     missing, which reads 0 - both streams sit at half weight on the whole
  //     ribbon, which is dimmer and blended, never wrong.
  //     uTrafBoost 0 => the branch never runs => the R19 fragment exactly.
  if ( uTrafBoost > 0.0 ) {
    float tph = ra / uTrafDash.x;
    float dH = wbDash( fract( tph - uRoadT * uTrafDash.z ), uTrafDash.y );
    float dT = wbDash( fract( tph + uRoadT * uTrafDash.z + 0.5 ), uTrafDash.y );
    float laneH = ${r23?.cinematic ? 'exp(-pow((vRoadSide - 0.42) / 0.09, 2.0))' : 'mix( 0.5, 0.5 + 0.5 * vRoadSide, uLaneK )'};
    float laneT = ${r23?.cinematic ? 'exp(-pow((vRoadSide + 0.42) / 0.09, 2.0))' : 'mix( 0.5, 0.5 - 0.5 * vRoadSide, uLaneK )'};
    diffuseColor.rgb += ( uTrafHead * dH * laneH + uTrafTail * dT * laneT )
      * mArtery * uRoadNight * uTrafBoost * rwEff;
  }`
      : ''
  }
}`
  ...
  // Round 23 (B): the armed program carries a new attribute, a new varying and
  // the (H) colour term, so it takes its OWN final key. Disarmed, every string
  // above is byte-identical to R19's and this stays '-r19' — the shipped tree
  // compiles the same program it always did (verify-groundlife's ROAD_CACHE_KEY
  // is asserted against that state).
  material.customProgramCacheKey = () =>
    r23?.cinematic ? 'world-bend-road-satnight-cinematic-v1' : r23 ? 'world-bend-road-satnight-r23' : 'world-bend-road-satnight-r19';
  material.needsUpdate = true;
}

/**
 * Round 23 (B): write the NIGHT_CITY_R23.roads knobs that live only in the
 * armed program. Called at apply time and by the dev handle (window.__flyNightCity)
 * so an A/B leg is a uniform write, not a re-boot — the control leg restores the
 * R16/R19 swept values and IS the "measured A/B against the swept baseline" the
 * knob-move discipline requires.
 */
export function setNightCityRoads(cfg) {
  if (!cfg) return;
  const U = roadSatR23Uniforms;
  // EVERY write is undefined-guarded. A partial cfg (an A/B leg that names one
  // knob) must leave the others where they are — writing `undefined` into a
  // vec3's .x drops three's uniform upload off the {x,y,z} fast path into
  // `uniform3fv(plain object)`, which throws once per frame per material. That
  // is not hypothetical: the first shader-bench run produced 128 of them from a
  // one-field traffic patch, and a partial leg is the NORMAL way this is driven.
  const num = (v, cur) => (typeof v === 'number' && Number.isFinite(v) ? v : cur);
  if (cfg.lamp) {
    U.uRoadLamp.value.x = num(cfg.lamp.gain, U.uRoadLamp.value.x);
    U.uRoadLamp.value.y = num(cfg.lamp.sharp, U.uRoadLamp.value.y);
  }
  const t = cfg.traffic;
  if (t) {
    U.uTrafDash.value.x = num(t.dashLenM, U.uTrafDash.value.x);
    U.uTrafDash.value.y = num(t.dashDuty, U.uTrafDash.value.y);
    U.uTrafDash.value.z = num(t.speed, U.uTrafDash.value.z);
    if (typeof t.head === 'string') _hexToVec3(t.head, U.uTrafHead.value);
    if (typeof t.tail === 'string') _hexToVec3(t.tail, U.uTrafTail.value);
    U.uTrafBoost.value = num(t.boost, U.uTrafBoost.value);
    U.uLaneK.value = num(t.lanes, U.uLaneK.value);
  }
}

/** sRGB hex → the linear-ish 0..1 triple the additive road material adds. */
function _hexToVec3(hex, out) {
...
/**
 * Round 23 (B): the live road-night uniforms, read back off what the GPU is
 * actually using (the getSatRoadNight()/getEdgeFade() discipline). The A/B
 * captures and any future gate assert THIS, never the constants.
 */
export function getNightCityRoads() {
  const U = roadSatR23Uniforms;
  return {
    glow: roadSatUniforms.uRoadGlow.value,
    street5: roadSatUniforms.uStreet56.value.x,
    street6: roadSatUniforms.uStreet56.value.y,
    streamBoost: roadSatUniforms.uStreamBoost.value,
    lamp: { gain: U.uRoadLamp.value.x, sharp: U.uRoadLamp.value.y },
    traffic: {
      boost: U.uTrafBoost.value,
      lanes: U.uLaneK.value,
      dash: { lenM: U.uTrafDash.value.x, duty: U.uTrafDash.value.y, speed: U.uTrafDash.value.z },
      head: [U.uTrafHead.value.x, U.uTrafHead.value.y, U.uTrafHead.value.z],
      tail: [U.uTrafTail.value.x, U.uTrafTail.value.y, U.uTrafTail.value.z],
    },
  };
}

/**
 * Round 23 (B): the road-glow / suburban-parity knobs an A/B leg moves. Kept
 * separate from setNightCityRoads because these three write R16/R19 uniforms
 * (the shadowing half of the re-sweep) — restoring {2.4, 0.24, 0.34} here is
 * exactly the swept-baseline control leg.
 */
export function setNightCityRoadGain(intensity, c5, c6, streamBoost) {
  roadSatUniforms.uRoadGlow.value = intensity;
  roadSatUniforms.uStreet56.value.x = c5;
  roadSatUniforms.uStreet56.value.y = c6;
  if (streamBoost != null) roadSatUniforms.uStreamBoost.value = streamBoost;
}
