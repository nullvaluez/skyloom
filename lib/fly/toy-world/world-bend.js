 ...
 *                            so the FINAL key bumped '-r13' → '-r19'. REACH: this
...
export function applyHillshade(material, cfg) {
  ...
  material.customProgramCacheKey = () => 'world-bend-fade-hill-r19';
  material.needsUpdate = true;
}
 ...
 */
export function applyBendAnchorSat(material, r23) {
  if (!material || material.userData.__worldBend) return;
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    shader.uniforms.uBendCenter = uniforms.uBendCenter;
    shader.uniforms.uBendK = uniforms.uBendK;
    shader.uniforms.uSatBldgFade = satBldgFadeUniforms.uSatBldgFade;
...
uniform float uNCTint;${satBldgHashGLSL}`
            : ''
        }`
      )
      .replace(
 ...
 */
export function applyBendAnchorSatSkyline(material) {
  if (!material || material.userData.__worldBend) return;
  ...
  material.onBeforeCompile = (shader, renderer) => {
    ...
    shader.uniforms.uSkyFade = satSkylineUniforms.uSkyFade;
    shader.uniforms.uSatHaze = satHazeUniforms.uSatHaze;
    shader.uniforms.uSatHazeColor = satHazeUniforms.uSatHazeColor;
    shader.uniforms.uSatHazeMax = satHazeUniforms.uSatHazeMax;
    shader.vertexShader = shader.vertexShader
...
export function setSatContentHaze(startM, endM, r, g, b, max) {
  ...
  satHazeUniforms.uSatHazeColor.value.r = r;
  satHazeUniforms.uSatHazeColor.value.g = g;
  satHazeUniforms.uSatHazeColor.value.b = b;
...
export function getRimColor(out) {
  const c = uniforms.uEdgeColor.value;
  if (out) {
    out.r = c.r;
    out.g = c.g;
    out.b = c.b;
