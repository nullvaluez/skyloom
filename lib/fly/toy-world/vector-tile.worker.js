...
//     cleanly.
const WORKER_PROTOCOL = 22; // terrain surface masks and explicit water boundaries
...
function buildSatRoads(vt, frame) {
  ...
  const airfield=airportSceneryMask(vt,256,tileSpan/k,true);
...
function buildSatVeg(vt, frame) {
  ...
  const airportMask=airportSceneryMask(vt,AVOID_GRID_N,tileSpan/frame.k);
  let avoidMask = null;
...
const api = {
  ...
  async buildTile(z, x, y, detail = 'full', options = {}) {
    ...
    if (detail === 'earth-surface') {
      ...
      const surface = deriveWorldContent(vt,combineWorldCover(buildEarthSurfaceMask(vt, options.size),await loadWorldCover(z,x,y)),span,k);
