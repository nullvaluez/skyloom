...
//     cleanly.
const WORKER_PROTOCOL = 21; // terrain surface masks and explicit water boundaries
...
function architectureContext(vt, frame) {
  ...
  return { density: vt.layers.building.length / ((frame.tileSpan / frame.k / 1000) ** 2), regions };
}
function architectureStyle(item, context, extent, k) {
  ...
  return inferBuildingStyle({ id: item.id, height: buildingClassificationHeight(item.rawH,item.areaM2/(k*k),item.id), areaM2: item.areaM2/(k*k),
    ...
    density: context.density, landuse: region?.cls, tags: item.archTags });
}
...
function buildSatBuildings(vt, frame) {
  ...
  for (const item of selected) {
    ...
    if (visuals && !item.elevated && entrances < 64 && item.h > 5 && item.areaM2 / (k*k) > 80) {
...
const api = {
  ...
  async buildTile(z, x, y, detail = 'full', options = {}) {
    ...
    if (detail === 'earth-surface') {
      const surface = buildEarthSurfaceMask(vt, options.size);
      return transferResult({ v: WORKER_PROTOCOL, surface }, [surface.classes.buffer, surface.exclusion.buffer, surface.waterEdges.buffer]);
    }
