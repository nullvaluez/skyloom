import { findAirportSurface } from '../operations-airports';
import { operationsFootprint } from '../operations-scenery';
/**
...
import { buildEarthSurfaceMask } from '../earth-surface-mask';
import { emitArchitectureRelief, steppedBuildingLevels, emitSteppedBuilding } from '../living-architecture';
import { architectureRegion } from '../living-regions';
import { LIVING_EARTH } from '../living-earth';
import { deriveWorldContent } from '../world-content';
import { buildRoadsideInfill } from '../settlement-infill';
import { loadWorldCover, combineWorldCover, worldCoverAt } from '../world-cover';
import { airportSceneryMask } from '../airport-scenery-mask';
import { EARTH_SURFACE, surfaceClass, stylizedEarthOn } from '../stylized-earth';
...
import { inferBuildingStyle, buildingClassificationHeight, architecturalColor, buildingStyleVertex, ARCHITECTURE_UV_PERIOD } from '../building-profiles';
...
//     cleanly.
const WORKER_PROTOCOL = 23; // terrain surface masks and explicit water boundaries
...
function clipRing(ring, extent) {
  ...
  // R24 B (RING_DEDUPE): the ONE place every polygon ring passes through, so
  // the closing clone dies once for the toy AND satellite pipelines together.
...
/**
 * R24 B (RING_DEDUPE, recon WB-1 / A1 / A1b) — THE DEFECT AT ITS SOURCE.
 *
 * `@mapbox/vector-tile` closes every polygon ring with a clone of its first
 * point (index.js:98, `line.push(line[0].clone()); // closePolygon`), and
 * clipRing above keeps an inside closing point verbatim. Every wall extruder
 * then walks the ring as if it were OPEN
 * (`for (let e = 0, j = ring.length - 1; e < ring.length; j = e++)`), so the
 * wrap-around edge is zero-length and emits two exactly-degenerate triangles
 * per ring AND per hole — plus 6 more per parapet and 2 per crown step, which
 * is why the measured population is ~7.4 degenerate triangles per ring, not 2
 * (scripts/r24-b-worker-proof.js gate 6, on E's own fixture tile).
 *
 * FLASH_GUARD removes them on the main thread at finalize, which is the
 * CORRECTNESS fix and moves no bundle bytes. This is the SOURCE fix: it also
 * stops them being built, transferred and draped, so the buffers shrink.
 *
 * IT ALSO FIXES A1b's SKYLINE DEFECT FOR FREE. `simplifyRing` walks the ring
 * cyclically and drops any point whose two edges are collinear; for the
 * closing clone `bc` is (0,0) and for ring[0] `ab` is (0,0), so BOTH are
 * discarded and every z14 block-mass ring silently loses its genuine first
 * corner. With the clone gone, ring[0]'s previous neighbour is a real corner
 * and it survives on its own merits — no change to simplifyRing itself.
 *
 * Returns the SAME array when nothing was dropped, so a flag-off session and
 * a flag-on session over already-clean rings are identical.
 */
function dedupeRing(pts) {
  if (!RING_DEDUPE.enabled) return pts;
  const n = pts.length;
  if (n < 2) return pts;
  const out = [];
  for (let i = 0; i < n; i++) {
    ...
    if (q && q.x === p.x && q.y === p.y) continue; // consecutive duplicate
    out.push(p);
  }
  // …and the wrap-around duplicate (the closePolygon clone).
  while (out.length > 1) {
    const a = out[0];
...
function architectureContext(vt, frame) {
  const land = vt.layers.landuse;
  const regions = [];
  if (land) for (let i = 0; i < land.length; i++) {
    const f = land.feature(i);
    const cls = f.properties.class;
    if (f.type !== 3 || !['residential', 'industrial', 'commercial', 'retail'].includes(cls)) continue;
    const polys = classifyRingsSat(f.loadGeometry());
    for (const poly of polys) regions.push({ poly, cls, extent: land.extent });
  }
  const airports=vt.layers.aeroway;
  if(airports)for(let i=0;i<airports.length;i++){
    const f=airports.feature(i);
    if(f.type===3 && ['aerodrome','apron','terminal','hangar'].includes(f.properties.class))
      for(const poly of classifyRingsSat(f.loadGeometry()))regions.unshift({poly,cls:'airport',extent:airports.extent});
  }
  const lon=frame.cx/6378137*180/Math.PI,lat=(2*Math.atan(Math.exp(-frame.cz/6378137))-Math.PI/2)*180/Math.PI;
  return { density: vt.layers.building.length / ((frame.tileSpan / frame.k / 1000) ** 2), regions, geography:architectureRegion(lat,lon) };
}
function architectureStyle(item, context, extent, k) {
  const ring = item.polys[0].outer;
  let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
  for (const p of ring) { x0 = Math.min(x0,p.x); x1 = Math.max(x1,p.x); y0 = Math.min(y0,p.y); y1 = Math.max(y1,p.y); }
  const x = (x0+x1)*0.5/extent, y = (y0+y1)*0.5/extent;
  const region = context.regions.find((r) => pointInPoly(r.poly,x*r.extent,y*r.extent));
  return inferBuildingStyle({ id: item.id, height: buildingClassificationHeight(item.rawH,item.areaM2/(k*k),item.id), areaM2: item.areaM2/(k*k),
    aspect: Math.max(x1-x0,y1-y0)/Math.max(1e-6,Math.min(x1-x0,y1-y0)),
    density: context.density, landuse: region?.cls, tags: item.archTags, region:context.geography.style });
}
...
function buildSatBuildings(vt, frame) {
  ...
  const visuals = frame.visuals === true;
  if (!layer) {
  ...
  const archContext = visuals ? architectureContext(vt, frame) : null;
  ...
  for (let i = 0; i < layer.length; i++) {
    ...
    if (PC.enabled) {
      ...
      for (let pi = 0; pi < polys.length; pi++) {
        ...
        const it = { polys: [p], area: a, areaM2: aM2, rawH, minY, id: pid, ...(visuals ? { archTags: Object.values(f.properties).filter((v) => typeof v === 'string').join(' ') } : {}) };
        ...
        if (!operationsFootprint(p.outer,frame,layer.extent)) items.push(it);
      }
      continue;
    }
    let area = 0;
    ...
    items.push({ polys, area, areaM2, rawH, minY, id: fid, ...(visuals ? { archTags: Object.values(f.properties).filter((v) => typeof v === 'string').join(' ') } : {}) });
  }
  ...
  for (const it of items) {
    ...
    h = Math.min(h, B.maxH);
    if (visuals && Number.isFinite(Number(it.rawH)) && Number(it.rawH) > 0) h = Number(it.rawH); // Mapped heights are never stylized in cinematic.
    it.h = h;
    it.hash = hash;
  ...
  // Living Earth: all mapped footprints retain a body. The former 500-polygon
  // sample becomes the detailed subset, rather than holes through a real town.
  const detailed = new Set(selected);
  // Physical ledges go to the most prominent mapped buildings. Roof forms
  // retain the larger detail set; every footprint below still gets its body.
  const relief = new Set(selected.slice().sort((a,b)=>b.areaM2*b.h-a.areaM2*a.h).slice(0,LIVING_EARTH.architecture.reliefPerTile));
  if (visuals) selected = built;
  const satB = { pos: [], col: [], idx: [], anchor: [], uv: [], style: [], vtx: 0 };
  const families = [0,0,0,0,0,0,0,0];
  const cityClusters = Array.from({ length: 4 }, () => ({ x: 0, z: 0, count: 0, warmth: 0 }));
  let entrances = 0;
  ...
  for (const item of selected) {
    ...
    const style = visuals ? architectureStyle(item, archContext, layer.extent, k) : null;
    if (style) families[style.family] += 1;
    const wall = style ? architecturalColor(style.walls[style.variant]) : hexToRGB(pickByHash(TONE.wallTones, item.id)).map((c) => c * jit(S.wallJitter, hash2));
      ...
      tyRoof === 'mid'
        ...
        : item.h <= RT.lowMaxH
          ...
          : item.h >= RT.tallMinH
            ? TONE.roofTall
            : TONE.roofMid;
    const roofCol = style ? architecturalColor(style.roofs[style.variant]) : hexToRGB(pickByHash(band, (item.id ^ 0x5bf03635) >>> 0)).map(
    ...
    if (style) {
      const c = cityClusters[(anchorX > 0 ? 1 : 0) + (anchorZ > 0 ? 2 : 0)];
      c.x += anchorX; c.z += anchorZ; c.count += 1; c.warmth += style.warmth;
    }
    ...
    const pushV = (px, py, y, colArr, u, v) => {
      ...
      if (style) satB.style.push(...buildingStyleVertex(style, u !== undefined));
    ...
    const neutralWall = !visuals && !!item.typo && TY.neutralWalls;
    ...
    const levels = visuals && detailed.has(item) && !item.elevated ? steppedBuildingLevels(item.polys[0],item.h,style) : null;
    for (const poly of item.polys) {
      if(levels){emitSteppedBuilding(satB,pushV,poly,levels,wallBottomY,wallBase,wallTop,roofCol,mTrue,uPeriod,vPeriod);continue;}
      // roof cap
      const flat = [];
      const holeIdx = [];
      for (const p of poly.outer) flat.push(p.x, p.y);
      ...
      for (const ring of [poly.outer, ...poly.holes]) {
        for (let e = 0, j = ring.length - 1; e < ring.length; j = e++) {
          const a = ring[j];
          const b = ring[e];
          // R24 B (RING_DEDUPE): dedupeRing already removed the exact
          // duplicates; this catches the clip-intersection case where two
          // points land a few float ulps apart — the SLIVER half of WB-1.
          if (
            RING_DEDUPE.enabled &&
            Math.hypot(b.x - a.x, b.y - a.y) < RING_DEDUPE.minEdgeTile
          )
            continue;
          const halfU =
    ...
    if(visuals&&!detailed.has(item)&&style.family===0&&item.h<16&&(style.roofRise??1)>0){
      const simple=simplifyRing(outer,RS.simplifyTolTile);
      if(simple.length===4)pushGable(satB,pushV,simple,roofY,(2+item.hash)*(style.roofRise??1),roofCol);
    }
    if (visuals && relief.has(item) && !item.elevated && !levels) {
      for (const poly of item.polys) emitArchitectureRelief(satB, pushV, poly, item, style, mTrue);
    }
    // A bounded entry canopy on the longest frontage. One shared chunk draw,
    // no footprint/height change, and the explicit roof mask keeps it unlit.
    if (visuals && detailed.has(item) && !item.elevated && entrances < 128 && item.h > 5 && item.areaM2 / (k*k) > 80) {
      let edge = null, longest = 0;
      for (let i = 0; i < outer.length; i++) {
        const a = outer[i], b = outer[(i+1)%outer.length];
        const length = Math.hypot(b.x-a.x,b.y-a.y);
        if (length > longest) { longest = length; edge = [a,b]; }
      }
      if (edge && longest*mTrue > 5) {
        const [a,b] = edge, mx = (a.x+b.x)*0.5, my = (a.y+b.y)*0.5;
        const tx = (b.x-a.x)/longest, ty = (b.y-a.y)/longest;
        let nx = -ty, ny = tx;
        if ((mx-cxT)*nx+(my-cyT)*ny < 0) { nx=-nx; ny=-ny; }
        const half = Math.min(1.8/mTrue,longest*0.2), depth = 0.95/mTrue;
        const y = Math.min(3.1,item.h*0.45), col = roofCol.map((c) => c*0.86);
        const i0 = pushV(mx-tx*half,my-ty*half,y,col);
        const i1 = pushV(mx+tx*half,my+ty*half,y,col);
        const i2 = pushV(mx+tx*half+nx*depth,my+ty*half+ny*depth,y,col);
        const i3 = pushV(mx-tx*half+nx*depth,my-ty*half+ny*depth,y,col);
        satB.idx.push(i0,i2,i1,i0,i3,i2); entrances += 1;
      }
    }
    ...
    if (S.roofDetail && RS.enabled && (!visuals || detailed.has(item)) && !levels) {
      ...
      const rise = (RS.small.riseM[0] + item.hash * (RS.small.riseM[1] - RS.small.riseM[0])) * (style?.roofRise ?? 1);
      const MID_MAX = RT.tallMinH; // 45 — the band edge the roof palette already uses
      const SUPER_MIN = RS.super.minH; // 120
      let treated = false;

      if(visuals && rise===0 && h<RS.small.maxH){
        pushParapet(satB,pushV,outer,roofY,.45,.035,parapetCol);forms.parapet++;treated=true;
      } else if (h < RS.small.maxH && item.areaM2 < RS.small.maxAreaM2) {
        ...
        if (visuals) {
          // Ordinary mapped towers receive a mechanical roof inside the envelope.
          // Landmark silhouettes are supplied by the separately mapped models.
          pushParapet(satB, pushV, outer, roofY-.6, .6, ROOFS.parapet.insetFrac, parapetCol);
          forms.parapet += 1;
          treated = true;
        } else if (item.hash < RS.super.crownFrac) {
      ...
      // Complex low commercial roofs gain an actual inset lip under the SAME
      // parapet cap. Houses retain their pitched forms and chimney dispatch.
      if(visuals&&!treated&&h>=8&&h<RS.small.maxH&&nC>6&&item.areaM2/(k*k)>180&&nParapet<RS.caps.parapet){
        pushParapet(satB,pushV,outer,roofY,ROOFS.parapet.heightM,ROOFS.parapet.insetFrac,parapetCol);
        nParapet++;forms.parapet++;treated=true;
      }
      if (!treated) {
        if (h < RS.small.maxH) {
  ...
  if (satB.idx.length > 0) {
    ...
    if (visuals) {
      const style = new Uint8Array(satB.style);
      out.satBuilding.style = style;
      meta.families = families; meta.entrances = entrances;
      meta.provenance={footprints:'openfreemap',appearance:'inferred-regional',region:archContext.geography,coverageBodies:selected.length,detailedBodies:detailed.size,reliefBodies:relief.size,revision:1};
      meta.cityClusters = cityClusters.filter((c) => c.count >= 3).map((c) => ({ x:c.x/c.count,z:c.z/c.count,intensity:Math.min(1,c.count/60),warmth:c.warmth/c.count }));
      transfer.push(style.buffer);
    }
    out.empty = false;
...
function buildSatRoads(vt, frame) {
  ...
  const airfield=airportSceneryMask(vt,256,tileSpan/k,true,frame);
  const inAirfield=(chain,extent)=>{
    if(!airfield)return false;
    for(let i=1;i<chain.length;i++){
      const a=chain[i-1],b=chain[i],steps=Math.max(1,Math.ceil(Math.hypot(b.x-a.x,b.y-a.y)*256/extent));
      for(let j=0;j<=steps;j++){
        const x=Math.floor((a.x+(b.x-a.x)*j/steps)*256/extent),y=Math.floor((a.y+(b.y-a.y)*j/steps)*256/extent);
        if(x>=0&&x<256&&y>=0&&y<256&&airfield[y*256+x])return true;
      }
    }
    return false;
  };
      ...
      if (!spec) {
        if (serviceChains && f.properties.class === 'service') {
          eachClippedChain(f, tLayer.extent, (chain) => {
            if(inAirfield(chain,tLayer.extent))return;
...
function buildSatSkyline(vt, frame) {
  ...
  for (let i = 0; i < layer.length; i++) {
    ...
    if (PC.enabled) {
      ...
      for (let pi = 0; pi < polys.length; pi++) {
        ...
        if (h > B.kneeM) h = B.kneeM + (h - B.kneeM) * B.kneeSlope;
        h = Math.min(h, B.maxH);
        if (visuals && Number.isFinite(Number(rawH)) && Number(rawH) > 0) h = Number(rawH);
        const it = { polys: [p], areaM2: aM2, h, rawH, id: pid, hatch: hatchPick, ...(visuals ? { archTags: Object.values(f.properties).filter((v) => typeof v === 'string').join(' ') } : {}) };
        if (!PC.perPolyDrape && pi > 0) it.anchorPoly = polys[0];
        items.push(it);
      }
      continue;
    ...
    h = Math.min(h, B.maxH);
    if (visuals && Number.isFinite(Number(rawH)) && Number(rawH) > 0) h = Number(rawH);
    items.push({ polys, areaM2, h, rawH, id, hatch: hatchPick, ...(visuals ? { archTags: Object.values(f.properties).filter((v) => typeof v === 'string').join(' ') } : {}) });
  }
  ...
  const acc = { pos: [], col: [], idx: [], anchor: [], uv: [], style: [], vtx: 0 };
  const families = [0,0,0,0,0,0,0,0];
  const haze = hexToRGB(SK.hazeColor);
  for (const item of selected) {
    const style = visuals ? architectureStyle(item, archContext, layer.extent, k) : null;
    if (style) families[style.family] += 1;
    const wall = style ? architecturalColor(style.walls[style.variant]) : hexToRGB(pickByHash(S.wallTones, item.id)).map(
    ...
    const pushV = (px, py, y, colArr, u, v) => {
      ...
      if (style) { acc.style.push(...buildingStyleVertex(style, u !== undefined)); acc.uv.push(u ?? 0.25,v ?? 0.25); }
      return acc.vtx++;
    };
    const roofY = item.h;
    ...
    for (const poly of item.polys) {
      ...
      if (tris.length === 0) continue;
      const base = acc.vtx;
      const roof = style ? architecturalColor(style.roofs[style.variant]) : wall;
      for (const p of ring) pushV(p.x, p.y, roofY, roof);
      for (let t = 0; t < tris.length; t += 3) {
      ...
      for (let e = 0, j = ring.length - 1; e < ring.length; j = e++) {
        const a = ring[j];
        const b = ring[e];
        const halfU = Math.hypot(b.x-a.x,b.y-a.y)*scale/k/(2*ARCHITECTURE_UV_PERIOD[0]);
        const vBot = bottomY/ARCHITECTURE_UV_PERIOD[1], vTop = roofY/ARCHITECTURE_UV_PERIOD[1];
        const i0 = pushV(a.x, a.y, bottomY, wall, 0.5-halfU, vBot);
        const i1 = pushV(b.x, b.y, bottomY, wall, 0.5+halfU, vBot);
        const i2 = pushV(b.x, b.y, roofY, wall, 0.5+halfU, vTop);
        const i3 = pushV(a.x, a.y, roofY, wall, 0.5-halfU, vTop);
  ...
  if (acc.idx.length > 0) {
    ...
    if (visuals) {
      const style = new Uint8Array(acc.style), uv = new Float32Array(acc.uv);
      out.satSkyline.style = style; out.satSkyline.uv = uv; out.satSkyline.families = families;
      transfer.push(style.buffer,uv.buffer);
    }
    out.empty = false;
...
function buildSatVeg(vt, frame) {
  ...
  const vegCls = [];
  const cover=frame.worldCover;
  const coverMask=cover?buildEarthSurfaceMask(vt,256):null;
  ...
  const airportMask=airportSceneryMask(vt,AVOID_GRID_N,tileSpan/frame.k,false,frame);
  let avoidMask = null;
    ...
    for (let i = 0; i < layer.length; i++) {
      ...
      for (const poly of polys) {
        ...
        for (let tries = want * 8; tries > 0 && want > 0; tries--) {
          ...
          if (!pointInPoly(poly, sx, sy)) continue;
          if(cover){
            const gx=Math.min(255,Math.floor(sx/layer.extent*256)),gy=Math.min(255,Math.floor(sy/layer.extent*256));
            // An administrative lawn/residential polygon is no longer enough
            // to plant an airport or town full of arbitrary trees.
            const observed=worldCoverAt(cover,gx,gy);
            if((observed&&observed!==10)||coverMask.exclusion[gy*256+gx])continue;
          }
          if(airportMask){
            const gx=Math.max(0,Math.min(AVOID_GRID_N-1,Math.floor(sx/layer.extent*AVOID_GRID_N)));
            const gy=Math.max(0,Math.min(AVOID_GRID_N-1,Math.floor(sy/layer.extent*AVOID_GRID_N)));
            if(airportMask[gy*AVOID_GRID_N+gx])continue;
          }
          ...
          if(findAirportSurface(cx+lx,cz+lz,15))continue;
  ...
  // Administrative parks do not imply trees. Physical landcover supplies the
  // supported vegetation; the separate Neon scatter below keeps its own style.
  scatter('landcover', (p) => p.class === 'wood', 0.35, 2);
  scatter('landcover', (p) => surfaceClass('landcover', p) === EARTH_SURFACE.grass, 0.08, 3);
  ...
  if(cover&&veg.length/4<V.maxPerChunk){
    // Fill missing mapped woodland in the existing pool. Same transferable
    // layout, cap, terrain support, fades and shadows; zero additional draws.
    const occupied=new Uint8Array(128*128),candidates=[];
    const occupiedAt=(lx,lz)=>Math.max(0,Math.min(127,Math.floor((lz+tileSpan/2)/tileSpan*128)))*128+Math.max(0,Math.min(127,Math.floor((lx+tileSpan/2)/tileSpan*128)));
    for(let i=0;i<veg.length;i+=4)occupied[occupiedAt(veg[i],veg[i+1])]=1;
    for(let gy=2;gy<254;gy+=3)for(let gx=2;gx<254;gx+=3){
      const index=gy*256+gx;
      if(cover[index]!==10||coverMask.exclusion[index])continue;
      let clear=true;
      for(let dz=-2;dz<=2&&clear;dz++)for(let dx=-2;dx<=2;dx++){
        const at=index+dz*256+dx;if(cover[at]!==10||coverMask.exclusion[at]){clear=false;break;}
      }
      if(!clear)continue;
      if(airportMask?.[Math.floor(gy/2)*128+Math.floor(gx/2)])continue;
      // Integer global pixel coordinates make ordering stable across re-visits.
      const mixed=Math.imul((x*256+gx)^Math.imul(y*256+gy,374761393),668265263);
      const rank=((mixed^(mixed>>>13))>>>0)/4294967296;
      const cell=Math.imul(Math.floor((x*256+gx)/12),1597334677)^Math.imul(Math.floor((y*256+gy)/12),3812015801);
      const stand=((Math.imul(cell^(cell>>>16),2246822519)>>>0)/4294967296);
      // Nearby accepted trees form woodland stands instead of uniform dots.
      // Radius uses its own hash: sorting the size seed selected only small trees.
      const radiusSeed=((Math.imul(mixed^0x51ed270b,3266489917)>>>0)/4294967296);
      candidates.push({gx,gy,rank:stand+rank*.004,radiusSeed});
    }
    candidates.sort((a,b)=>a.rank-b.rank);
    for(const {gx,gy,radiusSeed} of candidates){
      if(veg.length/4>=V.maxPerChunk)break;
      const lx=(gx+.5)/256*tileSpan-tileSpan/2,lz=(gy+.5)/256*tileSpan-tileSpan/2;
      const at=occupiedAt(lx,lz);if(occupied[at])continue;occupied[at]=1;
      // WorldCover says tree cover, not species. Avoid inventing conifers.
      if(findAirportSurface(cx+lx,cz+lz,15))continue;
      veg.push(lx,lz,T.minR+radiusSeed*(T.maxR-T.minR),0);if(perClass)vegCls.push(2);
    }
  }
      ...
      for (let i = 0; i < layer.length; i++) {
        ...
        for (const poly of classifyRingsSat(rings.filter((r) => r.length >= 3))) {
          ...
          for (let tries = want * 8; tries > 0 && want > 0; tries--) {
            ...
            const [lx, lz] = toLocal(sx, sy);
            if(findAirportSurface(cx+lx,cz+lz,30))continue;
  ...
  const parcelKind=new Uint8Array(parcelPts.length/2+384),parcelYaw=new Float32Array(parcelKind.length);
  if(PARCEL_HOMES.enabled){
    const extra=buildRoadsideInfill(vt,cover,tileSpan/frame.k);
    const oldLength=parcelPts.length;
    for(const p of extra){
      const lx=(p.x-.5)*tileSpan,lz=(p.y-.5)*tileSpan;
      if(findAirportSurface(cx+lx,cz+lz,30))continue;
      // Existing parcel clusters own their neighbourhood; supplement only gaps.
      let occupied=false;for(let i=0;i<oldLength;i+=2)if(Math.hypot(lx-parcelPts[i],lz-parcelPts[i+1])<85*frame.k){occupied=true;break;}
      if(occupied)continue;
      const i=parcelPts.length/2;parcelKind[i]=1;parcelYaw[i]=p.yaw;parcelPts.push(lx,lz);
    }
  }
  ...
  if (parcelPts.length > 0) {
    ...
    out.satParcelKind=parcelKind.slice(0,satParcel.length/2);out.satParcelYaw=parcelYaw.slice(0,satParcel.length/2);
    out.parcelProvenance={legacy:'residential-landuse',roadside:'residential-or-worldcover-2021-built-up-with-mapped-roads'};
    out.empty = false;
    transfer.push(satParcel.buffer);
    transfer.push(out.satParcelKind.buffer,out.satParcelYaw.buffer);
  }
...
const api = {
   ...
   */
  async buildTile(z, x, y, detail = 'full', options = {}) {
    ...
    if (detail === 'earth-surface') {
      const span=WORLD_SIZE/2**z,lat=Math.atan(Math.sinh(Math.PI*(1-2*(y+.5)/2**z))),k=1/Math.cos(lat);
      const frame={tileSpan:span,mercX0:x*span-WORLD_SIZE/2,mercYTop:WORLD_SIZE/2-y*span,cx:(x+.5)*span-WORLD_SIZE/2,cz:(y+.5)*span-WORLD_SIZE/2,k};
      const surface = deriveWorldContent(vt,combineWorldCover(buildEarthSurfaceMask(vt, options.size),await loadWorldCover(z,x,y)),span,k,frame);
      return transferResult({ v: WORKER_PROTOCOL, surface }, [surface.classes.buffer, surface.blend.buffer, surface.exclusion.buffer, surface.waterEdges.buffer,surface.settlement.buffer,surface.forest.buffer,surface.airport.pos.buffer,surface.airport.uv.buffer,surface.airport.style.buffer]);
    }
    ...
    if (detail === 'sat-buildings') {
      return buildSatBuildings(vt, { tileSpan, mercX0, mercYTop, cx, cz, k, t0, visuals: options.visuals === true });
    }
    ...
    if (detail === 'sat-skyline') {
      return buildSatSkyline(vt, { tileSpan, mercX0, mercYTop, cx, cz, k, t0, visuals: options.visuals === true });
    }
    if (detail === 'sat-veg') {
      return buildSatVeg(vt, { tileSpan, mercX0, mercYTop, cx, cz, k, z, x, y, t0,worldCover:await loadWorldCover(z,x,y) });
    }
      ...
      for (let i = 0; i < layer.length; i++) {
        ...
        if (NEON_COVER.enabled) {
          ...
          for (let pi = 0; pi < polys.length; pi++) {
            ...
            const pid = (Math.imul(fid, 2654435761) ^ Math.imul(pi + 1, 40503)) >>> 0;
            if (!operationsFootprint(p.outer,{tileSpan,mercX0,mercYTop,cx,cz},layer.extent)) items.push({ polys: [p], area: a, areaM2: aM2, rawH, minY: minY0, id: pid });
          }
          continue;
        }
        let area = 0;
        ...
        for (const poly of item.polys) {
          ...
          for (const ring of [poly.outer, ...poly.holes]) {
            for (let e = 0, j = ring.length - 1; e < ring.length; j = e++) {
              const a = ring[j];
              const b = ring[e];
              // R24 B (RING_DEDUPE): the toy twin of the sliver guard above.
              if (
                RING_DEDUPE.enabled &&
                Math.hypot(b.x - a.x, b.y - a.y) < RING_DEDUPE.minEdgeTile
              )
