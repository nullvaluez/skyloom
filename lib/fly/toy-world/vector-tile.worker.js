...
//         drops the bundle and warns — no buildings, no roads, no crash.
//     All FOUR protocol pins move in lockstep with this line (sat-building-
//     engine, sat-road-engine, toy-world-engine — the R15 lesson: forgetting
//     the toy pin spams a false "stale worker" warn through every toy session
//     even though the toy bundle is byte-unchanged, which it is here too).
const WORKER_PROTOCOL = 14;
...
function buildSatBuildings(vt, frame) {
  ...
  for (const it of items) {
    ...
    // compress the [5,8] band to [6,8] and undo half the fix).
    if (!inferredHouse) h = Math.max(h, S.minH);
  ...
  if (RS.enabled) {
    ...
    const room = Math.max(0, S.maxPerChunk - anchors.length);
    if (rest.length <= room) {
    ...
    selected = built.slice(0, S.maxPerChunk);
  }
  ...
  for (const item of selected) {
    ...
    const band = item.h <= RT.lowMaxH ? RT.low : item.h >= RT.tallMinH ? TONE.roofTall : TONE.roofMid;
    ...
    const vBot = wallBottomY / vPeriod;
    const vTop = roofY / vPeriod;
  ...
  if (satB.idx.length > 0) {
    ...
    const meta = { total: items.length, kept: selected.length, smallKept, forms };
...
function buildSatSkyline(vt, frame) {
  ...
  for (let i = 0; i < layer.length; i++) {
    ...
    if (!(rawH >= SK.minH || areaM2 >= SK.minAreaM2)) continue;
    ...
    let h = rawH > 0 ? rawH : clamp01Range(12 + Math.sqrt(areaM2) * 0.6, 18, 60) * (0.9 + hash * 0.2);
    if (h > B.kneeM) h = B.kneeM + (h - B.kneeM) * B.kneeSlope; // same soft knee as the near ring
    h = Math.min(h, B.maxH);
    items.push({ polys, areaM2, h, id });
  }
  if (items.length === 0) {
    out.tessMs = performance.now() - t0;
    return out;
  ...
  for (const item of items.slice(0, SK.maxPerChunk)) {
...
function buildSatVeg(vt, frame) {
  ...
  const scatter = (layerName, classFilter, coniferFrac) => {
    ...
    for (let i = 0; i < layer.length; i++) {
      ...
      for (const poly of polys) {
        ...
        let want = Math.min(
          Math.floor(areaM2 / T.areaPerTreeM2),
  ...
  scatter('park', null, 0.1);
  scatter('landcover', (p) => p.class === 'wood', 0.35);
  scatter('landcover', (p) => p.class === 'grass', 0.08);
  ...
  const sampleInside = (layerName, classFilter, outArr, maxPts, clearanceM) => {
    ...
    for (let i = 0; i < layer.length; i++) {
      if (outArr.length / 2 >= maxPts) return;
      const f = layer.feature(i);
      if (f.type !== 3) continue;
      if (classFilter && !classFilter(f.properties)) continue;
      const rings = f.loadGeometry().map((r) => clipRing(r, layer.extent));
      ...
      for (const poly of polys) {
        ...
        for (let tries = 14; tries > 0; tries--) {
          const sx = minX + rand() * (maxX - minX);
          const sy = minY + rand() * (maxY - minY);
          ...
          if (!pointInPoly(poly, sx, sy)) continue;
          if (
            !pointInPoly(poly, sx - clr, sy - clr) ||
            !pointInPoly(poly, sx + clr, sy + clr) ||
            !pointInPoly(poly, sx - clr, sy + clr) ||
            !pointInPoly(poly, sx + clr, sy - clr)
          )
          ...
          const [lx, lz] = toLocal(sx, sy);
          outArr.push(lx, lz);
          break; // at most one anchor per polygon — spread, not clumps
        }
      }
    }
  };
