...
import { BEACONS, ROOFS, RUNWAY_LIGHTS, SAT_BUILDINGS, SAT_ROADS, SAT_WATER, TOY_WORLD } from '../fly-constants';
...
function buildSatBuildings(vt, frame) {
  ...
  for (const item of built.slice(0, S.maxPerChunk)) {
    ...
    const wall = hexToRGB(pickByHash(S.wallTones, item.id)).map((c) => c * jit(S.wallJitter, hash2));
    ...
    const baseMul = S.wallBaseMul[0] + (S.wallBaseMul[1] - S.wallBaseMul[0]) * aoT;
    ...
    const band = item.h <= RT.lowMaxH ? RT.low : item.h >= RT.tallMinH ? RT.tall : RT.mid;
  ...
  if (satB.idx.length > 0) {
    ...
    out.satBuilding = { pos, col, idx, anchor, uv };
    out.empty = false;
...
const api = {
   ...
   * ribbons, out.satRoads; also returns early.
