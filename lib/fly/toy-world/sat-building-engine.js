  ...
  _finalizePending(nowSec = 0) {
    ...
    for (let i = 0; i < this.pendingFinalize.length && done < S.finalizePerFrame; i++) {
      ...
      if (bld) {
        ...
        // index is uploaded and before normals are computed: a degenerate
        // contributes a zero/NaN face normal to computeVertexNormals(), so
        // stripping it here also stops it perturbing its neighbours' shading.
