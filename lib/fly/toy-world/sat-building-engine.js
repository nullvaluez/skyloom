...
import { applyBendAnchorSat, applyBendWaterSat, getSatBldgFade, setSatBldgFade } from './world-bend';
...
// R24 B: E CERT's stall-attribution hook (no-op with FRAME_STATS off).
...
export class SatBuildingEngine {
  constructor({ groundAt }) {
    ...
    this._fadePool = new TwinPool(
      () => {
        ...
        applyBendAnchorSat(material, uniform);
    ...
    this._stat = { errorRetries: 0, evictions: 0, heals: 0, degenerateDropped: 0, healsInPlace: 0, healsNoop: 0, fadeBudgetMiss: 0 };
  ...
  _queueRedrape(key, chunk) {
    if (!HEAL_IN_PLACE.enabled || !chunk.redrape || !chunk.mesh) return false;
    if (this._redrape.length >= HEAL_IN_PLACE.maxConcurrent) return false;
    if (this._redrape.some((j) => j.key === key)) return true; // already running
  ...
  _pumpRedrape() {
    ...
    for (let k = this._redrape.length - 1; k >= 0; k--) {
      ...
      while (j.i < R.gy.length) {
        ...
        j.i += 1;
        if (performance.now() - t0 > HEAL_IN_PLACE.budgetMs) return;
      }
      ...
      if (bld) {
        ...
        for (let v = 0, vi = 0; v < pos.length; v += 3, vi += 1) {
          ...
          if (ax !== runAx || az !== runAz) {
            flushRun();
            runStart.push(vi); // R24 B (HEAL_IN_PLACE): this run's first vertex
            runAx = ax;
            runAz = az;
            runTop = -Infinity;
            runR2 = 0;
