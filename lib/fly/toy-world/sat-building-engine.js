...
export class SatBuildingEngine {
  constructor({ groundAt }) {
    ...
    this._stat = {
      ...
      heals: 0,
      contactHeals: 0,
      // R22.1 (C): FLASH_GUARD instrument — verify-flash-guard reads these.
      degenDropped: 0,
      degenScanned: 0,
      degenChunks: 0,
    };
      ...
      if (bld) {
        ...
        const flushRun = (end) => {
          ...
          if (drapeRuns) {
            ...
            const run = { start: runStart, end, ax: runAx, az: runAz, ground: runGY,
        ...
        // Round 22.1 (C) — FLASH_GUARD. Drop zero-area triangles before the
        // index is uploaded. Placing it before computeVertexNormals() does NOT
        // change the normals and is not meant to: the face normal of a
        // coincident or collinear triangle is cross(c-b, a-b) === (0,0,0), so a
        // degenerate contributes exactly nothing to the accumulation. Verified
        // against this three build — normals byte-identical with and without.
        // That is the point worth recording: since these triangles also cover
        // no pixels when they rasterize correctly, removing them cannot change
        // any well-behaved frame. The only frames that change are the broken
        // ones. (Earlier revisions of this comment claimed the filter stopped
        // degenerates perturbing neighbour shading. They never did.)
        const deg = dropDegenerateTris(bld.idx, pos);
        this._stat.degenDropped += deg.dropped;
        this._stat.degenScanned += deg.tris;
        if (deg.dropped > 0) this._stat.degenChunks += 1;
        geo.setIndex(new BufferAttribute(deg.idx, 1));
