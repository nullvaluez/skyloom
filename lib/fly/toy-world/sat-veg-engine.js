...
import { NearGroundSupport } from '../near-ground-support';
import { satelliteVisualsOn } from '../satellite-visuals';
import { fillSkylineGrid as fillMeasuredGrid } from '../skyline-drape';
import { finalizePaceOn } from '../finalize-pace';
import { FINALIZE_PACE, LEAD_SAFE, RING_HOLD, SAT_VEG, STREAM_KEEPER } from '../fly-constants';
...
export class SatVegEngine {
  constructor({ groundAt, maxChunks }) {
    ...
    this.visuals = satelliteVisualsOn('ground');
    ...
    this._supportRevision = 0;
    this.nearSupport = new NearGroundSupport((x, z) => this._sampleWorld(x, z));
    this._vx = 0;
    this._vz = 0;
  ...
  update(nowSec, playerX, playerZ, eyeAglM, nearOn = false, epoch = 0, tier = 'high', mercatorK = 1) {
    if (this._disposed || !this.worker) return;
    this._now = nowSec;
    this.nearSupport.step(nowSec, playerX, playerZ, eyeAglM, mercatorK, tier, epoch, nearOn);
    ...
    while (this.building < SAT_VEG.maxBuilds && this.queue.length > 0) {
      ...
      this.worker
        ...
        .then((result) => {
          ...
          chunk.parcel = parcel;
          chunk.parcelKind=result.satParcelKind;chunk.parcelYaw=result.satParcelYaw;
          chunk.veg = veg;
  ...
  //            low-resolution. Legacy mode commits and heals it later;
  //            cinematic scenery holds until local samples can ground it.
  _samplePending() {
    ...
    for (const p of this.pendingSample) {
      if (!p.grid) {
        ...
        if(this.visuals)p.gridZoom=new Uint8Array(N);
        p.gi = 0;
        p.miss = 0;
        p.coarse = 0;
      }
      ...
      while (p.gi < N) {
        ...
        const valid=Number.isFinite(s?.elev)&&(!this.visuals||s.tileZ>=SAT_VEG.demZ);
        if (!valid) p.miss += 1;
        else if (s.tileZ < SAT_VEG.demZ) p.coarse += 1;
        // A continental sample can be hundreds of metres above the destination.
        // Never seed the near-contact smoothing cache with that wrong height.
        p.grid[p.gi] = valid?s.elev:this.visuals?NaN:0;
        if(p.gridZoom)p.gridZoom[p.gi]=valid?s.tileZ:0;
        p.gi += 1;
  ...
  _commitPending(nowSec) {
    ...
    // R24 A (FINALIZE_PACE, recon A9): every OTHER streaming engine budgets its
    // finalize per frame; this one flipped every finished chunk ready on one
    // tick, and the pooled canopy layers all re-sign on that same tick. No
    // geometry is built here, so it is a synchronised POP rather than a stall —
    // but it is the only uncapped commit left.
    // R24 E — the sixth budget site, and the one the harness scaler missed.
    // `budgetK()` is exactly 1 unless a harness sets FLY_FINALIZE_BUDGET_K, so
    // production arithmetic is byte-identical; at this venue's ~1 fps an
    // unscaled cap of 1 is ONE veg chunk per second, which leaves any veg
    // reading partly populated and indistinguishable from a feature that did
    // not run. Same one-line idiom as the other five sites (sat-building,
    // sat-skyline, toy-world finalize; sat-building, sat-skyline, sat-road,
    // toy-world drape).
    const cap = finalizePaceOn() ? Math.max(1, FINALIZE_PACE.vegPerFrame * budgetK()) : Infinity;
    let committed = 0;
    for (let i = 0; i < this.pendingSample.length && committed < cap; i++) {
      ...
      if(this.visuals&&!fillMeasuredGrid(p.grid,p.gridZoom,S.gridSegments)){
        // Keep old ready geometry during a heal; a new forest waits for local
        // terrain. Missing DEM never becomes a sea-level or previous-area forest.
        if(nowSec>=(p.retryAt??0)){p.retryAt=nowSec+S.missRetrySec;p.gi=0;p.miss=0;p.coarse=0;}
        continue;
      }
      ...
      chunk.grid = p.grid;
      // Placement must see the COMMIT, not the earlier heal request. Counts
      // stay unchanged while a ready chunk's support heights are replaced.
      this._supportRevision += 1;
  ...
  /** Bounded exact contacts for the two near-ground consumers. */
  groundAtNear(x, z, fallbackY, kind = 'canopy') { return this.nearSupport.heightAt(x, z, fallbackY, kind); }
  ...
  get stats() {
    ...
    if (this._parked) {
      return {
        ...
        supportRevision: this._supportRevision,
        nearSupport: this.nearSupport.stats,
      };
    }
