import { SAT_VEG } from '../fly-constants';
 ...
 *
 * A ready chunk record is { cx, cz, veg, water, ind, grid, span, d } — cx/cz
 * are the ABSOLUTE tile centre, and veg/water/ind hold the worker's rows in
 * TILE-LOCAL coordinates (add cx/cz for world).
 */
export class SatVegEngine {
  ...
  _pumpQueue() {
    while (this.building < SAT_VEG.maxBuilds && this.queue.length > 0) {
      ...
      this.chunks.set(key, { key, state: 'building', cx, cz, span, tile: e });
      this.building += 1;
      this.worker
        ...
        .then((result) => {
          ...
          // No satVeg AND no satPts = ocean, desert, bare rock, or a tile whose
          // park/landcover/water/industrial layers are all absent. Nothing to
          // draw, nothing to sample: this chunk costs zero of everything.
