...
export class SatSkylineEngine {
  ...
  update(nowSec, playerX, playerZ, eyeAglM, refGroundY = 0) {
    ...
    this._writeChoreography(eyeAglM);
  ...
  _writeChoreography(eyeAglM) {
    ...
    setSatSkyline(SK.hole.radiusM * (1 - open), SK.hole.featherM, fade);
  }
  ...
  _pumpQueue() {
    ...
    while (this.building < SK.maxBuilds && this.queue.length > 0) {
      ...
      const chunk = { state: 'building', mesh: null, group: e, pending: g * g, parts: [], stale: false };
      ...
      for (let j = 0; j < g; j++) {
        for (let i = 0; i < g; i++) {
          ...
          if (tx >= nTiles || ty >= nTiles) {
            this._childDone(key, null, 0, 0);
            continue;
          }
          this.worker
            .buildTile(e.z, tx, ty, 'sat-skyline')
            .then((result) => this._childDone(key, result, dx, dz))
            .catch((err) => {
              this._childDone(key, null, dx, dz);
  ...
  _childDone(key, result, dx, dz) {
    ...
    chunk.state = 'draping';
    this.pendingFinalize.push({ key, group: chunk.group, parts: chunk.parts, grid: null, gi: 0 });
  }
  ...
  get stats() {
    let ready = 0; // = skyline draw calls (one merged mesh per non-empty group)
    let empty = 0;
    let tris = 0;
    let verts = 0;
