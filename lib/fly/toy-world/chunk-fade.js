...
/**
 * R24 B (W3) — THE FRAME-COUNT FLOOR. Progress along a ramp is the SMALLER of
 * its seconds term and its frames term:
 *
 *     min(elapsed / sec, framesSince / CHUNK_FADE.minFrames)
 *
 * A ramp in seconds alone is right on a machine that renders in milliseconds
 * and useless on one that does not: at the cloud venue's measured 2.84 s/frame
 * a 0.3 s eviction ramp begins and ends between two samples, so a fade that is
 * genuinely running is indistinguishable from no fade — and a HITCH on a real
 * machine does exactly the same thing for one frame, which is the moment a pop
 * is most visible. The floor makes a ramp span at least `minFrames` frames
 * however long they are.
 *
 * At 60 Hz it changes nothing BY CONSTRUCTION: the 0.3 s eviction ramp is 18
 * frames and the 0.4 s birth ramp 24, both far above 4, so the seconds term is
 * the smaller one and governs alone. The floor binds only when a frame is
 * longer than sec/minFrames (75 ms) — i.e. only when the seconds term would
 * have skipped the fade entirely.
 *
 * @param {{t0:number, f0:number}} e a birth or death record
 * @param {number} nowSec  the frame clock
 * @param {number} sec     the ramp's duration in seconds
 * @param {number} frame   the engine's monotone fade-frame counter
 */
export function rampT(e, nowSec, sec, frame) {
  const bySec = sec > 0 ? (nowSec - e.t0) / sec : 1;
  const minF = CHUNK_FADE.minFrames | 0;
  if (minF <= 0) return bySec;
  return Math.min(bySec, (frame - e.f0) / minF);
}
