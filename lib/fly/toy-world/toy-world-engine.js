import { airportTerrainHeight } from '../operations-terrain';
import { markPhase } from '../frame-stats';
import { mayFinalize, finalizePaceOn } from '../finalize-pace';
import {
  BufferAttribute,
  BufferGeometry,
  Color,
...
import { BEACONS, BEND_LEAD, FOAM, GLOBE, NEON_COVER, ROAD_PULSE, ROOF_CONTENT, RUNWAY_LIGHTS, STREAM_KEEPER, TOY, TOY_WORLD, WATER_MOON, WINDOW_GRID } from '../fly-constants';
...
/**
 * R24 A (recon WB-10). three's own threshold for a 32-bit index buffer, copied
 * from `arrayNeedsUint32` (three r185, three.core.js:1779-1791). It is 65535
 * and NOT 65536 on purpose — three #24565, PRIMITIVE_RESTART_FIXED_INDEX — so
 * this mirrors the number rather than re-deriving it.
 */
const INDEX_U32_MIN = 65535;

/**
 * True when any element of `arr` is >= `min`. Scanned from the end, matching
 * three's own comment that larger values usually sit last; the ORDER is only a
 * speed choice, the answer is order-independent.
 *
 * This exists so the FINALIZE_PACE branch of the land index build can pick the
 * same container type three would have picked for the flag-off branch, instead
 * of assuming Uint32 and doubling every toy land index buffer.
 */
function anyAtLeast(arr, min) {
  if (min <= 0) return arr.length > 0;
  for (let i = arr.length - 1; i >= 0; i--) if (arr[i] >= min) return true;
  return false;
}
...
const EXPECTED_WORKER_PROTOCOL = 23; // shared worker surface-mask contract
...
function newStreamStats() {
  ...
  return { errorRetries: 0, evictions: 0, heals: 0, degenerateDropped: 0 };
}
...
export class ToyWorldEngine {
  ...
  _finalizePending(nowSec = 0) {
    ...
    for (let i = 0; i < this.pendingFinalize.length && done < TOY_WORLD.finalizePerFrame * budgetK(); i++) {
      // R24 A (FINALIZE_PACE, recon WB-3): the shared brake REPLACES the
      // engine-local one when armed. The local rule let the first chunk
      // through unconditionally (`done > 0 &&`) and could not see the other
      // three engines spending their own budgets in the same frame.
      if (!mayFinalize(done)) break;
      markPhase('finalize:toy-world'); // R24 A: E's FRAME_STATS attribution
      if (
        !finalizePaceOn() &&
        done > 0 &&
      ...
      const height = (lx, lz) => {
        ...
        const raw = (h00 * (1-tx) * (1-tz) + h10*tx*(1-tz) + h01*(1-tx)*tz + h11*tx*tz)*EX;
        return airportTerrainHeight(cx+lx,cz+lz,raw+TOY_WORLD.groundLift)-TOY_WORLD.groundLift;
      };
      ...
      for (let gz = 0; gz < w; gz++) {
        for (let gx = 0; gx < w; gx++) {
          ...
          const h = airportTerrainHeight(cx+lx,cz+lz,p.grid[i]*EX+TOY_WORLD.groundLift)-TOY_WORLD.groundLift;
          groundPos[i * 3] = lx;
      ...
      // Merge ground + worker land overlays into ONE draw
      {
        ...
        // R24 A (recon WB-10): build the merged index into a TYPED array. The
        // spread built a plain JS array of up to ~100k boxed numbers per chunk,
        // purely to hand it to BufferAttribute, which then copies it again.
        // Identical values and identical order — only the container changes.
        //
        // AND IT MUST BE WRAPPED. `BufferGeometry.setIndex` wraps its argument
        // ONLY when `Array.isArray(index)` (three r185, three.core.js:18404):
        // a typed array is assigned RAW as `geometry.index`, so it has no
        // `.array` and WebGLAttributes throws on `attribute.array.byteLength`
        // at first upload — once per land mesh. That is the pass-2b toy-boot
        // pageerror (x31 on ladder-fix, x3 on ladder-red, both arms), attributed
        // by B's headless attribute census: FINALIZE_PACE ON gave 80 broken LAND
        // meshes, OFF gave 0. The OFF branch below is a plain array, which three
        // wraps — which is exactly why pass 1 was clean and 2b was not.
        //
        // The container TYPE is chosen the way three chooses it for the OFF
        // path (`arrayNeedsUint32`, three.core.js:1779), so the two branches
        // agree in values AND in type: Uint16 unless some index is >= 65535.
        // The bound is 65535 and not 65536 in three's own source, for
        // PRIMITIVE_RESTART_FIXED_INDEX (three #24565) — mirrored, not rounded.
        // Defaulting to Uint32 would have silently doubled every toy land index
        // buffer, which is the opposite of what WB-10 is for.
        let idx;
        if (finalizePaceOn()) {
          const base = groundIdx.length;
          const extra = data ? data.idx.length : 0;
          const off = w * w;
          // An element of the merged array is either a groundIdx value or
          // `data.idx[k] + off`, so three's "any element >= 65535" decomposes
          // exactly. Scanned, not inferred from `total - 1`: a bound is not the
          // same claim as a maximum, and only the scan is provably three's answer.
          const needs32 =
            anyAtLeast(groundIdx, INDEX_U32_MIN) ||
            (extra > 0 && anyAtLeast(data.idx, INDEX_U32_MIN - off));
          const merged = new (needs32 ? Uint32Array : Uint16Array)(base + extra);
          merged.set(groundIdx, 0);
          for (let k = 0; k < extra; k++) merged[base + k] = data.idx[k] + off;
          idx = new BufferAttribute(merged, 1);
        } else {
          idx = data
      ...
      if (p.result.water) {
        ...
        // R24 B (FLASH_GUARD) — cheap insurance on the toy water sheet (its
        // polygons come from earcut and its foam ribbons are length-guarded, so
        // the expected drop here is 0). Byte-identical when the flag is off.
        const fgW = guardIndex(data.idx, pos);
        this._stat.degenerateDropped += fgW.dropped;
        geo.setIndex(new BufferAttribute(fgW.idx, 1));
      ...
      if (p.result.building) {
        ...
        // R24 B (FLASH_GUARD) — THE toy twin of the A1 defect: the toy wall
        // extruder (vector-tile.worker.js:4285) walks the CLOSED ring exactly
        // like the satellite one, on a MeshToon DoubleSide material under the
        // same per-vertex bend. Never censused before R24 (recon A1b). Filter
        // before normals; byte-identical when the flag is off.
        const fgB = guardIndex(data.idx, pos);
        this._stat.degenerateDropped += fgB.dropped;
        geo.setIndex(new BufferAttribute(fgB.idx, 1));
  ...
  censusDegenerate() {
    const meshes = [];
    for (const c of this.chunks.values()) if (c.meshes) meshes.push(...c.meshes);
