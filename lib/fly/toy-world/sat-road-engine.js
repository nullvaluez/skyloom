...
import { satelliteRoadLighting } from '../night-city';
...
//
// This ring has NO per-instance consumer range (the ribbons are drawn whole, per
// chunk), so LEAD_SAFE's cap for it is set to exactly its present effective
// maximum, 0.35 × 12000 = 4200 — i.e. the cap is a PROVEN NO-OP here and exists
// only so that a future change to `SAT_ROADS.ring.r` cannot silently re-open the
// coverage hole that LEAD_SAFE closed for veg and clutter. RING_HOLD, by
// contrast, is live: this ring offers 32 candidates for 16 slots, the widest
// over-subscription in the app, and its chunks carry the road network the user
// reads as part of the ground surface.
...
export class SatRoadEngine {
  constructor({ groundAt }) {
    ...
    // R24 B (W3) — NO FADE CHANNEL, DECLARED. This engine is deliberately
    // excluded from CHUNK_FADE: verify-sat-night's frozen gate asserts that ONE
    // material instance carries every road mesh (by uuid, at two censuses
    // minutes apart) and that meshes === ready === visible, and a pooled fade
    // twin or a deferred eviction turns that into a load-decided coin — the
    // exact R21 anti-pattern. Road ribbons are also the least visible pop in
    // the set. So every road mesh reads presence 1 on its first displayed
    // frame, BY DESIGN — and a presence probe must ATTRIBUTE that rather than
    // count it as an unexplained hard birth. This marker is how it can.
    this.material.userData.__noFade =
      'sat-roads: excluded from CHUNK_FADE — verify-sat-night pins one material instance for this engine';
    ...
    // Round 23 (B CITY-LIGHT): the fourth argument is NIGHT_CITY_R23.roads and
    // ONLY when the block is armed — same rule, one level further. null (the
    // shipped state) ⇒ world-bend compiles the R19 program verbatim under the
    // R19 key. Resolved ONCE here because it selects the shader source, and a
    // material's source is fixed at apply time.
    this.r23 = satelliteRoadLighting();
    applyBendRoadSat(this.material, SAT_ROADS, SUBURB_NIGHT, this.r23);
  ...
  _leadCenter(nowSec, px, pz, ringR) {
    ...
    // R24 (B) — LEAD_SAFE. A proven no-op for this ring (cap === 0.35 × ringR);
    // see the header block for why the entry exists anyway.
    const lead = Math.min(sp * L.leadSec, leadCapM('satRoads', ringR));
  ...
  _refreshDesired(px0, pz0, eyeAglM, nowSec = 0) {
    ...
    // R24 (B) — RING_HOLD. Still `S.maxChunks` at most (the hard draw bound is
    // this ring's whole point) and still rank-sorted.
    const keyOf = (e) => `${e.z}/${e.x}/${e.y}`;
    const kept = ringHoldKeep(desired, S.maxChunks, this.chunks, keyOf, (c) => !!c.mesh);
    const keep = new Set(kept.map(keyOf));
    const held = residencyHeld(this.chunks, keep, nowSec, px0, pz0, S.ring.r);
  ...
  _finalizePending(nowSec = 0) {
    ...
    for (let i = 0; i < this.pendingFinalize.length && done < perFrame; i++) {
      ...
      // Round 23 (B CITY-LIGHT) — THE RIBBON SIDE, derived here rather than in
      // the worker. `pushSatRoadQuads` emits every quad as
      //   (a + n), (a − n), (b + n), (b − n)
      // and always four verts at a time, so global vertex-index PARITY is the
      // side: even = +normal, odd = −normal. That is a property of the emitter,
      // not of the data, which is why reading it here costs no protocol bump, no
      // payload change and no worker edit (WORKER_PROTOCOL stays 18). Built ONLY
      // when the two-colour traffic term is armed — disarmed, not one byte of
      // this buffer is allocated. Absent on the GPU ⇒ 0 ⇒ both light streams at
      // half weight (the shader's fail-blended default).
      if (this.r23) {
        const n = rd.cls.length;
        const side = new Float32Array(n);
        for (let i = 0; i < n; i++) side[i] = i & 1 ? -1 : 1;
        geo.setAttribute('aRoadSide', new BufferAttribute(side, 1));
      }
      ...
      // Reverse-Z in Three r185 reverses explicit ordering too. The preview
      // road overlay must follow terrain(0) and cinematic water(-3).
      mesh.renderOrder = satelliteVisualsOn('lighting') ? -4 : 3;
      this.object.add(mesh);
      chunk.mesh = mesh;
      chunk.state = 'ready';
      // R24 (B) — RING_HOLD (2) reads both. This engine's record carried no
      // resolved centre before (the mesh held it), and `residencyHeld` refuses
      // to judge a record it cannot locate, so the stamp is what makes the hold
