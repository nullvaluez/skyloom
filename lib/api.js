...
/**
 * Fetch aircraft photo from Planespotters (via our keyless proxy)
 *
 * Round 15: a TRANSPORT failure now throws so React Query can retry it — the
 * hook caches with `staleTime: Infinity`, and the old blanket `return null`
 * turned one flaky fetch into "this aircraft has no photo" for the rest of
 * the session. A resolved-but-empty answer still returns null (that is a real
 * "no photo on file", and it should stay cached).
 *
 * @param {string} hex - ICAO 24-bit address
 * @returns {Promise<Object|null>} - Photo record or null when none exists
 */
export async function fetchAircraftPhoto(hex) {
  ...
  if (!response.ok) {
    // The proxy degrades to 200 + { photos: [] } for every upstream problem,
    // so a non-OK here means OUR route broke — worth a retry.
    throw httpError(`Photo lookup failed: ${response.status}`, response.status);
  }

  const data = await response.json();
  return data?.photos?.length ? data.photos[0] : null;
}

/**
 * Fetch the aircraft REGISTRY record (manufacturer / real model name /
 * registered owner / country) from our keyless adsbdb → hexdb proxy.
 *
 * The proxy never throws and never 500s: a genuine miss is
 * `{ found: false }` (military, blocked and PIA hexes legitimately have no
 * public registry entry), so only a broken local route reaches the throw.
 *
 * @param {string} hex - ICAO 24-bit address
 * @returns {Promise<Object>} - Normalized registry record ({ found: false } on miss)
 */
export async function fetchAircraftInfo(hex) {
  if (!hex) return { found: false };

  const response = await fetch(`/api/aircraft/${hex}/info`);

  if (!response.ok) {
    throw httpError(`Registry lookup failed: ${response.status}`, response.status);
  }
