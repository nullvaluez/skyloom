...
The user requested merging into main when complete. Fetching revealed that GitHub
main (`0ff2a3f`, 2026-09-07) is a divergent Round21 descendant from the local main
used for this work (`44ec502`), rather than a simple fast-forward. Merge base is
`3592656`; there are 429 remote-only /105 implementation-HEAD-only commits and
24 overlapping edited app/rendering paths. Remote terrain residency/crossfades,
worker contracts, building pool fades and atmosphere/shader changes require semantic
reconciliation. No whole-file ours/theirs resolution or main merge has been made.
The user has been asked whether to integrate latest GitHub main or merge local main
only. Until that choice, the completed overhaul and its evidence remain isolated.
